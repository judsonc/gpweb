...
gpweb\modulos\calendario\expediente.class.php		

Classe CExpediente para manipulação do expendiente dos usuários e OMs.																																						
...
class CExpediente {
	...
	function __construct($cia_id=0, $usuario_id=0, $projeto_id=0, $recurso_id=0, $tarefa_id=0){
		global $Aplic;
		if($usuario_id) {
			$q = new BDConsulta;
		...
		elseif($recurso_id) {
			...
			$q->adOnde('recurso_id = '.(int)$recurso_id);
			$resultado=$q->Resultado();
			$q->limpar();	
			if ($resultado) $this->cia_id=$resultado;
			else $this->cia_id=$Aplic->usuario_cia;
	...
	function calendarioMesAtual($sobrecarga=false){
		...
		else getExpedienteLinks($primeira_data, $ultima_data, $links, $this->cia_id, $this->usuario_id, $this->projeto_id, $this->recurso_id, $this->tarefa_id);
	...
	function getSobrecargaLinks(&$links) {
		...
		$q->adOnde('date(tarefa_inicio) <= \''.$ultima_data->format($d).'\' AND date(tarefa_fim)>= \''.$primeira_data->format($d).'\'');
		$tarefas=$q->Lista();
		$q->limpar();
		...
		$q->adOnde('date(evento_inicio) <= \''.$ultima_data->format($d).'\'');
		$q->adOnde('date(evento_fim)>= \''.$primeira_data->format($d).'\'');
		$eventos=$q->Lista();
		$q->limpar();
		...
		foreach ($tarefas as $tarefa) {
			...
			for ($i = 0, $i_cmp = $data_inicial->dataDiferenca($data_final); $i <= $i_cmp; $i++) {
				if (!isset($horas_diasuteis[$data->format(FMT_TIMESTAMP_DATA)])) $horas_diasuteis[$data->format(FMT_TIMESTAMP_DATA)]= round($this->horas_dia($data->format($d)), 2);
