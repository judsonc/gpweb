...
class CEvento extends CAplicObjeto {
	...
	static function getEventoParaPeriodo($data_inicio, $data_fim, $evento_filtro='', $usuario_id=null, $cia_id=null, $dept_id=null,
		...
		if ($Aplic->profissional){
			...
			elseif ($objetivo_id) $sql->adOnde('evento_gestao_objetivo='.(int)$objetivo_id);
			elseif ($fator_id) $sql->adOnde('evento_gestao_fator='.(int)$fator_id);
			...
			elseif ($mswot_id) $sql->adOnde('evento_gestao_mswot='.(int)$mswot_id);
			...
			if ($objetivo_id || ($dept_id && $nada_selecionado)) $sql->esqUnir('objetivo', 'objetivo', 'objetivo_id = e.evento_objetivo');
			if ($pg_estrategia_id || ($dept_id && $nada_selecionado)) $sql->esqUnir('estrategias', 'estrategias', 'pg_estrategia_id = e.evento_estrategia');
			if ($fator_id && ($dept_id && $nada_selecionado)) $sql->esqUnir('fator', 'fator', 'fator_id = e.evento_fator');
			...
			if ($dept_id && $nada_selecionado){
				...
				$sql->esqUnir('objetivo_dept', 'objetivo_dept', 'objetivo_dept_objetivo = evento_objetivo');
				$sql->esqUnir('fator_dept', 'fator_dept', 'fator_dept_fator = evento_fator');
				...
				$sql->adOnde('tarefa_depts.departamento_id IN ('.$dept_id.')
					...
					OR objetivo_dept_dept IN ('.$dept_id.')
			...
			if ($objetivo_id > 0) $sql->adOnde('e.evento_objetivo = '.(int)$objetivo_id);
			...
			if ($fator_id > 0) $sql->adOnde('e.evento_fator = '.(int)$fator_id);
	...
	function notificar($designados, $atualizar = false, $conflito = false) {
		...
		if ($this->evento_objetivo) {
			...
			$sql->adOnde('objetivo_id ='.(int)$this->evento_objetivo);
