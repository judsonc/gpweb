...
global $tab, $localidade_tipo_caract, $data, $dialogo, $ver_subordinadas, $ver_dept_subordinados;
if (!$dialogo) $Aplic->salvarPosicao();

$podeAdicionar = $Aplic->checarModulo('eventos', 'adicionar');
...
if (!$dialogo){
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar os eventos pelo campo de matriz SWOT relacionado.').'Campo SWOT:'.dicaF();
	...
	$procurar_filtro='<tr><td align=right>'.dica('Filtrar Eventos', 'Filtrar os eventos por uma das três opções:</br></br><li><b>Meus Eventos</b> - eventos em que sejas um dos endereçados, como reunião, '.$config['tarefas'].' e outras atividades.</li><li><b>Eventos que Eu Criei</b> - eventos que tenha inserido neste Sistema, com'.$config['genero_usuario'].' '.$config['usuario'].'.</li><li><b>Todos os Eventos</b> - Não será aplicado nenhum tipo de filtro.').'Eventos:'.dicaF().'</td><td>'.selecionaVetor($evento_filtro_lista, 'evento_filtro', 'onchange="document.env.submit()" class="texto" style="width:250px;"', $evento_filtro).'</td></tr>';
	
	}

if (!$dialogo&& $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Calendário Diário', 'calendario.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo_evento=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Novo Evento', 'Criar um novo evento.<br><br>Os eventos são atividades com data e hora específicas podendo estar relacionados com '.$config['projetos'].', '.$config['tarefas'].' e '.$config['usuarios'].' específicos.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'editar\'; env.submit();" >'.imagem('calendario_novo.png').'</a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr>' : '');

	$imprimir='<tr><td>'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir os eventos agendados para este dia.').'<a href="javascript: void(0);" onclick ="url_passar(1,\'m=calendario&a=ver_dia&tab='.$tab.'&dialogo=1&data='.$data.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$botoesTitulo->adicionaBotao('m=calendario&a=ver_semana&data='.$data, 'visão semanal ','','Visão Semanal','Visualizar a semana inteira.');


	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$procurar_filtro.$popFiltro.$filtros.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_evento.$imprimir.$icone_periodo_todo.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo&& $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Calendário Diário', 'calendario.png', $m, $m.'.'.$a);
	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.'<tr><td colspan=20>&nbsp;</td></tr></table>');
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0>'.$procurar_filtro.$popFiltro.$filtros.$icone_periodo_todo.'</table>');
	if ($podeAdicionar) $botoesTitulo->adicionaBotaoCelula('', 'env.a.value=\'editar\'; env.submit();', 'evento', '', 'Novo Evento', 'Criar um novo evento.<br><br>Os eventos são atividades com data e hora específicas podendo estar relacionados com '.$config['projetos'].', '.$config['tarefas'].' e '.$config['usuarios'].' específicos');
	$botoesTitulo->adicionaCelula(dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir os eventos agendados para este dia.').'<a href="javascript: void(0);" onclick ="url_passar(1,\'m=calendario&a=ver_dia&tab='.$tab.'&dialogo=1&data='.$data.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->adicionaBotao('m=calendario&a=ver_semana&data='.$data, 'visão semanal ','','Visão Semanal','Visualizar a semana inteira.');
	$botoesTitulo->mostrar();
	}
...
echo '<table class="std" width="100%" cellspacing=4 cellpadding=0>';
$titulo=dia_semana($este_dia->format('%A')).', '.$este_dia->format($df);
echo '<tr><td valign="top"><table border=0 cellspacing=0 cellpadding=0 width="100%" class="motitulo">';
echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_dia&data='.$dia_ant->format(FMT_TIMESTAMP_DATA).'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif'), 'Dia Anterior', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif')).' para exibir o dia anterior.').'</a></td><td width="100%" align=center><b>'.$titulo.'</b></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_dia&data='.$dia_prox->format(FMT_TIMESTAMP_DATA).'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif'), 'Próximo Dia', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif')).' para exibir o próximo dia.').'</a></td></tr>';
echo '</table>';


if (!$dialogo){
	$caixaTab = new CTabBox('m=calendario&a=ver_dia&data='.$este_dia->format(FMT_TIMESTAMP_DATA), '', $tab);
	$caixaTab->adicionar(BASE_DIR.'/modulos/calendario/ver_eventos_dia', 'Eventos',null,null,'Eventos','Visualizar os eventos marcados para este dia.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/email/ver_compromissos_dia', 'Compromissos',null,null,'Compromissos','Visualizar os compromissos marcados para este dia.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/calendario/ver_tarefas_dia', ucfirst($config['tarefas']),null,null,ucfirst($config['tarefas']),'Visualizar '.$config['genero_tarefa'].'s '.$config['tarefas'].' que devem estar sendo realizadas neste dia.<br><br>Será necessário selecionar de qual '.$config['usuario'].' desejará ver '.$config['genero_tarefa'].'s '.$config['tarefas'].' designadas.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/calendario/ver_acoes_dia', ucfirst($config['acoes']),null,null,ucfirst($config['acoes']),'Visualizar os '.$config['acoes'].' em execução neste dia.');
	if ($ata_ativo) $caixaTab->adicionar(BASE_DIR.'/modulos/calendario/ver_atas_dia', 'Atas de Reunião',null,null,'Atas de Reunião','Visualizar as ações de atas de reunião em execução neste dia.');
	$caixaTab->mostrar('','','','',true);
	}
else{
	if ($tab==0) include_once(BASE_DIR.'/modulos/calendario/ver_eventos_dia.php');
	elseif ($tab==1) include_once(BASE_DIR.'/modulos/email/ver_compromissos_dia.php');
	elseif ($tab==2) include_once(BASE_DIR.'/modulos/calendario/ver_tarefas_dia.php');
	elseif ($tab==3) include_once(BASE_DIR.'/modulos/calendario/ver_acoes_dia.php');
	elseif ($tab==4) include_once(BASE_DIR.'/modulos/calendario/ver_atas_dia.php');
	}

echo '</td>';

if (!$dialogo && $config['cal_ver_dia_mostrar_minical']) {
  echo '<td valign="top" width="175">';
	...
	$minutoiCal = new CCalendarioMes($este_dia);
	...
	$ultimaData->setTime(23, 59, 59);
	$links = array();
	if ($tab==0) $links=getEventoLinks($evento_filtro, $periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $cia_id, $dept_id, 
		$tarefa_id, 
		$projeto_id, 
		$pg_perspectiva_id, 
		$tema_id, 
		...
		$tr_id,
		$me_id);
	elseif ($tab==1) $links=getCompromissoLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $Aplic->usuario_id, $agenda_tipo_id);
	elseif ($tab==2) $links=getTarefaLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $cia_id, $dept_id, 
		$tarefa_id, 
		$projeto_id, 
		$pg_perspectiva_id, 
		$tema_id, 
		...
		$tr_id,
		$me_id);
	elseif ($tab==3) $links=getAcaoLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $cia_id, $dept_id, 
		$tarefa_id, 
		$projeto_id, 
		$pg_perspectiva_id, 
		$tema_id, 
		...
		$tr_id,
		$me_id);
	elseif ($tab==4) $links=getAtaLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $cia_id, $dept_id, 
		$tarefa_id, 
		$projeto_id, 
		$pg_perspectiva_id, 
		$tema_id, 
...
function clicarDia( dataInicio, dataFim ) {
	url_passar(0, 'm=calendario&a=ver_dia&data='+dataInicio+'&tab=0');
	}
