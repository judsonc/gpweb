...
for ($i = 0; $i < 7; $i++) {
	...
	echo '<td class="'.$classe.'" style="width:14.29%;" ondblclick="url_passar(0, \'m=calendario&a=editar&data='.$diaFormato.'&evento_projeto='.$projeto_id.'&evento_pratica='.$pratica_id.'&evento_indicador='.$pratica_indicador_id.'&evento_acao='.$plano_acao_id.'&evento_calendario='.$calendario_id.'&evento_objetivo='.$objetivo_id.'&evento_tema='.$tema_id.'&evento_estrategia='.$pg_estrategia_id.'&evento_meta='.$pg_meta_id.'&evento_fator='.$fator_id.'&cia_id='.$cia_id.'&dept_id='.$dept_id.'\');">';
