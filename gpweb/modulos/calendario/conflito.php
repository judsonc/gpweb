...
if (getParam($_REQUEST, 'aceitar', null)){
	...
	if ($Aplic->profissional && getParam($_REQUEST, 'uuid', null)){
		...
		elseif ($linha['evento_gestao_objetivo'] && $qnt==1) $endereco='m=praticas&a=obj_estrategico_ver&objetivo_id='.$linha['evento_gestao_objetivo'];
		elseif ($linha['evento_gestao_fator'] && $qnt==1) $endereco='m=praticas&a=fator_ver&fator_id='.$linha['evento_gestao_fator'];
		...
		elseif ($linha['evento_gestao_instrumento'] && $qnt==1) $endereco='m=instrumento&a=instrumento_ver&instrumento_id='.$linha['evento_gestao_instrumento'];
		...
		$Aplic->redirecionar('m=calendario&projeto_id='.$obj->evento_projeto.'&pratica_indicador_id='.$obj->evento_indicador.'&pratica_id='.$obj->evento_pratica.'&evento_calendario='.$obj->evento_calendario.'&tema_id='.$obj->evento_tema.'&objetivo_id='.$obj->evento_objetivo.'&pg_estrategia_id='.$obj->evento_estrategia.'&plano_acao_id='.$obj->evento_acao.'&fator_id='.$obj->evento_fator.'&pg_meta_id='.$obj->evento_meta);
		exit();
		}
	}
