...
if ($exibir['inicio']) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&ordemPor=projeto_data_inicio&ordemDir='.$ordemDir.($projeto_expandido ? '&projeto_expandido='.$projeto_expandido : '').'\');" class="hdr">'.dica('Início Previsto', 'Clique para ordenar os portfolios pela data de início previsto.').($ordenarPor=='projeto_data_inicio' ? imagem('icones/'.$seta[$ordemDir]) : '').'Início Previsto'.dicaF().'</a></th>';
if ($exibir['termino']) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&ordemPor=projeto_data_fim&ordemDir='.$ordemDir.($projeto_expandido ? '&projeto_expandido='.$projeto_expandido : '').'\');" class="hdr">'.dica('Término Previsto', 'Clique para ordenar os portfolios pela data de término previsto.').($ordenarPor=='projeto_data_fim' ? imagem('icones/'.$seta[$ordemDir]) : '').'Término Previsto'.dicaF().'</a></th>';
if ($exibir['provavel_inicio']) echo '<th nowrap="nowrap">'.dica('Início', 'Data de início baseada na data inicial d'.$config['genero_tarefa'].' primeir'.$config['genero_tarefa'].' '.$config['tarefa'].' do portfólio.').'Início'.dicaF().'</th>';
if ($exibir['provavel_termino']) echo '<th nowrap="nowrap">'.dica('Término', 'Data de término baseada na data final d'.$config['genero_tarefa'].' últim'.$config['genero_tarefa'].' '.$config['tarefa'].' do portfólio.').'Término'.dicaF().'</th>';
...
if ($exibir['recursos']) echo '<th nowrap="nowrap">'.dica('Recursos Financeiro', 'Somatório dos recursos financeiros alocados n'.$config['genero_projeto'].'s '.$config['projetos'].'.').'Recursos'.dicaF().'</th>';
...
foreach ($projetos as $linha){
	if ($Aplic->usuario_super_admin || permiteAcessar($linha['projeto_acesso'], $linha['projeto_id'])){
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['projeto_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
