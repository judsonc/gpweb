...
if ($projeto_id){
    ...
    if (!$Aplic->profissional){
        ...
        echo '<td id="ver_agil">'.botao('gantt interativo', 'Gantt Interativo', 'Abre a interface de criação e edição dest'.($config['genero_projeto']=='o'? 'e' : 'a').' '.$config['projeto'].' com foco no planejamento do físico.','','ver_agil();').'</td>';
        ...
        if ($podeEditar) {
	        ...
	        if (!$portfolio) {
	            ...
	            if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado a '.($config['genero_tarefa']=='o' ? 'este' : 'esta').' '.$config['tarefa'].'.').'Fórum'.dicaF(), "javascript: void(0);' onclick='adicionarForum();");
