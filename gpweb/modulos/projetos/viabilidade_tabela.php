<?php
global $estilo_interface, $dialogo, $tab, $cia_id, $dept_id, $lista_depts, $lista_cias, $pesquisar_texto, $podeEditar;
...
if ($tab!=0){
	...
	$sql->adCampo('projeto_viabilidade.projeto_viabilidade_id, projeto_viabilidade_nome, projeto_viabilidade_responsavel, projeto_viabilidade_acesso, projeto_viabilidade_cor, projeto_viabilidade_necessidade, projeto_viabilidade_aprovado');
...
if ($tab!=0) $sql->adGrupo('projeto_viabilidade.projeto_viabilidade_id');
else $sql->adGrupo('demandas.demanda_id');
...
if ($tab!=0) {
	...
	if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=projeto_viabilidade_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='projeto_viabilidade_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se o estudo de viabilidade foi aprovado.').'Aprovado'.dicaF().'</a></th>';
...
foreach ($projeto_viabilidade as $linha) {
	...
	if ($tab!=0) {
		...
		if (permiteAcessarViabilidade($linha['projeto_viabilidade_acesso'],$linha['projeto_viabilidade_id'])){
			...
			if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['projeto_viabilidade_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		if (permiteAcessarDemanda($linha['demanda_acesso'],$linha['demanda_id'])){
			...
			if (!$dialogo) echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar a viabilidade.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=demanda_editar&demanda_id='.$linha['demanda_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
