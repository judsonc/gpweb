<?php
global $estilo_interface, $dialogo, $tab, $cia_id, $dept_id, $lista_cias, $pesquisar_texto, $licaostatus, $licaocategoria, $licaotipo, $usuario_id, $projeto_id, $lista_depts, $data_inicio, $data_fim, $filtro_extra_lista, $usar_periodo, $favorito_id,  $podeEditar, $filtro_prioridade_licao;
...
$sql->adTabela('licao');
$sql->adCampo('licao.licao_id, licao_nome, licao_projeto, licao_responsavel, licao_acesso, licao_cor, licao_categoria, licao_tipo, licao_aprovado');
if ($pesquisar_texto) $sql->adOnde('licao_nome LIKE \'%'.$pesquisar_texto.'%\' OR licao_ocorrencia LIKE \'%'.$pesquisar_texto.'%\' OR licao_consequencia LIKE \'%'.$pesquisar_texto.'%\' OR licao_acao_tomada LIKE \'%'.$pesquisar_texto.'%\' OR licao_aprendizado LIKE \'%'.$pesquisar_texto.'%\'');
if ($usar_periodo) {
	$sql->adOnde('licao_data_final >=\''.$data_inicio->format("%Y-%m-%d").'\'');
	$sql->adOnde('licao_data_final <=\''.$data_fim->format("%Y-%m-%d").'\'');
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=licao_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='licao_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_licao) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($licao as $linha) {
	...
	if (permiteAcessarLicao($linha['licao_acesso'],$linha['licao_id'])){
		...
		echo '<tr>';
		if (!$dialogo) echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar a lição aprendida.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=licao_editar&licao_id='.$linha['licao_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['licao_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_licao) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
