...
$sql->adOnde('favorito_trava_demanda=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('me_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('me_favorito') !== null ? $Aplic->getEstado('me_favorito') : 0;
...
if (count($vetor_favoritos)) {
	...
	$favoritos='<tr><td align="right" nowrap="nowrap">'.dica('Favoritos', 'Escolha um favorito para mostrar '.$config['genero_me'].'s '.$config['me'].' pertencentes.').'Favoritos:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($vetor_favoritos, 'favorito_id', 'class="texto" style="width:250px;" onchange="document.env.submit()"', $favorito_id).'</td></tr>';
	}
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Demandas', 'demanda.gif', $m, $m.'.'.$a);
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar as atas de reunião pelo campo de matriz SWOT que estão relacionadas.').'Campo SWOT:'.dicaF();
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito='<tr><td nowrap="nowrap" align="right">'.dica('Favoritos', 'Criar ou editar um grupo de demandas favoritas, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&demanda=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_responsavel.$procurar_supervisor.$procurar_autoridade.$procurar_cliente.$procura_pesquisa.$procura_setor.$procura_segmento.$procura_intervencao.$procura_tipo_intervencao.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$botao_filtrar.$novo.$botao_prioridade.$imprimir.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Demandas</center></font>';
	}	
...
if ($config['mostrar_total']){
	$total1=' ('.(int)demandas_quantidade(0, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
	...
	$total2=' ('.(int)demandas_quantidade(1, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
	...
	$total3=' ('.(int)demandas_quantidade(2, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
	...
	$total4=' ('.(int)demandas_quantidade(3, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda,  $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
	...
	$patrocinador_id,
	$template_id).')';
	$total5=' ('.(int)demandas_quantidade(4, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
	...
	$total6=' ('.(int)demandas_quantidade(5, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id, 
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
