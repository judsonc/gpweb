<?php
global $filtro_prioridade_demanda, $estilo_interface, $dialogo, $tab, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $favorito_id, $podeEditar,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
...
$xtotalregistros=demandas_quantidade($tab, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
...
$sql->adCampo('demandas.demanda_id, demanda_nome, demanda_usuario, demanda_acesso, demanda_cor, demanda_identificacao, formatar_data(demanda_data, \'%d/%m/%Y\') AS data, demanda_aprovado');
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('demanda_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('demanda_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('demanda_gestao_mswot='.(int)$mswot_id);
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=demanda_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='demanda_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovada', 'Neste campo consta se a demanda foi aprovada.').'Aprovada'.dicaF().'</a></th>';

if ($filtro_prioridade_demanda) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
for ($j = 0; $j < count($demandas); $j++) {
	...
	if (permiteAcessarDemanda($linha['demanda_acesso'],$linha['demanda_id'])){
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['demanda_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		if ($Aplic->profissional){
			...
			if (count($lista)) {
				foreach($lista as $gestao_data){
					...
					elseif ($gestao_data['demanda_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['demanda_gestao_mswot']);
