...
function corpo_cabecalho(){
	global $obj, $baseline_id, $m, $a, $config, $projeto_id, $sql, $Aplic, $label_projeto, $objp, $dados_projeto, $portfolio_lista, $codigo, $moedas;
	...
	if ($custo_estimado > 0){
	  ...
	  $saida.= '<tr><th>Nome</th><th>Descrição</th><th>Unidade</th><th>Qnt.</th><th>Valor</th><th>ND</th><th>Total</th><th>Responsável</th></tr>';
	  $total=array();
	  $custo=array();
	  $tarefa=0;
	  $projeto_corrente = 0;
	    ...
	    $saida.= '<table width="100%">';
	      ...
	      $saida.= '<tr><td>';
	        $saida.= '<table border=0 cellpadding=0 cellspacing=0 class="tbl1">';
	          $saida.= '<tr style="text-align: center;"><th>&nbsp;Orçado&nbsp;</th><th>&nbsp;Realizado&nbsp;</th><th>&nbsp;Variação&nbsp;</th><th>&nbsp;%&nbsp;</th></tr>';
	...
	if (count($tarefas)){
	  ...
	  if($portfolio_lista){
          $src = BASE_URL.($Aplic->pdf_print ? '/pdfimg.php' : '/index.php').'?m=projetos&a=gantt&sem_cabecalho=1&portfolio='.$projeto_id.'&mostrarLegendas=0&ordenarTarefasPorNome=0&mostrarInativo=1&mostrarTodoGantt=0&width="100%"';
      }
	  else{
          $src = BASE_URL.($Aplic->pdf_print ? '/pdfimg.php' : '/index.php').'?m=tarefas&a=gantt&sem_cabecalho=1&mostrarLegendas=1&proFiltro=&mostrarInativo=1&mostrarTodoGantt=1&projeto_id='.$projeto_id.'&width="100%"';
      }
	...
	if (!$Aplic->profissional && $config['barra_projeto'] && $codigo) $saida.= '<tr><td><table width="100%"><tr><td align=center><img src="'.BASE_URL.'/lib/barras/barcode.php?quality=75&barcode='.$codigo.'\" alt=""></td></tr></table></td></tr>';
	$saida.= '</table>';
