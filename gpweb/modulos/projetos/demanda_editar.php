...
if ($demanda_id){
	...
	if (!$demanda_id && ($demanda_acao || $demanda_pratica  || $demanda_indicador || $demanda_objetivo || $demanda_tema || $demanda_estrategia)){
		...
		if ($demanda_objetivo) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
		...
		if ($demanda_fator) $sql->esqUnir('fator','fator','fator_cia=cias.cia_id');
		...
		elseif ($demanda_objetivo) $sql->adOnde('objetivo_id = '.$demanda_objetivo);
		...
		elseif ($demanda_fator) $sql->adOnde('fator_id = '.$demanda_fator);
...
$botoesTitulo = new CBlocoTitulo(($demanda_id ? 'Editar Demanda' : 'Criar Demanda'), 'demanda.gif', $m, $m.'.'.$a);
...
echo '<tr><td align="right">'.dica('Nome', 'Toda demanda necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="demanda_nome" value="'.$obj->demanda_nome.'" style="width:284px;" class="texto" /> *</td></tr>';
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores relacionados mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'demanda_principal_indicador', 'class="texto" style="width:284px;"', $obj->demanda_principal_indicador).'</td></tr>';
	}
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'demanda_moeda', 'class=texto size=1', ($obj->demanda_moeda ? $obj->demanda_moeda : 1)).'</td></tr>';
	}	
else echo '<input type="hidden" name="demanda_moeda" id="demanda_moeda" value="'.($obj->demanda_moeda ? $obj->demanda_moeda : 1).'" />';
