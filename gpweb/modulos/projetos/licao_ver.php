...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_licao=".$licao_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lição Aprendida</center></font>';
	}
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada na meta.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->licao_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se '.$config['genero_licao'].' '.$config['licao'].' se encontra aprovad'.$config['genero_licao'].'.').'Aprovad'.$config['genero_licao'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->licao_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
