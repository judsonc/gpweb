...
$sql->adOnde('favorito_trava_licao=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('licao_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('licao_favorito') !== null ? $Aplic->getEstado('licao_favorito') : 0;

if (isset($_REQUEST['filtro_prioridade_licao']))	$Aplic->setEstado('filtro_prioridade_licao', getParam($_REQUEST, 'filtro_prioridade_licao', null));
$filtro_prioridade_licao = $Aplic->getEstado('filtro_prioridade_licao') !== null ? $Aplic->getEstado('filtro_prioridade_licao') : null;
...
if ($houve_filtro) $Aplic->setEstado('usar_periodo', getParam($_REQUEST, 'usar_periodo', null));
$usar_periodo = $Aplic->getEstado('usar_periodo') !== null ? $Aplic->getEstado('usar_periodo') : null;


if (isset($_REQUEST['licaoinicio']) || $houve_filtro)	$Aplic->setEstado('licaoinicio', getParam($_REQUEST, 'licaoinicio', null));
$licaoinicio = $Aplic->getEstado('licaoinicio') !== null ? $Aplic->getEstado('licaoinicio') : null;

if (isset($_REQUEST['licaofim']) || $houve_filtro)	$Aplic->setEstado('licaofim', getParam($_REQUEST, 'licaofim', null));
...
if (isset($_REQUEST['licaostatus']) || $houve_filtro)	$Aplic->setEstado('licaostatus', getParam($_REQUEST, 'licaostatus', null));
$licaostatus = $Aplic->getEstado('licaostatus') !== null ? $Aplic->getEstado('licaostatus') : ($houve_filtro ? null : -1);

if (isset($_REQUEST['licaocategoria']) || $houve_filtro)	$Aplic->setEstado('licaocategoria', getParam($_REQUEST, 'licaocategoria', null));
$licaocategoria = $Aplic->getEstado('licaocategoria') !== null ? $Aplic->getEstado('licaocategoria') : ($houve_filtro ? null : -1);

if (isset($_REQUEST['licaotipo']) || $houve_filtro)	$Aplic->setEstado('licaotipo', getParam($_REQUEST, 'licaotipo', null));
$licaotipo = $Aplic->getEstado('licaotipo') !== null ? $Aplic->getEstado('licaotipo') : ($houve_filtro ? null : -1);
...
echo '<input type="hidden" name="filtro_prioridade_licao" id="filtro_prioridade_licao" value="'.$filtro_prioridade_licao.'" />';
...
if (!$dialogo && $Aplic->profissional){
	...
	$botoesTitulo = new CBlocoTitulo('Lista de Lições Aprendidas', 'licoes.gif', $m, $m.'.'.$a);
  ...
  $btnCustomizados = '<tr><td nowrap="nowrap" align="right">'.dica('Filtar por campos customizados', 'Clique neste ícone '.imagem('custom_field_search.png').' para visualizar uma janela com filtros para campos customizados.').'<a href="javascript: void(0)" onclick="javascript:popFiltroCamposCustomizados();"><img src="'.acharImagem('custom_field_search.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	$botao_favorito='<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&licao=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	if($filtro_prioridade_licao) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$procurar_status.$procurar_categoria.$procurar_tipo.$procuraBuffer.$procura_projeto.$procurar_periodo.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0 align=right>'.$botao_filtrar.$nova_licao.$botao_prioridade.$imprimir.$btnCustomizados.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	if ($podeAdicionar) $botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nova Lição Aprendida', 'Criar um nova licão aprendida.').'<a class="botao" href="javascript: void(0)" onclick="javascript:env.a.value=\'licao_editar\'; env.submit();" ><span>nova</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Lições Aprendidas</center></font>';
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_licao.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_licao){
	env.filtro_prioridade_licao.value=filtro_prioridade_licao;
	env.submit();
	}	
</script>