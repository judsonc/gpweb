...
if (!$Aplic->profissional){
	...
	if (($Aplic->checarModulo('projetos', 'adicionar', $Aplic->usuario_id, 'abertura')) && $obj->projeto_viabilidade_viavel && !$demanda['demanda_termo_abertura']) $botoesTitulo->adicionaBotao('m=projetos&a=termo_abertura_editar&projeto_viabilidade_id='.$projeto_viabilidade_id.'&demanda_id='.$demanda['demanda_id'], 'elaborar&nbsp;termo&nbsp;de&nbsp;abertura','','Elaborar Termo de Abertura', 'Entrará na tela em que se elaborará o termo de abertura d'.$config['genero_projeto'].' '.$config['projeto'].'.');
...
if ($Aplic->profissional && !$dialogo){	
	...
	$bloquear=($obj->projeto_viabilidade_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);

	if (isset($assinar['assinatura_id']) && $assinar['assinatura_id'] && !$bloquear) $km->Add("acao","acao_assinar", ($assinar['assinatura_data'] ? dica('Mudar Assinatura', 'Entrará na tela em que se pode mudar a assinatura no estudo de viabilidade.').'Mudar Assinatura'.dicaF() : dica('Assinar', 'Entrará na tela em que se pode assinar o estudo de viabilidade.').'Assinar'.dicaF()), "javascript: void(0);' onclick='url_passar(0, \"m=sistema&u=assinatura&a=assinatura_assinar&projeto_viabilidade_id=".$projeto_viabilidade_id."\");"); 
	
	if ($podeEditar && ($Aplic->checarModulo('projetos', 'aprovar', $Aplic->usuario_id, 'viabilidade') || $Aplic->usuario_super_admin)) {
		if (!$bloquear) $km->Add("acao","acao_editar",dica('Editar Estudo de Viabilidade','Editar os detalhes deste estudo de viabilidade.').'Editar Estudos de Viabilidade'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=viabilidade_editar&projeto_viabilidade_id=".$projeto_viabilidade_id."\");");
		if ($podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir este estudo de viabilidade.').'Excluir Estudo de Viabilidade'.dicaF(), "javascript: void(0);' onclick='excluir()");
		}
	...
	$km->Add("acao_imprimir","acao_imprimir1",dica('Estudo de Viabilidade', 'Visualize o estudo de viabilidade.').'Estudo de Viabilidade.'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=projetos&a=viabilidade_imprimir&dialogo=1&projeto_viabilidade_id=".$projeto_viabilidade_id."\");");	
	$km->Add("acao_imprimir","acao_imprimir2",dica('Detalhes deste Estudo de Viabilidade', 'Visualize os detalhes deste estudo de viabilidade.').'Detalhes deste Estudo de Viabilidade'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=".$m."&a=".$a."&dialogo=1&projeto_viabilidade_id=".$projeto_viabilidade_id."\");");
...
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o estudo de viabilidade se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->projeto_viabilidade_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
