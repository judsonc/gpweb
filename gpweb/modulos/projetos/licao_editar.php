...
echo '<input type="hidden" name="uuid" id="uuid" value="'.($licao_id ? null : uuid()).'" />';
...
echo '<tr><td align="right">'.dica('Nome da Lição Aprendida', 'Toda lição aprendida necessita ter um nome para identificação.').'*Nome:'.dicaF().'</td><td><input type="text" name="licao_nome" value="'.$obj->licao_nome.'" style="width:284px;" class="texto" /></td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'licao_moeda', 'class=texto size=1', ($obj->licao_moeda ? $obj->licao_moeda : 1)).'</td></tr>';
	}	
...
if ($Aplic->profissional) {
	echo '<tr><td colspan=2 align="center"><a href="javascript: void(0);" onclick="javascript:incluir_arquivo();">'.dica('Anexar arquivos','Clique neste link para anexar um arquivo.<br>Caso necessite anexar múltiplos arquivos basta clicar aqui sucessivamente para criar os campos necessários.').'<b>Anexar arquivos</b>'.dicaF().'</a></td></tr>';
