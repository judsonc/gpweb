...
$sql->adTabela('demanda_custo', 'demanda_custo');
$sql->esqUnir('demandas', 'demandas', 'demandas.demanda_id=t.demanda_custo_demanda');
$sql->adCampo('demanda_nome');
$sql->adCampo('demanda_custo.*,((demanda_custo_quantidade*demanda_custo_custo)*((100+demanda_custo_bdi)/100)) AS valor');
$sql->adOnde('demanda_custo.demanda_custo_demanda IN ('.$obj->demandas_subordinadas.')');
...
foreach ($linhas as $linha) {
	...
	echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['demanda_custo_moeda']].' '.number_format($linha['demanda_custo_custo'], 2, ',', '.').'</td>';
	if ($config['bdi']) echo '<td align="right">'.number_format($linha['demanda_custo_bdi'], 2, ',', '.').'</td>';

	echo '<td nowrap="nowrap">'.$nd.'</td>';
	echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['demanda_custo_moeda']].' '.number_format($linha['valor'], 2, ',', '.').'</td>';
	...
	if (isset($custo[$linha['demanda_custo_moeda']][$nd])) $custo[$linha['demanda_custo_moeda']][$nd] += (float)$linha['valor'];
	else $custo[$linha['demanda_custo_moeda']][$nd]=(float)$linha['valor'];
	
	if (isset($total[$linha['demanda_custo_moeda']])) $total[$linha['demanda_custo_moeda']]+=$linha['valor'];
	else $total[$linha['demanda_custo_moeda']]=$linha['valor']; 
