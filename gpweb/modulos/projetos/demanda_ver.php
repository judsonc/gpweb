...
if (!$dialogo && $Aplic->profissional){	
	...
	$bloquear=($obj->demanda_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_demanda=".$demanda_id."\");");
	...
	if ($editar && !$bloquear) $km->Add("acao","acao_editar",dica('Editar Demanda','Editar os detalhes desta demanda.').'Editar Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_id=".$demanda_id."\");");
	...
	if (isset($assinar['assinatura_id']) && $assinar['assinatura_id'] && !$bloquear) $km->Add("acao","acao_assinar", ($assinar['assinatura_data'] ? dica('Mudar Assinatura', 'Entrará na tela em que se pode mudar a assinatura na demanda.').'Mudar Assinatura'.dicaF() : dica('Assinar', 'Entrará na tela em que se pode assinar a demanda.').'Assinar'.dicaF()), "javascript: void(0);' onclick='url_passar(0, \"m=sistema&u=assinatura&a=assinatura_assinar&demanda_id=".$demanda_id."\");"); 
	if ($editar && $obj->demanda_aprovado && ($Aplic->checarModulo('projetos', 'aprovar', $Aplic->usuario_id, 'viabilidade') || $Aplic->usuario_super_admin) && $obj->demanda_caracteristica_projeto && !$obj->demanda_viabilidade) $km->Add("acao","acao_analisar",dica('Analisar Viabilidade', 'Entrará na tela em que se marcará se esta demanda é um'.($config['genero_projeto']=='o' ? '' : 'a').' '.$config['projeto'].' viável.').'Analisar Viabilidade'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=viabilidade_editar&demanda_id=".$demanda_id."\");");
	...
	if ($podeExcluir && $editar && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir esta demanda do sistema.').'Excluir Demanda'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->demanda_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Demanda</center></font>';
	}

$cor_indicador=cor_indicador('demanda', null, null, null, null, $obj->demanda_principal_indicador);
echo '<table id="tblObjetivos" cellpadding=0 cellspacing=1 '.(!$dialogo ? 'class="std" ' : '').' width="100%">';

echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->demanda_cor.'" colspan="2"><b>'.$obj->demanda_nome.'<b>'.$cor_indicador.'</td></tr>';
...
if ($Aplic->profissional){
	...
	if (count($lista)) {
		...
		foreach($lista as $gestao_data){
			...
			elseif (isset($gestao_data['demanda_gestao_mswot']) && $gestao_data['demanda_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['demanda_gestao_mswot']);
...
if ($Aplic->profissional && $exibir['moeda']) echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada na demanda.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->demanda_moeda].'</td></tr>';
	
		
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovada', 'Se a demanda se encontra aprovada.').'Aprovada:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->demanda_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


$acesso = getSisValor('NivelAcesso','','','sisvalor_id');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nível de Acesso', 'Pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido</b> - Todos podem ver, porem apenas o responsável e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante</b> - Somente o responsável e os designados podem ver e editar</li><li><b>Privado</b> - Somente o responsável e os designados podem ver, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.(isset($acesso[$obj->demanda_acesso]) ? $acesso[$obj->demanda_acesso] : '').'</td></tr>';
...
if (!$dialogo){
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('log_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('evento_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(evento_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=false;		
	...
	if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('arquivo_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(arquivo_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;		
	...
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('link_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(link_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=false;	
	...
	if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('forum_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(forum_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=false;		
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(pratica_indicador_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=false;		
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('plano_acao_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(plano_acao_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=false;		
	...
	if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('projeto_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		...
		$sql->adOnde('msg_gestao_demanda = '.(int)$demanda_id);
		$sql->adCampo('count(msg_gestao_demanda)');
		$existe=$sql->resultado();
		$sql->limpar();
		...
		if ($config['doc_interno']) {
			...
			$sql->adOnde('modelo_gestao_demanda = '.(int)$demanda_id);
			$sql->adCampo('count(modelo_gestao_demanda)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		...
		$sql->adOnde('ata_gestao_demanda = '.(int)$demanda_id);
		$sql->adCampo('count(ata_gestao_demanda)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		...
		$sql->adOnde('problema_gestao_demanda = '.(int)$demanda_id);
		$sql->adCampo('count(problema_gestao_demanda)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		...
		$sql->adOnde('risco_gestao_demanda = '.(int)$demanda_id);
		$sql->adCampo('count(risco_gestao_demanda)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
		...
		$sql->adOnde('risco_resposta_gestao_demanda = '.(int)$demanda_id.' OR risco_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		...
		$sql->adOnde('instrumento_gestao_demanda = '.(int)$demanda_id);
		$sql->adCampo('count(instrumento_gestao_demanda)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
