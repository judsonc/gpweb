...
if (!$dialogo && $Aplic->profissional){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Banco de Possíveis '.ucfirst($config['projetos']),'banco_projeto.gif', $m, $m.'.'.$a);
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$procuraBuffer = '<tr><td align=right>'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="bancotextobusca" onChange="document.frm_filtro.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=estrategia_lista&bancotextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';

	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0>'.$procurar_om.$procuraBuffer.'</table>');
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
...
$caixaTab->adicionar('banco_projeto_tabela', 'Sem Termo de Abertura',null,null,'Sem Ternmo de Abertura','Possíveis '.$config['projetos'].' que ainda não tem um termo de abertura criado.');
$caixaTab->adicionar('banco_projeto_tabela', 'Com Minuta do Termo de Abertura',null,null,'Com Minuta do Termo de Abertura','Possíveis '.$config['projetos'].' que já tem o termo de abertura criado, entretanto aguardam aprovação da autoridade competente.');
