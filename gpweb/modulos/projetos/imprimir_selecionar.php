...
echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_visao_geral&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Visão geral</a></td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if ($existe) {
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'&jquery=1&m=projetos&a=financeiro_pizza_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Estágios da despesa (empenho, liquidação e pagamento)</a></td></tr>';
		if ($config['projeto_siafi'] && $Aplic->modulo_ativo('financeiro')) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'&jquery=1&m=projetos&a=financeiro_siafi_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Estágios da despesa vs anexos d'.$config['genero_siafi'].' '.$config['siafi'].'</a></td></tr>';
		...
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_geral_tarefas_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Geral d'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_status_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Status d'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_inconsistencia_status_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Inconsistência do status d'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_cha_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Competências dos envolvidos n'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		}
	...
	echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_consolidacao&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Consolidação d'.$config['genero_projeto'].' '.$config['projeto'].'</a></td></tr>';
	
	echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_consolidacao2&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Consolidação extensa d'.$config['genero_projeto'].' '.$config['projeto'].'</a></td></tr>';
	...
	if ($existe) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=priorizacao_imprimir_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Priorização</a></td></tr>';
	}
...
if ($config['anexo_mpog']){
	...
	$linha1=$sql->linha();
	$sql->limpar();
	if ($linha1['demanda_id']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=demanda_imprimir&dialogo=1&demanda_id='.$linha1['demanda_id'].'\');">Documento de Oficialização da Demanda (DOD)</a></td></tr>';
	if ($linha1['demanda_caracteristica_projeto']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=demanda_analise_imprimirs&dialogo=1&demanda_id='.$linha1['demanda_id'].'\');">Mensuração do Tamanho d'.$config['genero_projeto'].' '.$config['projeto'].' (MTP)</a></td></tr>';
	if ($linha1['demanda_viabilidade']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=viabilidade_imprimirs&dialogo=1&projeto_viabilidade_id='.$linha1['demanda_viabilidade'].'\');">Análise de Viabilidade d'.$config['genero_projeto'].' '.$config['projeto'].' (AVP)</a></td></tr>';
	if ($linha1['demanda_termo_abertura']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=termo_abertura_imprimir&dialogo=1&projeto_abertura_id='.$linha1['demanda_termo_abertura'].'\');">Termo de Abertura d'.$config['genero_projeto'].' '.$config['projeto'].' (TAP)</a></td></tr>';
	...
	$linha3=$sql->linha();
	$sql->limpar();
	if ($linha3['projeto_qualidade_usuario']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=qualidade_imprimir&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Plano de Qualidade (PQ)</a></td></tr>';
	...
	$linha4=$sql->linha();
	$sql->limpar();
	if ($linha4['projeto_comunicacao_usuario']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=comunicacao_imprimir&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Plano de Comunicação (PC)</a></td></tr>';
	...
	$linha5=$sql->linha();
	$sql->limpar();
	if ($linha5['projeto_risco_usuario']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=risco_imprimir&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Plano de Gerenciamento de Riscos (PGR)</a></td></tr>';
	...
	$linha7=$sql->linha();
	$sql->limpar();
	if ($linha7['projeto_encerramento_responsavel']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=encerramento_imprimir&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Termo de Encerramento de Projeto (TEP)</a></td></tr>';
...
if ($Aplic->profissional) {
	...
	if ($config['anexo_eb']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=relatorios&a=index&relatorio_tipo=status_negapeb_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Situação dos documentos d'.$config['genero_anexo_eb_nome'].' '.$config['anexo_eb_nome'].'</a></td></tr>';
...
function ir_para(m, a, u){
	url_passar(0, 'm='+m+'&a='+a+'&u='+u+'&dialogo=1<?php echo "&baseline_id=".$baseline_id."&projeto_id=".$projeto_id ?>');
	}
...
function imprimir_ata_reuniao(){
	...
	var pdf=document.getElementById('pdf').checked;
	var url = 'm=projetos&a=ata_imprimir&dialogo=1&ata_id='+ elmId.value+'<?php echo '&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id?>';
	if(pdf) url += '&sem_cabecalho=1&pdf=1&page_orientation=P';
	url_passar(0, url);
	elmId.selectedIndex = 0;
	}	
