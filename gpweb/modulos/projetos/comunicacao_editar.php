...
$ttl = ($obj->projeto_comunicacao_usuario ? 'Editar Plano de Comunicação' : 'Criar Plano de Comunicação');
...
echo '<input type="hidden" name="fazerSQL" value="fazer_sql_comunicacao" />';
echo '<input type="hidden" name="dialogo" value="1" />';
echo '<input type="hidden" name="projeto_comunicacao_projeto" id="projeto_comunicacao_projeto" value="'.$projeto_id.'" />';
echo '<input type="hidden" name="projeto_comunicacao_data" id="projeto_comunicacao_data" value="'.date('Y-m-d H:i:s').'" />';
echo '<input type="hidden" name="salvar" value="" />';
echo '<input type="hidden" name="excluir" value="" />';

echo '<input type="hidden" name="antigo" value="'.($obj->projeto_comunicacao_usuario ? 1 : 0).'" />';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Descrição', 'Descrever de forma clara a justificativa como se processa o plano de comunicação.').'Descrição:'.dicaF().'</td><td><textarea data-gpweb-cmp="ckeditor" name="projeto_comunicacao_descricao" style="width:800px;" class="textarea">'.$obj->projeto_comunicacao_descricao.'</textarea></td></tr>';

echo '<tr><td align="right" nowrap="nowrap">'.dica('Responsável', 'O responsável pelo plano de comunicação.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="projeto_comunicacao_usuario" name="projeto_comunicacao_usuario" value="'.($obj->projeto_comunicacao_usuario ? $obj->projeto_comunicacao_usuario : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om(($obj->projeto_comunicacao_usuario ? $obj->projeto_comunicacao_usuario : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';


$campos_customizados = new CampoCustomizados('projeto_comunicacao', $projeto_id, 'editar');
$campos_customizados->imprimirHTML();

echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre o plano de comunicação.').'Notificar:'.dicaF().'</td>';
echo '<td>';
...
echo '<table><tr><td>';
if ($Aplic->ModuloAtivo('contatos') && $Aplic->checarModulo('contatos', 'acesso')) echo botao('outros contatos', 'Outros Contatos','Abrir uma caixa de diálogo onde poderá selecionar outras pessoas que serão informadas por e-mail sobre este plano de comunicacao.','','popEmailContatos()');
echo '</td><td>'.dica('Destinatários Extra', 'Preencha neste campo os e-mail, separados por vírgula, dos destinatários extras que serão avisados deste plano de comunicacao.').'Destinatários extra:'.dicaF().'<input type="text" class="texto" name="email_extras" maxlength="255" size="30" /></td></tr></table></td></tr>';
echo '<tr><td colspan="2" valign="bottom" align="right"></td></tr>';
echo '</td></table></td></tr>';


echo '<tr><td><table cellspacing=0 cellpadding=0 width="100%"><tr><td >'.botao('salvar', 'Salvar', 'Salvar os dados.','','env.submit();').'</td><td align="right">'.botao('cancelar', 'Cancelar', 'Cancelar a '.($obj->projeto_comunicacao_usuario ? 'edição' : 'criação').' do plano de comunicacao.','','if(confirm(\'Tem certeza que deseja cancelar?\')){url_passar(0, \''.$Aplic->getPosicao().'\');}').'</td></tr></table></td></tr>';
...
function popResponsavel(campo) {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["usuario"])?>', 500, 500, 'm=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setResponsavel&usuario_id='+document.getElementById('projeto_comunicacao_usuario').value, window.setResponsavel, window);
	else window.open('./index.php?m=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setResponsavel&usuario_id='+document.getElementById('projeto_comunicacao_usuario').value, '<?php echo ucfirst($config["usuario"])?>','height=500,width=500,resizable,scrollbars=yes, left=0, top=0');
	}

function setResponsavel(usuario_id, posto, nome, funcao, campo, nome_cia){
	document.getElementById('projeto_comunicacao_usuario').value=usuario_id;
...
function excluir() {
	if (confirm( "Tem certeza que deseja excluir este plano de comunicacao?")) {
		var f = document.env;
		f.excluir.value=1;
