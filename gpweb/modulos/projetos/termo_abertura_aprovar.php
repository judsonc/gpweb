...
$botoesTitulo = new CBlocoTitulo('Aprovar o Termo de Abertura', 'anexo_projeto.png', $m, $m.'.'.$a);
...
echo '<tr><td align="right" width=100>'.dica('Justificativa', 'Justificativa para a aprovação do termo de abertura.').'Justificativa:'.dicaF().'</td><td align=left><textarea data-gpweb-cmp="ckeditor" name="projeto_abertura_aprovacao" id="projeto_abertura_aprovacao"class="textarea"></textarea></td></tr>';
...
function	salvar_termo_abertura(){
	...
	if (confirm('Tem certeza que deseja aprovar este termo de abertura? Caso positivo será automaticamente criado um novo projeto.')) env.submit();
	}
</script>