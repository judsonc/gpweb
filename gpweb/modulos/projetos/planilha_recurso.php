...
echo '<tr><th>Nome</th><th width="110">Início</th><th width="110">Fim</th><th width="50">Duração</th><th width="80">Valor '.$config['simbolo_moeda'].'</th></tr>';
$soma=0;
$soma2=0;
$soma3=0;
$gasto2=array();
...
foreach($existe as $linha) {
		...
		if (count($gastos)) {
			...
			foreach ($gastos as $item) {
				...
				if ($ponto_relatorio_tipo=='data_custo' || $ponto_relatorio_tipo=='data_custo_arquivo'){
					...
					echo '<td width="10" nowrap="nowrap">'.($item['recurso_ponto_gasto_categoria_economica'] && $item['recurso_ponto_gasto_grupo_despesa'] && $item['recurso_ponto_gasto_modalidade_aplicacao'] ? $item['recurso_ponto_gasto_categoria_economica'].'.'.$item['recurso_ponto_gasto_grupo_despesa'].'.'.$item['recurso_ponto_gasto_modalidade_aplicacao'].'.' : '&nbsp;').$item['recurso_ponto_gasto_nd'].'</td>';
					...
					echo '<td width="10" nowrap="nowrap">'.($item['recurso_ponto_gasto_data_limite']? retorna_data($item['recurso_ponto_gasto_data_limite'],false) : '&nbsp;').'</td>';
					echo '</tr>';
					}
	...
	if ($ponto_relatorio_tipo=='data_custo_arquivo' || $ponto_relatorio_tipo=='data_custo_resumido_arquivo'){
		...
		$sql->adCampo('recurso_ponto_arquivo_id, recurso_ponto_arquivo_data, recurso_ponto_arquivo_ordem, recurso_ponto_arquivo_nome, recurso_ponto_arquivo_endereco');
