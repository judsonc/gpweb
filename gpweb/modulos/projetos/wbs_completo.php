...
$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_projeto.$procurar_usuario.'</table></td></tr></table>';
$saida.= '</div></div>';
...
function popProjeto2() {
  if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["projeto"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto&edicao=1&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, window.setProjeto2, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto2&edicao=1&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, '<?php echo ucfirst($config["projeto"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
