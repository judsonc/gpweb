...
if (!$dialogo){
  echo '<div id="container_detalhes_projeto">';
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_projeto=".$projeto_id."\");");
	...
	$bloquear=($obj->projeto_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($obj->projeto_principal_indicador) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'O indicador mais representativo da situação geral d'.($obj->projeto_portfolio ? $config['genero_portfolio'].' '.$config['portfolio'] : $config['genero_projeto'].' '.$config['projeto']).'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->projeto_principal_indicador).'</td></tr>';
  ...
  if (count($lista)){
  	...
  	if ($Aplic->profissional) require_once BASE_DIR.'/modulos/projetos/template_pro.class.php';
		...
		foreach($lista as $gestao_data){
			...
			elseif ($gestao_data['projeto_gestao_mswot']) echo ($usado++? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['projeto_gestao_mswot']);
			...
			elseif ($gestao_data['projeto_gestao_template']) echo ($usado++? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['projeto_gestao_template']);
...
if ($Aplic->profissional) echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada n'.($obj->projeto_portfolio ? $config['genero_portfolio'].' '.$config['portfolio'] : $config['genero_projeto'].' '.$config['projeto']).'.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->projeto_moeda].'</td></tr>';
...
if (!$dialogo){
	...
	if ($Aplic->profissional) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'tarefa_entrega','tarefa_entrega');
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefas','tarefas','tarefa_id=tarefa_entrega_tarefa');
		if ($baseline_id){
			$sql->adOnde('tarefa_entrega.baseline_id='.(int)$baseline_id);
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'evento_gestao','evento_gestao');
			if ($baseline_id)	$sql->adOnde('evento_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('evento_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(evento_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/praticas/ver_eventos', 'Eventos',null,null,'Eventos','Visualizar os eventos relacionados.');
		}
		
	if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'arquivo_gestao','arquivo_gestao');
			if ($baseline_id)	$sql->adOnde('arquivo_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('arquivo_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(arquivo_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/praticas/ver_arquivos', 'Arquivos',null,null,'Arquivos','Visualizar os arquivos relacionados.');
		}
	
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'link_gestao','link_gestao');
			if ($baseline_id)	$sql->adOnde('link_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('link_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(link_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/links/index_tabela', 'Links',null,null,'Links','Visualizar os links relacionados.');
		}
	
	if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'forum_gestao','forum_gestao');
			if ($baseline_id)	$sql->adOnde('forum_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('forum_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(forum_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/foruns/forum_tabela', 'Fóruns',null,null,'Fóruns','Visualizar os fóruns relacionados.');
		}
		
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'pratica_indicador_gestao','pratica_indicador_gestao');
			if ($baseline_id)	$sql->adOnde('pratica_indicador_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('pratica_indicador_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(pratica_indicador_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/praticas/indicadores_ver', 'Indicadores',null,null,'Indicadores','Visualizar os indicadores relacionados.');
		}
		
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'plano_acao_gestao','plano_acao_gestao');
			if ($baseline_id)	$sql->adOnde('plano_acao_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('plano_acao_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(plano_acao_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx', ucfirst($config['acoes']),null,null,ucfirst($config['acoes']),'Visualizar '.$config['genero_acao'].'s '.$config['acoes'].' relacionad'.$config['genero_acao'].'s.');
		}
		
	if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'msg_gestao','msg_gestao');
		if ($baseline_id)	$sql->adOnde('msg_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('msg_gestao_projeto = '.(int)$projeto_id);
		$sql->adCampo('count(msg_gestao_projeto)');
		$existe=$sql->resultado();
		$sql->limpar();
		...
		if ($config['doc_interno']) {
			$sql->adTabela(($baseline_id ? 'baseline_' : '').'modelo_gestao','modelo_gestao');
			if ($baseline_id)	$sql->adOnde('modelo_gestao.baseline_id='.(int)$baseline_id);
			$sql->adOnde('modelo_gestao_projeto = '.(int)$projeto_id);
			$sql->adCampo('count(modelo_gestao_projeto)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/email/ver_modelo_pro', 'Documentos',null,null,'Documentos','Os documentos relacionados.');
			}
		}
		
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'ata_gestao','ata_gestao');
		if ($baseline_id)	$sql->adOnde('ata_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('ata_gestao_projeto = '.(int)$projeto_id);
		$sql->adCampo('count(ata_gestao_projeto)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/atas/ata_tabela', 'Atas',null,null,'Atas','Visualizar as atas de reunião relacionadas.');
		}
		
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'problema_gestao','problema_gestao');
		if ($baseline_id)	$sql->adOnde('problema_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('problema_gestao_projeto = '.(int)$projeto_id);
		$sql->adCampo('count(problema_gestao_projeto)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/problema/problema_tabela', ucfirst($config['problemas']),null,null,ucfirst($config['problemas']),'Visualizar '.$config['genero_problema'].'s '.$config['problemas'].' relacionad'.$config['genero_problema'].'s.');
		}
		
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'risco_gestao','risco_gestao');
		if ($baseline_id)	$sql->adOnde('risco_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('risco_gestao_projeto = '.(int)$projeto_id);
		$sql->adCampo('count(risco_gestao_projeto)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/praticas/risco_pro_ver_idx', ucfirst($config['riscos']),null,null,ucfirst($config['riscos']),'Visualizar '.$config['genero_risco'].'s '.$config['riscos'].' relacionad'.$config['genero_risco'].'s.');
		
		
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'risco_resposta_gestao', 'risco_resposta_gestao');
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'risco_gestao','risco_gestao', 'risco_resposta_gestao_risco=risco_gestao_risco');
		if ($baseline_id)	$sql->adOnde('risco_resposta_gestao.baseline_id='.(int)$baseline_id);
		if ($baseline_id)	$sql->adOnde('risco_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('risco_resposta_gestao_projeto = '.(int)$projeto_id.' OR risco_gestao_projeto = '.(int)$projeto_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		$sql->adTabela(($baseline_id ? 'baseline_' : '').'instrumento_gestao','instrumento_gestao');
		if ($baseline_id)	$sql->adOnde('instrumento_gestao.baseline_id='.(int)$baseline_id);
		$sql->adOnde('instrumento_gestao_projeto = '.(int)$projeto_id);
		$sql->adCampo('count(instrumento_gestao_projeto)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', ucfirst($config['instrumentos']),null,null,ucfirst($config['instrumentos']),'Visualizar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' relacionad'.$config['genero_instrumento'].'s.');
		}
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('financeiro') && $Aplic->checarModulo('financeiro', 'acesso')) {
		...
		$sql->adOnde('financeiro_rel_nc_projeto '.($lista_projeto ? 'IN('.$lista_projeto.')' : '='.(int)$projeto_id));
		$sql->adCampo('count(financeiro_rel_nc_id)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/financeiro/financeiro_rel_nc', 'NC',null,null,'Notas de Crédito','Visualizar as notas de crédito relacionadas a '.($config['genero_projeto']=='a' ? 'esta ': 'este ').$config['projeto'].'.');
		
		$sql->adTabela('financeiro_rel_ne');
		$sql->adOnde('financeiro_rel_ne_projeto '.($lista_projeto ? 'IN('.$lista_projeto.')' : '='.(int)$projeto_id));
		$sql->adCampo('count(financeiro_rel_ne_id)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/financeiro/financeiro_rel_ne', 'NE',null,null,'Notas de Empenho','Visualizar as notas de empenho relacionadas a '.($config['genero_projeto']=='a' ? 'esta ': 'este ').$config['projeto'].'.');
		
		$sql->adTabela('financeiro_rel_ns');
		$sql->adOnde('financeiro_rel_ns_projeto '.($lista_projeto ? 'IN('.$lista_projeto.')' : '='.(int)$projeto_id));
		$sql->adCampo('count(financeiro_rel_ns_id)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/financeiro/financeiro_rel_ns', 'NS',null,null,'Notas de Sistema','Visualizar as notas de sistema relacionadas a '.($config['genero_projeto']=='a' ? 'esta ': 'este ').$config['projeto'].'.');
		
		$sql->adTabela('financeiro_rel_ob');
		$sql->adOnde('financeiro_rel_ob_projeto '.($lista_projeto ? 'IN('.$lista_projeto.')' : '='.(int)$projeto_id));
		$sql->adCampo('count(financeiro_rel_ob_id)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) $caixaTab->adicionar(BASE_DIR.'/modulos/financeiro/financeiro_rel_ob', 'OB',null,null,'Ordens Bancárias','Visualizar as ordens bancárias relacionadas a '.($config['genero_projeto']=='a' ? 'esta ': 'este ').$config['projeto'].'.');
		}
...
function imprimir(){
	var baseline_id = 0;
	if(document.getElementById('baseline_id')) baseline_id = document.getElementById('baseline_id').value;
	url_passar(Math.floor(Math.random()*1000), 'm=projetos&a=imprimir_selecionar&dialogo=1&baseline_id='+baseline_id+'&projeto_id='+document.getElementById('projeto_id').value);
	}
