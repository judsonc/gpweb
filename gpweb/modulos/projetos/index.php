...
if (count($vetor_favoritos)) {
	$favoritos='<tr><td align="right" nowrap="nowrap">'.dica('Favoritos', 'Escolha um favorito para mostrar '.$config['genero_projeto'].'s '.$config['projeto'].' pertencentes.').'Favoritos:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($vetor_favoritos, 'favorito_id', 'class="texto"'.($Aplic->profissional ? ' multiple' :'').' style="width:200px;"', $favorito_id).'</td></tr>';
	}
...
if (!$dialogo && $Aplic->profissional){
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo daMatriz SWOT', 'Filtrar as atas de reunião pelo campo de matriz SWOT que estão relacionadas.').'Campo SWOT:'.dicaF();
  ...
  $botao_custom_field=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="javascript:popFiltroCamposCustomizados();">'.imagem('icones/custom_field_search.png', 'Campo Customizado', 'Clique neste ícone '.imagem('icones/custom_field_search.png').' para filtrar '.$config['projetos'].' utilizando os campos customizados.').'</a></td></tr>' : '<tr><td></td></tr>');
	...
	$saida.='<td style="vertical-align:top;"><table cellspacing=0 cellpadding=0><tr><td valign=top><table cellspacing=3 cellpadding=0>'.$botao_filtrar.$botao_projeto.$botao_superiores.$botao_favorito.$botao_trava.$botao_gestao.$botao_imprimir.$botao_custom_field.'</table></td><td><table cellspacing=3 cellpadding=0>'.$botao_area.$botao_excel.$botao_mysql.$botao_pizza_geral.$botao_graficos.$botao_prioridade.$botao_pdf.$botao_campos.$vazio.'</table></td><td></td></tr></table></td>';
