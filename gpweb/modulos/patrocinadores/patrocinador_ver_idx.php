...
global $estilo_interface, $sql, $Aplic, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $usuario_id, $tab, $ordem, $ordenar, $dialogo, $favorito_id, $podeEditar, $filtro_prioridade_patrocinador,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('patrocinador_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('patrocinador_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('patrocinador_gestao_mswot='.(int)$mswot_id);
...
if ($filtro_prioridade_patrocinador){
	$sql->esqUnir('priorizacao', 'priorizacao', 'patrocinadores.patrocinador_id=priorizacao_patrocinador');
	$sql->adOnde('priorizacao_modelo IN ('.$filtro_prioridade_patrocinador.')');
	}


if ($favorito_id){
	$sql->internoUnir('favorito_lista', 'favorito_lista', 'patrocinadores.patrocinador_id=favorito_lista_campo');
...
$sql->adCampo('patrocinadores.patrocinador_id, patrocinador_nome, patrocinador_responsavel, patrocinador_acesso, patrocinador_cor, patrocinador_descricao, patrocinador_aprovado');

if ($filtro_prioridade_patrocinador){
	$sql->esqUnir('priorizacao', 'priorizacao', 'patrocinadores.patrocinador_id=priorizacao_patrocinador');
	if ($config['metodo_priorizacao']) $sql->adCampo('(SELECT round(exp(sum(log(coalesce(priorizacao_valor,1))))) FROM priorizacao WHERE priorizacao_patrocinador = patrocinadores.patrocinador_id AND priorizacao_modelo IN ('.$filtro_prioridade_patrocinador.')) AS priorizacao');
	else $sql->adCampo('(SELECT SUM(priorizacao_valor) FROM priorizacao WHERE priorizacao_patrocinador = patrocinadores.patrocinador_id AND priorizacao_modelo IN ('.$filtro_prioridade_patrocinador.')) AS priorizacao');
	$sql->adOnde('priorizacao_modelo IN ('.$filtro_prioridade_patrocinador.')');
	}
...
if ($Aplic->profissional) $sql->adCampo('(SELECT count(assinatura_id) FROM assinatura WHERE assinatura_aprova=1 AND assinatura_patrocinador=patrocinadores.patrocinador_id) AS tem_aprovacao');	
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=patrocinador_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='patrocinador_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_patrocinador) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
for ($i = 0; $i < count($patrocinadores); $i++) {
	...
	if (permiteAcessarPatrocinador($linha['patrocinador_acesso'],$linha['patrocinador_id'])){
		...
		if ($Aplic->profissional) $bloquear=($linha['patrocinador_aprovado'] && $config['trava_aprovacao'] && $linha['tem_aprovacao'] && !$Aplic->usuario_super_admin);
		else $bloquear=0;
		echo '<tr>';
		if (!$impressao && !$dialogo) echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar Patrocinador', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o patrocinador.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=patrocinador_editar&patrocinador_id='.$linha['patrocinador_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['patrocinador_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_patrocinador) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
