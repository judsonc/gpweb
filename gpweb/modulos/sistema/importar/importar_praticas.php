...
if ($importar){
    ...
    if(!$abortar){
        ...
        if ($lista_plano_gestao){		
            ...
            $sql->adOnde('plano_gestao_objetivo_plano_gestao IN ('.$lista_plano_gestao.')');
            $sql->exec();
            $sql->limpar(); 
...
if (!$abortar && $importar_parte1){
  ...
  while ($buffer !=FALSE) {
	...
	if (isset($lista_tabelas['objetivo'])){
		foreach($lista_tabelas['objetivo'] as $linha_tabela){
			$sql->adTabela('objetivo');
			foreach($linha_tabela as $chave => $valor) if ($chave!='objetivo_id') $sql->adInserir($chave, $valor);
			$sql->sem_chave_estrangeira();
			$sql->exec();
			$objetivo_id=$bd->Insert_ID('objetivo','objetivo_id');
			$novo_obj_estrategico[$linha_tabela['objetivo_id']]=$objetivo_id;
			$sql->limpar();	
			}
		}
	...
	importar_tabela('fator', 'fator_id', 'fator_pg_id','fator_usuario');	
