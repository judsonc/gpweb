...
$botoesTitulo = new CBlocoTitulo('Importar do Web2Project/DotProject', 'importar.jpg', $m, "$m.$a");
...
if (!$nomeBd){
	...
	echo '<tr><td align="right" width="300"  align="right">'.dica('Sistema à Importar', 'Escolha de qual sistema deseja irá importar os dados.').'Sistema à importar os dados:'.dicaF().'</td><td align="left"><select class="texto" name="banco" size="1" style="width:202px;"><option value="dotproject" >Web2Project/DotProject</option></select></td></tr>';
	echo '<tr><td align="right">'.dica('Endereço do Servidor Hospedeiro do Banco de Dados', 'Caso o '.$config['gpweb'].' (páginas PHP) esteja instalado na mesma máquina onde esteja o banco de dados MySQL provavelmente o endereço seja 127.0.0.1 (localhost).').'Endereço do Servidor(Host) do Banco de Dados:'.dicaF().'</td><td align="left"><input type="text" class="texto" style="width:200px" name="hospedadoBd" value="'.$config['hospedadoBd'].'" /></td></tr>';
	echo '<tr><td align="right">'.dica('Prefixo das Tabelas', 'Caso o haja um prefixo nas tabelas deverá ser prenchido<br>Exemplo:<br>Padrão dotproject: companies<br>Atual: dotp_companies<br>Prefixo: dotp_').'Prefixo das Tabelas:'.dicaF().'</td><td align="left"><input type="text" class="texto" style="width:200px" name="prefixo" value="" /></td></tr>';
	echo '<tr><td align="right">'.dica('Nome do Banco de Dados', 'Nome da base de dados que conterá todas as tabelas do '.$config['gpweb'].'.').'Nome do Banco de Dados:'.dicaF().'</td><td align="left"><input type="text" name="nomeBd" value="dotproject" class="texto" style="width:200px" /></td></tr>';
	echo '<tr><td align="right">'.dica('Login do Administrador', 'Nome do administrador configurado no servidor.<br> Por <i>default</i> tem o nome <b>root</b>.').'Login do administrador do SGDB:'.dicaF().'</td><td align="left"><input type="text" class="texto" style="width:200px" name="usuarioBd" value="'.$config['usuarioBd'].'" /></td></tr>';
	echo '<tr><td align="right">'.dica('Senha do Administrador', 'Senha utilizada pelo administrador para acessar o SGDB.').'Senha do Administrador do SGDB:'.dicaF().'</td><td align="left"><table cellpadding=0 cellspacing=0><tr><td><input type="password" name="senhaBd" class="texto" style="width:200px" value="'.$config['senhaBd'].'" /></td><td>'.botao('testar&nbsp;conexão', 'Testar Conexão', 'Testar a conexão configurada para acessar a base de dados.','','testar();').'</td></tr></table></td></tr>';
	echo '<tr><td align="right">'.dica('Número de linhas à Importar de Cada Vez', 'Defina o número de linhas a serem importadas de cada vez.<br><br>Caso escolha um número muito elevado há uma chance de o servidor interromper a importação por ter estourado o limite de tempo (em média 5 minutos).').'Número de linhas à importar de cada vez:'.dicaF().'</td><td align="left"><input type="text" name="linhas" class="texto" style="width:200px" value="500" /></td></tr>';
	echo '<tr><td align="right">'.dica('Perfil de Acesso', 'Defina qual o perfil de acesso d'.$config['genero_usuario'].'s '.$config['usuarios'].' importados.').'Perfil de acesso d'.$config['genero_usuario'].'s '.$config['usuarios'].':'.dicaF().'</td><td align="left">'.selecionaVetor($perfis_arr, 'usuario_perfil', 'style="width:202px;" size="1" class="texto"', '').'</td></tr>';
	...
	try {
	  $db = new PDO('mysql:dbname='.$nomeBd.';host='.$hospedadoBd, $usuarioBd, $senhaBd);
		...
		echo '<tr><td align="left">'.botao('retornar', 'Retornar', 'Clique neste botão para retornar aos parâmetros.','','env.nomeBd.value=\'\'; env.submit();').'</td></tr>';
...
//importar dotproject
	...
	if ($tabela=='files' && $ok){
		...
		if (!$feito){
			...
			if (!$sql->exec()) echo ('Não foi possível excluir os dados da tabela arquivo!');
			$sql->limpar();
			}
		if ($quantidade <= $feito){
		...
		foreach ($lista as $linha) {
			...
			$sql->sem_chave_estrangeira();
			if (!$sql->exec()) echo ('Não foi possível inserir na tabela arquivo!');
			$sql->limpar();
			}
		echo '<script>env.feito.value='.($feito+$linhas).'; env.historico.value="'.$historico.'"; env.submit();</script>';
		}
	...
	if ($tabela=='fim' && $ok){
		...
		echo estiloTopoCaixa();
		echo '<table class="std" width="100%" border=0 cellpadding=0 cellspacing="2">';	
		echo '<tr><td>Copie e cole as pastas dentro de files no dotproject (dotproject/files) para a pasta arquivos/projetos no '.$config['gpweb'].' (server/arquivos/projetos)</td></tr>';
		echo '</table>';	
