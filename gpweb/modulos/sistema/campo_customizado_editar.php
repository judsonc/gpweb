...
echo '<tr><td align="right" width="100">'.dica ('Identificação','Cada campo deverá ter um nome distinto e não pode conter espaços entre as palavras.').'Identificação:'.dicaF().'</td><td><input type="text" class="texto" name="campo_nome" id="campo_nome" maxlength="100" value="'.$campo_nome.'" onblur=\'this.value=this.value.replace(/[^a-z|^A-Z|^0-9]*/gi,"");\' /></td></tr>';
...
function adicionarItemSelecionado() {
	if (env.campo_nome.value.length < 1) {
		alert('Escreva um Identificação válida');
		env.campo_nome.focus();
		}
	else env.submit();
	}
