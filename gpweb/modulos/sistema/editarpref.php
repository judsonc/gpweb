...
echo '<input type="hidden" name="usuario" id="usuario" value="'.$usuario_id.'" />';
...
function popAssinatura(){
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Assinatura', 500, 500, 'm=admin&a=assinatura&dialogo=1&usuario_id='+document.getElementById('usuario').value, null, window);
	else window.open('./index.php?m=admin&a=assinatura&dialogo=1&usuario_id='+document.getElementById('usuario').value, 'Assinatura','left=0,top=0,height=350,width=600, scrollbars=yes, resizable');
	}	
