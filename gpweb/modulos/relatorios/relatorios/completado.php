...
$data = new CData();	
$titulo=ucfirst($config['tarefas']).' concluíd'.$config['genero_tarefa'].'s nos últimos sete dias'.($projeto_id  ? ' n'.$config['genero_projeto'].' '.$config['projeto'].' '.$mostrarNomeProjeto : ' em tod'.$config['genero_projeto'].'s '.$config['genero_projeto'].'s '.$config['projetos']).' - '.$data->format($df);
if (!$dialogo){
...
$sql->adOnde('tarefa_fim BETWEEN adiciona_data(now(), -7, \'DAY\') AND  now()');
