...
$titulo = 'Horas atribuídas '.($usuario_id ? 'ao '.link_usuario($usuario_id) : 'a todos '.$config['genero_usuario'].'s '.$config['usuarios'].' ').($grupo=='designado' ? ' designado'.($usuario_id ? '':'s') : ' responsáve'.($usuario_id ? 'l':'is')).($projeto_id ? ' n'.$config['genero_projeto'].' '.$config['projeto'].' '.nome_projeto($projeto_id) : ' em tod'.$config['genero_projeto'].'s '.$config['genero_projeto'].'s '.$config['projetos'].'.');
if (!$dialogo){
...
echo '<table cellspacing=2 cellpadding=0 border=0 width="100%" '.(!$dialogo ? 'class="std"' : '').'>';
if (!$dialogo){ 
...
if ($fazer_relatorio || $dialogo) {
	if ($grupo=='designado' && $usuario_id) echo '<tr><td colspan=20 align=center><h2>Designado para '.$config['genero_tarefa'].'s '.$config['tarefas'].': '.link_usuario($usuario_id).'</h2></td></tr>';
	elseif ($usuario_id) echo '<tr><td colspan=20 align=center><h2>Responsável pel'.$config['genero_tarefa'].'s '.$config['tarefas'].': '.link_usuario($usuario_id).'</h2></td></tr>';
	...
	if ($grupo=='designado') $sql->adCampo('td.perc_designado, td.usuario_id');
	$sql->adOrdem(($grupo=='designado' ? 'usuario_id' : 'tarefa_dono').', tarefa_projeto, tarefa_inicio');
	$lista=$sql->Lista();
	$sql->limpar();
	...
	echo '<tr><th>Usuario</th><th>Projeto</th><th>Tarefa</th><th>Horas</th></tr>';
	foreach($lista as $linha){
		echo '<tr>';
		
		if (($grupo=='designado' && $linha['usuario_id']!=$usuario) || (($grupo!='designado' && $linha['tarefa_dono']!=$usuario))){
			
			if ($usuario) {
				echo '<tr><td align=center colspan='.($usuario_id ? 2 : 3).'><b>Total</b></td><td align=right><b>'.$soma_horas.'</b></td></tr>';
				$soma_horas=0;
				}
			
			
			echo '<td>'.($grupo=='designado' ? link_usuario($linha['usuario_id']) : link_usuario($linha['tarefa_dono'])).'</td>';
			$usuario=($grupo=='designado' ? $linha['usuario_id'] : $linha['tarefa_dono']);
			}
		else echo '<td>'.($grupo=='designado' ? nome_usuario($linha['usuario_id']) : nome_usuario($linha['tarefa_dono'])).'</td>';
		...
		echo '<td align=right>'.($grupo=='designado' ? (int)($linha['tarefa_duracao']*$linha['perc_designado']/100) : (int)$linha['tarefa_duracao']).'</td>';
		echo'</tr>';
		$soma_horas+=($grupo=='designado' ? (int)($linha['tarefa_duracao']*$linha['perc_designado']/100) : (int)$linha['tarefa_duracao']);
		}
	
	if (count($lista)) {
		echo '<tr><td align=center colspan=3><b>Total</b></td><td align=right><b>'.$soma_horas.'</b></td></tr>';
		}
