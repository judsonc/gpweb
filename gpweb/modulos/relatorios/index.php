...
if (isset($_REQUEST['filtro_me']))	$Aplic->setEstado('filtro_me', getParam($_REQUEST, 'filtro_me', null));
...
if (count($vetor_favoritos)) {
	...
	$favoritos='<tr><td align="right" nowrap="nowrap">'.dica('Favorito', 'Escolha um favorito.').'Favorito:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($vetor_favoritos, 'favorito_id', 'class="texto"'.($Aplic->profissional ? ' multiple' :'').' style="width:200px;"', $favorito_id).'</td></tr>';
	}
...
if ($relatorio_tipo) {
	...
	//relatórios manuais
		if ($Aplic->profissional){
			...
			if ($config['projeto_siafi'] && $Aplic->modulo_ativo('financeiro')) echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=relatorios&a=index&projeto_id='.$projeto_id.'&relatorio_tipo=financeiro_siafi_pro\');">'.dica('Estágios da Despesa vs Anexos d'.$config['genero_siafi'].' '.$config['siafi'],'Planilha correlacionando as fases da despesa com os arquivos anexados de busca n'.$config['genero_siafi'].' '.$config['siafi'].'.').'Estágios da despesa vs anexos d'.$config['genero_siafi'].' '.$config['siafi'].'</a></td><td>Planilha correlacionando as fases da despesa com os arquivos anexados de busca n'.$config['genero_siafi'].' '.$config['siafi'].'</td></tr>';
			}
	echo '</table>';
