...
$sql->adOnde('favorito_trava_forum=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('forum_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('forum_favorito') !== null ? $Aplic->getEstado('forum_favorito') : 0;
...
if (!$dialogo){
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar os fóruns pelo campo de matriz SWOT que estão relacionados.').'Campo SWOT:'.dicaF();
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Fóruns', 'forum.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito='<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&forum=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_forum.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Fóruns</center></font>';
	}	
