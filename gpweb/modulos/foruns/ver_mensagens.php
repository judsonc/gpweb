...
echo '<td width="100%" align="right"><table><tr><td>'.botao('ordenar '.($ordenar == 'asc' ? 'ascendente' : 'descendente'), 'Ordenar '.($ordenar == 'asc' ? 'Ascendente' : 'Descendente'), 'Ordenar as respostas de forma '.($ordenar == 'asc' ? 'ascendente' : 'descendente'),'','url_passar(0, \'m=foruns&a=ver&forum_id='.$forum_id.'&mensagem_id='.$mensagem_id.'&ordenar='.$ordenar.'\');','','',0).'</td>';
if ($podeEditar && !$dialogo) echo '<td>'.botao('postar resposta', 'Postar Resposta', 'Acrescentar uma mensagem a este tópico.','','url_passar(0, \'m=foruns&a=ver&forum_id='.$forum_id.'&mensagem_superior='.$mensagem_id.'&postar_mensagem=1\');','','',0).'</td><td>'.botao('novo tópico', 'Novo Tópico', 'Criar um novo tópico dentro deste fórum.','','url_passar(0, \'m=foruns&a=ver&forum_id='.$forum_id.'&mensagem_id=0&postar_mensagem=1\');','','',0).'</td>';
echo '</tr></table></td>';
echo '</tr></table></td></tr>';
echo '<tr>';
