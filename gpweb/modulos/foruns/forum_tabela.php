...
global $Aplic, $filtro_prioridade_forum, $ver_subordinadas, $estilo_interface, $lista_cias, $lista_depts, $tab, $usuario_id, $cia_id, $dept_id, $dialogo, $favorito_id, $podeEditar,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('forum_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('forum_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('forum_gestao_mswot='.(int)$mswot_id);
	...
	if ($objetivo_id) $sql->adOnde('forum_objetivo = '.(int)$objetivo_id);
	...
	if ($fator_id) $sql->adOnde('forum_fator = '.(int)$fator_id);
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordemPor=forum_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordemPor=='forum_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_forum) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordemPor=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordemPor=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if (isset($linha['forum_id']) && $linha['forum_id'] && permiteAcessarForum($linha['forum_acesso'],  $linha['forum_id'])){
		...
		echo '<td nowrap="nowrap" align="center" width=16>';
				if ($editar && $podeEditar && !$bloquear) echo dica('Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' caso deseja editar este fórum.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=foruns&a=editar&forum_id='.$linha['forum_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF();
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['forum_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		if ($Aplic->profissional){
		  ...
		  if (count($lista)) {
				...
				foreach($lista as $gestao_data){
					...
					elseif ($gestao_data['forum_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['forum_gestao_mswot']);
					...
					elseif ($gestao_data['forum_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['forum_gestao_template']);
