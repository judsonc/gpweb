...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_forum='.(int)$forum_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->forum_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Fórum</center></font>';
	}
...
if (count($lista)) {
	...
	foreach($lista as $gestao_data){
		...
		elseif (isset($gestao_data['forum_gestao_mswot']) && $gestao_data['forum_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['forum_gestao_mswot']);
		...
		elseif (isset($gestao_data['forum_gestao_template']) && $gestao_data['forum_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['forum_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->forum_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o fórum se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->forum_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional) {
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('log_forum = '.(int)$forum_id);
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('evento_gestao_forum = '.(int)$forum_id);
			$sql->adCampo('count(evento_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('arquivo_gestao_forum = '.(int)$forum_id);
			$sql->adCampo('count(arquivo_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('link_gestao_forum = '.(int)$forum_id);
			$sql->adCampo('count(link_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(pratica_indicador_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('plano_acao_gestao_forum = '.(int)$forum_id);
			$sql->adCampo('count(plano_acao_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('projeto_gestao_forum = '.(int)$forum_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		...
		$sql->adOnde('msg_gestao_forum = '.(int)$forum_id);
		$sql->adCampo('count(msg_gestao_forum)');
		$existe=$sql->resultado();
		$sql->limpar();
		...
		if ($config['doc_interno']) {
			...
			$sql->adOnde('modelo_gestao_forum = '.(int)$forum_id);
			$sql->adCampo('count(modelo_gestao_forum)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		...
		$sql->adOnde('ata_gestao_forum = '.(int)$forum_id);
		$sql->adCampo('count(ata_gestao_forum)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		...
		$sql->adOnde('problema_gestao_forum = '.(int)$forum_id);
		$sql->adCampo('count(problema_gestao_forum)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		...
		$sql->adOnde('risco_gestao_forum = '.(int)$forum_id);
		$sql->adCampo('count(risco_gestao_forum)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
		...
		$sql->adOnde('risco_resposta_gestao_forum = '.(int)$forum_id.' OR risco_gestao_forum = '.(int)$forum_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		...
		$sql->adOnde('instrumento_gestao_forum = '.(int)$forum_id);
		$sql->adCampo('count(instrumento_gestao_forum)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
