...
if ($mostrarEvento){
	...
	$q->esqUnir('evento_usuarios','eu','eu.evento_id=eventos.evento_id');
...
$grafico->scale->actinfo->SetColTitles(array('Nome da '.$config['tarefa'], ucfirst($config['projeto']), '%','hr.',' Início ', 'Término'), array(70, 70, 10,10, 70, 70));
...
if (is_array($tarefas)) {
	foreach ($tarefas as $t) {
		...
		if ($t['tarefa_marco'] && $mostrarMarco) {
			...
			$barra = new MileStone($linha++, array($nome, $nomeProjeto, $t['tarefa_percentagem'], $t['tarefa_duracao'], $datainicio->format($df), ''), substr($inicio, 0, 10));
			$barra->title->SetFont(FF_FONT1, FS_NORMAL, 8);
			$barra->title->SetColor('#CC0000');
			} 

		if (!$t['tarefa_marco']){
			$barra = new GanttBar($linha++, array($nome, $nomeProjeto, $t['tarefa_percentagem'], $t['tarefa_duracao'], $datainicio->format($df), $datafim->format($df)), $inicio, $fim, $cap, 0.6);
