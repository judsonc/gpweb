...
if (!$usuario) {
	...
	if (!$dialogo && !$Aplic->profissional){
		...
		if ($podeAcessar && ($Aplic->usuario_admin || $Aplic->usuario_super_admin)) $botoesTitulo->adicionaBotao('m=admin', 'lista de '.$config['usuarios'],'','Lista de '.ucfirst($config['usuarios']),'Visualizar a lista de '.$config['usuarios'].' do Sistema.');
		if ($podeEditar || $usuario_id == $Aplic->usuario_id || $Aplic->usuario_admin || $Aplic->usuario_super_admin) {
			...
			if (!(isset($config['restrito']) && $config['restrito'])) {
				$botoesTitulo->adicionaBotaoDireita(dica('Mudar Senha', 'Mudar a senha de entrada no Sistema.').'<a style="float:right;" href="javascript: void(0);" class="botao" onclick="popMudarSenha();return false"><span>mudar senha</span></a>'.dicaF());
				$botoesTitulo->adicionaBotaoDireita(dica('Mudar Pergunta/Resposta', 'Mudar a pergunta/resposta para o caso de esquecer a senha.').'<a style="float:right;" href="javascript: void(0);" class="botao" onclick="popMudarPergunta();return false"><span>mudar pergunta/resposta</span></a>'.dicaF());
				}
			}
		if ($podeAdicionar && ($Aplic->usuario_admin || $Aplic->usuario_super_admin)) $botoesTitulo->adicionaBotaoDireita(dica('Adicionar '.ucfirst($config['usuario']), 'Adicionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].' no Sistema.').'<a style="float:right;" href="javascript: void(0);" class="botao" onclick="url_passar(0, \'m=admin&a=editar_usuario\');"><span>adicionar '.$config['usuario'].'</span></a>'.dicaF());
		$botoesTitulo->mostrar();
	...
	if ($dialogo) echo '<tr><td colspan=2 style="font-weight: bold; font-size: 150%">Detalhes do '.ucfirst($config['usuario']).'<br>&nbsp;</td></tr>';
		...
		if ($usuario['contato_tel']) echo '<tr><td nowrap="nowrap" align="right">'.dica('Telefone Comercial', 'O telefone comercial d'.$config['genero_usuario'].' '.$config['usuario'].', para se comunicar com o mesmo.').'Telefone Comercial:'.dicaF().'</td><td class="realce" style="text-align: justify;">('.$usuario['contato_dddtel'].')'.$usuario['contato_tel'].'</td></tr>';
		if ($usuario['contato_tel2']) echo '<tr><td nowrap="nowrap" align="right">'.dica('Telefone Residencial', 'O telefone residencial d'.$config['genero_usuario'].' '.$config['usuario'].', para se comunicar com o mesmo.').'Telefone Residencial:'.dicaF().'</td><td class="realce" >('.$usuario['contato_dddtel2'].')'.$usuario['contato_tel2'].'</td></tr>';
		...
		if ($usuario['usuario_assinatura']) echo '<tr><td align="right" nowrap="nowrap">'.dica('Assinatura', 'A assinatura d'.$config['genero_usuario'].' '.$config['usuario'].' para os documentos internos.').'Assinatura:'.dicaF().'</td><td class="realce"><img src="'.($config['url_arquivo'] ? $config['url_arquivo'] : BASE_URL).'/arquivos/assinaturas/'.$usuario['usuario_assinatura'].'" /></td></tr>';
	...
	else echo '<script type="text/javascript">self.print();</script>';	
	}
...
function popMudarSenha() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Mudar Senha', 400,300,'m=publico&a=checarsenha&dialogo=1&usuario_id=<?php echo $usuario["usuario_id"]; ?>', null, window);
	else window.open( './index.php?m=publico&a=checarsenha&dialogo=1&usuario_id=<?php echo $usuario["usuario_id"]; ?>', 'Mudar Senha', 'top=250,left=250,width=350, height=220, scrollbars=no');
	}
