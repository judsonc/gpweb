...
if (!$usuario && $usuario_id) {
	...
	echo '<table cellpadding=0 cellspacing=0 class="std" width="100%">';
	echo '<tr><td align="right" width="100">'.dica('Login', 'Escreva o nome de acesso ao sistema.').'Login:'.dicaF().'</td><td><input type="text" class="texto" name="usuario_login" value="'.(isset($usuario['usuario_login']) ? $usuario['usuario_login'] : '').'" maxlength="255" style="width:260px;" /></td></tr>';
	...
	if (!isset($usuario['usuario_login']) || !$usuario['usuario_id']) {
		...
		echo '<tr><td align="right" width="150">'.dica('Pergunta', 'Escreva uma pergunta para a eventualidade de esquecer o login/senha de acesso ao sistema.').'Pergunta:'.dicaF().'</td><td colspan="2"><input type="text" class="texto" name="usuario_frase" value="'.(isset($usuario['usuario_frase']) ? $usuario['usuario_frase']:'').'" maxlength="255" style="width:260px;" /></td></tr>';
		echo '<tr><td align="right" width="150">'.dica('Resposta', 'Escreva uma resposta a pergunta para a eventualidade de esquecer o login/senha de acesso ao sistema.').'Resposta:'.dicaF().'</td><td colspan="2"><input type="password" class="texto" name="usuario_resposta" value="'.(isset($usuario['usuario_resposta']) ? $usuario['usuario_resposta']:'').'" maxlength="255" style="width:260px;" /></td></tr>';
		echo '<tr><td align="right" width="150">'.dica('Confirmar Resposta', 'Escreva novamennte a resposta a pergunta para a eventualidade de esquecer o login/senha de acesso ao sistema.').'Confirmar resposta:'.dicaF().'</td><td colspan="2"><input type="password" class="texto" name="usuario_resposta_confirmar" value="'.(isset($usuario['usuario_resposta']) ? $usuario['usuario_resposta']:'').'" maxlength="255" style="width:260px;" /></td></tr>';
		}
	echo '<tr><td>&nbsp;</td><td>
	...
	echo '<tr id="criar_contato" style="display:'.(!isset($usuario['usuario_grupo_dept']) || (isset($usuario['usuario_grupo_dept']) && !$usuario['usuario_grupo_dept']) ? '' : 'none').' "><td colspan=20>
	<table align="left"  cellpadding=0 cellspacing=0>';
	echo '<tr><td align="right" nowrap="nowrap" width=208>'.dica('Identidade', 'Escreva a identidade.').($config['id_usuario_identidade'] ? '' : '').'Identidade:'.dicaF().'</td><td><input type="text" class="texto" name="contato_identidade" id="contato_identidade" value="'.(isset($usuario['contato_identidade']) ? $usuario['contato_identidade'] : '').'" maxlength="25" style="width:260px;" /> </td></tr>';
	...
	if ($Aplic->usuario_super_admin || $Aplic->usuario_admin) {
		...
		echo '<tr><td align="right" nowrap="nowrap" width=208>'.dica(ucfirst($config['departamentos']).' Envolvid'.$config['genero_dept'].'s', 'Quais '.strtolower($config['departamentos']).' estão envolvid'.$config['genero_dept'].'s.').ucfirst($config['departamentos']).' envolvid'.$config['genero_dept'].'s:'.dicaF().'</td><td><table cellpadding=0 cellspacing=0><tr><td style="width:288px;"><div id="combo_depts">'.$saida_depts.'</div></td><td>'.botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamentos'],'popDepts()').'</td></tr></table></td></tr>';
		}
	echo '</table></td></tr>';
...
function enviarDados(){
	...
	<?php if ($podeEditar && !$usuario_id) { ?>
		...
		alert("A sua senha não está igual em ambos os campos.");
		form.usuario_senha.focus();
		}
	else if (form.usuario_resposta.value !=  form.usuario_resposta_confirmar.value) {
		alert("A resposta não está igual em ambos os campos.");
		form.usuario_resposta.focus();
		}	
...
function popAssinatura(){
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Assinatura', 500, 500, 'm=admin&a=assinatura&dialogo=1&usuario_id='+document.getElementById('usuario_id').value, null, window);
	else window.open('./index.php?m=admin&a=assinatura&dialogo=1&usuario_id='+document.getElementById('usuario_id').value, 'Assinatura','left=0,top=0,height=350,width=600, scrollbars=yes, resizable');
	}
