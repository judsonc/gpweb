...
foreach ($usuario_perfis as $chave => $nome) echo '<tr><td width="100%">'.$nome.'</td><td nowrap>'.(!(isset($config['restrito']) && $config['restrito']) && ($Aplic->usuario_admin || $Aplic->usuario_super_admin) ? dica('Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir este perfil de acesso.').'<a href="javascript:excluir('.$chave.');" >'.imagem('icones/remover.png').'</a>'.dicaF() : '').'</td></tr>';
		
echo '</table></td>';
echo '<td width="50%" valign="top">';
if ($podeEditar && ($Aplic->usuario_admin || $Aplic->usuario_super_admin)) { 
	...
	echo'<tr><td colspan="2" align="center"><table cellspacing=0 cellpadding=0><tr><td align=right>'.selecionaVetor($perfis_arr, 'usuario_perfil', 'style="width:200px;" size="1" class="texto"', '').'</td><td>'.(!(isset($config['restrito']) && $config['restrito']) && ($Aplic->usuario_admin || $Aplic->usuario_super_admin) ? '<a href="javascript:void(0);" onclick="frmPerms.submit();">'.imagem('icones/adicionar.png', 'Adicionar', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar o perfil de acesso selecionado.').'</a>': '').'</td></tr></table></td></tr>';
