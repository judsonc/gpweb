...
//$sql->esqUnir('usuario_tarefa_marcada', 'marcada', 'tarefas.tarefa_id = marcada.tarefa_id AND marcada.usuario_id = '.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
if ($baseline_id){
	$sql->adOnde('tarefas.baseline_id='.(int)$baseline_id);
	$sql->adOnde('p.baseline_id='.(int)$baseline_id);
	}
$sql->adCampo('tarefas.tarefa_id, tarefa_acao, tarefa_acesso, tarefa_superior, tarefa_nome, tarefa_inicio, tarefa_fim, tarefa_dinamica, tarefa_prioridade, tarefa_percentagem, tarefa_duracao, tarefa_duracao_tipo, tarefa_projeto, tarefa_descricao, tarefa_dono, tarefa_status');

//$sql->adCampo('count(tarefas.tarefa_superior) as subordinada, tarefa_marcada');
...
switch ($f) {
	...
	case 'minhasterminadas7dias':
		$sql->adOnde('ut.usuario_id ='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
	...
	case 'todasterminadas7dias':
		...
		$sql->adOnde('tarefa_designados.usuario_id ='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
		$sql->adOnde('tarefa_percentagem = 100');
		$sql->adOnde('tarefa_fim >= \''.date('Y-m-d 00:00:00', mktime(0, 0, 0, date('m'), date('d') - 7, date('Y'))).'\'');
		break;
	case 'subordinada':
	...
	case 'subordinadaProfunda':
		...
		break;
	case 'meuProj':
		$sql->adOnde('projeto_responsavel ='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
		break;
	case 'minhaCia':
	...
	case 'meu':
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_designados','tarefa_designados','tarefa_designados.tarefa_id = tarefas.tarefa_id');
		if ($baseline_id) $sql->adOnde('tarefa_designados.baseline_id='.(int)$baseline_id);
		$sql->adOnde('tarefa_designados.usuario_id ='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id).' OR tarefa_dono='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
		break;
	...
	case 'semDesignado':
		...
		break;
	case 'tarefaCriada':
		$sql->adOnde('tarefa_criador ='.(int)($usuario_id ? $usuario_id : $Aplic->usuario_id));
		break;
	default:
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_designados','tarefa_designados','tarefa_designados.tarefa_id = tarefas.tarefa_id');
		if ($baseline_id) $sql->adOnde('tarefa_designados.baseline_id='.(int)$baseline_id);
