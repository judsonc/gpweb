...
global $Aplic, $tarefa_id, $obj, $percentual, $cal_sdf, $podeEditar, $editar, $projeto_id, $obj;
...
if (!($editar && ($podeEditar || $Aplic->checarModulo('tarefa_log', ($tarefa_log_id ? 'editar' :'adicionar'))))) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if ($podeEditar){
	...
	echo '<tr><td align="right">'.dica('Valor Gasto', 'Para um cáculo dinâmico do custo d'.$config['genero_tarefa'].' '.$config['tarefa'].', pode utilizar o registro de trabalho para adicionar gastos nas diversas contas.').'Valor Gasto:'.dicaF().'</td><td><input type="text" style="text-align:right; width:180px;" class="texto" onkeypress="return somenteFloat(event)" name="tarefa_log_custo" value="'.($log->tarefa_log_custo!=0 ? number_format($log->tarefa_log_custo, 2, ',', '.') : '').'" size="40" /></td></tr>';  
	...
	echo '<input name="tarefa_log_custo" type="hidden" value="'.($log->tarefa_log_custo!=0 ? number_format($log->tarefa_log_custo, 2, ',', '.') : '').'" />';
	echo '<input name="tarefa_log_horas" type="hidden" value="'.($log->tarefa_log_horas!=0 ? number_format($log->tarefa_log_horas, 2, ',', '.'): '').'" />';
	}
