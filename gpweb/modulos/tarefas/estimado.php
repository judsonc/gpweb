...
if ($acao=='primeira' && $item && ($ordem > 1)){
	...
	foreach ($anteriores as $linha) {
		...
		$sql->adOnde('tarefa_custos_id = '.$linha['tarefa_custos_id']);
...
if ($acao!='inserir' && $acao!='editar'){
	...
	(count($pi)>1 ? '<th>'.dica('PI', 'PI do item.').'PI'.dicaF().'</th>' : '').(!$imprimindo ? '<th></th>' : '').
	'</tr>';
	$total=0;
	$custo=array();
	foreach ($linhas as $linha) {
		...
		echo '<td>'.$unidade[$linha['tarefa_custos_tipo']].'</td><td>'.number_format($linha['tarefa_custos_quantidade'], 2, ',', '.').'</td>';
		echo '<td align="right">'.number_format($linha['tarefa_custos_custo'], 2, ',', '.').'</td>';
		...
		echo '<td>'.($linha['tarefa_custos_data_limite']? retorna_data($linha['tarefa_custos_data_limite'],false) : '&nbsp;').'</td>';
		if (count($pi)>1) echo '<td align="center">'.$linha['tarefa_custos_pi'].'</td>';
		if (!$imprimindo) {
			echo '<td width="72" align="right">';
			echo dica('Primeira Posição', 'Clique neste ícone '.imagem('icones/2setacima.gif').' para mover para a primeira posição o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=primeira&ordem='.$linha['tarefa_custos_ordem'].'&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');"><img src="'.acharImagem('icones/2setacima.gif').'" border=0/></a>'.dicaF();
			echo dica('Mover Acima', 'Clique neste ícone '.imagem('icones/1setacima.gif').' para mover acima o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=acima&ordem='.$linha['tarefa_custos_ordem'].'&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');"><img src="'.acharImagem('icones/1setacima.gif').'" border=0/></a>'.dicaF();
			echo dica('Mover Abaixo', 'Clique neste ícone '.imagem('icones/1setabaixo.gif').' para mover abaixo o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=abaixo&ordem='.$linha['tarefa_custos_ordem'].'&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');"><img src="'.acharImagem('icones/1setabaixo.gif').'" border=0/></a>'.dicaF();
			echo dica('Última Posição', 'Clique neste ícone '.imagem('icones/2setabaixo.gif').' para mover para a última posição o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=ultima&ordem='.$linha['tarefa_custos_ordem'].'&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');"><img src="'.acharImagem('icones/2setabaixo.gif').'" border=0/></a>'.dicaF();
			echo dica('Editar Item', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=editar&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF();
			echo dica('Excluir Item', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir o item '.$linha['tarefa_custos_nome'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=estimado&acao=excluir&tarefa_id='.$tarefa_id.'&item='.$linha['tarefa_custos_id'].'\');">'.imagem('icones/remover.png').'</a>'.dicaF();
			echo '</td>';
			}
		echo '</tr>';
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="tarefa_custos_custo" id="tarefa_custos_custo" value="'.(isset($atual['tarefa_custos_custo']) ? number_format($atual['tarefa_custos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
...
function valor(){
	...
	var bdi=moeda2float(document.getElementById('tarefa_custos_bdi').value);
	if (custo=='') custo=0;
	if (valor=='') valor=0;
