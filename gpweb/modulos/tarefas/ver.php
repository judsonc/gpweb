...
if (!$dialogo && !$Aplic->profissional){
	...
	if ($podeEditar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_tarefa=".$tarefa_id.'&forum_projeto='.$obj->tarefa_projeto."\");");
	...
	if ($editar && $podeEditar)	$km->Add("acao","acao_editar",dica('Editar est'.($config['genero_tarefa']=='a' ?  'a' : 'e').' '.ucfirst($config['tarefa']),'Editar os detalhes '.($config['genero_tarefa']=='o' ? 'deste' : 'desta').' '.$config['tarefa'].'.').'Editar '.ucfirst($config['tarefa']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=tarefas&a=editar&tarefa_id=".$obj->tarefa_id."\");");
	if ($editar && $podeExcluir) $km->Add("acao","acao_excluir",dica('Excluir '.$config['genero_tarefa'].' '.ucfirst($config['tarefa']),'Excluir '.($config['genero_tarefa']=='o' ? 'este' : 'esta').' '.$config['tarefa'].' do sistema.<br><br>Todas '.$config['genero_tarefa'].'s '.$config['tarefas'].' pertencentes a '.($config['genero_tarefa']=='o' ? 'este' : 'esta').' '.$config['tarefa'].' também serão excluídas.').'Excluir '.ucfirst($config['tarefa']).dicaF(), "javascript: void(0);' onclick='excluir()");
...
if ($obj->tarefa_principal_indicador && $Aplic->profissional) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'O indicador mais representativo da situação geral d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->tarefa_principal_indicador).'</td></tr>';
...
if ($Aplic->checarModulo('tarefa_log', 'acesso')) {
	if ($Aplic->profissional){
	...
	if ($existe) {
		$caixaTab->adicionar(BASE_DIR.'/modulos/tarefas/ver_logs', 'Registros',null,null,'Registros d'.$config['genero_tarefa'].' '.ucfirst($config['tarefa']),'Visualizar os registros d'.$config['genero_tarefa'].' '.$config['tarefa'].'.');
		$qnt_aba++;
		}
	}
	
if ($editar && ($podeEditar || $Aplic->checarModulo('tarefa_log', 'adicionar')) && !$Aplic->profissional) $caixaTab->adicionar(BASE_DIR.'/modulos/tarefas/ver_log_atualizar', 'Registrar',null,null,'Registrar','Inserir um novo registro d'.$config['genero_tarefa'].' '.$config['tarefa'].'.');
...
if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(evento_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(arquivo_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(link_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(forum_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(pratica_indicador_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(plano_acao_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		}
	else $existe=true;	
...
if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
	...
	$sql->adCampo('count(msg_gestao_tarefa)');
	$existe=$sql->resultado();
	$sql->limpar();
	if ($existe) {
	...
	if ($config['doc_interno']) {
		...
		$sql->adCampo('count(modelo_gestao_tarefa)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
...
if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
	...
	$sql->adCampo('count(ata_gestao_tarefa)');
	$existe=$sql->resultado();
	$sql->limpar();
	if ($existe) {
...
if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
	...
	$sql->adCampo('count(problema_gestao_tarefa)');
	$existe=$sql->resultado();
	$sql->limpar();
	if ($existe) {
...
if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
	...
	$sql->adCampo('count(risco_gestao_tarefa)');
	$existe=$sql->resultado();
	$sql->limpar();
	if ($existe) {
