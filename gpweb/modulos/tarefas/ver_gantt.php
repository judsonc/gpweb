...
if (!$ver_min && !($m=='tarefas' && $a=='ver') && !($m=='projetos' && $a=='ver')) {
	...
	$botoesTitulo->adicionaBotao('m=tarefas&a=index', 'lista de '.$config['tarefas'], '', 'Lista de '.ucfirst($config['tarefa']), 'Visualizar '.$config['genero_tarefa'].'s '.$config['tarefas'].' '.($config['genero_projeto']=='o' ? 'deste' : 'desta').' '.$config['projeto'].'.');
