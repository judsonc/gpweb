...
<th>'.dica('Valor Unitátio', 'O valor de uma unidade do item.').'Valor Unit.'.dicaF().'</th>'.
...
foreach ($linhas as $linha) {
	...
	if ($linha['tarefa_custos_tarefa']!=$tarefa_atual) {
		...
		echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['tarefa_custos_moeda']].' '.number_format($linha['tarefa_custos_custo'], 2, ',', '.').'</td>';
		if ($config['bdi']) echo '<td align="right">'.number_format($linha['tarefa_custos_bdi'], 2, ',', '.').'</td>';
		echo '<td nowrap="nowrap" align="right">'.$nd.'</td>';
		echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['tarefa_custos_moeda']].' '.number_format($linha['valor'], 2, ',', '.').'</td>';
		...
		echo '<td nowrap="nowrap">'.number_format($linha['tarefa_gastos_quantidade'], 2, ',', '.').'</td>';
		echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['tarefa_gastos_moeda']].' '.number_format($linha['tarefa_gastos_custo'], 2, ',', '.').'</td>';
		if ($config['bdi']) echo '<td align="right">'.number_format($linha['tarefa_gastos_bdi'], 2, ',', '.').'</td>';
		echo '<td nowrap="nowrap" align="right">'.$nd.'</td>';
		echo '<td align="right" nowrap="nowrap">'.$moedas[$linha['tarefa_gastos_moeda']].' '.number_format($linha['valor'], 2, ',', '.').'</td>';
