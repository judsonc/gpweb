...
echo '<tr><td><b><center>Nome</th><td><b><center>Descrição</center></b></td><td><b><center>Unidade</center></b></td><td width="40"><b><center>Qnt.</center></b></td><td><b><center>Valor</center></b></td><td><b><center>ND</center></b></td><td width="100"><b><center>Total</center></b></td><td><b><center>Responsável</center></b></td></tr>';
$total=0;
$custo=array();
foreach ($linhas as $linha) {
	if ($tipo=='estimado'){
		$data = new CData($linha['tarefa_custos_data_inicio']);
		echo '<tr align="center"><td align="left">'.++$qnt.' - '.$linha['tarefa_custos_nome'].'</td><td align="left">'.$linha['tarefa_custos_descricao'].'</td><td nowrap="nowrap">'.$unidade[$linha['tarefa_custos_tipo']].'</td><td nowrap="nowrap">'.$linha['tarefa_custos_quantidade'].'</td><td align="right" nowrap="nowrap">'.number_format($linha['tarefa_custos_custo'], 2, ',', '.').'</td><td nowrap="nowrap">'.dica('Natureza da Despesa', $nd[$linha['tarefa_custos_nd']]).($linha['tarefa_custos_categoria_economica'] && $linha['tarefa_custos_grupo_despesa'] && $linha['tarefa_custos_modalidade_aplicacao'] ? $linha['tarefa_custos_categoria_economica'].'.'.$linha['tarefa_custos_grupo_despesa'].'.'.$linha['tarefa_custos_modalidade_aplicacao'].'.' : '').$linha['tarefa_custos_nd'].dicaF().'</td><td align="right" nowrap="nowrap">'.number_format($linha['valor'], 2, ',', '.').'</td><td align="left" nowrap="nowrap">'.nome_usuario($linha['tarefa_custos_usuario']).'</td><tr>';
		$custo[$linha['tarefa_custos_nd']] += (float)$linha['valor'];	
		}
	else{
		$data = new CData($linha['tarefa_gastos_data_inicio']);
		echo '<tr align="center"><td align="left">'.++$qnt.' - '.$linha['tarefa_gastos_nome'].'</td><td align="left">'.$linha['tarefa_gastos_descricao'].'</td><td nowrap="nowrap">'.$unidade[$linha['tarefa_gastos_tipo']].'</td><td nowrap="nowrap">'.$linha['tarefa_gastos_quantidade'].'</td><td align="right" nowrap="nowrap">'.number_format($linha['tarefa_gastos_custo'], 2, ',', '.').'</td><td nowrap="nowrap">'.dica('Natureza da Despesa', (isset($nd[$linha['tarefa_gastos_nd']]) ? $nd[$linha['tarefa_gastos_nd']] : 'Sem natureza de despesa')).($linha['tarefa_gastos_categoria_economica'] && $linha['tarefa_gastos_grupo_despesa'] && $linha['tarefa_gastos_modalidade_aplicacao'] ? $linha['tarefa_gastos_categoria_economica'].'.'.$linha['tarefa_gastos_grupo_despesa'].'.'.$linha['tarefa_gastos_modalidade_aplicacao'].'.' : '').$linha['tarefa_gastos_nd'].dicaF().'</td><td align="right" nowrap="nowrap">'.number_format($linha['valor'], 2, ',', '.').'</td><td align="left" nowrap="nowrap">'.nome_usuario($linha['tarefa_gastos_usuario']).'</td><tr>';
