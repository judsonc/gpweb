...
echo '<input type="hidden" name="uuid" id="uuid" value="'.($recurso_id ? null : uuid()).'" />';
...
if ($Aplic->profissional) {
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['organizacoes']).' Envolvid'.$config['genero_organizacao'].'s', 'Quais '.strtolower($config['organizacoes']).' estão envolvid'.$config['genero_organizacao'].'s.').ucfirst($config['organizacoes']).' envolvid'.$config['genero_organizacao'].'s:'.dicaF().'</td><td><table cellpadding=0 cellspacing=0><tr><td style="width:486px;"><div id="combo_cias">'.$saida_cias.'</div></td><td>'.botao_icone('organizacao_p.gif','Selecionar', 'selecionar '.$config['organizacoes'],'popCias()').'</td></tr></table></td></tr>';
	}
...
echo '<tr id="linha_quantidade"><td align="right" width=200>'.dica('Quantidade', 'Insira a quantidade total deste recurso.').'Quantidade:'.dicaF().'</td><td><span id="cifrao" style="display:none"></span><input type="text" onkeypress="return entradaNumerica(event, this, true, true);" class="texto" onchange="javascript:valor();" onclick="javascript:valor();" name="recurso_quantidade" id="recurso_quantidade" value="'.(isset($obj->recurso_quantidade) && $obj->recurso_quantidade ? number_format($obj->recurso_quantidade, 2, ',', '.') :'').'" size="40" /></td></tr>';
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores relacionados mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'recurso_principal_indicador', 'class="texto" style="width:284px;"', $obj->recurso_principal_indicador).'</td></tr>';
	}
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'recurso_moeda', 'class=texto size=1', ($obj->recurso_moeda ? $obj->recurso_moeda : 1)).'</td></tr>';
	}	
