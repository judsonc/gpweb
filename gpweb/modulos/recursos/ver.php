...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_recurso='.(int)$recurso_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_recurso=".$recurso_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->recurso_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Recurso</center></font>';
	}	
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->recurso_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o recurso se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->recurso_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
if ($Aplic->profissional) include_once BASE_DIR.'/modulos/recursos/ver_pro.php';	
...
if (!$dialogo){
	...
	if ($Aplic->profissional) {
		...
		$sql->adCampo('count(recurso_tarefa_id)');
		$sql->adOnde('recurso_tarefas.recurso_id = '.(int)$recurso_id);
		...
		if ($Aplic->profissional) {
			...
			$sql->adOnde('log_recurso = '.(int)$recurso_id);
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('evento_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(evento_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('arquivo_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(arquivo_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('link_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(link_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('forum_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(forum_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(pratica_indicador_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('plano_acao_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(plano_acao_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('projeto_gestao_recurso = '.(int)$recurso_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adOnde('msg_gestao_recurso = '.(int)$recurso_id);
			$sql->adCampo('count(msg_gestao_recurso)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adOnde('modelo_gestao_recurso = '.(int)$recurso_id);
				$sql->adCampo('count(modelo_gestao_recurso)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adOnde('ata_gestao_recurso = '.(int)$recurso_id);
			$sql->adCampo('count(ata_gestao_recurso)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adOnde('problema_gestao_recurso = '.(int)$recurso_id);
			$sql->adCampo('count(problema_gestao_recurso)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adOnde('risco_gestao_recurso = '.(int)$recurso_id);
			$sql->adCampo('count(risco_gestao_recurso)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_recurso = '.(int)$recurso_id.' OR risco_gestao_recurso = '.(int)$recurso_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adOnde('instrumento_gestao_recurso = '.(int)$recurso_id);
			$sql->adCampo('count(instrumento_gestao_recurso)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
