...
global $estilo_interface, $Aplic, $recurso_pesquisa, $dialogo, $podeEditar, $cia_id, $dept_id, $lista_depts, $tab, $lista_cias, $usuario_id,$recurso_tipo,$recurso_ano,$recurso_ugr,$recurso_ptres,$dept_id,$recurso_credito_adicional,$recurso_movimentacao_orcamentaria,$recurso_identificador_uso, $favorito_id, $filtro_prioridade_recurso;
...
$sql->adTabela('recursos');
$sql->adCampo('recursos.recurso_id, recurso_chave, recurso_nome, recurso_responsavel, recurso_quantidade, recurso_nd, recurso_nivel_acesso, recurso_tipo, recurso_aprovado');
...
echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').'&ordenar=recurso_chave&ordem='.($ordem ? '0' : '1').'\');">'.dica('Código', 'Recomenda-se que todo recurso tenha um código para facilitar a catalogação.').($ordenar=='recurso_chave' ? imagem('icones/'.$seta[$ordem]) : '').'Código'.dicaF().'</a></th>';
echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').'&ordenar=recurso_nome&ordem='.($ordem ? '0' : '1').'\');">'.dica('Nome do Recurso', 'Todo recurso precisa de um nome para facilitar a identificação.').($ordenar=='recurso_nome' ? imagem('icones/'.$seta[$ordem]) : '').'Nome do Recurso'.dicaF().'</a></th>';
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=recurso_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='recurso_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_recurso) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').'&ordenar=recurso_quantidade&ordem='.($ordem ? '0' : '1').'\');">'.dica('Total', 'Total deste recurso que foi disponibilizado para '.$config['genero_tarefa'].'s '.$config['tarefas'].'.').($ordenar=='recurso_quantidade' ? imagem('icones/'.$seta[$ordem]) : '').'Total'.dicaF().'</a></th>';
if ($recurso_tipo==5) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').'&ordenar=recurso_nd&ordem='.($ordem ? '0' : '1').'\');">'.dica('Natureza da Despesa', 'Natureza de Despesa(ND) deste recurso, se for o caso.').($ordenar=='recurso_nd' ? imagem('icones/'.$seta[$ordem]) : '').'ND'.dicaF().'</a></th>';
echo '</tr>';
...
foreach ($recursos as $linha) {
	if (permiteAcessarRecurso($linha['recurso_nivel_acesso'], $linha['recurso_id'])){
		...
		if (!$dialogo) echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o recurso.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=editar&recurso_id='.$linha['recurso_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['recurso_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_recurso) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
