...
if (getParam($_REQUEST, 'enviado', 0)) {
	...
	echo '<script type="text/javascript">if(parent && parent.gpwebApp){parent.gpwebApp._popupCallback(\''.$selecionados_id.'\');} else {'.$chamarVolta_string.'self.close();}</script>';
	}
...
echo '<script type="text/javascript">function setFechar(chave, valor){if (chave!=0) window.opener.'.$chamarVolta.'(chave, valor); else window.opener.'.$chamarVolta.'(null, ""); window.close(); }</script>';
