...
$valor=intval(getParam($_REQUEST, 'valor', 0));
...
switch ($tabela) {
	case 'pratica_indicador':
		...
		$indicadores=vetor_com_pai_generico('pratica_indicador', 'pratica_indicador_id', 'pratica_indicador_nome', 'pratica_indicador_superior', $valor, $cia_id, 'pratica_indicador_cia', TRUE, FALSE, 'pratica_indicador_acesso', 'indicador', $nenhum, false, $filtro);
		...
		echo '<tr><td colspan=2><fieldset><legend class=texto style="color: black;">&nbsp;<b>Indicadores Disponíveis</b>&nbsp</legend>'.dica().'<div id="combo_lista">'.selecionaVetor($indicadores, 'lista', 'style="width:100%;" size="15" class="texto" ondblclick="mudar_indicadores();"').'</div></fieldset></td><tr>';
		break;					
	...
	case 'checklist':
		...
		$checklist=vetor_com_pai_generico('checklist', 'checklist_id', 'checklist_nome', 'checklist_superior', $valor, $cia_id, 'checklist_cia', TRUE, FALSE, 'checklist_acesso', 'checklist', $nenhum);
		echo '<tr><td colspan=2><table><tr><td align=right>'.ucfirst($config['organizacao']).':'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om($cia_id, 'cia_id', 'class=texto size=1 style="width:250px;" onchange="javascript:mudar_om();"').'</div></td><td><a href="javascript:void(0);" onclick="mudar_checklist()">'.imagem('icones/atualizar.png').'</a></td></tr></table></td></tr>';
		echo '<tr><td colspan=2><fieldset><legend class=texto style="color: black;">&nbsp;<b>Checklist Disponíveis</b>&nbsp</legend>'.dica().'<div id="combo_lista">'.selecionaVetor($checklist, 'lista', 'style="width:100%;" size="15" class="texto" ondblclick="mudar_checklist();"').'</div></fieldset></td><tr>';
		break;	

	}


	echo '<tr><td>'.botao('selecionar', '', '','','selecionar()').'</td>'.(!$Aplic->profissional ? '<td align=right>'.botao('cancelar', '', '','','window.opener = window; window.close()').'</td>' : '').'</tr></table></form>';
...
function selecionar(){
	var valor=document.env.lista.options[document.env.lista.selectedIndex].text;
	var chave=document.env.lista.value;	
	
	if(parent && parent.gpwebApp) {
		if (chave>0) parent.gpwebApp._popupCallback(chave, valor); 
		else parent.gpwebApp._popupCallback(null, "");
		} 
	else {
		<?php echo 'window.opener.'.$chamarVolta.'(chave, valor);' ?>
		window.close();
		}	
	}
...
function mudar_checklist(){
	xajax_mudar_checklist_ajax(document.getElementById('cia_id').value, document.getElementById('lista').value,  '<?php echo $nenhum ?>'); 	
	}	
