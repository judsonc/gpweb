...
if ($Aplic->profissional && $arquivo){
	...
	AND arquivo_pasta_gestao_mswot IS NULL
	...
	foreach($lista as $linha){
		...
		elseif ($linha['arquivo_gestao_mswot']) $vetor[]='arquivo_pasta_gestao_mswot='.(int)$linha['arquivo_gestao_mswot'];
	...
	foreach($lista as $linha){
		...
		elseif ($linha['arquivo_pasta_gestao_mswot']) $vetor[]='arquivo_pasta_gestao_mswot='.(int)$linha['arquivo_pasta_gestao_mswot'];
	...
	elseif ($objetivo_id) $filtro='arquivo_pasta_gestao_objetivo='.(int)$objetivo_id;
	elseif ($fator_id) $filtro='arquivo_pasta_gestao_fator='.(int)$fator_id;
	...
	elseif ($mswot_id) $filtro='arquivo_pasta_gestao_mswot='.(int)$mswot_id;
...
if ($Aplic->profissional){
	...
	else if ($objetivo_id) $sql->adOnde('arquivo_pasta_objetivo = '.$objetivo_id);
	else if ($pg_estrategia_id) $sql->adOnde('arquivo_pasta_estrategia = '.$pg_estrategia_id);
	else if ($fator_id) $sql->adOnde('arquivo_pasta_fator = '.$fator_id);
	...
	else if ($usuario) $sql->adOnde('arquivo_pasta_usuario = '.$usuario);
	else $sql->adOnde('(
		arquivo_pasta_tarefa IS NULL
		AND arquivo_pasta_projeto IS NULL
		AND arquivo_pasta_perspectiva IS NULL
		AND arquivo_pasta_tema IS NULL
		AND arquivo_pasta_objetivo IS NULL
		AND arquivo_pasta_fator IS NULL
		AND arquivo_pasta_estrategia IS NULL
		AND arquivo_pasta_meta IS NULL
		AND arquivo_pasta_pratica IS NULL
		AND arquivo_pasta_acao IS NULL
		AND arquivo_pasta_indicador IS NULL
		AND arquivo_pasta_calendario IS NULL
		AND arquivo_pasta_ata IS NULL
		AND arquivo_pasta_instrumento IS NULL
		AND arquivo_pasta_demanda IS NULL
		AND arquivo_pasta_usuario IS NULL)');
	}
...
function subniveis($pasta_id, $subnivel){
	global $Aplic, $edicao, $arquivo_pasta_id, $chamarVolta, $campo, $sql, $sem_nada, $filtro, $nenhum_filtro,
	$usuario,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
