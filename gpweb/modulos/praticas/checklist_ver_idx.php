...
global $estilo_interface, $sql, $perms, $Aplic, $cia_id, $lista_cias, $pesquisar_texto, $usuario_id, $dept_id, $lista_depts, $tab, $ordem, $ordenar, $favorito_id, $dialogo, $podeEditar, $filtro_prioridade_checklist,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('checklist_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('checklist_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('checklist_gestao_mswot='.(int)$mswot_id);
...
$sql->adCampo('checklist.checklist_id, checklist_nome, checklist_descricao, checklist_responsavel, checklist_cor, checklist_acesso, count(checklist_lista_id) AS qnt, checklist_aprovado');
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=checklist_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='checklist_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_checklist) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($checklists as $linha) {
	if (permiteAcessarChecklist($linha['checklist_acesso'],$linha['checklist_id'])){
		...
		echo '<tr>';
		if (!$impressao) echo '<td nowrap="nowrap" width="16">'.($podeEditar && $editar && !$bloquear ? dica('Editar Checklist', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o checklist.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=checklist_editar&checklist_id='.$linha['checklist_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['checklist_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_checklist) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';
		...
		if ($Aplic->profissional) {
			...
			if (count($lista)){	
				foreach($lista as $gestao_data){	
					...
					elseif (isset($gestao_data['checklist_gestao_mswot']) && $gestao_data['checklist_gestao_mswot']) echo ($qnt_gestao++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['checklist_gestao_mswot']);
