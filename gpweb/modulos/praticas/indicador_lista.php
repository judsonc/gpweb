...
if (isset($_REQUEST['filtro_prioridade_indicador']))	$Aplic->setEstado('filtro_prioridade_indicador', getParam($_REQUEST, 'filtro_prioridade_indicador', null));
$filtro_prioridade_indicador = $Aplic->getEstado('filtro_prioridade_indicador') !== null ? $Aplic->getEstado('filtro_prioridade_indicador') : null;

$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_indicador=1');
...
$sql->adOnde('favorito_indicador=1');
...
if (!$dialogo){
	...
	echo '<input type="hidden" name="filtro_prioridade_indicador" id="filtro_prioridade_indicador" value="'.$filtro_prioridade_indicador.'" />';
...
$tipos=array(
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar os indicadores pelo campo de matriz SWOT que estão relacionados.').'Campo SWOT:'.dicaF();
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Indicadores', 'indicador.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito='<tr><td nowrap="nowrap" align=right>'.dica('Favoritos', 'Criar ou editar um grupo de indicadores favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&indicador=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	if($filtro_prioridade_indicador) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$pesquisa_pauta.$procuraBuffer.$selecao_ano.$tipo.$popFiltro.$filtros.$favoritos.'</table></td><td align=right><table cellspacing=0 cellpadding=0>'.$filtrar.$novo_indicador.$botao_imprimir.$botao_prioridade.$filtro_expandido.$botao_favorito.$botao_trava.$botao_excel.$botao_campos.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0><tr><td>'.dica('Novo Indicador', 'Criar um novo indicador.').'<a class="botao" href="javascript: void(0)" onclick="javascript:env.a.value=\'indicador_editar\'; env.submit();" ><span>novo</span></a>'.dicaF().'</td></tr></table>');
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Indicadores</center></font>';
	}
...
foreach ((array)$praticas_criterios as $chave => $criterio) {
	...
	if ($filtro_prioridade_indicador){
		$sql->esqUnir('priorizacao', 'priorizacao', 'pratica_indicador.pratica_indicador_id=priorizacao_indicador');
		$sql->adOnde('priorizacao_modelo IN ('.$filtro_prioridade_indicador.')');
		}
	
	if ($favorito_id){
		$sql->internoUnir('favorito_lista', 'favorito_lista', 'pratica_indicador.pratica_indicador_id=favorito_lista_campo');
	...
	if ($Aplic->profissional){
		...
		elseif ($objetivo_id && !$indicador_expandido) $sql->adOnde('pratica_indicador_gestao_objetivo='.(int)$objetivo_id);
		elseif ($fator_id && !$indicador_expandido) $sql->adOnde('pratica_indicador_gestao_fator='.(int)$fator_id);
		...
		elseif ($mswot_id && !$indicador_expandido) $sql->adOnde('pratica_indicador_gestao_mswot='.(int)$mswot_id);
		...
		elseif ($objetivo_id && !$indicador_expandido) $sql->adOnde('pratica_indicador_objetivo_estrategico='.(int)$objetivo_id);
		...
		elseif ($fator_id && !$indicador_expandido) $sql->adOnde('pratica_indicador_fator='.(int)$fator_id);
...
if ($pratica_modelo_id && isset($praticas_criterios[$tab-2]['pratica_criterio_resultado']) && !$praticas_criterios[$tab-2]['pratica_criterio_resultado']){
	...
	if ($filtro_prioridade_indicador){
		$sql->esqUnir('priorizacao', 'priorizacao', 'pratica_indicador.pratica_indicador_id=priorizacao_indicador');
		if ($config['metodo_priorizacao']) $sql->adCampo('(SELECT round(exp(sum(log(coalesce(priorizacao_valor,1))))) FROM priorizacao WHERE priorizacao_indicador = pratica_indicador.pratica_indicador_id AND priorizacao_modelo IN ('.$filtro_prioridade_indicador.')) AS priorizacao');
		else $sql->adCampo('(SELECT SUM(priorizacao_valor) FROM priorizacao WHERE priorizacao_indicador = pratica_indicador.pratica_indicador_id AND priorizacao_modelo IN ('.$filtro_prioridade_indicador.')) AS priorizacao');
		$sql->adOnde('priorizacao_modelo IN ('.$filtro_prioridade_indicador.')');
		}
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_indicador.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_indicador){
	env.filtro_prioridade_indicador.value=filtro_prioridade_indicador;
	env.submit();
	}
