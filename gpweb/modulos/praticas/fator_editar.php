...
if(!($podeEditar && permiteEditarFator($obj->fator_acesso,$fator_id))) $Aplic->redirecionar('m=publico&a=acesso_negado');
if ((!$podeEditar && $fator_id) || (!$podeAdicionar && !$fator_id)) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if ($fator_id) {
	...
	$sql->adOnde('fator_objetivo_fator = '.(int)$obj->fator_id);
...
echo '<input type="hidden" name="uuid" id="uuid" value="'.($fator_id ? null : uuid()).'" />';
...
echo '<tr><td align="right" nowrap="nowrap" width="145">'.dica(ucfirst($config['organizacao']).' Responsável', 'A qual '.$config['organizacao'].' pertence '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om(($obj->fator_cia ? $obj->fator_cia : $cia_id), 'fator_cia', 'class=texto size=1 style="width:404px;" onchange="javascript:mudar_om();"').'</div></td></tr>';
if ($Aplic->profissional) {
	$saida_cias='';
...
if ($Aplic->profissional) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['departamento']).' Responsável', 'Escolha pressionando o ícone à direita qual '.$config['genero_dept'].' '.$config['dept'].' responsável por '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td><input type="hidden" name="fator_dept" id="fator_dept" value="'.($fator_id ? $obj->fator_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept)).'" /><input type="text" class="texto" name="dept_nome" id="dept_nome" value="'.nome_dept(($fator_id ? $obj->fator_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept))).'" style="width:401px;" READONLY />'.botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamento'],'popDept()').'</td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap" width="135">'.dica(ucfirst($config['objetivo']), 'A qual '.$config['objetivo'].' está relacionad'.($config['genero_fator']=='a' ? 'a esta' : 'o este').' '.$config['fator'].'.').''.ucfirst($config['objetivo']).':'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="fator_objetivo" id="fator_objetivo" value="" /><input type="text" id="nome_objetivo" name="nome_objetivo" value="" style="width:400px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popObjetivo();">'.imagem('icones/obj_estrategicos_p.gif','Selecionar '.ucfirst($config['objetivo']).'','Clique neste ícone '.imagem('icones/obj_estrategicos_p.gif').' para selecionar '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' '.$config['objetivo'].'.').'</a></td></tr></table></td></tr>';
...
if (count($objetivos)) {
	echo '<table cellspacing=0 cellpadding=0 class="tbl1" align=left><tr><th></th><th>'.dica('Nome', 'Mome do objeto relacionado.').'Nome'.dicaF().'</th><th></th></tr>';
...
if ($exibir['fator_descricao'])  echo '<tr><td align="right" nowrap="nowrap" >'.dica('Descrição', 'Descrição sobre '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Descrição:'.dicaF().'</td><td width="100%" colspan="2"><textarea data-gpweb-cmp="ckeditor" name="fator_descricao" style="width:284px;" rows="2" class="textarea">'.$obj->fator_descricao.'</textarea></td></tr>';


if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_tema'].' '.$config['tema'].' mais representativo da situação geral do mesmo.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'fator_principal_indicador', 'class="texto" style="width:400px;"', $obj->fator_principal_indicador).'</td></tr>';
...
if ($exibir['fator_oque']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('O Que', 'Sumário sobre o que se trata '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'O Que:'.dicaF().'</td><td colspan="2"><textarea name="fator_oque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_oque.'</textarea></td></tr>';
if ($exibir['fator_quem']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Quem:'.dicaF().'</td><td colspan="2"><textarea name="fator_quem" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_quem.'</textarea></td></tr>';
if ($exibir['fator_quando']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Quando', 'Quando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Quando:'.dicaF().'</td><td colspan="2"><textarea name="fator_quando" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_quando.'</textarea></td></tr>';
if ($exibir['fator_onde']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Onde', 'Onde '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Onde:'.dicaF().'</td><td colspan="2"><textarea name="fator_onde" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_onde.'</textarea></td></tr>';
if ($exibir['fator_porque']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Por Que', 'Por que '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' será executad'.$config['genero_fator'].'.').'Por que:'.dicaF().'</td><td colspan="2"><textarea name="fator_porque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_porque.'</textarea></td></tr>';
if ($exibir['fator_como']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Como', 'Como '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Como:'.dicaF().'</td><td colspan="2"><textarea name="fator_como" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_como.'</textarea></td></tr>';
if ($exibir['fator_quanto']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Quanto', 'Custo para executar '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Quanto:'.dicaF().'</td><td colspan="2"><textarea name="fator_quanto" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->fator_quanto.'</textarea></td></tr>';
if ($cincow2h) {
	echo '</table></fieldset></td></tr>';
	}
...
if ($exibir['fator_desde_quando']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Desde Quando é Feita', 'Desde quando '.$config['genero_fator'].' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Desde quando:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="fator_desde_quando" cols="60" rows="2" class="textarea">'.$obj->fator_desde_quando.'</textarea></td></tr>';
if ($exibir['fator_controle'])echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Método de Controle', 'Como '.$config['genero_fator'].' '.$config['fator'].' é controlad'.$config['genero_fator'].'.').'Controle:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="fator_controle" cols="60" rows="2" class="textarea">'.$obj->fator_controle.'</textarea></td></tr>';
...
if ($exibir['fator_melhorias']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Melhorias Efetuadas n'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Quais as melhorias realizadas n'.$config['genero_fator'].' '.$config['fator'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="fator_melhorias" cols="60" rows="2" class="textarea">'.$obj->fator_melhorias.'</textarea></td></tr>';
if ($bsc) {
	echo '</table></fieldset></td></tr>';
	}
...
echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($fator_id > 0 ? 'modificação' : 'criação').' d'.$config['genero_fator'].' '.$config['fator'].'.').'Notificar:'.dicaF().'</td>';
echo '<td>';
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('fator_cia').value, window.setObjetivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('fator_cia').value, 'Objetivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popMe() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["me"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setMe&edicao=1&tabela=me&cia_id='+document.getElementById('fator_cia').value, window.setMe, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setMe&edicao=1&tabela=me&cia_id='+document.getElementById('fator_cia').value, '<?php echo ucfirst($config["me"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function mudar_posicao_objetivo(ordem, fator_objetivo_id, direcao){
	xajax_mudar_posicao_objetivo(ordem, fator_objetivo_id, direcao, document.getElementById('fator_id').value, document.getElementById('uuid').value);
	__buildTooltip();
	}

function excluir_objetivo(fator_objetivo_id){
	xajax_excluir_objetivo(fator_objetivo_id, document.getElementById('fator_id').value, document.getElementById('uuid').value);
	__buildTooltip();
	}
