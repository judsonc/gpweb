...
$meta_mswot = getParam($_REQUEST, 'meta_mswot', null);
...
if ($Aplic->profissional && (
	...
	elseif ($meta_objetivo) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
	elseif ($meta_fator) $sql->esqUnir('fator','fator','fator_cia=cias.cia_id');
	...
	elseif ($meta_mswot) $sql->esqUnir('mswot','mswot','mswot_cia=cias.cia_id');
	...
	elseif ($meta_arquivo) $sql->esqUnir('arquivo','arquivo','arquivo_cia=cias.cia_id');
	...
	elseif ($meta_objetivo) $sql->adOnde('objetivo_id = '.(int)$meta_objetivo);
	elseif ($meta_fator) $sql->adOnde('fator_id = '.(int)$meta_fator);
	...
	elseif ($meta_mswot) $sql->adOnde('mswot_id = '.(int)$meta_mswot);
...
if (!$Aplic->profissional && !$pg_meta_id) {
	...
	if ($obj->pg_meta_objetivo_estrategico || $obj->pg_meta_tema || $obj->pg_meta_estrategia || $obj->pg_meta_fator || $obj->pg_meta_perspectiva){
		...
		if ($obj->pg_meta_objetivo_estrategico) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
		if ($obj->pg_meta_estrategia) $sql->esqUnir('estrategias','estrategias','pg_estrategia_cia=cias.cia_id');
		if ($obj->pg_meta_fator) $sql->esqUnir('fator','fator','fator_cia=cias.cia_id');
		...
		elseif ($obj->pg_meta_objetivo_estrategico) $sql->adOnde('objetivo_id = '.$obj->pg_meta_objetivo_estrategico);
		elseif ($obj->pg_meta_estrategia) $sql->adOnde('pg_estrategia_id = '.$obj->pg_meta_estrategia);
		elseif ($obj->pg_meta_fator) $sql->adOnde('fator_id = '.$obj->pg_meta_fator);
...
echo '<tr><td align="right" width=125>'.dica('Nome', 'Tod'.$config['genero_meta'].' '.$config['meta'].' necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="pg_meta_nome" value="'.$obj->pg_meta_nome.'" style="width:284px;" class="texto" /> *</td></tr>';
...
if (!$Aplic->profissional){
	...
	echo '<input type="hidden" name="meta_mswot" value="" id="mswot" /><input type="hidden" id="mswot_nome" name="mswot_nome" value="">';
	...
	if ($swot_ativo) {
		echo '<tr '.(isset($meta_mswot) && $meta_mswot ? '' : 'style="display:none"').' id="mswot" ><td align="right" nowrap="nowrap">'.dica('Matriz SWOT', 'Caso '.$config['genero_meta'].' '.$config['meta'].' seja específic'.$config['genero_meta'].' de uma matriz SWOT neste campo deverá constar o nome da matriz SWOT').'Matriz SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="meta_mswot" value="'.(isset($meta_mswot) ? $meta_mswot : '').'" /><input type="text" id="mswot_nome" name="mswot_nome" value="'.nome_mswot((isset($meta_mswot) ? $meta_mswot : null)).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popMSWOT();">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Selecionar Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para selecionar uma matriz SWOT.').'</a></td></tr></table></td></tr>';
		echo '<tr '.(isset($meta_swot) && $meta_swot ? '' : 'style="display:none"').' id="swot" ><td align="right" nowrap="nowrap">'.dica('Campo SWOT', 'Caso '.$config['genero_meta'].' '.$config['meta'].' seja específic'.$config['genero_meta'].' de um campo de matriz SWOT neste campo deverá constar o nome do campo de matriz SWOT').'Campo SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="meta_swot" value="'.(isset($meta_swot) ? $meta_swot : '').'" /><input type="text" id="swot_nome" name="swot_nome" value="'.nome_swot((isset($meta_swot) ? $meta_swot : null)).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popSWOT();">'.imagem('../../../modulos/swot/imagens/swot_p.png','Selecionar Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para selecionar um campo de matriz SWOT.').'</a></td></tr></table></td></tr>';
		}
	else {
		echo '<input type="hidden" name="meta_mswot" value="" id="mswot" /><input type="hidden" id="mswot_nome" name="mswot_nome" value="">';
		echo '<input type="hidden" name="meta_swot" value="" id="swot" /><input type="hidden" id="swot_nome" name="swot_nome" value="">';
  ...
  $sql->limpar();
	...
	foreach($lista as $gestao_data){
		...
		elseif ($gestao_data['meta_gestao_mswot']) echo '<td align=left>'.imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['meta_gestao_mswot']).'</td>';
		...
		elseif ($gestao_data['meta_gestao_template']) echo '<td align=left>'.imagem('icones/template_p.gif').link_template($gestao_data['meta_gestao_template']).'</td>';
...
if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_meta'].' '.$config['meta'].' mais representativo da situação geral da mesma.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'pg_meta_principal_indicador', 'class="texto" style="width:267px;"', $obj->pg_meta_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'pg_meta_moeda', 'class=texto size=1', ($obj->pg_meta_moeda ? $obj->pg_meta_moeda : 1)).'</td></tr>';
	}	
else echo '<input type="hidden" name="pg_meta_moeda" id="pg_meta_moeda" value="'.($obj->pg_meta_moeda ? $obj->pg_meta_moeda : 1).'" />';
...
echo '<tr><td colspan=20 style="background-color:#'.($estilo_interface=='metro' ? '006fc2' : 'a6a6a6').'" onclick="if (document.getElementById(\'valor_metas\').style.display) document.getElementById(\'valor_metas\').style.display=\'\'; else document.getElementById(\'valor_metas\').style.display=\'none\';"><a href="javascript: void(0);" class="aba">'.dica('Valor da Meta','Lista de valores da metas.').'<b>Valor da Meta</b>'.dicaF().'</a></td></tr>';
echo '<tr id="valor_metas" style="display:none"><td colspan=20><table cellspacing=0 cellpadding=0>';
...
echo '<input type="hidden" id="meta_meta_id" name="meta_meta_id" value="" /></table></td><td id="adicionar_meta" style="display:" width=50 align=center><a href="javascript: void(0);" onclick="incluir_meta();">'.imagem('icones/adicionar_g.png','Incluir','Clique neste ícone '.imagem('icones/adicionar.png').' para incluir a meta.').'</a></td>';
...
function popObjetivo2() {
	window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo2&tabela=objetivo&cia_id='+document.getElementById('pg_meta_cia').value, 'Objetivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popFator2() {
	window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setFator2&tabela=fator&cia_id='+document.getElementById('pg_meta_cia').value, 'Fator','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('pg_meta_cia').value, window.setObjetivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('pg_meta_cia').value, '<?php echo ucfirst($config["objetivo"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('pg_meta_cia').value, window.setFator, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('pg_meta_cia').value, '<?php echo ucfirst($config["fator"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popMSWOT() {
	parent.gpwebApp.popUp('Matriz SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setMSWOT&tabela=mswot&cia_id='+document.getElementById('pg_meta_cia').value, window.setMSWOT, window);
	}

function setMSWOT(chave, valor){
	limpar_tudo();
	document.env.meta_mswot.value = chave;
	document.env.mswot_nome.value = valor;
	<?php if ($Aplic->profissional) echo 'incluir_relacionado();' ?>
	}

function popSWOT() {
	parent.gpwebApp.popUp('Campo SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setSWOT&tabela=swot&cia_id='+document.getElementById('pg_meta_cia').value, window.setSWOT, window);
	}
...
function popArquivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Arquivo', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('pg_meta_cia').value, window.setArquivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('pg_meta_cia').value, 'Arquivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function limpar_tudo(){
	...
	<?php
	if($swot_ativo) {
		echo 'document.env.mswot_nome.value = \'\';	document.env.meta_mswot.value = null;';
		echo 'document.env.swot_nome.value = \'\';	document.env.meta_swot.value = null;';
		}
