...
Este plano_acao é parte do programa gpweb
...
if (!defined('BASE_DIR'))	die('Você não deveria acessar este plano de ação diretamente.');
...
$plano_acao_mswot = getParam($_REQUEST, 'plano_acao_mswot', null);
...
if (
	...
	elseif ($plano_acao_objetivo) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
	elseif ($plano_acao_fator) $sql->esqUnir('fator','fator','fator_cia=cias.cia_id');
	...
	elseif ($plano_acao_mswot) $sql->esqUnir('mswot','mswot','mswot_cia=cias.cia_id');
	...
	elseif ($plano_acao_arquivo) $sql->esqUnir('arquivo','arquivo','arquivo_cia=cias.cia_id');
	...
	elseif ($plano_acao_objetivo) $sql->adOnde('objetivo_id = '.(int)$plano_acao_objetivo);
	elseif ($plano_acao_fator) $sql->adOnde('fator_id = '.(int)$plano_acao_fator);
	...
	elseif ($plano_acao_mswot) $sql->adOnde('mswot_id = '.(int)$plano_acao_mswot);
...
echo '<tr><td align="right">'.dica('Nome d'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Tod'.$config['genero_acao'].' '.$config['acao'].' necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="plano_acao_nome" value="'.$obj->plano_acao_nome.'" style="width:286px;" class="texto" /> *</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', 'A qual '.$config['organizacao'].' pertence '.($config['genero_acao']=='o' ? 'este' : 'esta').' '.$config['acao'].'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om($cia_id, 'plano_acao_cia', 'class=texto size=1 style="width:286px;" onchange="javascript:mudar_om();"').'</div></td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Progresso', ucfirst($config['genero_acao']).' '.$config['acao'].' pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td nowrap="nowrap">'.selecionaVetor($percentual, 'plano_acao_percentagem', 'size="1" class="texto"'.($Aplic->profissional && (($exibir['porcentagem_item'] && $obj->plano_acao_calculo_porcentagem) || !$obj->plano_acao_id) ? ' disabled' : ''), (int)$obj->plano_acao_percentagem).'% </td></tr>';


if ($Aplic->profissional && $exibir['porcentagem_item']) echo '<tr><td align="right"  nowrap="nowrap">'.dica('Progresso Calculado', 'Caso esteja marcada, '.$config['genero_acao'].' '.$config['acao'].' terá o progresso calculado baseado nos ítens d'.$config['genero_acao'].' mesm'.$config['genero_acao'].'.').'Progresso calculado:'.dicaF().'</td><td><input type="checkbox" value="1" onchange="desabilirar_porcentagem();" name="plano_acao_calculo_porcentagem" id="plano_acao_calculo_porcentagem" '.($obj->plano_acao_calculo_porcentagem || !$plano_acao_id ? 'checked="checked"' : '').' /></td></tr>';



if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_acao'].' '.$config['acao'].' mais representativo da situação geral do mesmo.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'plano_acao_principal_indicador', 'class="texto" style="width:267px;"', $obj->plano_acao_principal_indicador).'</td></tr>';


if ($exibir['plano_acao_descricao'])  echo '<tr><td align="right" nowrap="nowrap" >'.dica('Descrição', 'Descrição sobre '.($config['genero_iniciativa']=='a' ? 'esta' : 'este').' '.$config['iniciativa'].'.').'Descrição:'.dicaF().'</td><td width="100%" colspan="2"><textarea data-gpweb-cmp="ckeditor" name="plano_acao_descricao" style="width:284px;" rows="2" class="textarea">'.$obj->plano_acao_descricao.'</textarea></td></tr>';
...
elseif ($plano_acao_mswot) $tipo='mswot';
...
if ($swot_ativo) {
	echo '<tr '.(isset($plano_acao_mswot) && $plano_acao_mswot ? '' : 'style="display:none"').' id="mswot" ><td align="right" nowrap="nowrap">'.dica('Matriz SWOT', 'Caso '.$config['genero_acao'].' '.$config['acao'].' seja específic'.$config['genero_acao'].' de uma matriz SWOT neste campo deverá constar o nome da matriz SWOT').'Matriz SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="plano_acao_mswot" value="'.(isset($plano_acao_mswot) ? $plano_acao_mswot : '').'" /><input type="text" id="mswot_nome" name="mswot_nome" value="'.nome_mswot((isset($plano_acao_mswot) ? $plano_acao_mswot : null)).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popMSWOT();">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Selecionar Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para selecionar uma matriz SWOT.').'</a></td></tr></table></td></tr>';
	echo '<tr '.(isset($plano_acao_swot) && $plano_acao_swot ? '' : 'style="display:none"').' id="swot" ><td align="right" nowrap="nowrap">'.dica('Campo SWOT', 'Caso '.$config['genero_acao'].' '.$config['acao'].' seja específic'.$config['genero_acao'].' de um campo de matriz SWOT neste campo deverá constar o nome do campo de matriz SWOT').'Campo SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="plano_acao_swot" value="'.(isset($plano_acao_swot) ? $plano_acao_swot : '').'" /><input type="text" id="swot_nome" name="swot_nome" value="'.nome_swot((isset($plano_acao_swot) ? $plano_acao_swot : null)).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popSWOT();">'.imagem('../../../modulos/swot/imagens/swot_p.png','Selecionar Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para selecionar um campo de matriz SWOT.').'</a></td></tr></table></td></tr>';
	}
else {
	echo '<input type="hidden" name="plano_acao_mswot" value="" id="mswot" /><input type="hidden" id="mswot_nome" name="mswot_nome" value="">';
	echo '<input type="hidden" name="plano_acao_swot" value="" id="swot" /><input type="hidden" id="swot_nome" name="swot_nome" value="">';
	}
if ($ata_ativo) echo '<tr '.(isset($plano_acao_ata) && $plano_acao_ata ? '' : 'style="display:none"').' id="ata" ><td align="right" nowrap="nowrap">'.dica('Ata de Reunião', 'Caso '.$config['genero_acao'].' '.$config['acao'].' seja específic'.$config['genero_acao'].' de uma ata de reunião neste campo deverá constar o nome da ata').'Ata de Reunião:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="plano_acao_ata" value="'.(isset($plano_acao_ata) ? $plano_acao_ata : '').'" /><input type="text" id="ata_nome" name="ata_nome" value="'.nome_ata((isset($plano_acao_ata) ? $plano_acao_ata : null)).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popAta();">'.imagem('../../../modulos/atas/imagens/ata_p.png','Selecionar Ata de Reunião','Clique neste ícone '.imagem('../../../modulos/atas/imagens/ata_p.png').' para selecionar uma ata de reunião.').'</a></td></tr></table></td></tr>';
else echo '<input type="hidden" name="plano_acao_ata" value="" id="ata" /><input type="hidden" id="ata_nome" name="ata_nome" value="">';
if ($operativo_ativo) echo '<tr '.($plano_acao_operativo ? '' : 'style="display:none"').' id="operativo" ><td align="right">'.dica('Plano operativo', 'Caso '.$config['genero_acao'].' '.$config['acao'].' seja específic'.$config['genero_acao'].' de um plano operativo, neste campo deverá constar o nome do plano operativo.').'Operativo:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="plano_acao_operativo" value="'.$plano_acao_operativo.'" /><input type="text" id="operativo_nome" name="operativo_nome" value="'.nome_operativo($plano_acao_operativo).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popOperativo();">'.imagem('icones/operativo_p.png','Selecionar Plano Operativo','Clique neste ícone '.imagem('icones/operativo_p.png').' para selecionar um plano operativo.').'</a></td></tr></table></td></tr>';
...
if ($Aplic->profissional){
  ...
  $sql->limpar();
	...
	foreach($lista as $gestao_data){
		...
		echo dica('Mover para Cima', 'Clique neste ícone '.imagem('icones/1setacima.gif').' para mover acima').'<a href="javascript:void(0);" onclick="javascript:mudar_posicao_gestao('.$gestao_data['plano_acao_gestao_ordem'].', '.$gestao_data['plano_acao_gestao_id'].', \'moverParaCima\');"><img src="'.acharImagem('icones/1setacima.gif').'" border=0/></a>'.dicaF();
		echo dica('Mover para Baixo', 'Clique neste ícone '.imagem('icones/1setabaixo.gif').' para mover abaixo').'<a href="javascript:void(0);" onclick="javascript:mudar_posicao_gestao('.$gestao_data['plano_acao_gestao_ordem'].', '.$gestao_data['plano_acao_gestao_id'].', \'moverParaBaixo\');"><img src="'.acharImagem('icones/1setabaixo.gif').'" border=0/></a>'.dicaF();
		...
		elseif ($gestao_data['plano_acao_gestao_mswot']) echo '<td align=left>'.imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['plano_acao_gestao_mswot']).'</td>';
		...
		elseif ($gestao_data['plano_acao_gestao_template']) echo '<td align=left>'.imagem('icones/template_p.gif').link_template($gestao_data['plano_acao_gestao_template']).'</td>';
		...
		echo '<td><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir?\')) {excluir_gestao('.$gestao_data['plano_acao_gestao_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir.').'</a></td></tr>';
		}
...
if ($Aplic->profissional){
	if ($exibir['plano_acao_codigo']) echo '<tr><td align="right">'.dica('Código', 'Escreva, caso exista, o código d'.$config['genero_acao'].' '.$config['acao'].'.').'Código:'.dicaF().'</td><td><input type="text" style="width:284px;" class="texto" name="plano_acao_codigo" value="'.(isset($obj->plano_acao_codigo) ? $obj->plano_acao_codigo : '').'" size="30" maxlength="255" /></td></tr>';
	if ($exibir['ano']) echo '<tr><td align="right" nowrap="nowrap">'.dica('Ano', 'A qual ano deverá '.$config['genero_acao'].' '.$config['acao'].' estar relacionad'.$config['genero_acao'].'.').'Ano:'.dicaF().'</td><td width="100%" colspan="2"><input type="text" name="plano_acao_ano" value="'.($obj->plano_acao_ano ? $obj->plano_acao_ano : date('Y')).'" size="4" class="texto" /></td></tr>';
...
$df = '%d/%m/%Y';
if($Aplic->profissional){
	$data_inicio = intval( $obj->plano_acao_inicio ) ? new CData( $obj->plano_acao_inicio ) : false;
	$data_fim = intval( $obj->plano_acao_fim ) ? new CData( $obj->plano_acao_fim ) : false;
	...
	if(!$data_inicio || !$data_fim){
		...
		$obj->plano_acao_duracao = $cache->horasPeriodo($data_inicio, $data_fim, (int)$cia_id);
...
echo '<tr><td align=right width="100" >'.dica('Data de Início', 'A data de início d'.$config['genero_acao'].' '.$config['acao'].'.').'Início:'.dicaF().'</td><td><input type="hidden" id="oculto_data_inicio" name="oculto_data_inicio"  value="'.$data_inicio->format('%Y-%m-%d').'" /><input type="text" onchange="setData(\'env\', \'data_inicio\'); data_ajax();" class="texto" style="width:70px;" id="data_inicio" name="data_inicio" value="'.$data_inicio->format('%d/%m/%Y').'" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data de início.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora do Início', 'Selecione na caixa de seleção a hora do ínicio d'.$config['genero_acao'].' '.$config['acao']). selecionaVetor($horas, 'inicio_hora', 'size="1" onchange="CompararDatas(); data_ajax();" class="texto"', $data_inicio->getHour()).' : '.dica('Minutos do Início', 'Selecione na caixa de seleção os minutos do ínicio d'.$config['genero_acao'].' '.$config['acao']).selecionaVetor($minutos, 'inicio_minuto', 'size="1" class="texto" onchange="CompararDatas(); data_ajax();"', $data_inicio->getMinute()).'</td></tr>';
echo '<tr><td align=right>'.dica('Data de Término', 'A data de término d'.$config['genero_acao'].' '.$config['acao'].'.').'Término:'.dicaF().'</td><td><input type="hidden" id="oculto_data_fim" name="oculto_data_fim" value="'.($data_fim ? $data_fim->format('%Y-%m-%d') : '').'" /><input type="text" onchange="setData(\'env\', \'data_fim\'); horas_ajax();" class="texto" style="width:70px;" id="data_fim" name="data_fim" value="'.($data_fim ? $data_fim->format('%d/%m/%Y') : '').'" />'.dica('Data de Término', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data de término.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora do Término', 'Selecione na caixa de seleção a hora do término.</p>Caso não saiba a hora provável de término, deixe em branco este campo e clique no botão <b>Data de Término</b>').selecionaVetor($horas, 'fim_hora', 'size="1" onchange="CompararDatas(); horas_ajax();" class="texto"', $data_fim ? $data_fim->getHour() : $fim).' : '.dica('Minutos do Término', 'Selecione na caixa de seleção os minutos do término. </p>Caso não saiba os minutos prováveis de término, deixe em branco este campo e clique no botão <b>Data de Término</b>').selecionaVetor($minutos, 'fim_minuto', 'size="1" class="texto" onchange="CompararDatas(); horas_ajax();"', $data_fim ? $data_fim->getMinute() : '00').'</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica('A duração d'.$config['genero_acao'].' '.$config['acao'].' em dias.').'Duração:'.dicaF().'</td><td nowrap="nowrap"><input type="text" onchange="data_ajax();" onkeypress="return somenteFloat(event)" class="texto" name="plano_acao_duracao" id="plano_acao_duracao" maxlength="30" size="2" value="'.float_brasileiro($obj->plano_acao_duracao/($config['horas_trab_diario'] ? $config['horas_trab_diario'] : 8)).'" />&nbsp;dias</td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'plano_acao_moeda', 'class=texto size=1', ($obj->plano_acao_moeda ? $obj->plano_acao_moeda : 1)).'</td></tr>';
	}	
else echo '<input type="hidden" name="plano_acao_moeda" id="plano_acao_moeda" value="'.($obj->plano_acao_moeda ? $obj->plano_acao_moeda : 1).'" />';

echo '<tr><td align="right" nowrap="nowrap">'.dica('Cor', 'Para facilitar a visualização pode-se escolher uma das 216 cores pré-definidas, bastando clicar no retângulo colorido na ponta direita. Caso deseje uma cor inexistente na paleta de cores deste programa insira o valor Hexadecimal da mesma, na caixa de texto logo à direita.').'Cor:'.dicaF().'</td><td nowrap="nowrap" align="left"><input type="text" name="plano_acao_cor" value="'.($obj->plano_acao_cor ? $obj->plano_acao_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="10" maxlength="6" onblur="setCor();" class="texto" />&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript: void(0);" onclick="if (window.parent.gpwebApp) parent.gpwebApp.popUp(\'Cor\', 300, 290, \'m=publico&a=selecao_cor&dialogo=1&chamar_volta=setCor\', window.setCor, window); else newwin=window.open(\'./index.php?m=publico&a=selecao_cor&dialogo=1&chamar_volta=setCor\', \'calwin\', \'width=310, height=300, scrollbars=no\');">'.dica('Mudar Cor', 'Para facilitar a visualização dos eventos pode-se escolher uma das 216 cores pré-definidas, bastando clicar no retângulo colorido. Caso deseje uma cor inexistente na paleta de cores deste programa insira o valor Hexadecimal da mesma, na caixa de texto à esquerda.').'Mudar cor&nbsp;&nbsp;<span id="teste" style="border:solid;border-width:1;background:#'.($obj->plano_acao_cor ? $obj->plano_acao_cor : 'FFFFFF').';">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></a>'.dicaF().'</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nível de Acesso', 'Os '.$config['objetivos'].' podem ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar '.$config['genero_acao'].' '.$config['acao'].'.</li><li><b>Protegido</b> - Todos podem ver, porem apenas o responsável e os designados para '.$config['genero_acao'].' '.$config['acao'].' podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante</b> - Somente o responsável e os designados para '.$config['genero_acao'].' '.$config['acao'].' ver e editar '.$config['genero_acao'].' '.$config['acao'].'</li><li><b>Privado</b> - Somente o responsável e os designados para '.$config['genero_acao'].' '.$config['acao'].' podem ver a mesma, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($plano_acao_acesso, 'plano_acao_acesso', 'class="texto"', ($plano_acao_id ? $obj->plano_acao_acesso  : $config['nivel_acesso_padrao'])).'</td></tr>';

echo '<tr><td align="right" width="100">'.dica('Ativo', 'Caso '.$config['genero_acao'].' '.$config['acao'].' ainda esteja ativ'.$config['genero_acao'].' deverá estar marcado este campo.').'Ativo:'.dicaF().'</td><td><input type="checkbox" value="1" name="plano_acao_ativo" '.($obj->plano_acao_ativo || !$plano_acao_id ? 'checked="checked"' : '').' /></td></tr>';
...
echo '<tr><td align="right" valign="top" nowrap="nowrap" width=120>'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($plano_acao_id > 0 ? 'modificação' : 'criação').' d'.$config['genero_acao'].' '.$config['acao'].'.').'Notificar:'.dicaF().'</td><td><input type="checkbox" name="email_responsavel" id="email_responsavel" '.($Aplic->getPref('informa_responsavel') ? 'checked="checked"' : '').' value="1" />'.dica('Responsável pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Caso esta caixa esteja selecionada, um e-mail será enviado para o responsável pel'.$config['genero_acao'].' '.$config['acao'].'.').'<label for="email_responsavel">Responsável</label>'.dicaF().'<input type="checkbox" name="email_designados" id="email_designados" '.($Aplic->getPref('informa_designados') ? 'checked="checked"' : '').' />'.dica('Designados para '.$config['genero_acao'].' '.ucfirst($config['acao']), 'Caso esta caixa esteja selecionada, um e-mail será enviado para os designados para '.$config['genero_acao'].' '.$config['acao'].'.').'<label for="email_designados">Designados</label>'.dicaF().'<input type="checkbox" name="email_contatos" id="email_contatos" '.($Aplic->getPref('informa_contatos') ? 'checked="checked"' : '').' />'.dica('Contatos d'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Caso esta caixa esteja selecionada, um e-mail será enviado para os contatos d'.$config['genero_acao'].' '.$config['acao'].'.').'<label for="email_contatos">Contatos</label>'.dicaF().'</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica('Texto', 'Os dados básicos d'.$config['genero_acao'].' '.$config['acao'].' são automaticamente acrescentado nas mnsagens enviadas, porem escreva na caixa de texto caso deseja enviar outras informações junto com a mensagem.').'Texto:'.dicaF().'</td><td><textarea data-gpweb-cmp="ckeditor" name="email_comentario" class="textarea" style="width:284px;" rows="1"></textarea></td></tr>';

echo '</table></td></tr>';

echo '<tr><td colspan=20><table cellspacing=0 cellpadding=0 width="100%"><tr><td>'.botao('salvar', 'Salvar', 'Salvar os dados.','','enviarDados()').'</td>'.(!$dialogo ? '<td align="right">'.botao('cancelar', 'Cancelar', 'Cancelar a '.($plano_acao_id ? 'edição' : 'criação').' do pratica.','','if(confirm(\'Tem certeza que deseja cancelar?\')){url_passar(0, \''.$Aplic->getPosicao().'\');}').'</td>':'').'</tr></table></td></tr>';
echo '</table>';
echo '</form>';
...
function horas_ajax(){
	...
	xajax_calcular_duracao(inicio, fim, document.getElementById('plano_acao_cia').value);
	}
...
function data_ajax(){
	var f=document.env;
	var inicio=f.oculto_data_inicio.value+' '+f.inicio_hora.value+':'+f.inicio_minuto.value+':00';
	var horas=f.plano_acao_duracao.value;
	xajax_data_final_periodo(inicio, horas, document.getElementById('plano_acao_cia').value);
	} 
...
function popGerente() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Responsável', 500, 500, 'm=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setGerente&cia_id='+document.getElementById('plano_acao_cia').value+'&usuario_id='+document.getElementById('plano_acao_responsavel').value, window.setGerente, window);
	else window.open('./index.php?m=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setGerente&cia_id='+document.getElementById('plano_acao_cia').value+'&usuario_id='+document.getElementById('plano_acao_responsavel').value, 'Responsável','height=500,width=500,resizable,scrollbars=yes, left=0, top=0');
	}
...
function enviarDados() {
	...
	if (f.existe_acao.value > 0) {
		alert('Já existe outr<?php echo $config["genero_acao"]." ".$config["acao"]?> com o mesmo nome');
		f.plano_acao_nome.focus();
		return;
		}

	if (f.plano_acao_nome.value.length < 3) {
		alert('Escreva um nome para <?php echo $config["genero_acao"]." ".$config["acao"]?> válido');
		f.plano_acao_nome.focus();
		return;
		}

	document.getElementById('plano_acao_inicio').value=document.getElementById('oculto_data_inicio').value+' '+document.getElementById('inicio_hora').value+':'+document.getElementById('inicio_minuto').value+':00';
	
	document.getElementById('plano_acao_fim').value=document.getElementById('oculto_data_fim').value+' '+document.getElementById('fim_hora').value+':'+document.getElementById('fim_minuto').value+':00';
...
<?php  if ($Aplic->profissional) { ?>
	...
	function popOdometro() {
		if (window.parent.gpwebApp) parent.gpwebApp.popUp('Odômetro', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setOdometro&tabela=painel_odometro&cia_id='+document.getElementById('plano_acao_cia').value, window.setOdometro, window);
		else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setOdometro&tabela=painel_odometro&cia_id='+document.getElementById('plano_acao_cia').value, 'Odômetro','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
		}
	...
	function popComposicaoPaineis() {
		if (window.parent.gpwebApp) parent.gpwebApp.popUp('Composição de Painéis', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setComposicaoPaineis&tabela=painel_composicao&cia_id='+document.getElementById('plano_acao_cia').value, window.setComposicaoPaineis, window);
		else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setComposicaoPaineis&tabela=painel_composicao&cia_id='+document.getElementById('plano_acao_cia').value, 'Composição de Painéis','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
		}
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('plano_acao_cia').value, window.setObjetivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('plano_acao_cia').value, '<?php echo ucfirst($config["objetivo"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('plano_acao_cia').value, window.setFator, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('plano_acao_cia').value, '<?php echo ucfirst($config["fator"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popAta() {
	parent.gpwebApp.popUp('Ata de Reunião', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setAta&tabela=ata&cia_id='+document.getElementById('plano_acao_cia').value, window.setAta, window);
	}
...
function popMSWOT() {
	parent.gpwebApp.popUp('Matriz SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setMSWOT&tabela=mswot&cia_id='+document.getElementById('plano_acao_cia').value, window.setMSWOT, window);
	}

function setMSWOT(chave, valor){
	limpar_tudo();
	document.env.plano_acao_mswot.value = chave;
	document.env.mswot_nome.value = valor;
	<?php if ($Aplic->profissional) echo 'incluir_relacionado();' ?>
	}

function popSWOT() {
	parent.gpwebApp.popUp('Campo SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setSWOT&tabela=swot&cia_id='+document.getElementById('plano_acao_cia').value, window.setSWOT, window);
	}
...
function popInstrumento() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Instrumento Jurídico', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setInstrumento&tabela=instrumento&cia_id='+document.getElementById('plano_acao_cia').value, window.setInstrumento, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setInstrumento&tabela=instrumento&cia_id='+document.getElementById('plano_acao_cia').value, 'Instrumento Jurídico','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popAvaliacao() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Avaliação', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setAvaliacao&tabela=avaliacao&cia_id='+document.getElementById('plano_acao_cia').value, window.setAvaliacao, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setAvaliacao&tabela=avaliacao&cia_id='+document.getElementById('plano_acao_cia').value, 'Avaliação','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popArquivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Arquivo', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('plano_acao_cia').value, window.setArquivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('plano_acao_cia').value, 'Arquivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popForum() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Fórum', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setForum&tabela=foruns&cia_id='+document.getElementById('plano_acao_cia').value, window.setForum, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setForum&tabela=foruns&cia_id='+document.getElementById('plano_acao_cia').value, 'Fórum','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function limpar_tudo(){
	...
	<?php
	if($swot_ativo) {
		echo 'document.env.mswot_nome.value = \'\';	document.env.plano_acao_mswot.value = null;';
		echo 'document.env.swot_nome.value = \'\';	document.env.plano_acao_swot.value = null;';
		}
...
if (document.getElementById('plano_acao_duracao').value==0) horas_ajax();
