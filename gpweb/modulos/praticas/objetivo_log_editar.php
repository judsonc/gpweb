...
global $Aplic, $objetivo_id, $obj, $percentual, $cal_sdf;
$niveis_acesso=getSisValor('NivelAcesso','','','sisvalor_id');
$Aplic->carregarCalendarioJS();
$objetivo_log_id = intval(getParam($_REQUEST, 'objetivo_log_id', 0));
require_once (BASE_DIR.'/modulos/praticas/obj_estrategico.class.php');
$log = new CObjetivoLog();
if ($objetivo_log_id) {
	$log->load($objetivo_log_id);
	} 
else {
	$log->objetivo_log_objetivo = $objetivo_id;
	}
$RefRegistroTarefa = getSisValor('RefRegistroTarefa');
$df = '%d/%m/%Y';
$log_data = new CData($log->objetivo_log_data);
echo '<a name="log"></a>';
echo '<form name="frmEditar" method="post" onsubmit=\'atualizarEmailContatos();\'>';
echo '<input type="hidden" name="m" value="praticas" />';
echo '<input name="a" type="hidden" value="vazio" />';
echo '<input name="u" type="hidden" value="" />';
echo '<input type="hidden" name="fazerSQL" value="objetivo_log_fazer_sql" />';
echo '<input type="hidden" name="objetivo_id" value="'.$objetivo_id.'" />';
echo '<input type="hidden" name="uniqueid" value="'.uniqid('').'" />';
echo '<input type="hidden" name="objetivo_log_id" value="'.$log->objetivo_log_id.'" />';
echo '<input type="hidden" name="objetivo_log_objetivo" value="'.$log->objetivo_log_objetivo.'" />';
echo '<input type="hidden" name="objetivo_log_criador" value="'.($log->objetivo_log_criador == 0 ? $Aplic->usuario_id : $log->objetivo_log_criador).'" />';
echo '<input type="hidden" name="objetivo_log_nome" value="Atualizado :'.$log->objetivo_log_nome.'" />';
...
echo '<tr><td align="right">'.dica('Data', 'Escolha qual a data deste registro de ocorrência.').'Data:'.dicaF().'</td><td nowrap="nowrap"><input type="hidden" name="objetivo_log_data" id="objetivo_log_data" value="'.$log_data->format(FMT_TIMESTAMP_DATA).'" /><input type="text" name="log_date" id="log_date" onchange="setData(\'frmEditar\', \'log_date\');" value="'.$log_data->format($df).'" class="texto" />'.dica('Data do Registro', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data deste registro.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
echo '<tr><td align="right">'.dica('Horas Trabalhadas', 'Horas trabalhadas n'.$config['genero_objetivo'].' '.$config['objetivo'].'.<br><br>Ex: Para inserir 1h30min digite 1.5').'Horas Trab.:'.dicaF().'</td><td nowrap="nowrap"><input type="text" style="text-align:right;" class="texto" name="objetivo_log_horas" value="'.$log->objetivo_log_horas.'" maxlength="8" size="4" /></td></tr>';
echo '<tr><td align="right">'.dica('Valor Gasto', 'Valor gasto n'.$config['genero_objetivo'].' '.$config['objetivo'].'.').'Valor Gasto:'.dicaF().'</td><td><input type="text" style="text-align:right;" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" name="objetivo_log_custo" value="'.($log->objetivo_log_custo ? number_format($log->objetivo_log_custo, 2, ',', '.') : '').'" size="40" /></td></tr>';  

$categoria_economica=array(''=>'')+getSisValor('CategoriaEconomica');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Categoria Econômica', 'Caso insira um valor gasto, seleciona a categoria econômica deste item.').'Categoria econômica:'.dicaF().'</td><td>'.selecionaVetor($categoria_economica, 'objetivo_log_categoria_economica', 'class=texto size=1 style="width:250px;" onchange="frmEditar.objetivo_log_nd.value=\'\'; mudar_nd();"', (isset($log->objetivo_log_categoria_economica) ? $log->objetivo_log_categoria_economica :'')).'</td></tr>';

$GrupoND=array(''=>'')+getSisValor('GrupoND');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Grupo de Despesa', 'Caso insira um valor gasto, seleciona o grupo de despesa deste item.').'Grupo de despesa:'.dicaF().'</td><td>'.selecionaVetor($GrupoND, 'objetivo_log_grupo_despesa', 'class=texto size=1 style="width:250px;" onchange="frmEditar.objetivo_log_nd.value=\'\'; mudar_nd();"', (isset($log->objetivo_log_grupo_despesa) ? $log->objetivo_log_grupo_despesa :'')).'</td></tr>';

$ModalidadeAplicacao=array(''=>'')+getSisValor('ModalidadeAplicacao');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Modalidade de Aplicação', 'Caso insira um valor gasto, seleciona a modalidade de aplicação deste item.').'Modalidade de aplicação:'.dicaF().'</td><td>'.selecionaVetor($ModalidadeAplicacao, 'objetivo_log_modalidade_aplicacao', 'class=texto size=1 style="width:250px;" onchange="frmEditar.objetivo_log_nd.value=\'\'; mudar_nd();"', (isset($log->objetivo_log_modalidade_aplicacao) ? $log->objetivo_log_modalidade_aplicacao :'')).'</td></tr>';


$nd=vetor_nd((isset($log->objetivo_log_nd) ? $log->objetivo_log_nd : ''), null, null, 3 ,(isset($log->objetivo_log_categoria_economica) ?  $log->objetivo_log_categoria_economica : ''), (isset($log->objetivo_log_grupo_despesa) ?  $log->objetivo_log_grupo_despesa : ''), (isset($log->objetivo_log_modalidade_aplicacao) ?  $log->objetivo_log_modalidade_aplicacao : ''));
echo '<tr><td align="right">'.dica('Natureza da Despesa', 'Caso insira um valor gasto, seleciona qual a natureza da despesa do mesmo.').'ND:'.dicaF().'</td><td><div id="combo_nd">'.selecionaVetor($nd, 'objetivo_log_nd', 'class=texto size=1 style="width:250px;" onchange="mudar_nd();"', (isset($log->objetivo_log_nd) && $log->objetivo_log_nd ? $log->objetivo_log_nd :'')).'</div></td></tr>';

echo '<tr><td align="right" nowrap="nowrap">'.dica('Nível de Acesso', 'O registro de ocorrência d'.$config['genero_objetivo'].' '.$config['objetivo'].' pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido</b> - Todos podem ver, porem apenas o responsável pel'.$config['genero_objetivo'].' '.$config['objetivo'].' e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante</b> - Somente o responsável pel'.$config['genero_objetivo'].' '.$config['objetivo'].' e os designados podem ver e editar</li><li><b>Privado</b> - Somente o responsável pel'.$config['genero_objetivo'].' '.$config['objetivo'].' e os designados podem ver, e o responsável editar.</li></ul>').'Nível de Acesso'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($niveis_acesso, 'objetivo_log_acesso', 'class="texto"', ($objetivo_log_id ? $log->objetivo_log_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';	
echo '</table></td>';
...
echo '<tr><td align="left"><input type="text" class="texto" name="objetivo_log_nome" value="'.$log->objetivo_log_nome.'" maxlength="255" size="30" /></td><td align="center">'.'<input type="checkbox" value="1" name="objetivo_log_problema" id="objetivo_log_problema" '.($log->objetivo_log_problema ? 'checked="checked"' : '').' />'.dica('Problema', 'Caso esta caixa esteja selecionada, este registro será marcado como de problema.<br><br>Ele se diferenciará dos outros registros por ter um fundo vermelho no sumário para chamar a atenção.').'<label for="objetivo_log_problema">Problema</label>'.dicaF().'</td></tr>';
echo '</table></td></tr>';
echo '<tr><td align="right" valign="middle">'.dica('Referência', 'Escolha de que forma chegou aos dados que aqui estão registrados.').'Referência:'.dicaF().'</td><td valign="middle">'.selecionaVetor($RefRegistroTarefa, 'objetivo_log_referencia', 'size="1" class="texto"', $log->objetivo_log_referencia).'</td></tr>';
echo '<tr><td align="right">'.dica('Endereço Eletrônico desta Referência', 'Escreva, caso exista, um link para página ou arquivo na rede que faz referência a este registro tal como visualiza na tela no Navegador Web.<br>Para link para páginas da internet é necessário escrever http://<br>Ex: <b>http://www.sistemagpweb.com</b>').'URL:'.dicaF().'</td><td><input type="text" class="texto" name="objetivo_log_url_relacionada" value="'.($log->objetivo_log_url_relacionada).'" size="50" maxlength="255" /></td></tr>';
echo '<tr><td align="right" valign="top">'.dica('Descrição', 'Escreva uma descrição pormenorizada sobre este registro.').'Descrição:'.dicaF().'</td><td><textarea name="objetivo_log_descricao" class="textarea" cols="50" rows="6">'.$log->objetivo_log_descricao.'</textarea></td></tr>';


echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($objetivo_log_id > 0 ? 'modificação' : 'criação').' do registro.').'Notificar:'.dicaF().'</td>';
echo '<td>';

$q = new BDConsulta;
$q->adTabela('objetivo_usuario');
$q->esqUnir('usuarios', 'usuarios', 'usuarios.usuario_id = objetivo_usuario_usuario');
$q->esqUnir('contatos', 'contatos', 'contatos.contato_id = usuarios.usuario_contato');
$q->adOnde('objetivo_usuario_objetivo = '.(int)$objetivo_id);
...
echo '<input type="hidden" name="email_objetivo_lista" id="email_objetivo_lista" value="'.implode(',',$designados).'" />';
...
function mudar_nd(){
	xajax_mudar_nd_ajax(frmEditar.objetivo_log_nd.value, 'objetivo_log_nd', 'combo_nd','class=texto size=1 style="width:250px;" onchange="mudar_nd();"', 3, frmEditar.objetivo_log_categoria_economica.value, frmEditar.objetivo_log_grupo_despesa.value, frmEditar.objetivo_log_modalidade_aplicacao.value);
	}

function updateTarefa() {
	var f = document.frmEditar;
	if (f.objetivo_log_descricao.value.length < 1) {
		alert( 'Por favor, insira uma descrição à ocorrência.' );
		f.objetivo_log_descricao.focus();
		} 
	else {
		f.objetivo_log_custo.value=moeda2float(f.objetivo_log_custo.value);
		f.submit();
		}
	}
...
var cal1 = Calendario.setup({
  ...
  if (date){
    ...
    document.getElementById("objetivo_log_data").value = Calendario.printDate(date, "%Y-%m-%d");
    }
	cal1.hide(); 
	}
});
...
function atualizarEmailContatos() {
	var email_outro = document.getElementById('email_outro');
	var objetivo_emails = document.getElementById('email_objetivo_lista');
