...
//editar_ajax
function mudar_posicao_objetivo($ordem, $fator_objetivo_id, $direcao, $fator_id=0, $fator_objetivo_uuid=''){
	...
	if($direcao&&$fator_objetivo_id) {
		...
		else $sql->adOnde('fator_objetivo_fator = '.$fator_id);
	...
	$saida=atualizar_objetivos($fator_id, $fator_objetivo_uuid);
...
function incluir_objetivo($fator_id=null, $fator_objetivo_uuid=null, $fator_objetivo_objetivo=null, $fator_objetivo_me=null){
	$sql = new BDConsulta;
	...
	if ($fator_objetivo_uuid) $sql->adOnde('fator_objetivo_uuid = \''.$fator_objetivo_uuid.'\'');
	else $sql->adOnde('fator_objetivo_fator ='.(int)$fator_id);	
  ...
  $sql->limpar();
	...
	if (!$fator_objetivo_id){
	  ...
	  $sql->limpar();
		...
		else $sql->adInserir('fator_objetivo_fator', $fator_id);
...
function excluir_objetivo($fator_objetivo_id, $fator_id, $fator_objetivo_uuid=''){
	$sql = new BDConsulta;
...
function atualizar_objetivos($fator_id=0, $fator_objetivo_uuid=''){
	...
	$saida='';
	if (count($objetivos)) {
		$saida.= '<table cellspacing=0 cellpadding=0 class="tbl1" align=left><tr><th></th><th>'.dica('Nome', 'Mome do objeto relacionado.').'Nome'.dicaF().'</th><th></th></tr>';
