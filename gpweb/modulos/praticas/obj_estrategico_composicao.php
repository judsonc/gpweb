...
if (count($objetivos_recebidas)){
	$sql->adTabela('objetivo');
	$sql->esqUnir('cias','cias','objetivo_cia=cia_id');
	$sql->adCampo('objetivo_id, concatenar_tres(objetivo_nome, \' - \', cia_nome) AS nome');
	$sql->adOnde('objetivo_id IN ('.implode(',',$objetivos_recebidas).')');
	$lista=$sql->Lista();
	$sql->limpar();
	
	foreach($lista as $linha) $objetivos_selecionados[$linha['objetivo_id']]=$linha['nome'];
...
if ($objetivo_id) $sql->adOnde('objetivo_id!='.$objetivo_id);
$sql->adOnde('objetivo_cia='.(int)$cia_id);
$lista=$sql->Lista();
$sql->limpar();

$objetivos=array();
foreach($lista as $linha) $objetivos[$linha['objetivo_id']]=$linha['nome'];
