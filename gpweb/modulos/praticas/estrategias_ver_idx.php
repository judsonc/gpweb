...
global $estilo_interface, $sql, $perms, $dialogo, $Aplic, $cia_id, $dept_id, $lista_depts, $tab, $lista_cias, $favorito_id, $usuario_id, $pesquisar_texto, $pg_id, $podeEditar, $filtro_prioridade_estrategia;
...
$sql->adTabela('estrategias');
$sql->adCampo('estrategias.pg_estrategia_id, pg_estrategia_inicio, pg_estrategia_fim, pg_estrategia_percentagem, pg_estrategia_nome, pg_estrategia_descricao, pg_estrategia_usuario, pg_estrategia_cor, pg_estrategia_acesso, pg_estrategia_ano, pg_estrategia_codigo, pg_estrategia_cia, pg_estrategia_aprovado');
...
if ($xtotal_paginas > 1) mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, ucfirst($config['iniciativa']), ucfirst($config['iniciativas']),'','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=pg_estrategia_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='pg_estrategia_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_estrategia) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($estrategias as $linha) {
	if (permiteAcessarEstrategia($linha['pg_estrategia_acesso'],$linha['pg_estrategia_id'])){
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['pg_estrategia_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_estrategia) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';
		...
		if ($Aplic->profissional && $exibir['percentagem'])echo '<td style="margin-bottom:0cm; margin-top:0cm;">'.($linha['pg_estrategia_percentagem'] ? number_format($linha['pg_estrategia_percentagem'], 1, ',', '.') : '&nbsp;').'</td>';
