...
if (isset($_REQUEST['filtro_prioridade_meta']))	$Aplic->setEstado('filtro_prioridade_meta', getParam($_REQUEST, 'filtro_prioridade_meta', null));
$filtro_prioridade_meta = $Aplic->getEstado('filtro_prioridade_meta') !== null ? $Aplic->getEstado('filtro_prioridade_meta') : null;

$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_meta=1');
...
if (isset($_REQUEST['metatextobusca'])) $Aplic->setEstado('metatextobusca',getParam($_REQUEST, 'metatextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('metatextobusca') ? $Aplic->getEstado('metatextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaMetaTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaMetaTab') !== null ? $Aplic->getEstado('ListaMetaTab') : 0);
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('meta_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('meta_favorito') !== null ? $Aplic->getEstado('meta_favorito') : 0;

$sql->adTabela('favorito');
$sql->adCampo('favorito_id, favorito_nome');
$sql->adOnde('favorito_geral!=1');
$sql->adOnde('favorito_ativo=1');
$sql->adOnde('favorito_meta=1');
...
echo '<input type="hidden" name="filtro_prioridade_meta" id="filtro_prioridade_meta" value="'.$filtro_prioridade_meta.'" />';
...
if (!$dialogo){
	...
	if($plano_acao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_acao'].' '.$config['acao'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['acao']).':'.dicaF();
		$nome=nome_acao($plano_acao_id);
		}
	elseif($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela agenda que estão relacionad'.$config['genero_meta'].'s.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo indicador que estão relacionad'.$config['genero_meta'].'s.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($link_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo link que estão relacionad'.$config['genero_meta'].'s.').'Link:'.dicaF();
		$nome=nome_link($link_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo monitoramento que estão relacionad'.$config['genero_meta'].'s.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela ata de reunião a qual estão relacionad'.$config['genero_meta'].'s.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}	
	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela matriz SWOT que estão relacionad'.$config['genero_meta'].'s.').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($swot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo campo SWOT', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo campo SWOT que estão relacionad'.$config['genero_meta'].'s.').'Campo SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo plano operativo que estão relacionad'.$config['genero_meta'].'s.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo instrumento jurídico que estão relacionad'.$config['genero_meta'].'s.').'Instrumento Jurídico:'.dicaF();
		$nome=nome_instrumento($instrumento_id);
		}	
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo recurso que estão relacionad'.$config['genero_meta'].'s.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela demanda que estão relacionad'.$config['genero_meta'].'s.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo evento que estão relacionad'.$config['genero_meta'].'s.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela avaliação que estão relacionad'.$config['genero_meta'].'s.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo brainstorm que estão relacionad'.$config['genero_meta'].'s.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pela matriz GUT que estão relacionad'.$config['genero_meta'].'s.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo diagrama de causa-efeito que estão relacionad'.$config['genero_meta'].'s.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($arquivo_id){
		$legenda_filtro=dica('Filtrar pelo Arquivo', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo arquivo que estão relacionad'.$config['genero_meta'].'s.').'Arquivo:'.dicaF();
		$nome=nome_arquivo($arquivo_id);
		}	
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo fórum que estão relacionad'.$config['genero_meta'].'s.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($checklist_id){
		$legenda_filtro=dica('Filtrar pelo Checklist', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo checklist que estão relacionad'.$config['genero_meta'].'s.').'Checklist:'.dicaF();
		$nome=nome_checklist($checklist_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo compromisso que estão relacionad'.$config['genero_meta'].'s.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo agrupamento que estão relacionad'.$config['genero_meta'].'s.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo patrocinador que estão relacionad'.$config['genero_meta'].'s.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar '.$config['genero_meta'].'s '.$config['metas'].' pelo modelo que estão relacionad'.$config['genero_meta'].'s.').'Modelo:'.dicaF();
	...
	elseif($tr_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tr'].' '.ucfirst($config['tr']), 'Filtrar pel'.$config['genero_tr'].' '.$config['tr'].' que estão relacionad'.$config['genero_meta'].'s.').ucfirst($config['tr']).':'.dicaF();
		$nome=nome_tr($tr_id);
		}
	elseif($me_id){
		...
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar '.$config['genero_meta'].'s '.$config['metas'].'.').'Filtro:'.dicaF();
		}
	...
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['metas']), 'meta.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito='';
	$botao_trava='';
	$nova_meta=($podeAdicionar ? '<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="javascript:env.a.value=\'meta_editar\'; env.submit();" >'.imagem('icones/meta_novo.png', 'Nov'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Clique neste ícone '.imagem('icones/meta_novo.png').' para criar uma nov'.$config['genero_meta'].' '.$config['meta'].'.').'</a></td></tr>' : '');
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['metas']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['metas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=meta_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	if($filtro_prioridade_meta) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	if ($Aplic->profissional){
		$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionado','A qual parte do sistema os link estão relacionad'.$config['genero_meta'].'s.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
		...
		$botao_favorito='<tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de planos de ação favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&meta=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$nova_meta.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['metas']).'</center></font>';
	}
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/metas_ver_idx', 'Ativ'.$config['genero_meta'].'s',null,null,'Ativ'.$config['genero_meta'].'s','Visualizar '.$config['genero_meta'].'s '.$config['metas'].' ativ'.$config['genero_meta'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/metas_ver_idx', 'Inativ'.$config['genero_meta'].'s',null,null,'Inativ'.$config['genero_meta'].'s','Visualizar '.$config['genero_meta'].'s '.$config['metas'].' inativ'.$config['genero_meta'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/metas_ver_idx', 'Tod'.$config['genero_meta'].'s',null,null,'Tod'.$config['genero_meta'].'s','Visualizar tod'.$config['genero_meta'].'s '.$config['genero_meta'].'s '.$config['metas'].'.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/metas_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_meta.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_meta){
	env.filtro_prioridade_meta.value=filtro_prioridade_meta;
	env.submit();
	}	
