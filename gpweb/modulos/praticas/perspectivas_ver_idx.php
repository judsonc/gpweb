...
global $estilo_interface, $sql, $perms, $dialogo, $Aplic, $cia_id, $lista_cias, $dept_id, $lista_depts, $tab,  $favorito_id, $usuario_id, $pesquisar_texto, $pg_id, $favorito_id, $podeEditar, $filtro_prioridade_perspectiva;
...
$sql->adCampo('perspectivas.pg_perspectiva_id, pg_perspectiva_nome, pg_perspectiva_usuario, pg_perspectiva_acesso, pg_perspectiva_cor, pg_perspectiva_descricao, pg_perspectiva_percentagem, pg_perspectiva_aprovado');
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='pg_perspectiva_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovad'.$config['genero_perspectiva'], 'Neste campo consta se '.$config['genero_perspectiva'].' '.$config['perspectiva'].' foi aprovad'.$config['genero_perspectiva'].'.').'Aprovad'.$config['genero_perspectiva'].dicaF().'</a></th>';
if ($filtro_prioridade_perspectiva) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($perspectivas as $linha) {
	if (permiteAcessarPerspectiva($linha['pg_perspectiva_acesso'],$linha['pg_perspectiva_id'])){
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['pg_perspectiva_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_perspectiva) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';
		...
		if ($Aplic->profissional) echo '<td nowrap="nowrap" align=right width=30>'.number_format($linha['pg_perspectiva_percentagem'], 2, ',', '.').'</td>';
		echo '</tr>';
		}
	}
