...
function mudar_posicao_perspectiva($ordem, $objetivo_perspectiva_id, $direcao, $objetivo_id=0, $objetivo_perspectiva_uuid=''){
	...
	if($direcao&&$objetivo_perspectiva_id) {
		...
		else $sql->adOnde('objetivo_perspectiva_objetivo = '.$objetivo_id);
	...
	$saida=atualizar_perspectivas($objetivo_id, $objetivo_perspectiva_uuid);
...
function incluir_perspectiva($objetivo_id=null, $objetivo_perspectiva_uuid=null, $objetivo_perspectiva_perspectiva=null, $objetivo_perspectiva_tema=null){
	global $Aplic;
	...
	if (!$Aplic->profissional){
		...
		if ($objetivo_perspectiva_uuid) $sql->adOnde('objetivo_perspectiva_uuid = \''.$objetivo_perspectiva_uuid.'\'');
		else $sql->adOnde('objetivo_perspectiva_objetivo ='.(int)$objetivo_id);	
		$sql->exec();
		$sql->limpar();
		}	
	  ...
	  $sql->limpar();
		...
		if (!$objetivo_perspectiva_id){
		  ...
		  $sql->limpar();
			...
			else $sql->adInserir('objetivo_perspectiva_objetivo', $objetivo_id);
...
function excluir_perspectiva($objetivo_perspectiva_id, $objetivo_id, $objetivo_perspectiva_uuid=''){
	$sql = new BDConsulta;
	$sql->setExcluir('objetivo_perspectiva');
	$sql->adOnde('objetivo_perspectiva_id='.$objetivo_perspectiva_id);
	$sql->exec();
...
function atualizar_perspectivas($objetivo_id=0, $objetivo_perspectiva_uuid=''){
	...
	if (count($perspectivas)) {
		$saida.= '<table cellspacing=0 cellpadding=0 class="tbl1" align=left><tr>'.($Aplic->profissional ? '<th></th>' : '').'<th>Nome</th><th></th></tr>';
		foreach ($perspectivas as $perspectiva) {
			...
			$saida.= '<td><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir?\')) {excluir_perspectiva('.$perspectiva['objetivo_perspectiva_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir.').'</a></td>';
			$saida.= '</tr>';
			}
		$saida.= '</table>';
		}
...
function atualizar_perspectiva_ajax($cia_id=1, $ano='', $posicao){
	...
	$perspectiva[0]='';
	$saida=selecionaVetor($perspectiva, 'objetivo_perspectiva', 'class="texto" size=1');
	$objResposta = new xajaxResponse();
	$objResposta->assign($posicao,"innerHTML", $saida);
	return $objResposta;
	}	
