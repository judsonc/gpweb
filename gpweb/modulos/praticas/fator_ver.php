...
if (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo->adicionaCelula(dica('Imprimir '.$config['genero_fator'].' '.ucfirst($config['fator']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir '.$config['genero_fator'].' '.$config['fator'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&dialogo=1&fator_id='.$fator_id.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de Ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar_pro&fator_id=".$fator_id."\");");
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_fator=".$fator_id."\");");
		...
		if ($Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'adicionar')) $km->Add("inserir","inserir_ata",dica('Nova Ata de Reunião', 'Inserir uma nova ata de reunião relacionada.').'Ata de reunião'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=atas&a=ata_editar&ata_fator=".$fator_id."\");");
	...
	if ($obj->fator_tipo_pontuacao) $km->Add("acao","acao_percentagem",dica('Recalcular Progresso','Recalcula o progresso '.($config['genero_fator']=='a' ? 'desta' : 'deste').' '.$config['fator'].' para os caso de haver suspeita que o progresso não esteja calculado corretamente.').'Recalcular Progresso'.dicaF(), "javascript: void(0);' onclick='recalcular(".$fator_id.",".$obj->fator_percentagem.")");
...
if ($obj->fator_cia) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->fator_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->fator_dept) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->fator_dept).'</td></tr>';
...
if ($obj->fator_usuario) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Responsável pel'.$config['genero_fator'].' '.ucfirst($config['fator']).'', ucfirst($config['usuario']).' responsável por gerenciar '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->fator_usuario, '','','esquerda').'</td></tr>';		
...
if ($obj->fator_descricao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Descrição', 'Descrição d'.$config['genero_fator'].' '.$config['fator'].'.').'Descrição:'.dicaF().'</td><td class="realce">'.$obj->fator_descricao.'</td></tr>';
...
if ($Aplic->profissional){
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Sistema de Percentagem', 'Qual forma a percentagem d'.$config['genero_fator'].' '.$config['fator'].' é calculada.').'Sistema de percentagem:'.dicaF().'</td><td class="realce">'.(isset($tipo_pontuacao[$obj->fator_tipo_pontuacao]) ? $tipo_pontuacao[$obj->fator_tipo_pontuacao] : '').'</td></tr>';
	...
	if ($obj->fator_tipo_pontuacao=='pontos_completos' || $obj->fator_tipo_pontuacao=='pontos_parcial') echo '<tr><td align=right>'.dica('Pontuação Alvo', 'A pontuação necessária da soma das filhas para que '.$config['genero_fator'].' '.$config['fator'].' fique com progresso em 100%.').'Pontuação Alvo:'.dicaF().'</td><td class="realce">'.number_format((float)$obj->fator_ponto_alvo, 2, ',', '.').'</td></tr>';
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Progresso', $config['fator'].' pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce">'.number_format((float)$obj->fator_percentagem, 2, ',', '.').'%</td></tr>';
	}
...
$sql->limpar();
if (count($lista)) {
	echo '<tr><td  align="right" nowrap="nowrap">'.dica('Relacionad'.$config['genero_fator'],'A qual parte do sistema '.$config['genero_fator'].' '.$config['fator'].' está relacionad'.$config['genero_fator'].'.').'Relacionad'.$config['genero_fator'].':'.dicaF().'</td><td width="100%" class="realce">';
	$qnt=0;
...
if ($obj->fator_principal_indicador) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'O indicador d'.$config['genero_fator'].' '.$config['fator'].' mais representativo da situação geral d'.$config['genero_fator'].' mesm'.$config['genero_fator'].'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->fator_principal_indicador).'</td></tr>';



if ($obj->fator_oque) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('O Que Fazer', 'Sumário sobre o que se trata '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'O Que:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_oque.'</td></tr>';
if ($obj->fator_porque) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Por Que Fazer', 'Por que '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' será executad'.$config['genero_fator'].'.').'Por que:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_porque.'</td></tr>';
if ($obj->fator_onde) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Onde Fazer', 'Onde '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Onde:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_onde.'</td></tr>';
if ($obj->fator_quando) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Quando Fazer', 'Quando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Quando:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_quando.'</td></tr>';
if ($obj->fator_quem) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Quem:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_quem.'</td></tr>';



if ($obj->fator_como) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Como Fazer', 'Como '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Como:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_como.'</td></tr>';
...
if ($obj->fator_desde_quando || $obj->fator_controle || $obj->fator_metodo_aprendizado || $obj->fator_melhorias){	
	...
	if ($obj->fator_desde_quando) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Desde Quando é Feita', 'Desde quando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Desde quando:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_desde_quando.'</td></tr>';
	if ($obj->fator_controle) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Método de Controle', 'Como '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é controlad'.$config['genero_fator'].'.').'Controle:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_controle.'</td></tr>';
	if ($obj->fator_metodo_aprendizado) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Método de Aprendizado', 'Como é realizado o aprendizado d'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Aprendizado:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_metodo_aprendizado.'</td></tr>';
	if ($obj->fator_melhorias) echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Melhorias Efetuadas n'.$config['genero_fator'].' '.ucfirst($config['fator']).'', 'Quais as melhorias realizadas n'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_melhorias.'</td></tr>';
	}
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->fator_moeda].'</td></tr>';	
	}
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Ativ'.$config['genero_fator'], 'Se '.$config['genero_fator'].' '.$config['fator'].' se encontra ativ'.$config['genero_fator'].'.').'Ativ'.$config['genero_fator'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->fator_ativo ? 'Sim' : 'Não').'</td></tr>';
...
function recalcular(fator_id, percentagem){
	parent.gpwebApp.popUp('Recalculo de Progresso do Físico', 550, 100, 'm=praticas&a=fator_recalcular_fisico_pro&dialogo=1&fator_id='+fator_id+'&percentagem='+percentagem, null, window);
	}
