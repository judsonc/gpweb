...
echo '<tr><td align="right">'.dica('Data', 'Escolha qual a data deste registro de ocorrência.').'Data:'.dicaF().'</td><td nowrap="nowrap"><input type="hidden" name="tema_log_data" id="tema_log_data" value="'.$log_data->format(FMT_TIMESTAMP_DATA).'" /><input type="text" name="log_date" id="log_date" onchange="setData(\'frmEditar\', \'log_date\');" value="'.$log_data->format($df).'" class="texto" />'.dica('Data do Registro', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data deste registro.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
...
echo '<tr><td align="right">'.dica('Valor Gasto', 'Valor gasto n'.$config['genero_tema'].' '.$config['tema'].'.').'Valor Gasto:'.dicaF().'</td><td><input type="text" style="text-align:right;" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" name="tema_log_custo" value="'.($log->tema_log_custo ? number_format($log->tema_log_custo, 2, ',', '.') : '').'" size="40" /></td></tr>';  

$categoria_economica=array(''=>'')+getSisValor('CategoriaEconomica');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Categoria Econômica', 'Caso insira um valor gasto, seleciona a categoria econômica deste item.').'Categoria econômica:'.dicaF().'</td><td>'.selecionaVetor($categoria_economica, 'tema_log_categoria_economica', 'class=texto size=1 style="width:250px;" onchange="frmEditar.tema_log_nd.value=\'\'; mudar_nd();"', (isset($log->tema_log_categoria_economica) ? $log->tema_log_categoria_economica :'')).'</td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nível de Acesso', 'O registro de ocorrência d'.$config['genero_tema'].' '.$config['tema'].' pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido</b> - Todos podem ver, porem apenas o responsável pel'.$config['genero_tema'].' '.$config['tema'].' e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante</b> - Somente o responsável pel'.$config['genero_tema'].' '.$config['tema'].' e os designados podem ver e editar</li><li><b>Privado</b> - Somente o responsável pel'.$config['genero_tema'].' '.$config['tema'].' e os designados podem ver, e o responsável editar.</li></ul>').'Nível de Acesso'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($niveis_acesso, 'tema_log_acesso', 'class="texto"', ($tema_log_id ? $log->tema_log_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';	
echo '</table></td>';
echo '<td width="60%" valign="top"><table width="100%" cellspacing=0 cellpadding=0>';
echo '<tr><td align="right">'.dica('Sumário', 'Escreva um texto curto que exprima o motivo deste registro d'.$config['genero_tema'].' '.$config['tema'].'.').'Sumário:'.dicaF().'</td><td valign="middle"><table width="100%">';
echo '<tr><td align="left"><input type="text" class="texto" name="tema_log_nome" value="'.$log->tema_log_nome.'" maxlength="255" size="30" /></td><td align="center">'.'<input type="checkbox" value="1" name="tema_log_problema" id="tema_log_problema" '.($log->tema_log_problema ? 'checked="checked"' : '').' />'.dica('Problema', 'Caso esta caixa esteja selecionada, este registro será marcado como de problema.<br><br>Ele se diferenciará dos outros registros por ter um fundo vermelho no sumário para chamar a atenção.').'<label for="tema_log_problema">Problema</label>'.dicaF().'</td></tr>';
echo '</table></td></tr>';
echo '<tr><td align="right" valign="middle">'.dica('Referência', 'Escolha de que forma chegou aos dados que aqui estão registrados.').'Referência:'.dicaF().'</td><td valign="middle">'.selecionaVetor($RefRegistroTarefa, 'tema_log_referencia', 'size="1" class="texto"', $log->tema_log_referencia).'</td></tr>';
echo '<tr><td align="right">'.dica('Endereço Eletrônico desta Referência', 'Escreva, caso exista, um link para página ou arquivo na rede que faz referência a este registro tal como visualiza na tela no Navegador Web.<br>Para link para páginas da internet é necessário escrever http://<br>Ex: <b>http://www.sistemagpweb.com</b>').'URL:'.dicaF().'</td><td><input type="text" class="texto" name="tema_log_url_relacionada" value="'.($log->tema_log_url_relacionada).'" size="50" maxlength="255" /></td></tr>';
echo '<tr><td align="right" valign="top">'.dica('Descrição', 'Escreva uma descrição pormenorizada sobre este registro.').'Descrição:'.dicaF().'</td><td><textarea name="tema_log_descricao" class="textarea" cols="50" rows="6">'.$log->tema_log_descricao.'</textarea></td></tr>';


echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($tema_log_id > 0 ? 'modificação' : 'criação').' do registro.').'Notificar:'.dicaF().'</td>';
echo '<td>';
