...
echo '<tr><td width="100%"><fieldset><legend class=texto style="color: black;">'.dica('Indicadores Disponíveis', 'Lista de indicadores que poderão ser acrescentados à composição. Dê um clique duplo em um dos indicadores nesta lista de seleção para adiciona-lo à lista de composição.<BR><BR>Outra opção é selecionar o indicador e clicar no botão Adicionar.').'&nbsp;<b>Indicadores Disponíveis</b>&nbsp</legend>'.dica().'<div id="combo_lista_indicadores">'.selecionaVetor($indicadores, 'lista', 'style="width:100%;" size="15" class="texto" ondblclick="mudar_indicadores_filhos();"').'</div></fieldset></td></tr>';

echo '<tr><td align="left" colspan=20><table cellspacing=0 cellpadding=0><tr><td>'.botao('adicionar', 'Adicionar', 'Utilize este botão para adicionar um indicador à lista dos selecionados.</p>','','adicionar_indicador()','','',0).'</td><td>&nbsp;&nbsp;&nbsp;'.dica('Peso', 'Peso do indicador a ser inserido.O valor do indicador final será a média ponderada dos valores dos indicadores selecionados pelos respectivos pesos.').'Peso:'.dicaF().'<input type="text" class="texto" name="pratica_indicador_composicao_peso" id="pratica_indicador_composicao_peso" value="1" style="width:50px;"></td></tr></table></td></tr>';
...
foreach($lista as $linha){
	...
	echo '<td width="16" align=center><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir?\')) {excluir_indicador('.$linha['pratica_indicador_composicao_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir.').'</a></td></tr>';
	}
if (count($lista)) echo '</table>';
echo '</div></td></tr>';


echo '<tr><td>'.botao('aceitar', 'Aceitar', 'Utilize este botão para aceitar a edição da composição.','','Retornar();','','',0).'</td><td>&nbsp;</td></td>'. (!$Aplic->profissional ? '<td  align="right">'.botao('cancelar', 'Cancelar', 'Utilize este botão para cancelar a edição de fórmula do indicador.','','window.opener = window; window.close()','','',0).'</td>' : '').'</tr>';
