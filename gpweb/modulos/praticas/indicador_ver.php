...
if (!$Aplic->profissional && !$dialogo){
	...
	elseif ($pratica_indicador['pratica_indicador_objetivo_estrategico'])$botoesTitulo->adicionaBotao('m=praticas&a=obj_estrategico_ver&objetivo_id='.(int)$pratica_indicador['pratica_indicador_objetivo_estrategico'], $config['objetivo'],'',ucfirst($config['objetivo']),'Visualizar '.$config['genero_objetivo'].' '.$config['objetivo'].' do qual este indicador faz parte.');
	...
	elseif ($pratica_indicador['pratica_indicador_fator'])$botoesTitulo->adicionaBotao('m=praticas&a=fator_ver&fator_id='.(int)$pratica_indicador['pratica_indicador_fator'], $config['fator'],'',ucfirst($config['fator']),'Visualizar '.$config['genero_fator'].' '.$config['fator'].' d'.$config['genero_fator'].' qual este indicador faz parte.');
...
if ($Aplic->profissional && !$dialogo){
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_indicador=".$pratica_indicador_id."\");");
	...
	$bloquear=($pratica_indicador['pratica_indicador_aprovado'] && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($Aplic->profissional){
	...
	if (count($lista)){
		...
		foreach($lista as $gestao_data){
			...
			elseif ($gestao_data['pratica_indicador_gestao_mswot']) echo ($usado++? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['pratica_indicador_gestao_mswot']);
			...
			elseif ($gestao_data['pratica_indicador_gestao_template']) echo ($usado++? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['pratica_indicador_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$pratica_indicador['pratica_indicador_moeda']].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o indicador se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($pratica_indicador['pratica_indicador_aprovado'] ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
if (!$pratica_indicador['pratica_indicador_composicao']){
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Meta','Lista de metas vinculadas a este indicador.').'Metas:'.dicaF().'</td><td>';
	...
	if ($Aplic->profissional) {
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(evento_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(arquivo_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(link_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(forum_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(plano_acao_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adCampo('count(msg_gestao_indicador)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adCampo('count(modelo_gestao_indicador)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adCampo('count(ata_gestao_indicador)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adCampo('count(problema_gestao_indicador)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adCampo('count(risco_gestao_indicador)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_indicador = '.(int)$pratica_indicador_id.' OR risco_gestao_indicador = '.(int)$pratica_indicador_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adCampo('count(instrumento_gestao_indicador)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
