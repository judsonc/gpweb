...
$sql->esqUnir('objetivo', 'objetivo', 'objetivo_perspectiva=pg_perspectiva_id');
$sql->adCampo('perspectivas.pg_perspectiva_id, pg_perspectiva_nome, pg_perspectiva_cor, count(objetivo.objetivo_id) as quantidade');
if ($pg_id) {
...
foreach($perspectivas as $perspectiva){
	...
	$sql->adCampo('objetivo.objetivo_id, objetivo_nome');
	$sql->adOnde('objetivo_perspectiva='.(int)$perspectiva['pg_perspectiva_id']);
	$sql->adOrdem('objetivo_ordem ASC');
	$objetivos=$sql->Lista();
	$sql->limpar();
...
foreach($perspectivas as $perspectiva){
	...
	foreach($objetivos as $objetivo){
		...
		$coluna[$i++]=$objetivo['objetivo_id'];
		
		$texto=converte_texto_grafico(urlLimpar($objetivo['objetivo_nome']));
...
foreach ($estrategias as $estrategia){
	...
	$sql->adTabela('objetivo');
	$sql->esqUnir('fator', 'fator', 'fator_objetivo=objetivo_id');
	$sql->esqUnir('estrategias', 'estrategias', 'pg_estrategia_fator=fator_id');
	$sql->adCampo('objetivo.objetivo_id');
