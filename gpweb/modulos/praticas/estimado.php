...
if ($acao=='excluir'&& $item){
	...
	if (!$sql->exec()) echo '<script>alert("Não foi possível inserir no histórico os dados do item '.$excluido['plano_acao_item_custos_nome'].'")</script>';
	$sql->limpar();
...
if ($acao=='c_editar' && $item){
	...
	$sql->adInserir('h_custos_data2', $data);
	if (!$sql->exec()) echo '<script>alert("Não foi possível inserir no histórico os dados do item editado '.$editado['plano_acao_item_custos_nome'].'")</script>';
	$sql->limpar();
	...
	if (!$sql->exec()) echo '<script>alert("Não foi possível alterar os dados do item '.$plano_acao_item_custos_nome.'")</script>';
	$sql->limpar();
...
if ($acao=='c_inserir'&& ($plano_acao_item_custos_plano_acao_item || $uuid)){	
	...
	if (!$sql->exec()) echo '<script>alert("Não foi possível inserir os dados do item '.$plano_acao_item_custos_nome.'")</script>';
	$sql->limpar();
...
echo '<table width="100%"><tr><td width="15%">&nbsp;</td><td width="70% align="center"><center><h1>Custos Estimados</h1></center></td><td align="right" width="15%">'.($historico && !$impressao ? dica('Histórico da Planilha', 'Clique neste ícone '.imagem('informacao.gif').' para visualizar as alterações na planilha de custos estimados.').'<a href="javascript: void(0);" onclick="javascript:window.open(\'./index.php?m=praticas&a=historico&dialogo=1&uuid='.$uuid.'&plano_acao_item_custos_plano_acao_item='.$plano_acao_item_custos_plano_acao_item.'&tipo=estimado\', \'Planilha\',\'height=500,width=1024,resizable,scrollbars=yes\')"><img src="'.acharImagem('informacao.gif').'" border=0 width="22" heigth="22" /></a>'.dicaF() : '').(!$impressao ? dica('Imprimir a Planilha', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a planilha.').'<a href="javascript: void(0);" onclick="javascript:window.open(\'./index.php?m='.$m.'&a='.$a.'&impressao=1&uuid='.$uuid.'&dialogo=1&plano_acao_item_custos_plano_acao_item='.$plano_acao_item_custos_plano_acao_item.'&tipo=estimado\', \'Planilha\',\'height=500,width=1024,resizable,scrollbars=yes\')">'.imagem('imprimir_p.png').'</a>'.dicaF() : '').'</td></tr></table>';
...
if ($acao!='inserir' && $acao!='editar'){
	...
	foreach ($linhas as $linha) {
		...
		echo '<td align="left" nowrap="nowrap">'.link_usuario($linha['plano_acao_item_custos_usuario'],'','','esquerda').'</td>';
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Nome', 'Escreva o nome deste item.').'Nome:'.dicaF().'</td><td><input type="text" class="texto" name="plano_acao_item_custos_nome" value="'.(isset($atual['plano_acao_item_custos_nome']) ? $atual['plano_acao_item_custos_nome']:'').'" maxlength="255" size="40" /></td></tr>';
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Unidade de Medida', 'Escolha a unidade de medida deste item.').'Unidade de medida:'.dicaF().'</td><td>'.selecionaVetor($unidade, 'plano_acao_item_custos_tipo', 'class=texto size=1', (isset($atual['plano_acao_item_custos_tipo']) ? $atual['plano_acao_item_custos_tipo']:'')).'</td></tr>';
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Quantidade', 'Insira a quantidade deste item.').'Quantidade:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();"name="plano_acao_item_custos_quantidade" id="plano_acao_item_custos_quantidade" value="'.(isset($atual['plano_acao_item_custos_quantidade']) ? number_format($atual['plano_acao_item_custos_quantidade'], 2, ',', '.'):'').'" maxlength="255" size="10" /></td></tr>';
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="plano_acao_item_custos_custo" id="plano_acao_item_custos_custo" value="'.(isset($atual['plano_acao_item_custos_custo']) ? number_format($atual['plano_acao_item_custos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
	
	$categoria_economica=array(''=>'')+getSisValor('CategoriaEconomica');
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Categoria Econômica', 'Escolha a categoria econômica deste item.').'Categoria econômica:'.dicaF().'</td><td>'.selecionaVetor($categoria_economica, 'plano_acao_item_custos_categoria_economica', 'class=texto size=1 style="width:395px;" onchange="plano_acao_item_custos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_custos_categoria_economica']) ? $atual['plano_acao_item_custos_categoria_economica']:'')).'</td></tr>';
	
	$GrupoND=array(''=>'')+getSisValor('GrupoND');
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Grupo de Despesa', 'Escolha o grupo de despesa deste item.').'Grupo de despesa:'.dicaF().'</td><td>'.selecionaVetor($GrupoND, 'plano_acao_item_custos_grupo_despesa', 'class=texto size=1 style="width:395px;" onchange="plano_acao_item_custos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_custos_grupo_despesa']) ? $atual['plano_acao_item_custos_grupo_despesa']:'')).'</td></tr>';
	
	$ModalidadeAplicacao=array(''=>'')+getSisValor('ModalidadeAplicacao');
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Modalidade de Aplicação', 'Escolha a modalidade de aplicação deste item.').'Modalidade de aplicação:'.dicaF().'</td><td>'.selecionaVetor($ModalidadeAplicacao, 'plano_acao_item_custos_modalidade_aplicacao', 'class=texto size=1 style="width:395px;" onchange="plano_acao_item_custos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_custos_modalidade_aplicacao']) ? $atual['plano_acao_item_custos_modalidade_aplicacao']:'')).'</td></tr>';
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Elemento de Despesa', 'Escolha o elemento de despesa (ED) deste item.').'Elemento de despesa:'.dicaF().'</td><td><div id="combo_nd">'.selecionaVetor($nd, 'plano_acao_item_custos_nd', 'class=texto size=1 style="width:395px;" onchange="mudar_nd();"', (isset($atual['plano_acao_item_custos_nd']) ? $atual['plano_acao_item_custos_nd']: '')).'</div></td></tr>';
	
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Total', 'O valor total do item.').'Total:'.dicaF().'</td><td><div id="total"></div></td></tr>';
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Descrição', 'Insira a descrição deste item.').'Descrição:'.dicaF().'</td><td><textarea cols="70" rows="5" class="textarea" name="plano_acao_item_custos_descricao">'.(isset($atual['plano_acao_item_custos_descricao']) ? $atual['plano_acao_item_custos_descricao']:'').'</textarea></td></tr>';
	...
	echo '<tr><td align="right">'.dica('Data','Data limite para o recebimento do ítem.').'Data:</td><td><table cellpadding=0 cellspacing=0><tr><td><td><input type="hidden" name="plano_acao_item_custos_data_limite" id="plano_acao_item_custos_data_limite" value="'.($data_texto ? $data_texto->format(FMT_TIMESTAMP_DATA) : '').'" /><input type="text" name="data_texto"  id="data_texto" style="width:70px;" onchange="setData(\'env\', \'data_texto\', \'plano_acao_item_custos_data_limite\');" value="'.($data_texto ? $data_texto->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Limite', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data limite para o recebimento do ítem.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr></table></td></tr>';
