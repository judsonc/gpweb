...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_checklist='.(int)$checklist_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
if (!permiteAcessarChecklist($obj->checklist_acesso ,$checklist_id)) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
$editar=($podeEditar&& permiteEditarChecklist($obj->checklist_acesso,$checklist_id));
...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_checklist=".$checklist_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->checklist_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Checklist</center></font>';
	}	
	
echo '<table id="tblObjetivos" cellpadding=0 cellspacing=1 width="100%" '.(!$dialogo ? 'class="std"' : '').' >';

echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->checklist_cor.'" colspan="2"><font color="'.melhorCor($obj->checklist_cor).'"><b>'.$obj->checklist_nome.'<b></font></td></tr>';

if ($obj->checklist_cia) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->checklist_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->checklist_dept) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por esta meta.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_secao($obj->checklist_dept).'</td></tr>';
...
if ($obj->checklist_descricao) echo '<tr><td align="right" >'.dica('Descrição', 'Descrição do checklist.').'Descrição:'.dicaF().'</td><td class="realce">'.$obj->checklist_descricao.'</td></tr>';
...
if ($Aplic->profissional) {
	...
	if (count($lista)){	
		...
		foreach($lista as $gestao_data){	
			...
			elseif (isset($gestao_data['checklist_gestao_mswot']) && $gestao_data['checklist_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['checklist_gestao_mswot']);
			...
			elseif ($gestao_data['checklist_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['checklist_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->checklist_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o checklist se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->checklist_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


	
if ($obj->checklist_principal_indicador) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->checklist_principal_indicador).'</td></tr>';
	
	
$acesso = getSisValor('NivelAcesso','','','sisvalor_id');
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nível de Acesso', 'Pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido</b> - Todos podem ver, porem apenas o responsável e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante</b> - Somente o responsável e os designados podem ver e editar</li><li><b>Privado</b> - Somente o responsável e os designados podem ver, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$acesso[$obj->checklist_acesso].'</td></tr>';

echo '<tr><td align="right" nowrap="nowrap">'.dica('Ativo', 'Se o checklist se encontra ativo.').'Ativo:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->checklist_ativo ? 'Sim' : 'Não').'</td></tr>';
...
if ($Aplic->profissional) include_once BASE_DIR.'/modulos/praticas/checklist_ver_pro.php';			
...
if (!$dialogo && $Aplic->profissional) {
	...
	if ($Aplic->profissional){
		...
		if ($Aplic->profissional) {
			...
			$sql->adOnde('log_checklist = '.(int)$checklist_id);
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('evento_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(evento_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('arquivo_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(arquivo_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('link_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(link_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('forum_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(forum_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(pratica_indicador_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('plano_acao_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(plano_acao_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('projeto_gestao_checklist = '.(int)$checklist_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adOnde('msg_gestao_checklist = '.(int)$checklist_id);
			$sql->adCampo('count(msg_gestao_checklist)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adOnde('modelo_gestao_checklist = '.(int)$checklist_id);
				$sql->adCampo('count(modelo_gestao_checklist)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adOnde('ata_gestao_checklist = '.(int)$checklist_id);
			$sql->adCampo('count(ata_gestao_checklist)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adOnde('problema_gestao_checklist = '.(int)$checklist_id);
			$sql->adCampo('count(problema_gestao_checklist)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adOnde('risco_gestao_checklist = '.(int)$checklist_id);
			$sql->adCampo('count(risco_gestao_checklist)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_checklist = '.(int)$checklist_id.' OR risco_gestao_checklist = '.(int)$checklist_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adOnde('instrumento_gestao_checklist = '.(int)$checklist_id);
			$sql->adCampo('count(instrumento_gestao_checklist)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
