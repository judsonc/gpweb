...
foreach($perspectivas as $perspectiva){
	...
	foreach($objetivos as $objetivo){
		...
		foreach($depts as $dept){
			...
			$sql->adOnde('objetivo_dept_dept='.(int)$dept['dept_id']);
		...
		$sql->adTabela('objetivo_dept');
		$sql->adCampo('count(objetivo_dept_dept)');
		$sql->adOnde('objetivo_dept_objetivo='.(int)$objetivo['objetivo_id']);
		$quantidade=$sql->Resultado();
		$sql->limpar();
	...
	foreach($depts as $dept){
		$sql->adTabela('objetivo_dept');
		$sql->adCampo('DISTINCT count(objetivo_dept_dept)');
		if(!empty($vetor_objetivo)) $sql->adOnde('objetivo_dept_objetivo IN ('.implode(',',$vetor_objetivo).')');
		$sql->adOnde('objetivo_dept_dept='.(int)$dept['dept_id']);
		$quantidade=$sql->Resultado();
		$sql->limpar();
