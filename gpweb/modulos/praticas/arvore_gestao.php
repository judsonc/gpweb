...
$sql->adCampo('objetivo.objetivo_id, objetivo_perspectiva_perspectiva AS objetivo_perspectiva, objetivo_perspectiva_tema AS objetivo_tema, objetivo_nome, objetivo_percentagem');
if ($pg_id){
...
if (!$Aplic->profissional) $sql->adGrupo('objetivo_id, objetivo_perspectiva, objetivo_perspectiva_tema');
$objetivos=$sql->Lista();
$sql->limpar();
...
$sql->adTabela('fator');
$sql->esqUnir('fator_objetivo', 'fator_objetivo','fator_objetivo_fator=fator.fator_id');
$sql->adCampo('fator.fator_id, fator_nome, fator_objetivo_objetivo AS fator_objetivo, fator_objetivo_me, fator_percentagem');
if ($pg_id){
...
if (!$Aplic->profissional) $sql->adGrupo('fator_id, fator_objetivo, fator_objetivo_me');
$fatores=$sql->Lista();
$sql->limpar();
$vetor_fator=array();
foreach($fatores as $linha){
	...
	elseif (isset($vetor_me[$linha['fator_objetivo_me']]) && $linha['fator_objetivo_me']){
		...
		for ($i=1; $i<=$vetor_me[$linha['fator_objetivo_me']]; $i++){
			$vetor_fator[$linha['fator_id']]=(isset($vetor_fator[$linha['fator_id']]) ? $vetor_fator[$linha['fator_id']]+1 : 1);
			$nodulo=$arvore->Add('y'.$linha['fator_objetivo_me'].'_'.$i, 'f'.$linha['fator_id'].'_'.$vetor_fator[$linha['fator_id']], $cor_indicador.' '.link_fator($linha['fator_id']),false, "fator_p.gif");
			$nodulo->addData("id", $linha['fator_id']);
			}
		}		
	}
