...
if (isset($_REQUEST['filtro_prioridade_objetivo']))	$Aplic->setEstado('filtro_prioridade_objetivo', getParam($_REQUEST, 'filtro_prioridade_objetivo', null));
$filtro_prioridade_objetivo = $Aplic->getEstado('filtro_prioridade_objetivo') !== null ? $Aplic->getEstado('filtro_prioridade_objetivo') : null;
...
if (isset($_REQUEST['objetivotextobusca'])) $Aplic->setEstado('objetivotextobusca', getParam($_REQUEST, 'objetivotextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('objetivotextobusca') ? $Aplic->getEstado('objetivotextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaObjEstrategicoTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaObjEstrategicoTab') !== null ? $Aplic->getEstado('ListaObjEstrategicoTab') : 0);
...
$sql->adOnde('favorito_objetivo=1');
...
$sql->adCampo('perspectivas.pg_perspectiva_id, pg_perspectiva_nome, pg_perspectiva_ativo');
$sql->adOrdem('perspectivas.pg_perspectiva_ordem');
...
foreach($perspectivas_ativas as $ativas) {
	...
	$cor[$ativas['pg_perspectiva_id']]='color:'.($ativas['pg_perspectiva_ativo'] ? 'black' : 'red');
	}
...
if (!$dialogo && $Aplic->profissional){
	...
	echo '<input type="hidden" name="filtro_prioridade_objetivo" id="filtro_prioridade_objetivo" value="'.$filtro_prioridade_objetivo.'" />';
	echo '<input type="hidden" name="houve_filtro" id="houve_filtro" value="1" />';
	
	
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['objetivos']), 'obj_estrategicos.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$filtra_perspectiva=(count($cor) ? '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['perspectiva']), 'Tod'.$config['genero_objetivo'].' '.$config['objetivo'].' deve estar inserido dentro de '.($config['genero_perspectiva']=='a' ? 'uma' : 'um').' '.$config['perspectiva'].'.').ucfirst($config['perspectiva']).':'.dicaF().'</td><td>'.selecionaVetor($perspectivas, 'pg_perspectiva_id', 'class="texto" style="width:250px;" onchange="env.submit();"', $pg_perspectiva_id,'','',$cor).'</td></tr>' : '');
	...
	$novo_objetivo=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']), 'Criar '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' nov'.$config['genero_objetivo'].' '.$config['objetivos'].'.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'obj_estrategico_editar\'; env.submit();" ><img src="'.acharImagem('obj_estrategicos_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr>' : '');

	$botao_favorito='<tr><td nowrap="nowrap" align="right">'.dica('Favoritos', 'Criar ou editar um grupo de '.$config['objetivos'].' favorit'.$config['genero_objetivo'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&objetivo=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	if($filtro_prioridade_objetivo) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['objetivos']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['objetivos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&tab='.$tab.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$filtra_perspectiva.$procuraBuffer.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_objetivo.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$selecionar_planejamento='<tr><td align="right" nowrap="nowrap">'.dica('Planejamentos Estratégico', 'Utilize esta opção para filtrar pelo planejamentos estratégico que está vinculad'.$config['genero_fator'].'.').'Planejamentos Estratégico:</td><td>'.dicaF().selecionaVetor($planos, 'pg_id', 'onchange="env.submit();" style="width:250px;" class="texto"', $pg_id).'</td></tr>';
	$procurar_usuario='<tr><td align=right nowrap="nowrap">'.dica(ucfirst($config['usuario']), 'Filtrar pel'.$config['genero_usuario'].' '.$config['usuario'].' escolhido na caixa de seleção à direita.').ucfirst($config['usuario']).':'.dicaF().'</td><td><input type="hidden" id="usuario_id" name="usuario_id" value="'.$usuario_id.'" /><input type="text" id="nome_responsavel" name="nome_responsavel" value="'.nome_usuario($usuario_id).'" style="width:250px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$filtra_perspectiva.$procuraBuffer.'</table>');
	if ($podeAdicionar) $botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']), 'Criar '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' nov'.$config['genero_objetivo'].' '.$config['objetivos'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:env.a.value=\'obj_estrategico_editar\'; env.submit();" ><span>nov'.$config['genero_objetivo'].'</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['objetivos']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['objetivos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&tab='.$tab.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	echo '</form>';
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['objetivos']).'</center></font>';
	}
...
$caixaTab = new CTabBox('m=praticas&a=obj_estrategico_lista', '', $tab);
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/obj_estrategicos_ver_idx', 'Ativ'.$config['genero_objetivo'].'s',null,null,'Ativ'.$config['genero_objetivo'].'s','Visualizar '.$config['genero_objetivo'].'s '.$config['objetivos'].' ativ'.$config['genero_objetivo'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/obj_estrategicos_ver_idx', 'Inativ'.$config['genero_objetivo'].'s',null,null,'Inativ'.$config['genero_objetivo'].'s','Visualizar '.$config['genero_objetivo'].'s '.$config['objetivos'].' inativ'.$config['genero_objetivo'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/obj_estrategicos_ver_idx', 'Tod'.$config['genero_objetivo'].'s',null,null,'Tod'.$config['genero_objetivo'].'s','Visualizar tod'.$config['genero_objetivo'].'s '.$config['genero_objetivo'].'s '.$config['objetivos'].'.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/obj_estrategicos_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_objetivo.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_objetivo){
	env.filtro_prioridade_objetivo.value=filtro_prioridade_objetivo;
	env.submit();
	}	
