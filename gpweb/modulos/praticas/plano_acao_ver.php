...
if (!$dialogo && !$Aplic->profissional){
	...
	if ($editar && !$Aplic->profissional){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $linha_inserir.='<td nowrap="nowrap">'.dica('Novo Fórum', 'Inserir um novo fórum relacionado a '.($config['genero_acao']=='o' ? 'este' : 'esta').' '.$config['acao'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=foruns&a=editar&forum_acao='.(int)$plano_acao_id.'\');" ><span>forum</span></a>'.dicaF().'</td>';
...
elseif (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_acao=".$plano_acao_id."\");");
	...
	$km->Add("acao_imprimir","acao_imprimir2",dica('Registro de Ocorrência', 'Imprimir os registros de ocorrência  d'.$config['genero_acao'].' '.$config['acao'].'.').'Registro de Ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=".$m."&a=log_ver_pro&dialogo=1&cia_id=".$obj->plano_acao_cia."&plano_acao_id=".$plano_acao_id."\");");
...
if ($obj->plano_acao_duracao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Duração', 'A duração d'.$config['genero_acao'].' '.$config['acao'].' em dias.').'Duração:'.dicaF().'</td><td width="100%" class="realce">'.number_format(($obj->plano_acao_duracao/($config['horas_trab_diario'] ? $config['horas_trab_diario'] : 8)), 2, ',', '.').'</td></tr>';



echo '<tr><td align="right" nowrap="nowrap">'.dica('Progresso', $config['acao'].' pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce" width="300">'.number_format($obj->plano_acao_percentagem, 2, ',', '.').'%'.($obj->plano_acao_calculo_porcentagem ? ' (calculado)' : '').'</td></tr>';
...
if ($Aplic->profissional) echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada n'.$config['genero_acao'].' '.$config['acao'].'.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->plano_acao_moeda].'</td></tr>';	
