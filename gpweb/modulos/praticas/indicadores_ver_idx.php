...
global $estilo_interface, $perms, $Aplic, $cia_id, $tab, $praticas_criterios, $dialogo, $indicador_expandido, $podeEditar, $ano, $filtro_prioridade_indicador;
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=pratica_indicador_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='pratica_indicador_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_indicador) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if ($ver){
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['pratica_indicador_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_indicador) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';
		...
		if ($exibir['pontuacao']) {
			...
			echo '<td nowrap="nowrap" align=center '.referencial($pontos).'>'.number_format($pontos, 0).'</td>';
			}
		...
		if ($Aplic->profissional && $exibir['relacionado']){
		  ...
		  if (count($lista)) {
				...
				foreach($lista as $gestao_data){
					...
					elseif ($gestao_data['pratica_indicador_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['pratica_indicador_gestao_mswot']);
					...
					elseif ($gestao_data['pratica_indicador_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['pratica_indicador_gestao_template']);
...
elseif ($exibir['pontuacao']) {
	...
	echo '<tr><td colspan='.($Aplic->profissional ? 5 : 4).'>&nbsp;</td><td align=center '.referencial($ponto_final).'>'.$ponto_final.'</td><td colspan=20>&nbsp;</td><tr>';
	}
