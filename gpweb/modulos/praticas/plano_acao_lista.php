...
$sql->adOnde('favorito_trava_acao=1');
...
if (isset($_REQUEST['filtro_prioridade_acao']))	$Aplic->setEstado('filtro_prioridade_acao', getParam($_REQUEST, 'filtro_prioridade_acao', null));
$filtro_prioridade_acao = $Aplic->getEstado('filtro_prioridade_acao') !== null ? $Aplic->getEstado('filtro_prioridade_acao') : null;


$painel_filtro = $Aplic->getEstado('painel_filtro') !== null ? $Aplic->getEstado('painel_filtro') : 0;

if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaPlanoAcaoTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaPlanoAcaoTab') !== null ? $Aplic->getEstado('ListaPlanoAcaoTab') : 0);
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('plano_acao_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('plano_acao_favorito') !== null ? $Aplic->getEstado('plano_acao_favorito') : 0;
...
if (isset($_REQUEST['plano_acao_ano'])) $Aplic->setEstado('plano_acao_ano', getParam($_REQUEST, 'plano_acao_ano', null));
$plano_acao_ano = $Aplic->getEstado('plano_acao_ano') !== null ? $Aplic->getEstado('plano_acao_ano') : '';
...
$sql->adCampo('DISTINCT plano_acao_ano');
if ($cia_id && !$lista_cias) $sql->adOnde('plano_acao_cia='.(int)$cia_id);
elseif ($lista_cias) $sql->adOnde('plano_acao_cia IN ('.$lista_cias.')');
if ($lista_depts) $sql->adOnde('plano_acao_dept IN ('.$lista_depts.')');
elseif ($dept_id) $sql->adOnde('plano_acao_dept='.(int)$dept_id);
$sql->adOrdem('plano_acao_ano DESC');
$listaanos=$sql->carregarColuna();
$sql->limpar();
$anos[]='';
foreach ($listaanos as $ano) $anos[$ano]=$ano;
...
if (!$dialogo){
	...
	echo '<input type="hidden" name="filtro_prioridade_acao" id="filtro_prioridade_acao" value="'.$filtro_prioridade_acao.'" />';
	...
	if($filtro_prioridade_acao) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	if($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela agenda que estão relacionad'.$config['genero_acao'].'s.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo indicador que estão relacionad'.$config['genero_acao'].'s.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionad'.$config['genero_acao'].'s.').''.ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($pg_meta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_meta'].' '.$config['meta'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['meta']).':'.dicaF();
		$nome=nome_meta($pg_meta_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo monitoramento que estão relacionad'.$config['genero_acao'].'s.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela ata de reunião a qual estão relacionados.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}	
	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela matriz SWOT que estão relacionad'.$config['genero_acao'].'s.').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($mswot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo campo de matriz SWOT que estão relacionad'.$config['genero_acao'].'s.').'Campo SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo plano operativo que estão relacionad'.$config['genero_acao'].'s.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo instrumento jurídico que estão relacionad'.$config['genero_acao'].'s.').'Instrumento Jurídico:'.dicaF();
		$nome=nome_instrumento($instrumento_id);
		}	
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo recurso que estão relacionad'.$config['genero_acao'].'s.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela demanda que estão relacionad'.$config['genero_acao'].'s.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo evento que estão relacionad'.$config['genero_acao'].'s.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($link_id){
		$legenda_filtro=dica('Filtrar pelo Link', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo link que estão relacionad'.$config['genero_acao'].'s.').'Link:'.dicaF();
		$nome=nome_link($link_id);
		}
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela avaliação que estão relacionad'.$config['genero_acao'].'s.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionad'.$config['genero_acao'].'s.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo brainstorm que estão relacionad'.$config['genero_acao'].'s.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pela matriz GUT que estão relacionad'.$config['genero_acao'].'s.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo diagrama de causa-efeito que estão relacionad'.$config['genero_acao'].'s.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($arquivo_id){
		$legenda_filtro=dica('Filtrar pelo Arquivo', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo arquivo que estão relacionad'.$config['genero_acao'].'s.').'Arquivo:'.dicaF();
		$nome=nome_arquivo($arquivo_id);
		}	
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo fórum que estão relacionad'.$config['genero_acao'].'s.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($checklist_id){
		$legenda_filtro=dica('Filtrar pelo Checklist', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo checklist que estão relacionad'.$config['genero_acao'].'s.').'Checklist:'.dicaF();
		$nome=nome_checklist($checklist_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo compromisso que estão relacionad'.$config['genero_acao'].'s.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo agrupamento que estão relacionad'.$config['genero_acao'].'s.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo patrocinador que estão relacionad'.$config['genero_acao'].'s.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar '.$config['genero_acao'].'s '.$config['acoes'].' pelo modelo que estão relacionad'.$config['genero_acao'].'s.').'Modelo:'.dicaF();
	...
	elseif($me_id){
		...
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar '.$config['genero_acao'].'s '.$config['acoes'].'.').'Filtro:'.dicaF();
		}


	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionad'.$config['genero_acao'],'A qual parte do sistema '.$config['genero_acao'].'s '.$config['acoes'].' estão relacionad'.$config['genero_acao'].'s.').'Relacionad'.$config['genero_acao'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$procuraAno = '<tr><td align=right>'.dica('Ano', 'Pesquisar pelo ano d'.$config['genero_acao'].' '.$config['acao'].'.').'Ano:'.dicaF().'</td><td>'.selecionaVetor($anos, 'plano_acao_ano', 'onchange="document.env.submit()" class="texto" style="width:250px;"', $plano_acao_ano).'</td></tr>';
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['acao']), 'plano_acao.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo_plano=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Criar nov'.$config['genero_acao'].' '.$config['acao'].'.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=plano_acao_editar\');" ><img src="'.acharImagem('plano_acao_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td><tr>' : '');
	$botao_favorito='<tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de planos de ação favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&acao=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir a Lista de '.ucfirst($config['acoes']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['acoes'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=plano_acao_lista&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$procuraBuffer.$procuraAno.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_plano.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo&& !$Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Planos de Ação', 'plano_acao.png', $m, $m.'.'.$a);
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0>'.$procuraBuffer.$procuraAno.'</table>');
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0>'.$procurar_om.$procurar_usuario.$popFiltro.$filtros.'</table>');
	$botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Criar nov'.$config['genero_acao'].' '.$config['acao'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=plano_acao_editar\');" ><span>nov'.$config['genero_acao'].'</span></a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['acoes']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['acoes'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=plano_acao_lista&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	echo '</form>';
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['acao']).'</center></font>';
	}
if($Aplic->profissional){
...
if (!$dialogo){
	$caixaTab = new CTabBox('m=praticas&a=plano_acao_lista', '', $tab);
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx','Em execução',null,null,'Em execução','Visualizar '.$config['genero_acao'].'s '.$config['acoes'].' que estão com porcentagem diferente de 100%.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx','Completados',null,null,'Completados','Visualizar '.$config['genero_acao'].'s '.$config['acoes'].' que estão com porcentagem em 100%.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx','Inativos',null,null,'Inativos','Visualizar '.$config['genero_acao'].'s '.$config['acoes'].' que foram marcad'.$config['genero_acao'].'s como inativ'.$config['genero_acao'].'s.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else include_once(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_acao.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_acao){
	env.filtro_prioridade_acao.value=filtro_prioridade_acao;
	env.submit();
	}
