...
if ($acao!='inserir' && $acao!='editar'){
	...
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="plano_acao_item_gastos_custo" id="plano_acao_item_gastos_custo" value="'.(isset($atual['plano_acao_item_gastos_custo']) ? number_format($atual['plano_acao_item_gastos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
...
function valor(){
	...
	if (gasto=='') gasto=0;
	if (valor=='') valor=0;
	document.getElementById('total').innerHTML ='<b>'+float2moeda(gasto*qnt)+'</b>';
	}	
