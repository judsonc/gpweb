...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nome d'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Tod'.$config['genero_tema'].' '.$config['tema'].' necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="tema_nome" value="'.$obj->tema_nome.'" style="width:284px;" class="texto" /> *</td></tr>';
...
if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_tema'].' '.$config['tema'].' mais representativo da situação geral do mesmo.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'tema_principal_indicador', 'class="texto" style="width:267px;"', $obj->tema_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'tema_moeda', 'class=texto size=1', ($obj->tema_moeda ? $obj->tema_moeda : 1)).'</td></tr>';
	}	
