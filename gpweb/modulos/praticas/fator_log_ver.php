...
echo '<tr><th></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=fator_ver&tab=0&fator_id='.$fator_id.'&ordenar=fator_log_data&ordem='.($ordem ? '0' : '1').'\');">'.dica('Data', 'Data de inserção do registro d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='fator_log_data' ? imagem('icones/'.$seta[$ordem]) : '').'Data'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=fator_ver&tab=0&fator_id='.$fator_id.'&ordenar=fator_log_referencia&ordem='.($ordem ? '0' : '1').'\');">'.dica('Referência', 'A forma como se chegou aos dados que estão registrandos.').($ordenar=='fator_log_referencia' ? imagem('icones/'.$seta[$ordem]) : '').'Ref.'.dicaF().'</a></th>';
echo '<th width="100"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=fator_ver&tab=0&fator_id='.$fator_id.'&ordenar=fator_log_nome&ordem='.($ordem ? '0' : '1').'\');">'.dica('Título', 'Título do registro d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='fator_log_nome' ? imagem('icones/'.$seta[$ordem]) : '').'Título'.dicaF().'</a></th>';
...
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=fator_ver&tab=0&fator_id='.$fator_id.'&ordenar=fator_log_nd&ordem='.($ordem ? '0' : '1').'\');">'.dica('ND', 'Número de Despesa já empenhado n'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='fator_log_nd' ? imagem('icones/'.$seta[$ordem]) : '').'ND'.dicaF().'</a></th>';
...
foreach ($logs as $linha) {
	...
	echo($podeEditar ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=fator_ver&tab=1&fator_id='.$fator_id.'&fator_log_id='.$linha['fator_log_id'].'\');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o registro').'</a>' : '&nbsp;');
	...
	echo '<td>'.($podeEditar ?  dica('Excluir Registro', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir este registro.').'<a href="javascript:excluir2('.$linha['fator_log_id'].');" >'.imagem('icones/remover.png').'</a>' : '&nbsp;').'</td></tr>';
