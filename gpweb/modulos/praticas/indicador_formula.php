...
$lista_formula = getParam($_REQUEST, 'lista_formula', '');
$pratica_indicador_calculo = getParam($_REQUEST, 'pratica_indicador_calculo', '');

$pratica_indicador_calculo=str_replace('@','+', $pratica_indicador_calculo);
...
$numero_selecionados=array();
$indicadores_recebidos=array();
$rocados=array();
$vetor=explode(',',$lista_formula);
$maior=0;

foreach((array)$vetor as $campo){
	if (isset($campo)&& $campo){
		$valor=explode(':',$campo);
		$indicadores_recebidos[]=$valor[0];
		$numero_selecionados[]=$valor[1];
		$rocados[]=$valor[2];
		if ($valor[1] > $maior)$maior=$valor[1];
		}
	}
...
echo '<input type="hidden" name="correto" id="correto" value="" />';
...
if (!$Aplic->profissional) echo '<input type="hidden"  value="0" name="rocar" id="rocar" />';
...
if ($swot_ativo) {
	echo '<tr '.(isset($pratica_indicador_mswot) && $pratica_indicador_mswot ? '' : 'style="display:none"').' id="mswot" ><td align="right" nowrap="nowrap">'.dica('Matriz SWOT', 'Caso o indicador seja específico de uma matriz SWOT neste campo deverá constar o nome do campo de matriz SWOT').'Matriz SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="pratica_indicador_mswot" value="'.(isset($pratica_indicador_mswot) ? $pratica_indicador_mswot : '').'" /><input type="text" id="mswot_nome" name="mswot_nome" value="'.nome_mswot((isset($pratica_indicador_mswot) ? $pratica_indicador_mswot : null)).'" style="width:280px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popMSWOT();">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Selecionar Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para selecionar uma matriz SWOT.').'</a></td></tr></table></td></tr>';
	echo '<tr '.(isset($pratica_indicador_swot) && $pratica_indicador_swot ? '' : 'style="display:none"').' id="swot" ><td align="right" nowrap="nowrap">'.dica('Campo SWOT', 'Caso o indicador seja específico de um campo de matriz SWOT neste campo deverá constar o nome do campo de matriz SWOT').'Campo SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="pratica_indicador_swot" value="'.(isset($pratica_indicador_swot) ? $pratica_indicador_swot : '').'" /><input type="text" id="swot_nome" name="swot_nome" value="'.nome_swot((isset($pratica_indicador_swot) ? $pratica_indicador_swot : null)).'" style="width:280px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popSWOT();">'.imagem('../../../modulos/swot/imagens/swot_p.png','Selecionar Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para selecionar um campo de matriz SWOT.').'</a></td></tr></table></td></tr>';
	}
else {
...
echo '<tr><td width="100%"><fieldset><legend class=texto style="color: black;">'.dica('Indicadores Disponíveis', 'Lista de indicadores que poderão ser acrescentados à fórmula. Dê um clique duplo em um dos indicadores nesta lista de seleção para adiciona-lo.').'&nbsp;<b>Indicadores Disponíveis</b>&nbsp</legend>'.dica().'<div id="combo_lista_indicadores">'.selecionaVetor($indicadores, 'lista', 'style="width:100%;" size="15" class="texto" ondblclick="mudar_indicadores_filhos();"').'</div></fieldset></td></tr>';
echo '<tr><td colspan="2" align="center"><table width="100%">';

echo ($Aplic->profissional ? '<tr><td align="left">'.dica('Deslocar', 'Caso esteja definido valor diferente de zero, o indicador selecionado terá seus valores deslocados à direita de N posições, em comparação com as datas<br>Ex: Antes [2014 50], [2015 60] e [2016 70]<br>transforma-se em [2015 50] e [2016 60] com deslocamento de 1 (uma) posição.').'&nbsp;&nbsp;&nbsp;Deslocar:'.dicaF().'</td><td><input type="text" value="0" name="rocar" class="texto" style="width:100px;" id="rocar" /></td></tr>' : '');

echo '<tr><td align="left" colspan=2><table cellspacing=0 cellpadding=0><tr><td><table cellspacing=0 cellpadding=0><tr><td>'.botao('adicionar', 'Adicionar', 'Utilize este botão para adicionar um indicador à lista dos selecionados</p>Caso deseje inserir múltiplos indicadores de uma única vez, mantenha o botão <i>CTRL</i> pressionado enquanto clica com o botão esquerdo do mouse nos indicadores da lista acima.','','adicionar_indicador()','','',0).'</td></tr></table></td></tr></table></td><td>&nbsp;</td></tr>';
...
else $sql->adOnde('pratica_indicador_formula_pai='.(int)$pratica_indicador_id);
$sql->adOrdem('pratica_indicador_formula_ordem');
$lista=$sql->Lista();
$sql->limpar();
echo '<tr><td></td><td><div id="combo_indicador">';
if (count($lista)) echo '<table class="tbl1" cellspacing=0 cellpadding=0 width=100%><tr><th></th><th>'.dica('Índice', 'Qual o índice do indicador a ser utilizado na fórmula').'I'.dicaF().'</th><th>'.dica('Nome', 'Qual o nome do indicador a ser utilizado na fórmula').'Nome'.dicaF().'</th><th>'.dica(ucfirst($config['organizacao']), ucfirst($config['organizacao']).' do indicador a ser utilizado na fórmula.').ucfirst($config['organizacao']).dicaF().'</th><th>'.dica('Deslocado', 'Se o indicador a ser utilizado na fórmula terá os valores deslocados à direita de uma posição.').'D'.dicaF().'</th><th></th></tr>';
foreach($lista as $linha){
	...
	echo '</td>';
	echo '<td width=20>I'.($linha['pratica_indicador_formula_ordem']< 10 ? '0' : '').$linha['pratica_indicador_formula_ordem'].'</td>';
	echo '<td align=left>'.$linha['pratica_indicador_nome'].'</td>';
	echo '<td align=left>'.$linha['cia_nome'].'</td>';
	echo '<td width="16" align=center>'.($linha['pratica_indicador_formula_rocado'] ? $linha['pratica_indicador_formula_rocado'] : '').'</td>';
	echo '<td width="16" align=center><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir?\')) {excluir_indicador('.$linha['pratica_indicador_formula_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir.').'</a></td></tr>';
	}
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Fórmula', 'Como os indicadores selecionados serão utilizando em uma fórmula.<br><br> Exemplo: Tenho 3 indicadores selecionados, ficando com a legenda I01, I02 e I03.<br><br>Para fazer uma fórmula que é a média simples dos três basta escrever no campo texto à esquerda:<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>(I01+I02+I03)/3</b>').'Fórmula:'.dicaF().'</td><td width="100%" colspan="2"><textarea style="font-family:Times New Roman, Times, serif; font-size:12pt; input.upper { text-transform: uppercase; }" name="pratica_indicador_calculo" id="pratica_indicador_calculo" cols="76" rows="3" class="textarea" onkeydown="f(this)" onkeyup="f(this)" onblur="f(this)" onclick="f(this)">'.($pratica_indicador_calculo ? $pratica_indicador_calculo : '').'</textarea></td></tr>';



echo '<tr><td>'.botao('aceitar', 'Aceitar', 'Utilize este botão para aceitar a edição da fórmula do indicador.','','Retornar();','','',0).'</td><td>&nbsp;</td></td>'. (!$Aplic->profissional ? '<td  align="right">'.botao('cancelar', 'Cancelar', 'Utilize este botão para cancelar a edição de fórmula do indicador.','','window.opener = window; window.close()','','',0).'</td>' : '').'</tr>';
echo '</table></td></tr>';

echo '<tr><td colspan=20 align="center"><b>Lista de Operadores e Funções</b></td></tr>';
echo '<tr><td colspan=20><table border=1cellpadding=0 cellspacing=0 class="tbl1" width=500 align="center">

<tr><td align="right">Ixx+Iyy</td><td>Soma de Ixx e Iyy</td></tr>
<tr><td align="right">Ixx-Iyy</td><td>Diferençade Ixx e Iyy</td></tr>
<tr><td align="right">Ixx*Iyy</td><td>Multiplicação Ixx e Iyy</td></tr>
<tr><td align="right">Ixx/Iyy</td><td>Divisão de Ixx por Iyy</td></tr>
<tr><td align="right">Ixx%Iyy</td><td>Resto da divisão de Ixx por Iyy</td></tr>
<tr><td align="right">Ixx**Iyy</td><td>Ixx elevado a Iyy (exponenciação)</td></tr>
<tr><td align="right">abs(Ixx)</td><td>Valor absoluto de Ixx (valor positivo)</td></tr>
<tr><td align="right">acos(Ixx)</td><td>Arco cosseno de Ixx</td></tr>
<tr><td align="right">acosh(Ixx)</td><td>Cosseno hiperbólico inverso de Ixx</td></tr>
<tr><td align="right">asin(Ixx)</td><td>Arco seno de Ixx</td></tr>
<tr><td align="right">asinh(Ixx)</td><td>Seno hiperbólico inverso de Ixx</td></tr>
<tr><td align="right">atan2(Ixx,Iyy)</td><td>Tangente inversa de Ixx e Iyy</td></tr>
<tr><td align="right">atan(Ixx)</td><td>Arco tangente de Ixx</td></tr>
<tr><td align="right">atanh(Ixx)</td><td>Tangente hiperbólica inversa de Ixx</td></tr>
<tr><td align="right" nowrap="nowrap">base_convert(Ixx, A, B)</td><td>Converte Ixx da base A par a base B</td></tr>
<tr><td align="right">bindec (Ixx)</td><td>Converte Ixx de binário para decimal</td></tr>
<tr><td align="right">ceil(Ixx)</td><td>Arredonda Ixx para cima</td></tr>
<tr><td align="right">cos(Ixx)</td><td>Coseno de Ixx</td></tr>
<tr><td align="right">cosh(Ixx)</td><td>Cosseno hiperbólico de Ixx</td></tr>
<tr><td align="right">decbin(Ixx)</td><td>Converte Ixx de decimal para  binário</td></tr>
<tr><td align="right">dechex(Ixx)</td><td>Converte Ixx de decimal para hexadecimal</td></tr>
<tr><td align="right">decoct(Ixx)</td><td>Converte Ixx de decimal para octal</td></tr>
<tr><td align="right">deg2rad(Ixx)</td><td>Converte o Ixxem graus para o equivalente radiano</td></tr>
<tr><td align="right">exp(Ixx)</td><td>Calcula o expoente e de Ixx</td></tr>
<tr><td align="right">expm1(Ixx)</td><td>Retorna exp(Ixx)- 1, calculado em uma forma que é precisa, mesmo quando o valor de Ixx é próximo de zero</td></tr>
<tr><td align="right">floor(Ixx)</td><td>Arredonda Ixx para baixo</td></tr>
<tr><td align="right">fmod(Ixx, Iyy)</td><td>Retorna o reste de ponto flutuante (módulo) da divisão Ixx/Iyy</td></tr>
<tr><td align="right">hexdec(Ixx)</td><td>Converte Ixx de hexadecimal para decimal</td></tr>
<tr><td align="right">hypot(Ixx, Iyy)</td><td>Calcular o comprimento da hipotenusa de um triângulo em ângulo reto com lados Ixx e Iyy</td></tr>
<tr><td align="right">intdiv(Ixx, Iyy)</td><td>Retorna a parte inteira do quociente da divisão de Ixx por Iyy.</td></tr>
<tr><td align="right">is_finite(Ixx)</td><td> Verifica se Ixx é um número finito. Retorna TRUE ou FALSE (booleano)</td></tr>
<tr><td align="right">is_infinite(Ixx)</td><td>Verifica se Ixx é um número infinito. Retorna TRUE ou FALSE (booleano)</td></tr>
<tr><td align="right">lcg_value()</td><td>Gera um pseudo valor real aleatório no intervalo (0, 1)</td></tr>
<tr><td align="right">log10(Ixx)</td><td>Logaritmo de base 10 de Ixx</td></tr>
<tr><td align="right">log1p(Ixx)</td><td>Retorna log (1 + Ixx), calculado em uma forma que é precisa, mesmo quando o valor de Ixx é próximo de zero</td></tr>
<tr><td align="right">log(Ixx)</td><td>Logaritmo natural de Ixx</td></tr>
<tr><td align="right">max(Ixx, Iyy, Izz, ...)</td><td>Procurar mais alto valor entre os passados</td></tr>
<tr><td align="right">min(Ixx, Iyy, Izz, ...)</td><td>Procurar menor valor entre os passados</td></tr>
<tr><td align="right">octdec(Ixx)</td><td>Converte Ixx de octal para decimal</td></tr>
<tr><td align="right">pi()</td><td>Obtem valor de pi</td></tr>
<tr><td align="right">pow(Ixx, Iyy)</td><td>Base Ixx pelo exponencial Iyy</td></tr>
<tr><td align="right">rad2deg(Ixx)</td><td>Converte o número radiano Ixx para o equivalente em graus</td></tr>
<tr><td align="right">rand(Ixx, Iyy)</td><td>Gera um inteiro aleatório entre os valores de Ixx e Iyy</td></tr>
<tr><td align="right">round(Ixx)</td><td>Arredonda o número real Ixx</td></tr>
<tr><td align="right">sin(Ixx)</td><td>Seno de Ixx</td></tr>
<tr><td align="right">sinh(Ixx)</td><td>Seno hiperbólico de Ixx</td></tr>
<tr><td align="right">sqrt(Ixx)</td><td>Raiz quadrada de Ixx</td></tr>
<tr><td align="right">tan(Ixx)</td><td>Tangente de Ixx</td></tr>
<tr><td align="right">tanh(Ixx)</td><td>Tangente hiperbólica de Ixx</td></tr>
</table></td></tr>';
echo '</table></td>';
echo '</table>';
echo '</form>';
...
function excluir_indicador(pratica_indicador_formula_id){
	xajax_excluir_indicador(document.getElementById('pratica_indicador_id').value, document.getElementById('uuid').value, pratica_indicador_formula_id);
	__buildTooltip();
	}

function mudar_posicao_indicador(ordem, pratica_indicador_formula_id, direcao){
	xajax_mudar_posicao_indicador(ordem, pratica_indicador_formula_id, direcao, document.getElementById('pratica_indicador_id').value, document.getElementById('uuid').value);
	__buildTooltip();
	}

function adicionar_indicador() {
	if(env.lista.value) xajax_adicionar_indicador(document.getElementById('pratica_indicador_id').value, document.getElementById('uuid').value, env.lista.value, env.rocar.value);
	}
...
function Retornar(){
	var pratica_indicador_calculo=document.getElementById('pratica_indicador_calculo').value;
	if(parent && parent.gpwebApp){
		parent.gpwebApp._popupCallback(pratica_indicador_calculo);
		return;
		}
	else{
		window.opener.SetFormula(pratica_indicador_calculo);
...
function mudar_indicadores(){
	...
	xajax_mudar_indicadores_ajax(document.getElementById('cia_id').value, null, vetor, null, null);
	<?php } ?>	
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('cia_id').value, window.setObjetivo, window);
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('cia_id').value, window.setFator, window);
...
function popMSWOT() {
	parent.gpwebApp.popUp('Matriz SWOT', 630, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setMSWOT&tabela=mswot&cia_id='+document.getElementById('cia_id').value, window.setMSWOT, window);
	}
...
function popSWOT() {
	parent.gpwebApp.popUp('Campo SWOT', 630, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setSWOT&tabela=swot&cia_id='+document.getElementById('cia_id').value, window.setSWOT, window);
	}
...
function popArquivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Arquivo', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('cia_id').value, window.setArquivo, window);
