...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Nome d'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Tod'.$config['genero_objetivo'].' '.$config['objetivo'].' necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="objetivo_nome" value="'.$obj->objetivo_nome.'" style="width:284px;" class="texto" /> *</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', 'A qual '.$config['organizacao'].' pertence '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].''.'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><div id="combo_cia">'.selecionar_om(($obj->objetivo_cia ? $obj->objetivo_cia : $cia_id), 'objetivo_cia', 'class=texto size=1 style="width:284px;" onchange="javascript:mudar_om();"').'</div></td></tr></table></td></tr>';
if ($Aplic->profissional) {
	$saida_cias='';
...
if ($Aplic->profissional) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['departamento']).' Responsável', 'Escolha pressionando o ícone à direita qual '.$config['genero_dept'].' '.$config['dept'].' responsável por '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td><input type="hidden" name="objetivo_dept" id="objetivo_dept" value="'.($objetivo_id ? $obj->objetivo_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept)).'" /><input type="text" class="texto" name="dept_nome" id="dept_nome" value="'.nome_dept(($objetivo_id ? $obj->objetivo_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept))).'" style="width:284px;" READONLY />'.botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamento'],'popDept()').'</td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Responsável pel'.$config['genero_objetivo'].' '.$config['objetivo'].'', 'Tod'.$config['genero_objetivo'].' '.$config['objetivo'].' deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="objetivo_usuario" name="objetivo_usuario" value="'.($obj->objetivo_usuario ? $obj->objetivo_usuario : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om(($obj->objetivo_usuario ? $obj->objetivo_usuario : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
...
if ($exibir['objetivo_superior']) echo '<tr><td align="right" nowrap="nowrap">'.dica(''.ucfirst($config['objetivo']).' Superior', 'Caso '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].' seja um desdobramento de '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' '.$config['objetivo'].' do escalão superior.').'Objetivo superior:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="objetivo_superior" id="objetivo_superior" value="'.$obj->objetivo_superior.'" /><input type="text" id="nome_objetivo" name="nome_objetivo" value="'.nome_objetivo($obj->objetivo_superior).'" style="width:284px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popObjetivo();">'.imagem('icones/obj_estrategicos_p.gif','Selecionar '.ucfirst($config['objetivo']).'','Clique neste ícone '.imagem('icones/obj_estrategicos_p.gif').' para selecionar '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' '.$config['objetivo'].'.').'</a></td></tr></table></td></tr>';

if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_objetivo'].' '.$config['objetivo'].' mais representativo da situação geral do mesmo.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'objetivo_indicador', 'class="texto" style="width:284px;"', $obj->objetivo_indicador).'</td></tr>';


if ($exibir['objetivo_composicao']) echo '<tr><td align="right" nowrap="nowrap">'.dica('Composição de '.$config['objetivos'].'', ' Marque caso '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].' seja compostos de outr'.$config['genero_objetivo'].'s '.$config['objetivos'].'.').'Composição:'.dicaF().'</td><td width="100%" colspan="2"><table cellspacing=0 cellpadding=0><tr><td><input type="checkbox" onclick="if (env.objetivo_composicao.checked) {document.getElementById(\'botao_composicao\').style.display=\'\';} else {document.getElementById(\'botao_composicao\').style.display=\'none\';}" class="texto" name="objetivo_composicao" value="1" '.($obj->objetivo_composicao ? 'checked="checked"' : '').' /></td><td id="botao_composicao" '.($obj->objetivo_composicao ? 'style="display:"' : 'style="display:none"').'>'.botao('composição', 'Composição','Abrir uma janela onde poderá selecionar quais são os '.''.$config['objetivos'].''.' que compoem este ora selecionado.','','popComposicao()').'</td></tr></table></td></tr>';

if ($exibir['objetivo_descricao'])  echo '<tr><td align="right" nowrap="nowrap" >'.dica('Descrição', 'Descrição sobre '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Descrição:'.dicaF().'</td><td width="100%" colspan="2"><textarea data-gpweb-cmp="ckeditor" name="objetivo_descricao" style="width:284px;" rows="2" class="textarea">'.$obj->objetivo_descricao.'</textarea></td></tr>';
...
echo '<tr><td align="right" width="100">'.dica('Ativo', 'Caso '.$config['genero_objetivo'].' '.$config['objetivo'].' ainda esteja ativo deverá estar marcado este campo.').'Ativo:'.dicaF().'</td><td><input type="checkbox" value="1" name="objetivo_ativo" '.($obj->objetivo_ativo || !$objetivo_id ? 'checked="checked"' : '').' /></td></tr>';
...
if ($exibir['objetivo_oque']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('O Que', 'Sumário sobre o que se trata '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'O Que:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_oque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_oque.'</textarea></td></tr>';
if ($exibir['objetivo_quem']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Quem:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_quem" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_quem.'</textarea></td></tr>';
if ($exibir['objetivo_quando']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Quando', 'Quando '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Quando:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_quando" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_quando.'</textarea></td></tr>';
if ($exibir['objetivo_onde']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Onde', 'Onde '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Onde:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_onde" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_onde.'</textarea></td></tr>';
if ($exibir['objetivo_porque']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Por Que', 'Por que '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].' será executad'.$config['genero_objetivo'].'.').'Por que:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_porque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_porque.'</textarea></td></tr>';
if ($exibir['objetivo_como']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Como', 'Como '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Como:'.dicaF().'</td><td colspan="2"><textarea name="objetivo_como" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->objetivo_como.'</textarea></td></tr>';
...
if ($exibir['objetivo_desde_quando']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Desde Quando é Feita', 'Desde quando '.$config['genero_objetivo'].' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Desde quando:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="objetivo_desde_quando" cols="60" rows="2" class="textarea">'.$obj->objetivo_desde_quando.'</textarea></td></tr>';
if ($exibir['objetivo_controle'])echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Método de Controle', 'Como '.$config['genero_objetivo'].' '.$config['objetivo'].' é controlad'.$config['genero_objetivo'].'.').'Controle:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="objetivo_controle" cols="60" rows="2" class="textarea">'.$obj->objetivo_controle.'</textarea></td></tr>';
...
if ($exibir['objetivo_melhorias']) echo '<tr><td align="right" nowrap="nowrap" style="width:150px">'.dica('Melhorias Efetuadas n'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']), 'Quais as melhorias realizadas n'.$config['genero_objetivo'].' '.$config['objetivo'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td colspan="2"><textarea data-gpweb-cmp="ckeditor" name="objetivo_melhorias" cols="60" rows="2" class="textarea">'.$obj->objetivo_melhorias.'</textarea></td></tr>';
if ($bsc) {
	echo '</table></fieldset></td></tr>';
	}
...
echo '<tr><td align="right" valign="top" nowrap="nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($objetivo_id > 0 ? 'modificação' : 'criação').' d'.$config['genero_objetivo'].' '.$config['objetivo'].'.').'Notificar:'.dicaF().'</td>';
echo '<td>';
...
echo '<tr><td colspan=2><table cellspacing=0 cellpadding=0 width="100%"><tr><td>'.botao('salvar', 'Salvar', 'Salvar os dados.','','enviarDados()').'</td><td align="right">'.botao('cancelar', 'Cancelar', 'Cancelar a '.($objetivo_id ? 'edição' : 'criação').' d'.$config['genero_objetivo'].' '.$config['objetivo'].'.','','if(confirm(\'Tem certeza que deseja cancelar?\')){url_passar(0, \''.$Aplic->getPosicao().'\');}').'</td></tr></table></td></tr>';
echo '</table>';
echo '</form>';
...
function popComposicao() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Composição', 900, 500, 'm=praticas&a=obj_estrategico_composicao&dialogo=1&cia_id='+document.getElementById('objetivo_cia').value+'&objetivo_id=<?php echo $objetivo_id ?>&lista_composicao='+document.getElementById('lista_composicao').value, window.setComposicao, window);
...
function popGerente() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Responsável', 500, 500, 'm=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setGerente&cia_id='+document.getElementById('objetivo_cia').value+'&usuario_id='+document.getElementById('objetivo_usuario').value, window.setGerente, window);
	else window.open('./index.php?m=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setGerente&cia_id='+document.getElementById('objetivo_cia').value+'&usuario_id='+document.getElementById('objetivo_usuario').value, 'Responsável','height=500,width=500,resizable,scrollbars=yes, left=0, top=0');
	}
