...
if ($filtro_prioridade_tema) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($temas as $linha) {
	if (permiteAcessarTema($linha['tema_acesso'],$linha['tema_id'])){
		...
		echo '<tr>';
		if (!$impressao && !$dialogo) echo '<td nowrap="nowrap" width="16">'.($podeEditar && $editar && !$bloquear ? dica('Editar '.ucfirst($config['tema']), 'Clique neste ícone '.imagem('icones/editar.gif').' para editar '.$config['genero_tema'].' '.$config['tema'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=tema_editar&tema_id='.$linha['tema_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['tema_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
