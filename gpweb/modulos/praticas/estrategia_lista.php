...
if (isset($_REQUEST['filtro_prioridade_estrategia']))	$Aplic->setEstado('filtro_prioridade_estrategia', getParam($_REQUEST, 'filtro_prioridade_estrategia', null));
$filtro_prioridade_estrategia = $Aplic->getEstado('filtro_prioridade_estrategia') !== null ? $Aplic->getEstado('filtro_prioridade_estrategia') : null;

$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_estrategia=1');
...
if (isset($_REQUEST['estrategiatextobusca'])) $Aplic->setEstado('estrategiatextobusca', getParam($_REQUEST, 'estrategiatextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('estrategiatextobusca') ? $Aplic->getEstado('estrategiatextobusca') : '');
...
$sql->adOnde('favorito_estrategia=1');
...
if (!$dialogo && $Aplic->profissional){
	...
	echo '<input type="hidden" name="filtro_prioridade_estrategia" id="filtro_prioridade_estrategia" value="'.$filtro_prioridade_estrategia.'" />';
	echo '<input type="hidden" name="houve_filtro" id="houve_filtro" value="1" />';
	
	
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['iniciativas']), 'estrategia.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$nova_estrategia=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Criar nov'.$config['genero_iniciativa'].' '.$config['iniciativa'].'.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'estrategia_editar\'; env.submit();" ><img src="'.acharImagem('estrategia_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr>' : '');
	$imprimir='<tr><td nowrap="nowrap" align="right"><a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=estrategia_lista&dialogo=1\');">'.imagem('imprimir_p.png', 'Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['iniciativas'].'.').'</a></td></tr>';
	$botao_favorito='<tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de '.$config['iniciativas'].' favorit'.$config['genero_iniciativa'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&estrategia=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	if($filtro_prioridade_estrategia) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$nova_estrategia.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$procurar_om='<tr><td align=right>'.dica('Filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'], 'Filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' selecionad'.$config['genero_organizacao'].'.').ucfirst($config['organizacao']).':'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om($cia_id, 'cia_id', 'class=texto size=1 style="width:250px;" onchange="javascript:mudar_om();"').'</div></td><td><a href="javascript:void(0);" onclick="document.env.submit();">'.imagem('icones/filtrar_p.png','Filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'],'Clique neste ícone '.imagem('icones/filtrar_p.png').' para filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' selecionad'.$config['genero_organizacao'].' a esquerda.').'</a></td>'.(($Aplic->usuario_pode_outra_cia || $Aplic->usuario_super_admin || $Aplic->usuario_pode_todas_cias) && !$ver_subordinadas ? '<td><a href="javascript:void(0);" onclick="document.env.ver_subordinadas.value=1; document.env.dept_id.value=\'\';  document.env.ver_dept_subordinados.value=0; document.env.submit();">'.imagem('icones/organizacao_p.gif','Incluir Subordinad'.$config['genero_organizacao'].'s','Clique neste ícone '.imagem('icones/organizacao_p.gif').' para incluir '.$config['genero_organizacao'].'s '.$config['organizacoes'].' subordinad'.$config['genero_organizacao'].'s '.($config['genero_organizacao']=='a' ? 'à' : 'ao').' selecionad'.$config['genero_organizacao'].'.').'</a></td>' : '').(($Aplic->usuario_pode_outra_cia || $Aplic->usuario_super_admin || $Aplic->usuario_pode_todas_cias) && $ver_subordinadas ? '<td><a href="javascript:void(0);" onclick="document.env.ver_subordinadas.value=0; document.env.submit();">'.imagem('icones/nao_sub_om.gif','Não Incluir Subordinad'.$config['genero_organizacao'].'s','Clique neste ícone '.imagem('icones/nao_sub_om.gif').' para deixar de incluir '.$config['genero_organizacao'].'s '.$config['organizacoes'].' subordinad'.$config['genero_organizacao'].'s '.($config['genero_organizacao']=='a' ? 'à' : 'ao').' selecionad'.$config['genero_organizacao'].'.').'</a></td>' : '').($Aplic->profissional ? '<td><input type="hidden" name="dept_id" id="dept_id" value="'.$dept_id.'" />'.(!$dept_id ? '<a href="javascript:void(0);" onclick="escolher_dept();">'.imagem('icones/secoes_p.gif',ucfirst($config['departamento']),'Clique neste ícone '.imagem('icones/secoes_p.gif').' para filtrar pel'.$config['genero_dept'].' '.$config['departamento'].' envolvid'.$config['genero_dept'].' ou don'.$config['genero_dept'].'.').'</a>' : '').'</td>' : '<input type="hidden" name="dept_id" id="dept_id" value="'.$dept_id.'" />').'</tr>'.
	($dept_id ? '<tr><td align=right>'.dica(ucfirst($config['departamento']), 'Filtrar pel'.$config['genero_dept'].' '.strtolower($config['departamento']).' envolvid'.$config['genero_dept'].'.').ucfirst($config['departamento']).':</td><td><input type="text" style="width:250px;" class="texto" name="dept_nome" id="dept_nome" value="'.nome_dept($dept_id).'"></td>'.($dept_id ? '<td><a href="javascript:void(0);" onclick="escolher_dept();">'.imagem('icones/secoes_p.gif',ucfirst($config['departamento']),'Clique neste ícone '.imagem('icones/secoes_p.gif').' para filtrar pel'.$config['genero_dept'].' '.$config['departamento'].' envolvid'.$config['genero_dept'].' ou don'.$config['genero_dept'].'.').'</a></td>'.(($Aplic->usuario_pode_dept_subordinado || $Aplic->usuario_super_admin || $Aplic->usuario_pode_todos_depts) && !$ver_dept_subordinados ? '<td><a href="javascript:void(0);" onclick="document.env.ver_dept_subordinados.value=1; document.env.submit();">'.imagem('icones/organizacao_p.gif','Incluir Subordinad'.$config['genero_dept'].'s','Clique neste ícone '.imagem('icones/organizacao_p.gif').' para incluir '.$config['genero_dept'].'s '.$config['departamentos'].' subordinad'.$config['genero_dept'].'s '.($config['genero_dept']=='a' ? 'à' : 'ao').' selecionad'.$config['genero_dept'].'.').'</a></td>' : '').(($Aplic->usuario_pode_dept_subordinado || $Aplic->usuario_super_admin || $Aplic->usuario_pode_todos_depts) && $ver_dept_subordinados ? '<td><a href="javascript:void(0);" onclick="document.env.ver_dept_subordinados.value=0; document.env.submit();">'.imagem('icones/nao_sub_om.gif','Não Incluir Subordinad'.$config['genero_dept'].'s','Clique neste ícone '.imagem('icones/nao_sub_om.gif').' para deixar de incluir '.$config['genero_dept'].'s '.$config['departamentos'].' subordinad'.$config['genero_dept'].'s '.($config['genero_dept']=='a' ? 'à' : 'ao').' selecionad'.$config['genero_dept'].'.').'</a></td>' : '') : '').'</tr>' : '');
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.'</table>');
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Criar nov'.$config['genero_iniciativa'].' '.$config['iniciativa'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:env.a.value=\'estrategia_editar\'; env.submit();" ><span>nov'.$config['genero_iniciativa'].'</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0><tr><td nowrap="nowrap" align="right">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['iniciativas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=estrategia_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->mostrar();
	echo '</form>';
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['iniciativas']).'</center></font>';
	}
...
if (!$dialogo){
	$caixaTab = new CTabBox('m=praticas&a=estrategia_lista', '', $tab);
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/estrategias_ver_idx','Em execução',null,null,'Em execução','Visualizar '.$config['genero_iniciativa'].'s '.$config['iniciativas'].' ativ'.$config['genero_iniciativa'].'s que estão com porcentagem diferente de 100%.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/estrategias_ver_idx','Completad'.$config['genero_iniciativa'].'s',null,null,'Completad'.$config['genero_iniciativa'].'s','Visualizar '.$config['genero_iniciativa'].'s '.$config['iniciativas'].' ativ'.$config['genero_iniciativa'].'s que estão com porcentagem em 100%.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/estrategias_ver_idx','Inativ'.$config['genero_iniciativa'].'s',null,null,'Inativ'.$config['genero_iniciativa'].'s','Visualizar '.$config['genero_iniciativa'].'s '.$config['iniciativas'].' que foram marcad'.$config['genero_iniciativa'].'s como inativ'.$config['genero_iniciativa'].'s.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else include_once(BASE_DIR.'/modulos/praticas/estrategias_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_estrategia.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_estrategia){
	env.filtro_prioridade_estrategia.value=filtro_prioridade_estrategia;
	env.submit();
	}	
