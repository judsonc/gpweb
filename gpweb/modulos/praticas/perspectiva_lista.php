...
if (isset($_REQUEST['filtro_prioridade_perspectiva']))	$Aplic->setEstado('filtro_prioridade_perspectiva', getParam($_REQUEST, 'filtro_prioridade_perspectiva', null));
$filtro_prioridade_perspectiva = $Aplic->getEstado('filtro_prioridade_perspectiva') !== null ? $Aplic->getEstado('filtro_prioridade_perspectiva') : null;

$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_perspectiva=1');
...
if (isset($_REQUEST['perspectivatextobusca'])) $Aplic->setEstado('perspectivatextobusca', getParam($_REQUEST, 'perspectivatextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('perspectivatextobusca') ? $Aplic->getEstado('perspectivatextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaPerspectivaTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaPerspectivaTab') !== null ? $Aplic->getEstado('ListaPerspectivaTab') : 0);
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('perspectiva_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('perspectiva_favorito') !== null ? $Aplic->getEstado('perspectiva_favorito') : 0;

$sql->adTabela('favorito');
$sql->adCampo('favorito_id, favorito_nome');
$sql->adOnde('favorito_perspectiva=1');
...
if (!$dialogo && $Aplic->profissional){
	...
	echo '<input type="hidden" name="filtro_prioridade_perspectiva" id="filtro_prioridade_perspectiva" value="'.$filtro_prioridade_perspectiva.'" />';
	echo '<input type="hidden" name="houve_filtro" id="houve_filtro" value="1" />';
	
	
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['perspectivas']), 'perspectiva.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$nova_perspectiva=($podeAdicionar ? '<tr><td nowrap="nowrap" align=right>'.dica('Nov'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']).'', 'Criar um nov'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'perspectiva_editar\'; env.submit();" ><img src="'.acharImagem('perspectiva_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['perspectivas']).'', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['perspectivas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	
	if($filtro_prioridade_perspectiva) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$botao_favorito='<tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de planos de ação favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&perspectiva=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$nova_perspectiva.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	echo '<input type="hidden" name="u" value="" />';
	echo '<input type="hidden" name="ver_subordinadas" value="'.$ver_subordinadas.'" />';
	$botoesTitulo = new CBlocoTitulo(ucfirst($config['perspectivas']), 'perspectiva.png', $m, $m.'.'.$a);
	$procuraBuffer = '<tr><td align=right nowrap="nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="perspectivatextobusca" onChange="document.env.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=perspectiva_lista&tab='.$tab.'&perspectivatextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	$selecionar_planejamento='<tr><td align="right" nowrap="nowrap">'.dica('Planejamentos Estratégico', 'Utilize esta opção para filtrar pelo planejamentos estratégico que está vinculad'.$config['genero_fator'].'.').'Planejamentos Estratégico:</td><td>'.dicaF().selecionaVetor($planos, 'pg_id', 'onchange="env.submit();" style="width:250px;" class="texto"', $pg_id).'</td></tr>';
	$procurar_usuario='<tr><td align=right nowrap="nowrap">'.dica(ucfirst($config['usuario']), 'Filtrar pel'.$config['genero_usuario'].' '.$config['usuario'].' escolhido na caixa de seleção à direita.').ucfirst($config['usuario']).':'.dicaF().'</td><td><input type="hidden" id="usuario_id" name="usuario_id" value="'.$usuario_id.'" /><input type="text" id="nome_responsavel" name="nome_responsavel" value="'.nome_usuario($usuario_id).'" style="width:250px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
	...
	if ($podeAdicionar) $botoesTitulo->adicionaCelula('<table><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']).'', 'Criar um nov'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:env.a.value=\'perspectiva_editar\'; env.submit();" ><span>nov'.$config['genero_perspectiva'].'</span></a>'.dicaF().'</td></tr><tr><td nowrap="nowrap"></td></tr></table>');
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['perspectivas']).'', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['perspectivas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	echo '</form>';
	}
else{
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['perspectivas']).'</center></font>';
	}
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/perspectivas_ver_idx', 'Ativ'.$config['genero_perspectiva'].'s',null,null,'Ativ'.$config['genero_perspectiva'].'s','Visualizar '.$config['genero_perspectiva'].'s '.$config['perspectivas'].' ativ'.$config['genero_perspectiva'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/perspectivas_ver_idx', 'Inativ'.$config['genero_perspectiva'].'s',null,null,'Inativ'.$config['genero_perspectiva'].'s','Visualizar '.$config['genero_perspectiva'].'s '.$config['perspectivas'].' inativ'.$config['genero_perspectiva'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/perspectivas_ver_idx', 'Tod'.$config['genero_perspectiva'].'s',null,null,'Tod'.$config['genero_perspectiva'].'s','Visualizar tod'.$config['genero_perspectiva'].'s '.$config['genero_perspectiva'].'s '.$config['perspectivas'].'.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/perspectivas_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_perspectiva.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_perspectiva){
	env.filtro_prioridade_perspectiva.value=filtro_prioridade_perspectiva;
	env.submit();
	}
