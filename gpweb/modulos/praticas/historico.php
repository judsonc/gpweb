...
echo '<center><h1>Histórico dos '.($tipo=='estimado' ? 'Custos Estimados' : 'Gastos').'  - '.link_acao_item($plano_acao_item).'</h1></center>';
...
$data = new CData($linha[$prefixo.'data2']);
	if ($linha[$prefixo.'excluido']){
		echo '<tr><td colspan="8" align="left">Excluído por '.link_usuario($linha[$prefixo.'usuario2'],'','','esquerda').' em '.$data->format('%d/%m/%Y '.$tf).'</td></tr>';
		...
		echo '<tr><td colspan="8" align="left">Alterado por '.link_usuario($linha[$prefixo.'usuario2'],'','','esquerda').' em '.$data->format('%d/%m/%Y '.$tf).'</td></tr>';
