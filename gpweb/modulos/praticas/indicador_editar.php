...
if (
	...
	elseif ($pratica_indicador_objetivo_estrategico) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
	...
	elseif ($pratica_indicador_objetivo_estrategico) $sql->adOnde('objetivo_id = '.(int)$pratica_indicador_objetivo_estrategico);
...
echo '<tr><td align="right">'.dica('Nome do Indicador', 'Todo indicador necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="pratica_indicador_nome" value="'.$obj->pratica_indicador_nome.'" style="width:280px;" maxlength="512" class="texto" /> *</td></tr>';
...
if ($Aplic->profissional){
  ...
  $sql->limpar();
	...
	foreach($lista as $gestao_data){
		...
		elseif ($gestao_data['pratica_indicador_gestao_mswot']) echo '<td align=left>'.imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['pratica_indicador_gestao_mswot']).'</td>';
...
if ($Aplic->profissional && $exibir['ano']) echo '<tr><td align="right" nowrap="nowrap">'.dica('Ano', 'Qual ano iniciou-se a utilização deste indicador.').'Ano inicial:'.dicaF().'</td><td width="100%" colspan="2"><input type="text" name="pratica_indicador_ano" value="'.($obj->pratica_indicador_ano ? $obj->pratica_indicador_ano : date('Y')).'" size="4" class="texto" /></td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'pratica_indicador_moeda', 'class=texto size=1', ($obj->pratica_indicador_moeda ? $obj->pratica_indicador_moeda : 1)).'</td></tr>';
	}	
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('pratica_indicador_cia').value, window.setObjetivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('pratica_indicador_cia').value, '<?php echo ucfirst($config["objetivo"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('pratica_indicador_cia').value, window.setFator, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('pratica_indicador_cia').value, '<?php echo ucfirst($config["fator"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popMSWOT() {
	parent.gpwebApp.popUp('Matriz SWOT', 630, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setMSWOT&tabela=mswot&cia_id='+document.getElementById('pratica_indicador_cia').value, window.setMSWOT, window);
	}
...
function popSWOT() {
	parent.gpwebApp.popUp('Campo SWOT', 630, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setSWOT&tabela=swot&cia_id='+document.getElementById('pratica_indicador_cia').value, window.setSWOT, window);
	}
...
function popArquivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Arquivo', 630, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('pratica_indicador_cia').value, window.setArquivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo&cia_id='+document.getElementById('pratica_indicador_cia').value, 'Arquivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
