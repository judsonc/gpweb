...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'OportunidadeMelhoria');
	}
...
if ($salvar){
	...
	if ($existe) {
		$sql->adTabela('plano_gestao2');
		$sql->adAtualizar('pg_oportunidade_melhoria', getParam($_REQUEST, 'pg_oportunidade_melhoria', ''));
		$sql->adOnde('pg_id ='.(int)$pg_id);
		$retorno=$sql->exec();
		$sql->limpar();
		}
		...
		$sql->adTabela('plano_gestao2');
		$sql->adInserir('pg_oportunidade_melhoria', getParam($_REQUEST, 'pg_oportunidade_melhoria', ''));
...
$sql->adTabela('plano_gestao2');
$sql->adCampo('pg_oportunidade_melhoria');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Ambiente Interno - Fraquezas</h1></td></tr>';
if ($editarPG || $pg['pg_oportunidade_melhoria']) echo '<tr><td colspan=2 align="left"><b>Informações gerais sobre as fraquezas n'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_oportunidade_melhoria" id="pg_oportunidade_melhoria" >'.$pg['pg_oportunidade_melhoria'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_oportunidade_melhoria']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_oportunidade_melhoria'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'OportunidadeMelhoria\'');
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\'ponto_forte\');').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'oportunidade_melhoria\');').'</td></tr></table></td></tr>';
