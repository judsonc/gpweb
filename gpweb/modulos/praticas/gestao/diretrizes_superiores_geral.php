...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'DiretrizesSuperiores');
	}
...
if ($salvar){
	...
	if ($existe) {
		$sql->adTabela('plano_gestao2');
		$sql->adAtualizar('pg_diretriz_superior', getParam($_REQUEST, 'pg_diretriz_superior', ''));
		$sql->adOnde('pg_id ='.(int)$pg_id);
		$retorno=$sql->exec();
		$sql->limpar();
		}
		...
		$sql->adTabela('plano_gestao2');
		$sql->adInserir('pg_diretriz_superior', getParam($_REQUEST, 'pg_diretriz_superior', ''));
...
$sql->adTabela('plano_gestao2');
$sql->adCampo('pg_diretriz_superior');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Diretrizes do Escalão Superior</h1></td></tr>';
if ($editarPG || $pg['pg_diretriz_superior']) echo '<tr><td colspan=2 align="left"><b>Informações gerais sobre as diretrizes do escalão superior d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_diretriz_superior" id="pg_diretriz_superior" >'.$pg['pg_diretriz_superior'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_diretriz_superior']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_diretriz_superior'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'DiretrizesSuperiores\'');
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\'principios\');').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'diretrizes_superiores\');').'</td></tr></table></td></tr>';
