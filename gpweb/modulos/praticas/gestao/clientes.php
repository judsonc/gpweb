...
if($direcao) {
		...
		$q->adOnde('pg_arquivo_campo = \'Clientes\'');
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'Clientes');
	}


if ($salvar){
	$sql->adTabela('plano_gestao');
	$sql->adAtualizar('pg_clientes', getParam($_REQUEST, 'pg_clientes', ''));
...
$sql->adTabela('plano_gestao');
$sql->adCampo('pg_clientes');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Clientes / Usuários</h1></td></tr>';
if ($editarPG || $pg['pg_clientes']) echo '<tr><td colspan=2 align="left"><b>Clientes / Usuários d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_clientes" id="pg_clientes" >'.$pg['pg_clientes'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_clientes'])  echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_clientes'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_campo=\'Clientes\'');
...
if ($exibir['processos'] && $exibir['perfil']) $retorno='processos_produtos_servicos';
