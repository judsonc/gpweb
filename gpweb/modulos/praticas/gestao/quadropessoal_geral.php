...
gpweb\modulos\praticas\gestao\quadropessoal.php		

Exibe e editar o campo quadro de pessoal, do plano de gestão																																					
																																												
********************************************************************************************/
global $config, $exibir;
$base_dir=($config['dir_arquivo'] ? $config['dir_arquivo'] : BASE_DIR);
if ($editarPG) echo '<script type="text/javascript" src="'.BASE_URL.'/lib/ckeditor/ckeditor.js"></script>';
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'PessoalInterno');
	}


if ($salvar){
	$sql->adTabela('plano_gestao');
	$sql->adAtualizar('pg_pessoalinterno', getParam($_REQUEST, 'pg_pessoalinterno', ''));
...
$sql->adTabela('plano_gestao');
$sql->adCampo('pg_pessoalinterno');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';  
echo '<tr><td colspan=2 align="left"><h1>Quadro de Pessoal</h1></td></tr>'; 
if ($editarPG || $pg['pg_pessoalinterno']) echo '<tr><td colspan=2 align="left"><b>Informações sobre o Quadro de Pessoal d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>'; 
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea rows="10" name="pg_pessoalinterno" id="pg_pessoalinterno" >'.$pg['pg_pessoalinterno'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_pessoalinterno']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_pessoalinterno'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'PessoalInterno\'');
...
if ($exibir['clientes'] && $exibir['perfil']) $retorno='clientes';
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.($retorno ? botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\''.$retorno.'\');') : '').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'quadropessoal\');').'</td></tr></table></td></tr>';

echo '</table>';
echo '</td></tr></table>';
if ($editarPG) echo "<script type='text/javascript'>CKEDITOR.replace('pg_pessoalinterno',{extraPlugins : 'uicolor',uiColor: '#d9dada',toolbar :[['Source','NewPage','PasteText','PasteFromWord','SpellChecker','Scayt','Find', 'Undo','Redo','RemoveFormat','NumberedList','BulletedList','Outdent','Indent','Blockquote', 'JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock','Link','Unlink','HorizontalRule','PageBreak'],['Bold','Italic','Underline','Strike','Subscript','Superscript','TextColor','BGColor','Image','Table','SpecialChar','Styles','Format','Font','FontSize']]} );</script>"
