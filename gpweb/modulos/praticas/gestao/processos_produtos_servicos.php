...
if($direcao) {
		...
		$q->adOnde('pg_arquivo_campo = \'ProcessosProdutosServicos\'');
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'ProcessosProdutosServicos');
	}


if ($salvar){
	$sql->adTabela('plano_gestao');
	$sql->adAtualizar('pg_processos_apoio', getParam($_REQUEST, 'pg_processos_apoio', ''));
	$sql->adAtualizar('pg_processos_finalistico', getParam($_REQUEST, 'pg_processos_finalistico', ''));
	$sql->adAtualizar('pg_produtos_servicos', getParam($_REQUEST, 'pg_produtos_servicos', ''));
...
$sql->adTabela('plano_gestao');
$sql->adCampo('pg_processos_apoio, pg_processos_finalistico, pg_produtos_servicos');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Processos e Produtos/Serviços</h1></td></tr>';

if ($editarPG || $pg['pg_processos_apoio']) echo '<tr><td colspan=2 align="left"><b>Principais Processos de Apoio d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_processos_apoio" id="pg_processos_apoio" >'.$pg['pg_processos_apoio'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_processos_apoio'])  echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_processos_apoio'].'</td></tr></table></td></tr>';


if ($editarPG || $pg['pg_processos_finalistico']) echo '<tr><td colspan=2 align="left"><b>Principais Processos Finalísticos d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_processos_finalistico" id="pg_processos_finalistico" >'.$pg['pg_processos_finalistico'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_processos_finalistico'])  echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_processos_finalistico'].'</td></tr></table></td></tr>';


if ($editarPG || $pg['pg_produtos_servicos']) echo '<tr><td colspan=2 align="left"><b>Principais Produtos/Serviços d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_produtos_servicos" id="pg_produtos_servicos" >'.$pg['pg_produtos_servicos'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_produtos_servicos'])  echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_produtos_servicos'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_campo=\'ProcessosProdutosServicos\'');
