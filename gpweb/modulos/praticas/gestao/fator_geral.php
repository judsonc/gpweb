...
$fator_nome=getParam($_REQUEST, 'fator_nome', '0');
$objetivo_estrategico_id=getParam($_REQUEST, 'objetivo_estrategico_id', '0');
$excluirdiretriz=getParam($_REQUEST, 'excluirdiretriz', '0');
$editardiretriz=getParam($_REQUEST, 'editardiretriz', '0');
$mudar_fator_id=getParam($_REQUEST, 'mudar_fator_id', '0');
...
echo '<input type="hidden" name="mudar_fator_id" value="" />';
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'FatorCritico');
	}
...
if ($salvar){
	...
	if ($existe) {
		$sql->adTabela('plano_gestao2');
		$sql->adAtualizar('pg_fator_critico', getParam($_REQUEST, 'pg_fator_critico', ''));
		$sql->adOnde('pg_id ='.(int)$pg_id);
		$retorno=$sql->exec();
		$sql->limpar();
		}
		...
		$sql->adTabela('plano_gestao2');
		$sql->adInserir('pg_fator_critico', getParam($_REQUEST, 'pg_fator_critico', ''));
...
$sql->adTabela('plano_gestao2');
$sql->adCampo('pg_fator_critico');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>'.ucfirst($config['fatores']).'</h1></td></tr>';
if ($editarPG || $pg['pg_fator_critico']) echo '<tr><td colspan=2 align="left"><b>Informações gerais sobre os fatores crítico de sucesso d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_fator_critico" id="pg_fator_critico" >'.$pg['pg_fator_critico'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_fator_critico']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_fator_critico'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'FatorCritico\'');
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\''.($Aplic->profissional && $config['exibe_me']  && $Aplic->checarModulo('praticas', 'acesso', null, 'me') ? 'mes_pro' : 'objetivo').'\');').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'fator\');').'</td></tr></table></td></tr>';
