...
if($direcao) {
		...
		$q->adOnde('pg_arquivo_campo = \'EstruturaOrganizacional\'');
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'EstruturaOrganizacional');
	}


if ($salvar){
	$sql->adTabela('plano_gestao');
	$sql->adAtualizar('pg_estrut_org', getParam($_REQUEST, 'pg_estrut_org', ''));
...
$sql->adTabela('plano_gestao');
$sql->adCampo('pg_estrut_org');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Estrutura Organizacional</h1></td></tr>';
if ($editarPG || $pg['pg_estrut_org']) echo '<tr><td colspan=2 align="left"><b>Descrição da Estrutura Organizacional d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_estrut_org" id="pg_estrut_org">'.$pg['pg_estrut_org'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_estrut_org'])  echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_estrut_org'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_campo=\'EstruturaOrganizacional\'');
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1;env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'fornecedores_insumos_geral\');').'</td></tr></table></td></tr>';
