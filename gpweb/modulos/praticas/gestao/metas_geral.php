...
$pg_meta_nome=getParam($_REQUEST, 'pg_meta_nome', '0');
$pg_meta_responsavel=getParam($_REQUEST, 'pg_meta_responsavel', '0');
$pg_meta_prazo=getParam($_REQUEST, 'pg_meta_prazo', '0');
...
$mudar_pg_meta_id=getParam($_REQUEST, 'mudar_pg_meta_id', '0');
...
echo '<input type="hidden" name="pg_meta_id" value="0" />';
echo '<input type="hidden" name="mudar_pg_meta_id" value="" />';
...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'Meta');
	}
...
if ($salvar){
	...
	if ($existe) {
		$sql->adTabela('plano_gestao2');
		$sql->adAtualizar('pg_meta', getParam($_REQUEST, 'pg_meta', ''));
		$sql->adOnde('pg_id ='.(int)$pg_id);
		$retorno=$sql->exec();
		$sql->limpar();
		}
		...
		$sql->adTabela('plano_gestao2');
		$sql->adInserir('pg_meta', getParam($_REQUEST, 'pg_meta', ''));
...
echo '<tr><td colspan=2 align="left"><h1>Metas Organizacionais</h1></td></tr>';
if ($editarPG || $pg['pg_meta']) echo '<tr><td colspan=2 align="left"><b>Informações gerais sobre as estratégias d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_meta" id="pg_meta" >'.$pg['pg_meta'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_meta']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_meta'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'Meta\'');
...
echo '<tr><td colspan=2 align="center"><table width="100%"><tr><td>'.botao('anterior', 'Anterior', 'Ir para a tela anterior.','','carregar(\'estrategias\');').'</td><td width="40%">&nbsp;</td><td>'.($editarPG ? botao('salvar', 'Salvar', 'Salvar os dados acima.','','env.salvar.value=1; env.submit();') : '&nbsp').'</td><td width="40%">&nbsp;</td><td>'.botao('próximo', 'Próximo', 'Ir para a próxima tela.','','carregar(\'metas\');').'</td></tr></table></td></tr>';
