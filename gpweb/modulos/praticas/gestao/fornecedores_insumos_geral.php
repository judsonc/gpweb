...
if ($salvaranexo){
	grava_arquivo_pg($pg_id, 'arquivo', 'FornecedoreseInsumos');
	}


if ($salvar){
	$sql->adTabela('plano_gestao');
	$sql->adAtualizar('pg_fornecedores', getParam($_REQUEST, 'pg_fornecedores', ''));
...
$sql->adTabela('plano_gestao');
$sql->adCampo('pg_fornecedores');
$sql->adOnde('pg_id='.(int)$pg_id);
$pg=$sql->Linha();
$sql->limpar();

echo '<table width="100%" >';
echo '<tr><td colspan=2 align="left"><h1>Fornecedores e Insumos</h1></td></tr>';
if ($editarPG || $pg['pg_fornecedores']) echo '<tr><td colspan=2 align="left"><b>Informações gerais sobre os fornecedores e insumos d'.$config['genero_organizacao'].' '.$config['organizacao'].'</b></td></tr>';
if ($editarPG) echo '<tr><td colspan=2 align="left"><table width="810"><tr><td style="width:800px; max-width:800px;"><textarea data-gpweb-cmp="ckeditor" rows="10" name="pg_fornecedores" id="pg_fornecedores" >'.$pg['pg_fornecedores'].'</textarea></td></tr></table></td></tr>';
elseif($pg['pg_fornecedores']) echo '<tr><td colspan=2 align="left"><table width="100%"><tr><td class="realce" width="100%">'.$pg['pg_fornecedores'].'</td></tr></table></td></tr>';
...
$sql->adOnde('pg_arquivo_pg_id='.(int)$pg_id);
$sql->adOnde('pg_arquivo_campo=\'FornecedoreseInsumos\'');
