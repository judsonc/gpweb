...
if (isset($_REQUEST['filtro_prioridade_checklist']))	$Aplic->setEstado('filtro_prioridade_checklist', getParam($_REQUEST, 'filtro_prioridade_checklist', null));
$filtro_prioridade_checklist = $Aplic->getEstado('filtro_prioridade_checklist') !== null ? $Aplic->getEstado('filtro_prioridade_checklist') : null;


$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_checklist=1');
...
if (isset($_REQUEST['checklisttextobusca'])) $Aplic->setEstado('checklisttextobusca', getParam($_REQUEST, 'checklisttextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('checklisttextobusca') ? $Aplic->getEstado('checklisttextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaChecklistTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaChecklistTab') !== null ? $Aplic->getEstado('ListaChecklistTab') : 0);
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('checklist_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('checklist_favorito') !== null ? $Aplic->getEstado('checklist_favorito') : 0;

$sql->adTabela('favorito');
$sql->adCampo('favorito_id, favorito_nome');
$sql->adOnde('favorito_geral!=1');
$sql->adOnde('favorito_ativo=1');
$sql->adOnde('favorito_checklist=1');
...
echo '<input type="hidden" name="filtro_prioridade_checklist" id="filtro_prioridade_checklist" value="'.$filtro_prioridade_checklist.'" />';
...
if (!$dialogo && $Aplic->profissional){
	...
	$botoesTitulo = new CBlocoTitulo('Lista de Checklists', 'todo_list.png', $m, $m.'.'.$a);
	...
	if($plano_acao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Filtrar os checklists pel'.$config['genero_acao'].' '.$config['acao'].' que estão relacionados.').ucfirst($config['acao']).':'.dicaF();
		$nome=nome_acao($plano_acao_id);
		}
	elseif($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar os checklists pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionados.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar os checklists pela agenda que estão relacionados.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar os checklists pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionados.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar os checklists pelo indicador que estão relacionados.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar os checklists pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionados.').''.ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar os checklists pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionados.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar os checklists pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionados.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar os checklists pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionados.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar os checklists pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionados.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar os checklists pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionados.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($pg_meta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar os checklists pel'.$config['genero_meta'].' '.$config['meta'].' que estão relacionados.').ucfirst($config['meta']).':'.dicaF();
		$nome=nome_meta($pg_meta_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar os checklists pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionados.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar os checklists pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionados.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar os checklists pelo monitoramento que estão relacionados.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		

	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar os checklists pela ata de reunião a qual estão relacionados.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}	

	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar os checklists pela matriz SWOT que estão relacionados.').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($mswot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelp Campo de Matriz SWOT', 'Filtrar os checklists pelo campo de matriz SWOT que estão relacionados.').'Campo SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar os checklists pelo plano operativo que estão relacionados.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar os checklists pelo instrumento jurídico que estão relacionados.').'Instrumento Jurídico:'.dicaF();
		$nome=nome_instrumento($instrumento_id);
		}	
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar os checklists pelo recurso que estão relacionados.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar os checklists pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionados.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar os checklists pela demanda que estão relacionados.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar os checklists pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionados.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar os checklists pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionados.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar os checklists pelo evento que estão relacionados.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($link_id){
		$legenda_filtro=dica('Filtrar pelo Link', 'Filtrar os checklists pelo link que estão relacionados.').'Link:'.dicaF();
		$nome=nome_link($link_id);
		}
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar os checklists pela avaliação que estão relacionados.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar os checklists pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionados.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar os checklists pelo brainstorm que estão relacionados.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar os checklists pela matriz GUT que estão relacionados.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar os checklists pelo diagrama de causa-efeito que estão relacionados.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($arquivo_id){
		$legenda_filtro=dica('Filtrar pelo Arquivo', 'Filtrar os checklists pelo arquivo que estão relacionados.').'Arquivo:'.dicaF();
		$nome=nome_arquivo($arquivo_id);
		}	
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar os checklists pelo fórum que estão relacionados.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar os checklists pelo compromisso que estão relacionados.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar os checklists pelo agrupamento que estão relacionados.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar os checklists pelo patrocinador que estão relacionados.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar os checklists pelo modelo que estão relacionados.').'Modelo:'.dicaF();
		$nome=nome_template($template_id);
		}		
	else{
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar os checklists.').'Filtro:'.dicaF();
		}
	...
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionad'.$config['genero_mensagem'],'A qual parte do sistema os checklists estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$procuraBuffer = '<tr><td align=right nowrap="nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="checklisttextobusca" onChange="document.env.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&tab='.$tab.'&checklisttextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';


	$novo_checklist=($podeAdicionar ? '<tr><td nowrap="nowrap">'.dica('Novo Checklist', 'Criar um novo checklist.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=checklist_editar\');" ><img src="'.acharImagem('todo_list_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$botao_favorito='<tr><td nowrap="nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de checklist favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&checklist=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir a Lista de Checklist', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de checklist.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=checklist_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	...
	if($filtro_prioridade_checklist) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$procuraBuffer.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_checklist.$botao_prioridade.$imprimir.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Lista de Checklist', 'todo_list.png', $m, $m.'.'.$a);
	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0>'.$procurar_om.'</table>');
	if ($podeAdicionar) $botoesTitulo->adicionaCelula(dica('Novo Checklist', 'Criar um novo checklist.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=checklist_editar\');" ><span>novo</span></a>'.dicaF());
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir a Lista de Checklist', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de checklist.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=checklist_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Checklists</center></font>';
	}
echo '</form>';
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/checklist_ver_idx', 'Ativos',null,null,'Ativos','Visualizar os checklist ativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/checklist_ver_idx', 'Inativos',null,null,'Inativos','Visualizar os checklist inativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/checklist_ver_idx', 'Todos',null,null,'Todos','Visualizar todos os checklist.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/checklist_ver_idx.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_checklist.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_checklist){
	env.filtro_prioridade_checklist.value=filtro_prioridade_checklist;
	env.submit();
	}	
