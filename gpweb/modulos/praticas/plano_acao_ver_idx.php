...
global $estilo_interface, $Aplic, $cia_id, $tab, $favorito_id, $dialogo, $usuario_id, $pesquisar_texto, $lista_cias, $dept_id, $lista_depts, $plano_acao_ano, $filtro_prioridade_acao, $podeEditar,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('plano_acao_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('plano_acao_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('plano_acao_gestao_mswot='.(int)$mswot_id);
...
$sql->adCampo('plano_acao.plano_acao_id, plano_acao_inicio, plano_acao_fim, plano_acao_percentagem, plano_acao_nome, plano_acao_descricao, plano_acao_responsavel, plano_acao_cor, plano_acao_acesso, count(plano_acao_item_id) AS qnt, plano_acao_ano, plano_acao_codigo, plano_acao_cia, plano_acao_aprovado');
...
if ($exibir['cor']) echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor', 'Neste campo fica a cor de identificação d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Cor'.dicaF().'</a></th>';
echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Nome'.dicaF().'</a></th>';

if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_acao) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';

if ($exibir['descricao']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição', 'Neste campo fica a descrição d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Descrição'.dicaF().'</a></th>';
...
if ($exibir['responsavel']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'O '.$config['usuario'].' responsável pel'.$config['genero_acao'].'s '.$config['acoes'].'.').'Responsável'.dicaF().'</a></th>';
...
if ($exibir['inicio']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_inicio&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_inicio' ? imagem('icones/'.$seta[$ordem]) : '').dica('Início', 'A data de ínicio d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Início'.dicaF().'</a></th>';
if ($exibir['fim']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_fim&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_fim' ? imagem('icones/'.$seta[$ordem]) : '').dica('Término', 'A data de término d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Término'.dicaF().'</a></th>';
if ($exibir['percentagem']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_percentagem&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_percentagem' ? imagem('icones/'.$seta[$ordem]) : '').dica('Percentagem', 'A percentagem executada n'.$config['genero_acao'].'s '.$config['acoes'].'.').'%'.dicaF().'</a></th>';
if ($exibir['linhas']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=qnt&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='qnt' ? imagem('icones/'.$seta[$ordem]) : '').dica('Quantidade', 'A quantidade de linhas inseridas n'.$config['genero_acao'].'s '.$config['acoes'].'.').'Qnt'.dicaF().'</a></th>';
if ($exibir['cia_id']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_cia&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_cia' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['organizacao']), 'As '.$config['organizacoes'].' d'.$config['genero_acao'].'s '.$config['acoes'].'.').$config['organizacao'].dicaF().'</a></th>';
if ($exibir['ano']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_ano&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_ano' ? imagem('icones/'.$seta[$ordem]) : '').dica('Ano', 'O ano base dos'.$config['genero_acao'].'s '.$config['acoes'].'.').'Ano'.dicaF().'</a></th>';
if ($exibir['codigo']) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').($tab ? '&tab='.$tab : '').'&ordenar=plano_acao_codigo&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='plano_acao_codigo' ? imagem('icones/'.$seta[$ordem]) : '').dica('Código', 'Os códigos d'.$config['genero_acao'].'s '.$config['acoes'].'.').'Código'.dicaF().'</a></th>';
echo '</tr>';
$qnt=0;
$agora =date('Y-m-d');
...
foreach ($plano_acoes as $linha) {
	if (permiteAcessarPlanoAcao($linha['plano_acao_acesso'],$linha['plano_acao_id'])){
		...
		echo '<tr>';
		if (!$dialogo) echo '<td nowrap="nowrap" width="16">'.($podeEditar && $editar && !$bloquear ? dica('Editar '.ucfirst($config['acao']), 'Clique neste ícone '.imagem('icones/editar.gif').' para editar '.$config['genero_acao'].'s '.$config['acoes'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=plano_acao_editar&plano_acao_id='.$linha['plano_acao_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['plano_acao_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		if ($Aplic->profissional) {
			...
			if (count($lista)){	
				foreach($lista as $gestao_data){	
					...
					elseif ($gestao_data['plano_acao_gestao_mswot']) echo ($qnt_gestao++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['plano_acao_gestao_mswot']);
