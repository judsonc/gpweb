...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo->adicionaCelula('<td nowrap="nowrap" align="right">'.dica('Imprimir '.ucfirst($config['projetos']).' por '.ucfirst($config['objetivos']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir '.$config['projetos'].' por '.$config['objetivos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=projetos_por_obj_estrategicos&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
...
foreach($perspectivas as $perspectiva){
	...
	$sql->adTabela('objetivo');
	$sql->esqUnir('objetivo_perspectiva', 'objetivo_perspectiva','objetivo_perspectiva_objetivo=objetivo.objetivo_id');
	$sql->esqUnir('tema', 'tema', 'tema_id=objetivo_perspectiva_tema');
