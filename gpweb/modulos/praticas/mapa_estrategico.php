...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_link='<tr><td><a href="javascript: void(0)" onclick="javascript:exportar_link();">'.imagem('icones/arvore_exporta.png', 'Exportar Link', 'Clique neste ícone '.imagem('icones/arvore_exporta.png').' para criar um endereço web para visualização do mapa estratégico em ambiente externo.').'</a></td></tr>';
	

	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$selecao_plano.$por_percentagem.$filtro_por_responsavel.$filtro_por_dept.$por_designado.'</table></td><td><table cellspacing=0 cellpadding=0>'.$imprimir.$botao_link.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif($dialogo && $Aplic->profissional) {
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Mapa Estratégico</center></font>';
	}	
...
foreach($perspectivas as $perspectiva){
	...
	foreach($temas as $tema){
		...
		$sql->esqUnir('objetivo_perspectiva', 'objetivo_perspectiva','objetivo_perspectiva_objetivo=objetivo.objetivo_id');
		...
		foreach($objetivos as $objetivo) {
			$saida.=($qnt_inserida++ ? '<br>': '').link_objetivo($objetivo['objetivo_id'],null,null,null,null,null,true, null, $inicio, $fim).($percentagem ? ' '.(int)$objetivo['objetivo_percentagem'].'%' : '');
			}
	...
	foreach($objetivos as $objetivo){
		...
		echo '<tr><td>'.link_objetivo($objetivo['objetivo_id'],'','','','','',true, null, $inicio, $fim).($percentagem ? ' '.(int)$objetivo['objetivo_percentagem'].'%' : '').'</td></tr>';
		if ($por_responsavel){
			$saida='';
		...
		if ($por_dept){
			...
			$qnt_inserida=0;
			foreach($depts as $dept) {
				$saida.=($qnt_inserida++ && $qnt_inserida!=$qnt ? ', ': '').($qnt_inserida==$qnt && $qnt>1 ? ' e ': '').link_secao($dept['objetivo_dept_dept']);
				}
			if ($saida) echo '<tr><td align=center>'.$saida.'</td></tr>';
			}
		if ($por_designados){
...
function exportar_link() {
	parent.gpwebApp.popUp('Link', 900, 100, 'm=publico&a=exportar_link&dialogo=1&tipo=generico&cia_id='+document.getElementById('cia_id').value, null, window);
	}
