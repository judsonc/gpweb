...
global $Aplic, $cal_sdf, $cia_id, $dialogo, $ano;
//Caso de impressão
...
echo '<td>&nbsp; &nbsp;</td><td style="border-style:solid;border-width:1px" bgcolor="#ffd9cf">&nbsp; &nbsp;</td><td>'.dica('Poucos Indicadores', 'Poucos indicadores atendem ao requisito.').'Poucos 0-24%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#ffe9cf">&nbsp; &nbsp;</td><td>'.dica('Muitos Indicadores', 'Muitos indicadores atendem ao requisito.').'Muitos 25-49%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
...
echo '<td style="border-style:solid;border-width:1px" bgcolor="#edffcf">&nbsp; &nbsp;</td><td>'.dica('Quase Todos os Indicadores', 'Quase todos os indicadores atendem ao requisito.').'Quase Todos 75-99%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#d8ffcf">&nbsp; &nbsp;</td><td>'.dica('Todos os Indicadores', 'Todos os indicadores atendem ao requisito.').'Todos 100%'.dicaF().'</td>';
...
if (!$dialogo){
	echo '<table width="100%"><tr><td width="22">&nbsp;</td><td align="center"><font size="4"><center>Pontos fortes dos resultados</center></font></td><td width="22"><a href="javascript: void(0);" onclick ="frm_filtro.target=\'popup\'; frm_filtro.dialogo.value=1; frm_filtro.submit();">'.imagem('imprimir_p.png', 'Imprimir o Relatório', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir o relatório a partir do navegador Web.').'</a></td></tr></table>';
	echo estiloTopoCaixa();
	}
else echo '<table width="100%"><tr><td align="center"><font size="4"><center>Pontos fortes dos resultados</center></font></td></tr></table>';		
...
function somatorio($vetor_indicadores){
	...
	foreach($vetor_indicadores as $chave => $pratica_indicador_id){
		foreach ($campos as $chave => $linha){
			if ($indicadores[$pratica_indicador_id['pratica_indicador_id']][$chave]) {
				$soma[$chave]++; 
				$nomes[$chave][$pratica_indicador_id['pratica_indicador_id']]=$indicadores[$pratica_indicador_id['pratica_indicador_id']]['pratica_indicador_nome'];
				}
			}
	...
	$saida='';
	foreach ($campos as $chave => $linha) $saida.=formata_soma($total, $soma[$chave], $nomes[$chave]);
