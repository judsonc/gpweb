...
global $Aplic, $cal_sdf, $cia_id, $dialogo, $ano, $usuario_id, $pratica_modelo_id;
...
echo '<td>&nbsp; &nbsp;</td><td style="border-style:solid;border-width:1px" bgcolor="#ffd9cf">&nbsp; &nbsp;</td><td>'.dica('Pouc'.($config['genero_pratica']=='a' ? 'as ': 'os ').ucfirst($config['praticas']), 'Pouc'.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas'].' atendem ao requisito.').'Poucas 0-24%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#ffe9cf">&nbsp; &nbsp;</td><td>'.dica('Muit'.($config['genero_pratica']=='a' ? 'as ': 'os ').ucfirst($config['praticas']), 'Muit'.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas'].' atendem ao requisito.').'Muitas 25-49%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#feffcf">&nbsp; &nbsp;</td><td>'.dica('Maioria d'.($config['genero_pratica']=='a' ? 'as ': 'os ').ucfirst($config['praticas']), 'Maioria d'.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas'].' atendem ao requisito.').'Maioria 50-74%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#edffcf">&nbsp; &nbsp;</td><td>'.dica('Quase Tod'.($config['genero_pratica']=='a' ? 'as as ': 'os os ').ucfirst($config['praticas']), 'Quase todas '.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas'].' atendem ao requisito.').'Quase Todas 75-99%'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
echo '<td style="border-style:solid;border-width:1px" bgcolor="#d8ffcf">&nbsp; &nbsp;</td><td>'.dica('Todas '.($config['genero_pratica']=='a' ? 'as ': 'os ').ucfirst($config['praticas']), 'Todas '.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas'].' atendem ao requisito.').'Todas 100%'.dicaF().'</td>';
...
if ($filtro_criterio) {
	if (!$dialogo){
		echo '<table width="100%"><tr><td width="22">&nbsp;</td><td align="center"><font size="4"><center>Pontos fortes d'.$config['genero_pratica'].'s '.$config['praticas'].'</center></font></td><td width="22"><a href="javascript: void(0);" onclick ="frm_filtro.target=\'popup\'; frm_filtro.dialogo.value=1; frm_filtro.submit();">'.imagem('imprimir_p.png', 'Imprimir o Relatório', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir o relatório a partir do navegador Web.').'</a></td></tr></table>';
		echo estiloTopoCaixa();
		}
	else echo '<table width="750"><tr><td align="center"><font size="4"><center>Pontos fortes d'.$config['genero_pratica'].'s '.$config['praticas'].'</center></font></td></tr></table>';		
...
function somatorio($vetor_praticas){
	...
	foreach($vetor_praticas as $chave => $pratica_id){
		foreach ($campos as $chave => $nome_campo){
			if ($praticas[$pratica_id['pratica_id']][$nome_campo]) {
				$soma[$nome_campo]++; 
				$nomes[$nome_campo][$pratica_id['pratica_id']]=$praticas[$pratica_id['pratica_id']]['pratica_nome'];
				}
			}
...
function formata_soma($total, $quantidade, $nomes=array()){
	...
	if ($percentagem<25) $cor='ffd9cf';
	elseif ($percentagem<50) $cor='ffe9cf';
	elseif ($percentagem<75) $cor='feffcf';
	elseif ($percentagem<100) $cor='edffcf';
	else $cor='d8ffcf';
	$saida='';
	if (count($nomes)){
		foreach($nomes as $pratica_id => $pratica_nome) {
		...
		return '<td style="text-align:center; background:#'.$cor.'">'.dica('Percentagem', $percentagem.'%').'&nbsp;0&nbsp;'.dicaF().'</td>';
		}	
	}	
