...
function atualizar_indicador($pratica_indicador_formula_pai=0, $uuid=''){
	...
	foreach($lista as $linha){
		...
		$saida.= '<td width="16" align=center><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir?\')) {excluir_indicador('.$linha['pratica_indicador_formula_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir.').'</a></td></tr>';
		}
	if (count($lista)) $saida.= '</table>';
	return $saida;
	}	
...
function testar_formula_ajax($texto){
  ...
  //Se chegar aqui é pq não houve erro
  $objResposta = new xajaxResponse();
