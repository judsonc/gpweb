...
if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores d'.$config['genero_perspectiva'].' '.$config['perspectiva'].' mais representativo da situação geral do mesmo.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'pg_perspectiva_principal_indicador', 'class="texto" style="width:284px;"', $obj->pg_perspectiva_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'pg_perspectiva_moeda', 'class=texto size=1', ($obj->pg_perspectiva_moeda ? $obj->pg_perspectiva_moeda : 1)).'</td></tr>';
	}	
else echo '<input type="hidden" name="pg_perspectiva_moeda" id="pg_perspectiva_moeda" value="'.($obj->pg_perspectiva_moeda ? $obj->pg_perspectiva_moeda : 1).'" />';
