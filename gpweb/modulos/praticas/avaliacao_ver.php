...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_avaliacao=".$avaliacao_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->avaliacao_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Avaliação</center></font>';
	}
...
if (!$dialogo && $Aplic->profissional) {
	...
	if($Aplic->profissional){
		...
		if ($Aplic->profissional) {
			...
			$sql->adOnde('log_avaliacao = '.(int)$avaliacao_id);
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('evento_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(evento_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('arquivo_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(arquivo_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('link_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(link_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('forum_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(forum_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('pratica_indicador_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(pratica_indicador_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('plano_acao_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(plano_acao_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('projeto_gestao_avaliacao = '.(int)$avaliacao_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adOnde('msg_gestao_avaliacao = '.(int)$avaliacao_id);
			$sql->adCampo('count(msg_gestao_avaliacao)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adOnde('modelo_gestao_avaliacao = '.(int)$avaliacao_id);
				$sql->adCampo('count(modelo_gestao_avaliacao)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adOnde('ata_gestao_avaliacao = '.(int)$avaliacao_id);
			$sql->adCampo('count(ata_gestao_avaliacao)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adOnde('problema_gestao_avaliacao = '.(int)$avaliacao_id);
			$sql->adCampo('count(problema_gestao_avaliacao)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adOnde('risco_gestao_avaliacao = '.(int)$avaliacao_id);
			$sql->adCampo('count(risco_gestao_avaliacao)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_avaliacao = '.(int)$avaliacao_id.' OR risco_gestao_avaliacao = '.(int)$avaliacao_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adOnde('instrumento_gestao_avaliacao = '.(int)$avaliacao_id);
			$sql->adCampo('count(instrumento_gestao_avaliacao)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
