...
if (getParam($_REQUEST, 'inserir', null)){
	...
	if ($existe) ver2('Já foi inserida esta ação social n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.');
	elseif ($existe2) ver2('Já foi negada esta ação social n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.');
	else {
		$sql->adTabela('social_familia_acao');
...
$msg = '';
$botoesTitulo = new CBlocoTitulo('Ação Social Vinculada '.($config['genero_beneficiario']=='o' ? 'ao' : 'a').' '.ucfirst($config['beneficiario']), '../../../modulos/Social/imagens/familia.gif', $m, $m.'.'.$a);
...
echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Inserir Ação Social','Inserir '.$config['genero_beneficiario'].' '.$config['beneficiario'].' em uma ação social.').'&nbsp;<b>Inserir Ação Social</b>&nbsp</legend><table width="100%" cellspacing=2 cellpadding=0>';
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social à inserir n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Programa:'.dicaF().'</td><td nowrap="nowrap" align="left">'.selecionaVetor($lista_programas, 'social_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao()"', $social_id).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social', 'Escolha a ação social à inserir n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Ação:'.dicaF().'</td><td nowrap="nowrap" align="left"><div id="acao_combo">'.selecionar_acao_para_ajax($social_id, 'acao_id', 'size="1" style="width:160px;" class="texto"', '', $acao_id, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social à esquerda n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'</a></td></tr></table></td></tr>';
echo '</table></fieldset></td></tr>';

echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Negadar Ação Social','Inserir uma ação social que será negada a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'&nbsp;<b>Negar Ação Social</b>&nbsp</legend><table width="100%" cellspacing=2 cellpadding=0>';
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social que será negada a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'Programa:'.dicaF().'</td><td nowrap="nowrap" align="left">'.selecionaVetor($lista_programas, 'social_id_negado', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao_negado();"', $social_id_negado).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social Negada', 'Escolha a ação social foi negada a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'Ação:'.dicaF().'</td><td nowrap="nowrap" align="left"><div id="acao_combo_negado">'.selecionar_acao_para_ajax($social_id_negado, 'acao_id_negado', 'size="1" style="width:160px;" class="texto" onchange="lista_negacao();"', '', $acao_id_negado, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao_negado();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social que foi negada à beneficiário.').'</a></td></tr></table></td></tr>';
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Justificativa', 'Escolha a justificativa para a negação desta ação social à beneficiário.').'Justificativa:'.dicaF().'</td><td nowrap="nowrap" align="left"><div id="combo_justificativa">'.selecionar_acao_negacao_para_ajax($acao_id_negado, 'negacao_id', 'size="1" style="width:355px;" class="texto"', '', '', false).'</div></td></tr></table></td></tr>';
echo '</table></fieldset></td></tr>';
...
foreach ($lista_acoes as $acao){
	$total=0;
	$marcado=0;
	echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica($acao['social_acao_nome'],'Lista de atividades da ação social vinculada à beneficiário.').'&nbsp;<b>'.$acao['social_acao_nome'].'</b>&nbsp</legend><table cellspacing=2 cellpadding=0>';
	
	
	echo '<tr><td colspan=20><table><tr><td align="right">'.dica('Código', 'O código desta ação n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Código:'.dicaF().'</td><td><input type="text" name="codigo_'.$acao['social_acao_id'].'" id="codigo_'.$acao['social_acao_id'].'" value="'.$acao['social_familia_acao_codigo'].'" style="width:100px;" class="texto" onchange="mudar_codigo('.$acao['social_acao_id'].')" /></td></tr></table></td></tr>';

	
	$data = intval($acao['social_familia_acao_data_previsao']) ? new CData($acao['social_familia_acao_data_previsao']) : new CData();
	
	echo '<tr><td colspan=20><table><tr><td align="right">'.dica('Previsão','Previsão de conclusão desta ação social n'.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'<b>Previsão:</b>'.dicaF().'</td><td><input type="hidden" name="previsao_'.$acao['social_acao_id'].'" id="previsao_'.$acao['social_acao_id'].'" value="'.($data ? $data->format(FMT_TIMESTAMP_DATA) : '').'" /><input type="text" name="data_'.$acao['social_acao_id'].'" style="width:70px;" id="data_'.$acao['social_acao_id'].'" onchange="setData(\'env\', \'data_'.$acao['social_acao_id'].'\', \'previsao_'.$acao['social_acao_id'].'\', '.$acao['social_acao_id'].');" value="'.($data ? $data->format('%d/%m/%Y') : '').'" class="texto" /></td><td>'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data de início da pesquisa d'.$config['genero_tarefa'].'s '.$config['tarefas'].'.<br><br>Somente serão apresentadas '.$config['genero_tarefa'].'s '.$config['tarefas'].' que tenham iniciado à partir desta data.').'<a href="javascript: void(0);" ><img id="btn_'.$acao['social_acao_id'].'" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr></table></td></tr>';	


	echo '<script type="text/javascript">var cal_'.$acao['social_acao_id'].' = Calendario.setup({
	...
	if ($podeExcluir) echo '<tr><td colspan=2>'.botao('excluir', 'Excluir','Ao pressionar este botão esta ação social será excluída d'.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'. Irá impactar na percentagem feita da tarefas referente a esta ação na comunidade d'.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].', caso a mesma estava finalizada.','','if(confirm(\'Tem certeza quanto à excluir?\')){env.excluir_acao.value='.$acao['social_acao_id'].'; env.submit();}').'</div></td></tr>';	
	
	echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Problemas','Lista de problemas relacionados à execução desta ação n'.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'&nbsp;<b>Problemas</b>&nbsp</legend><table cellspacing=0 cellpadding=0>';
	problema($acao['social_acao_id']);
	echo '</table></fieldset></td></tr>';
	
	echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Arquivos','Lista de arquivos relacionados à execução desta ação n'.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'&nbsp;<b>Arquivos</b>&nbsp</legend><table cellspacing=0 cellpadding=0>';
...
foreach ($lista_acoes as $linha) $saida.='<tr><td>'.$linha['social_acao_nome'].'</td><td>'.$linha['social_acao_negacao_justificativa'].'</td>'.($podeExcluir ? '<td><a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir esta negação?\')) {env.excluir_negacao.value='.$linha['social_acao_id'].'; env.submit();}">'.imagem('icones/remover.png', 'Excluir Negação', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir esta negação à concessão de ação social à beneficiário.').'</a></td>' : '').'</tr>';	
	
if ($saida){
	echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Negações','Lista de ações sociais que foram negadas a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'&nbsp;<b>Negações</b>&nbsp</legend><table cellspacing=0 cellpadding=0 class="tbl1"><tr><th>Ação</th><th>Justificativa</th>'.($podeExcluir ? '<th></th>' : '').'</tr>';
	echo $saida;
	echo '</table></fieldset></td></tr>';
	}
...
function problema($acao_id){
	...
	foreach ($lista as $linha) {
		...
		$saida.='<td><a href="javascript: void(0);" onclick="excluir_problema('.$acao_id.','.$linha['social_familia_problema_id'].');">'.imagem('icones/remover.png', 'Excluir Problema', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir este problema.').'</a></td>';
