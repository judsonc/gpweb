...
global $Aplic, $social_comunidade_id, $podeExcluir, $podeEditar, $df, $m, $tab;
...
echo '<th width="100%"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$m.'&tab='.$tab.'&social_comunidade_id='.$social_comunidade_id.'&ordenar=social_comunidade_log_descricao&ordem='.($ordem ? '0' : '1').'\');">'.dica('Comentários', 'Comentários sobre o registro d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='social_comunidade_log_descricao' ? imagem('icones/'.$seta[$ordem]) : '').'Comentários'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$m.'&tab='.$tab.'&social_comunidade_id='.$social_comunidade_id.'&ordenar=social_comunidade_log_nd&ordem='.($ordem ? '0' : '1').'\');">'.dica('ND', 'Número de Despesa já empenhado n'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='social_comunidade_log_nd' ? imagem('icones/'.$seta[$ordem]) : '').'ND'.dicaF().'</a></th>';
...
foreach ($logs as $linha) {
	...
	echo '</td><td nowrap="nowrap" >'.retorna_data($linha['social_comunidade_log_data'], false).'</td>';
	$imagem_referencia = '-';
	...
	echo '<td nowrap="nowrap" style="'.$estilo.'">'.($linha['social_comunidade_log_nome'] ? $linha['social_comunidade_log_nome'] : '&nbsp;').'</td>';
	echo '<td nowrap="nowrap">'.link_usuario($linha['social_comunidade_log_criador'],'','','esquerda').'</td>';
	echo '<td width="100" align="right">'.($linha['social_comunidade_log_horas'] ? sprintf('%.2f', $linha['social_comunidade_log_horas']) : '&nbsp;').'</td>';
	...
	echo '<td>&nbsp;</td>';
	echo '<td>'.($podeEditar ?  dica('Excluir Registro', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir este registro.').'<a href="javascript:excluir2('.$linha['social_comunidade_log_id'].');" >'.imagem('icones/remover.png').'</a>' : '&nbsp;').'</td></tr>';
