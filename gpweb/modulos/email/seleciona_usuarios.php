...
if ($msg_projeto || $msg_tarefa || $msg_pratica || $msg_indicador || $msg_acao || $msg_calendario || $msg_objetivo || $msg_tema || $msg_tema || $msg_estrategia || $msg_meta || $msg_perspectiva || $msg_canvas || $msg_fator || $msg_monitoramento || $msg_operativo){
	...
	if ($msg_objetivo) $sql->esqUnir('objetivo','objetivo','objetivo_cia=cias.cia_id');
	...
	if ($msg_fator) $sql->esqUnir('fator','fator','fator_cia=cias.cia_id');
	...
	elseif ($msg_objetivo) $sql->adOnde('objetivo_id = '.$msg_objetivo);
	...
	elseif ($msg_fator) $sql->adOnde('fator_id = '.$msg_fator);
...
function popObjetivo() {
	...
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('cia_id').value, 'Objetivos','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}		
