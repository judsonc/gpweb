...
if (!$permitido) {
	echo '<script language=Javascript>alert("Não tem permissão de acesso a esta Msg");window.open("./index.php?m=email&a=lista_msg&status=1", "_self");</script>';
	exit();
	}			
...
if (function_exists('openssl_sign') && $rs['assinatura']){	
	...
	if (!$ok) $assinado='&nbsp;'.dica('Assinatura Incorreta','A assinatura digital d'.$config['genero_mensagem'].' '.$config['mensagem'].' não confere! '.ucfirst($config['mensagem']).' possívelmente adulterad'.$config['genero_mensagem'].'.').'<img src="'.acharImagem('icones/assinatura_erro.gif').'" style="vertical-align:top" width="15" height="13" />'.dicaF();
	else $assinado='&nbsp;'.dica(''.ucfirst($config['mensagem']).' Assinad'.$config['genero_mensagem'],'A assinatura digital d'.$config['genero_mensagem'].' '.$config['mensagem'].' confere.').'<img src="'.acharImagem('icones/assinatura.gif').'" style="vertical-align:top" width="15" height="13" />'.dicaF();
	}
...
echo '<tr><td colspan=60 align="center" style="font-size:10pt;  padding-left: 5px; padding-right: 5px;"><b>'.strtoupper($config['mensagem']).' '.$msg_id.'</b></td></tr>';
echo '<tr>';
echo '<td width="20" align="center" nowrap="nowrap" style="font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;'.($config['msg_precedencia'] ? '' : ' display:none;').'">'.dica('Precedência','A precedência diz respeito à urgencia na leitura d'.$config['genero_mensagem'].' '.$config['mensagem'].'.').'Precedência'.dicaF().'</td>';
echo '<td width="20" align="center" nowrap="nowrap" style="font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;'.($config['msg_class_sigilosa'] ? '' : ' display:none;').'">'.dica('Classificação Sigilosa','A classificação sigilosa serve para restringir quais destinatários podem ler '.$config['genero_mensagem'].' '.$config['mensagem'].'.').'Class&nbsp;Sigilosa'.dicaF().'</td>';
echo '<td align="center" colspan=20 style="font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;" >'.dica('Assunto','O assunto serve para pesquisar '.$config['genero_mensagem'].'s '.$config['mensagens'].' e facilita na organização d'.$config['genero_mensagem'].'s mesm'.$config['genero_mensagem'].'s.').'Assunto'.dicaF().'</td></tr>';
echo '<tr>';
...
echo '<tr><td align="right" style="font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;">De:</td>';
echo '<td colspan=40 style="font-size:10pt; padding-left: 5px; padding-right: 5px;">';
...
//EUZ Postgres
$sql->adGrupo('usuarios.usuario_id, contatos.contato_posto, contato_nomeguerra, contato_funcao, msg_usuario.de_id, msg_usuario.nome_de, msg_usuario.funcao_de');
//EUD
...
echo '</td></tr>';
echo '<tr><td align="right" style="font-size:10pt;font-weight:Bold;  padding-left: 5px; padding-right: 5px;" >Para:</td>';
echo '<td colspan=40 style="font-size:9pt; padding-left: 5px; padding-right: 5px;">';
...
echo '<tr><td colspan=60 align="center" style="font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;">Texto d'.$config['genero_mensagem'].' '.ucfirst($config['mensagem']).'</td></tr>';

echo '<tr><td colspan=60 style="font-size:10pt; padding-left: 5px; padding-right: 5px;">'.$rs['texto'].'</td></tr>';


echo '<tr><td colspan=60 style="font-size:10pt; padding-left: 5px; padding-right: 5px ;">';
...
if ($Aplic->profissional){
  ...
  if (count($lista)) {
	  if ($Aplic->profissional) require_once BASE_DIR.'/modulos/projetos/template_pro.class.php';
		...
		echo '<tr><td style="font-size:10pt; padding-left: 5px; padding-right: 5px;"><b>Relacionamento:</b></td><td>';
		$qnt=0;
		foreach($lista as $gestao_data){
			...
			elseif ($gestao_data['msg_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['msg_gestao_mswot']);
			...
			elseif ($gestao_data['msg_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['msg_gestao_template']);
...
foreach ($anexos as $rs_anexo){ 
	...
	if (function_exists('openssl_sign') && $rs_anexo['assinatura']){	
		...
		if (!$ok) $assinado=dica('Assinatura Incorreta','A assinatura digital do anexo não confere! Anexo possívelmente adulterado.').'<img src="'.acharImagem('icones/assinatura_erro.gif').'" style="vertical-align:top" width="15" height="13" />'.dicaF();
		else $assinado=dica('Anexo Assinado','A assinatura digital do anexo confere.').'<img src="'.acharImagem('icones/assinatura.gif').'" style="vertical-align:top" width="15" height="13" />'.dicaF();
		}
...
if ($quantidade_anexos)  echo '</table></td></tr></table><br>';	
