...
if ($anexar_msg){
	...
	else 	echo '<script type="text/javascript">alert("Documento já foi anexado anteriormente.");</script>';
...
if (!$dialogo && $Aplic->profissional){
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo Matriz SWOT', 'Filtrar os documentos pelo campo de matriz SWOT que estão relacionad'.$config['genero_mensagem'].'s.').'Campo SWOT:'.dicaF();
...
if ($mover){
	...
	$sql->adOnde('para_id	'.($coletivo ? 'IN ('.$Aplic->usuario_lista_grupo.')' : '='.(int)$Aplic->usuario_id));
	$sql->adOnde('msg_id IN ('.grupo_msg($mover).')');
	$sql->exec();
	$sql->limpar();
...
if ($Aplic->profissional){	
	...
	elseif ($objetivo_id) $sql->adOnde('modelo_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('modelo_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('modelo_gestao_mswot='.(int)$mswot_id);
		  ...
		  foreach((array)$sql_resultadosc as $rs_anexo){
			...
			if ($Aplic->profissional){
				...
				foreach($lista as $linha) {	
					...
					elseif ($linha['modelo_gestao_fator']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=praticas&a=fator_ver&fator_id='.$linha['modelo_gestao_fator'].'\');">'.imagem('icones/fator_p.gif',ucfirst($config['fator']),'Clique neste ícone '.imagem('icones/fator_p.gif').' para ver '.$config['genero_fator'].' '.$config['fator'].' que este documento está vinculado.').'</a>';
					...
					elseif ($linha['modelo_gestao_mswot']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=swot&a=mswot_ver&mswot_id='.$linha['modelo_gestao_mswot'].'\');">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para ver a matriz SWOT que este documento está vinculado.').'</a>';
					elseif ($linha['modelo_gestao_swot']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=swot&a=swot_ver&swot_id='.$linha['modelo_gestao_swot'].'\');">'.imagem('../../../modulos/swot/imagens/swot_p.png','Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para ver o campo de matriz SWOT que este documento está vinculado.').'</a>';
