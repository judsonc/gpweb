...
echo '<tr><td align="right" width="150">'.dica('Nome do Compromisso', 'Qual o nome do compromisso.Cada compromisso deve ter um nome que facilite a compreensão do mesmo').'Nome do Compromisso:'.dicaF().'</td><td><input type="text" class="texto" size="25" name="agenda_titulo" value="'.$obj->agenda_titulo.'" maxlength="255" /></td></tr>';
echo '<tr><td align="right">'.dica('Data de Início', 'Digite ou escolha no calendário a data de início do agenda.').'Data de início:'.dicaF().'</td><td><input type="hidden" name="agenda_inicio" id="agenda_inicio" value="'.($data_inicio ? $data_inicio->format(FMT_TIMESTAMP_DATA) : '').'" /><input type="text" name="data_inicio" style="width:70px;" id="data_inicio" onchange="setData(\'env\', \'inicio\');" value="'.($data_inicio ? $data_inicio->format($df) : '').'" class="texto" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data de início deste agenda.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora de Início', 'Digite a hora de início do agenda.').'Hora:'.dicaF().selecionaVetor($horas, 'inicio_hora', 'size="1" class="texto" onchange="CompararHoras();"', $data_inicio->format('%H%M%S')).'</td></tr>';
echo '<tr><td align="right">'.dica('Data de Término', 'Digite ou escolha no calendário a data de término do agenda.').'Data de Término:'.dicaF().'</td><td><input type="hidden" name="agenda_fim" id="agenda_fim" value="'.($data_fim ? $data_fim->format(FMT_TIMESTAMP_DATA) : '').'" /><input type="text" name="data_fim" id="data_fim" style="width:70px;" onchange="setData(\'env\', \'fim\');" value="'.($data_fim ? $data_fim->format($df) : '').'" class="texto" />'.dica('Data de Término', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data de término deste agenda.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora de Término', 'Digite a hora de término do agenda.').'Hora:'.dicaF().selecionaVetor($horas, 'fim_hora', 'size="1" class="texto" onchange="CompararHoras();"', $data_fim->format('%H%M%S')).'</td></tr>';
...
echo '<tr><td align="right">'.dica('Lembrar', 'Envio de E-mail para lembrar do compromisso.').'Lembrar:'.dicaF().'</td><td>'.selecionaVetor($lembrar, 'agenda_lembrar', 'size="1" class="texto"', $obj->agenda_lembrar).' antes</td></tr>';
echo '<tr><td align="right">'.dica('Cor', 'Para facilitar a visualização pode-se escolher uma das 216 cores pré-definidas, bastando clicar no retângulo colorido na ponta direita. Caso deseje uma cor inexistente na paleta de cores deste programa insira o valor Hexadecimal da mesma, na caixa de texto logo à direita.').'Cor:'.dicaF().'</td><td align="left"><input type="text" name="agenda_cor" value="'.($obj->agenda_cor ? $obj->agenda_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="10" maxlength="6" onblur="setCor();" class="texto" />&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript: void(0);" onclick="if (window.parent.gpwebApp) parent.gpwebApp.popUp(\'Cor\', 300, 290, \'m=publico&a=selecao_cor&dialogo=1&chamar_volta=setCor\', window.setCor, window); else newwin=window.open(\'./index.php?m=publico&a=selecao_cor&dialogo=1&chamar_volta=setCor\', \'calwin\', \'width=310, height=300, scrollbars=no\');">'.dica('Mudar Cor', 'Para facilitar a visualização dos eventos pode-se escolher uma das 216 cores pré-definidas, bastando clicar no retângulo colorido. Caso deseje uma cor inexistente na paleta de cores deste programa insira o valor Hexadecimal da mesma, na caixa de texto à esquerda.').'Mudar cor&nbsp;&nbsp;<span id="teste" style="border:solid;border-width:1;background:#'.($obj->agenda_cor ? $obj->agenda_cor : 'FFFFFF').';">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></a>'.dicaF().'</td></tr>';
echo '<tr><td align="right">'.dica('Agenda', 'Escolha em qual agenda deseja que este compromisso seja armazenado.').'Agenda:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'agenda_tipo', 'class="texto"', ($obj->agenda_tipo ? $obj->agenda_tipo : $agenda_tipo_id) ).'</td></tr>';
echo '<tr><td align="right">'.dica('Mostrar Somente em Dias Úteis', 'Marque esta caixa para que a faixa de tempo do compromisso não inclua os fim-de-semana.').'<label for="agenda_diautil">Somente dias úteis:</label>'.dicaF().'</td><td><input type="checkbox" value="1" name="agenda_diautil" id="agenda_diautil" '.($obj->agenda_diautil ? 'checked="checked"' : '').' /></td></tr>';
...
echo '<tr><td align="right">'.dica('Descrição', 'Um resumo sobre o compromisso.').'Descrição:'.dicaF().'</td><td><textarea name="agenda_descricao" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->agenda_descricao.'</textarea></td></tr>';
...
if (!$tem_protegido || $Aplic->usuario_super_admin || $Aplic->usuario_admin) echo '<tr><td align=right>'.dica(ucfirst($config['organizacao']), 'Filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' selecionad'.$config['genero_organizacao'].'.').ucfirst($config['organizacao']).':'.dicaF().'</td><td><table cellpadding=0 cellspacing=0><tr><td><div id="combo_cia_designados">'.selecionar_om($Aplic->usuario_cia, 'cia_designados', 'class=texto size=1 style="width:250px;" onchange="javascript:mudar_om_designados();"','',1).'</div></td><td><a href="javascript:void(0);" onclick="mudar_usuarios_designados()">'.imagem('icones/atualizar.png','Atualizar os '.ucfirst($config['usuarios']),'Clique neste ícone '.imagem('icones/atualizar.png').' para atualizar a lista de '.$config['usuarios']).'</a></td></tr></table></td></tr>';
