...
foreach($vetor_msg_usuario as $chave => $valor){ 
	...
	if ($podeler_nota=='criador'){
		...
		$sql->adInserir('anotacao_usuario_anotacao', $anotacao_id);
		$sql->adInserir('anotacao_usuario_usuario', $de_id);
		if (!$sql->exec()) die('Não foi possível inserir os dados na tabela de anotacao_usuario!'.$bd->stderr(true));
		$sql->limpar();			
		}
	if ($podeler_nota=='remetentes'){
		...
		foreach($remetentes as $remetente){
			$sql->adTabela('anotacao_usuario');
			$sql->adInserir('anotacao_usuario_anotacao', $anotacao_id);
			$sql->adInserir('anotacao_usuario_usuario', $remetente['de_id']);
			if (!$sql->exec()) die('Não foi possível inserir os dados na tabela de anotacao_usuario!'.$bd->stderr(true));
			$sql->limpar();			
			}
		}
