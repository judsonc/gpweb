...
if (function_exists('openssl_sign') && $rs['assinatura']){	
	...
	if (!$ok) $assinado='&nbsp;'.dica(nome_funcao('','','','',$original['de_id']),'A assinatura digital d'.$config['genero_mensagem'].' '.$config['mensagem'].' não confere! '.ucfirst($config['mensagem']).' possívelmente adulterad'.$config['genero_mensagem'].'.').'<img src="'.acharImagem('icones/assinatura_erro.gif').'" style="vertical-align:top" width="15" height="13" />'.dicaF();
...
$km->Add("root","modelo_exibicao",dica('Modo de Exibição','Selecione outro modo de exibir '.($config['genero_mensagem']=='a' ? 'esta' : 'este').' '.$config['mensagem'].'.').'Modo de Exibição');	
...
$km->Add("modelo_exibicao","mod_padrao",dica('Padrão','Clique nesta opção para visualizar '.($config['genero_mensagem']=='a' ? 'esta' : 'este').' '.$config['mensagem'].' no formato padrão.').'Padrão'.dicaF(), "javascript: void(0);' onclick='env.a.value=\"exibe_msg\"; env.submit();");
echo $km->Render();
echo '</td></tr>';
echo '</table>';
...
echo '<tr width="100%"><td align="right" style="font-size:10pt;  padding-left: 5px; padding-right: 5px; background-color: #'.$cor['cor_msg'].'" width=110>'.$msg_id.($config['genero_mensagem']=='a' ? 'ª': 'º').' '.$config['msg'].':</td><td style="font-size:10pt; padding-left: 5px; padding-right: 5px; background-color: #'.$cor['cor_fundo'].'">'.$assinado.$rs['referencia'].'</td></tr>';
...
if ($Aplic->profissional){
  ...
  if (count($lista)) {
	  if ($Aplic->profissional) require_once BASE_DIR.'/modulos/projetos/template_pro.class.php';
		...
		echo '<table rules="ALL" border="1" cellspacing=0 cellpadding=0 align="center" width="770"><tr><td width=110 style="font-size:10pt; padding-left: 5px; padding-right: 5px ;background-color: #'.$cor['cor_msg'].';">Relacionamento:</td><td>';
		$qnt=0;
