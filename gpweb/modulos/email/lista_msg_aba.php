...
if ($Aplic->profissional){	
	...
	elseif ($objetivo_id) $sql->adOnde('msg_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('msg_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('msg_gestao_mswot='.(int)$mswot_id);
  ...
  foreach ($sql_resultadosc as $rs_anexo){
	...
	if ($Aplic->profissional){
		...
		foreach($lista as $linha) {	
			...
			elseif ($linha['msg_gestao_mswot']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=swot&a=mswot_ver&mswot_id='.$linha['msg_gestao_mswot'].'\');">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para ver a matriz SWOT que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
			elseif ($linha['msg_gestao_swot']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=swot&a=swot_ver&swot_id='.$linha['msg_gestao_swot'].'\');">'.imagem('../../../modulos/swot/imagens/swot_p.png','Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para ver o campo de matriz SWOT que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
