...
if ($Aplic->modulo_ativo('tr')) {
	require_once BASE_DIR.'/modulos/tr/tr.class.php';
	$tr_ativo=true;
	}
...
class pesquisa {
	...
	function pegarResultados(&$registro_contagem) {
		global $Aplic, $config, $atas_ativo,$swot_ativo,$operativo_ativo,$problema_ativo,$agrupamento_ativo,$patrocinadores_ativo, $tr_ativo;
		...
		if ($resultados) {
			...
			foreach ($resultados as $registros) {
				if ($Aplic->checarModulo($this->tabela_modulo, 'acesso')) {
					...
					elseif ($Aplic->profissional && $tr_ativo && ($this->funcao=='tr') && isset($this->tabela_chave)) $textoSaida .= '<tr><td>'.link_tr($registros[preg_replace('/^.*\.([^\.]+)$/', '$1', $this->tabela_chave)]).'<br>'.iluminar($mostrar_valor, $this->palavrasChave).'</a></span></td></tr>';
