...
global $estilo_interface, $sql, $perms, $Aplic, $cia_id, $dept_id, $lista_depts, $tab, $lista_cias, $usuario_id, $pesquisar_texto, $dialogo, $podeEditar, $podeExcluir, $instrumento_ano,  $instrumento_tipo, $instrumento_situacao, $podeEditar, $favorito_id, $filtro_prioridade_instrumento,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('instrumento_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('instrumento_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('instrumento_gestao_mswot='.(int)$mswot_id);
...
$sql->adTabela('instrumento');
$sql->adCampo('instrumento_id, instrumento_nome, instrumento_tipo, instrumento_objeto, instrumento_licitacao, instrumento_situacao, instrumento_valor, instrumento_data_inicio, instrumento_acesso, instrumento_cor, instrumento_responsavel, instrumento_aprovado');
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=instrumento_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='instrumento_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_instrumento) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($a ? '&a='.$a : '').'&ordenar=instrumento_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='instrumento_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'Neste campo fica o responsável pelo instrumento.').'Responsável'.dicaF().'</a></th>';
echo '</tr>';
...
foreach ($instrumentos as $linha) {
	if (permiteAcessarInstrumento($linha['instrumento_acesso'])){	
		$qnt++;
		$editar=permiteEditarInstrumento($linha['instrumento_acesso']);
		...
		echo '<tr>';
		echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar Instrumento', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=instrumento&a=instrumento_editar&instrumento_id='.$linha['instrumento_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		echo '<td id="ignore_td_" width="15" align="right" style="background-color:#'.$linha['instrumento_cor'].'">&nbsp;&nbsp;</td>';
		echo '<td>'.dica($linha['instrumento_nome'], 'Clique para visualizar os detalhes deste instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=instrumento&a=instrumento_ver&instrumento_id='.$linha['instrumento_id'].'\');">'.$linha['instrumento_nome'].'</a>'.dicaF().'</td>';
		
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['instrumento_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_instrumento) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
		...
		if ($Aplic->profissional){
		  ...
		  if (count($lista)) {
				...
				foreach($lista as $gestao_data){
					...
					elseif (isset($gestao_data['instrumento_gestao_mswot']) && $gestao_data['instrumento_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['instrumento_gestao_mswot']);
