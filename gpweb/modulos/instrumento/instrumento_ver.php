...
$sql->adOnde('campo_formulario_tipo = \'instrumento\'');
...
if (!$dialogo && !$Aplic->profissional){	
	...
	$botoesTitulo->adicionaBotao('m=instrumento&a=instrumento_lista', 'lista','','Lista de '.ucfirst($config['instrumentos']),'Visualizar a lista de instrumentos cadastrados.');
	if ($editar) $botoesTitulo->adicionaBotao('m=instrumento&a=instrumento_editar&instrumento_id='.$instrumento_id, 'editar','','Editar '.ucfirst($config['instrumento']),'Editar '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.');
	
	if ($Aplic->checarModulo('arquivos', 'adicionar')) $botoesTitulo->adicionaCelula('<table cellpadding=1 cellspacing=0><tr><td nowrap="nowrap">'.dica('Novo Arquivo', 'Inserir um novo arquivo relacionado com '.$config['genero_instrumento'].' '.$config['instrumento'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=arquivos&a=editar&arquivo_instrumento='.$instrumento_id.'\');" ><span>arquivo</span></a>'.dicaF().'</td></tr></table>');
	
	
	if ($podeExcluir && $instrumento_id > 0 && $editar) $botoesTitulo->adicionaBotaoExcluir('excluir', $podeExcluir, $msg, 'Excluir '.ucfirst($config['instrumento']), 'Excluir '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.' );
...
//Precisa das aprovações para Analisar o instrumento
	if ($Aplic->profissional){
...
if (!$dialogo && $Aplic->profissional){	
	...
	$km->Add("ver","ver_lista",dica('Lista de '.ucfirst($config['instrumentos']),'Clique neste botão para visualizar a lista de '.$config['instrumentos'].'.').'Lista de '.ucfirst($config['instrumentos']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=instrumento&a=instrumento_lista&instrumento_id=".$instrumento_id."\");");
	...
	if ($editar){
		...
		$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Criar um nov'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=instrumento&a=instrumento_editar\");");
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_instrumento=".$instrumento_id."\");");
	...
	$bloquear=($obj->instrumento_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
	...
	if ($podeEditar && $editar && !$bloquear) $km->Add("acao","acao_editar",dica('Editar '.ucfirst($config['instrumento']),'Editar os detalhes d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Editar '.ucfirst($config['instrumento']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=instrumento&a=instrumento_editar&instrumento_id=".$instrumento_id."\");");	
	if ($podeExcluir && $editar && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].' do sistema.').'Excluir '.ucfirst($config['instrumento']).dicaF(), "javascript: void(0);' onclick='excluir()");
...
echo '<tr><td align="right" nowrap="nowrap" style="width:140px;">'.dica('Nome', 'O nome para identificação d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Nome:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_nome.'</td></tr>';

if ($obj->instrumento_cia) echo '<tr><td align=right nowrap="nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_cia($obj->instrumento_cia).'</td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Número', 'O número de identificação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_numero.'</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica('Ano', 'O ano d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Ano:'.dicaF().'</td><td align="left" class="realce">',$obj->instrumento_ano.'</td></tr>';
...
if ($obj->instrumento_situacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Situação', 'A situação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Situação:'.dicaF().'</td><td align="left" class="realce">'.getSisValorCampo('SituacaoInstrumento', $obj->instrumento_situacao).'</td></tr>';
if ($obj->instrumento_licitacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Licitação', 'O tipo de licitação para '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Licitação:'.dicaF().'</td><td align="left" class="realce">'.getSisValorCampo('ModalidadeLicitacao',$obj->instrumento_licitacao).'</td></tr>';
if ($obj->instrumento_edital_nr) echo '<tr><td align="right" nowrap="nowrap">'.dica('Número do Edital', 'O número do edital da licitação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número do Edital:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_edital_nr.'</td></tr>';
if ($obj->instrumento_processo) echo '<tr><td align="right" nowrap="nowrap">'.dica('Número do Processo', 'O número do processo d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Número do Processo:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_processo.'</td></tr>';
if ($obj->instrumento_entidade) echo '<tr><td align="right" nowrap="nowrap">'.dica('Entidade', 'A entidade com a qual foi celebrado '.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Entidade:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_entidade.'</td></tr>';
if ($obj->instrumento_entidade_cnpj) echo '<tr><td align="right" nowrap="nowrap">'.dica('CNPJ da Entidade', 'O número do CNPJ da entidade com a qual foi celebrado '.$config['genero_instrumento'].' '.$config['instrumento'].'.').'CNPJ da entidade:'.dicaF().'</td><td align="left" class="realce">'.$obj->instrumento_entidade_cnpj.'</td></tr>';



if ($obj->instrumento_data_celebracao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Data de Celebração', 'Data em que '.$config['genero_instrumento'].' '.$config['instrumento'].' foi celebrad'.$config['genero_instrumento'].'.').'Data de celebração:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_celebracao, false).'</td></tr>';
if ($obj->instrumento_data_publicacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Data de Publicação', 'Data em que '.$config['genero_instrumento'].' '.$config['instrumento'].' foi publicad'.$config['genero_instrumento'].'.').'Data de publicação:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_publicacao, false).'</td></tr>';
if ($obj->instrumento_data_inicio) echo '<tr><td align="right" nowrap="nowrap">'.dica('Data de Início', 'Data de início d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Data de início:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_inicio, false).'</td></tr>';
if ($obj->instrumento_data_termino) echo '<tr><td align="right" nowrap="nowrap">'.dica('Data de Término', 'Data de término d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Data de término:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.retorna_data($obj->instrumento_data_termino, false).'</td></tr>';
if ($obj->instrumento_valor) echo '<tr><td align="right" nowrap="nowrap">'.dica('Valor', 'Insira o valor d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Valor:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$config['simbolo_moeda'].'&nbsp;'.number_format($obj->instrumento_valor, 2, ',', '.').'</td></tr>';
if ($obj->instrumento_valor_contrapartida) echo '<tr><td align="right" nowrap="nowrap">'.dica('Contrapartida', 'Insira o valor da contrapartida d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].', se for o caso.').'Contrapartida:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$config['simbolo_moeda'].'&nbsp;'.number_format($obj->instrumento_valor_contrapartida, 2, ',', '.').'</td></tr>';
if ($obj->instrumento_porcentagem) echo '<tr><td align="right" nowrap="nowrap">'.dica('Porcentusl realizado', 'Indique o porcentual d'.$config['genero_instrumento'].' '.$config['instrumento'].' já completad'.$config['genero_instrumento'].'.').'Realizad'.$config['genero_instrumento'].': '.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_porcentagem.'%</td></tr>';
...
echo '<tr><td align="right" nowrap="nowrap">'.dica('Responsável pel'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Tod'.$config['genero_instrumento'].' '.$config['instrumento'].' deve ter um responsável.').'Responsável:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_responsavel, '','','esquerda').'</td></tr>';

if ($obj->instrumento_supervisor) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['supervisor']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_supervisor, '','','esquerda').'</td></tr>';

if ($obj->instrumento_supervisor_data) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado pel'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_supervisor_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_supervisor_data).'</td></tr>';
if ($obj->instrumento_supervisor_obs) echo '<tr><td align="right" nowrap="nowrap">'.dica('Observação d'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), 'A observação redigida pel'.$config['genero_supervisor'].' '.$config['supervisor'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_supervisor_obs.'</td></tr>';

if ($obj->instrumento_autoridade) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['autoridade']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_autoridade, '','','esquerda').'</td></tr>';
if ($obj->instrumento_autoridade_data) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado pel'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_autoridade_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_autoridade_data).'</td></tr>';
if ($obj->instrumento_autoridade_obs) echo '<tr><td align="right" nowrap="nowrap">'.dica('Observação d'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), 'A observação redigida pel'.$config['genero_autoridade'].' '.$config['autoridade'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_autoridade_obs.'</td></tr>';

if ($obj->instrumento_cliente) echo '<tr><td align="right" nowrap="nowrap">'.dica(ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').ucfirst($config['cliente']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->instrumento_cliente, '','','esquerda').'</td></tr>';
if ($obj->instrumento_cliente_data) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado pel'.$config['genero_cliente'].' '.ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].' aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->instrumento_cliente_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->instrumento_cliente_data).'</td></tr>';
if ($obj->instrumento_cliente_obs) echo '<tr><td align="right" nowrap="nowrap">'.dica('Observação d'.$config['genero_cliente'].' '.ucfirst($config['cliente']), 'A observação redigida pel'.$config['genero_cliente'].' '.$config['cliente'].' d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Observação d'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->instrumento_cliente_obs.'</td></tr>';
...
if($saida_quem)echo '<tr><td align="right" nowrap="nowrap">'.dica('Designado', 'Quais '.$config['usuarios'].' estão designados para '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Designado:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_quem.'</td></tr>';
...
if($saida_quem)echo '<tr><td align="right" nowrap="nowrap">'.dica('Contatos', 'Quais são os contatos d'.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Contato:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_quem.'</td></tr>';
...
if ($saida_recurso) echo '<tr><td align="right" nowrap="nowrap">'.dica('Recurso', 'Qual recurso está relacionado à '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].'.').'Recurso:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$saida_recurso.'</td></tr>';
...
if ($Aplic->profissional){
  ...
  if (count($lista)) {
	  if ($Aplic->profissional) require_once BASE_DIR.'/modulos/projetos/template_pro.class.php';
		...
		echo '<tr><td align="right" nowrap="nowrap" valign="right">'.dica('Relacionado', 'A que área '.($config['genero_instrumento']=='a' ? 'esta' : 'este').' '.$config['instrumento'].' está relacionado.').'Relacionado:'.dicaF().'</td></td><td class="realce">';
		$qnt=0;
		foreach($lista as $gestao_data){
			...
			elseif ($gestao_data['instrumento_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['instrumento_gestao_mswot']);
			...
			elseif ($gestao_data['instrumento_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['instrumento_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada na meta.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->instrumento_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se '.$config['genero_instrumento'].' '.$config['instrumento'].' se encontra aprovad'.$config['genero_instrumento'].'.').'Aprovad'.$config['genero_instrumento'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->instrumento_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


echo '<tr><td align="right" nowrap="nowrap">'.dica('Ativo', 'Se '.$config['genero_instrumento'].' '.$config['instrumento'].' se encontra ativo.').'Ativo:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->instrumento_ativo ? 'Sim' : 'Não').'</td></tr>';
	...
	if ($Aplic->profissional){
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(evento_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(arquivo_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(link_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(forum_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(pratica_indicador_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(plano_acao_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adCampo('count(msg_gestao_instrumento)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adCampo('count(modelo_gestao_instrumento)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adCampo('count(ata_gestao_instrumento)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adCampo('count(problema_gestao_instrumento)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adCampo('count(risco_gestao_instrumento)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_instrumento = '.(int)$instrumento_id.' OR risco_gestao_instrumento = '.(int)$instrumento_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adOnde('instrumento_gestao_instrumento = '.(int)$instrumento_id);
			$sql->adCampo('count(instrumento_gestao_instrumento)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
