...
$sql->adOnde('favorito_trava_instrumento=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('instrumento_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('instrumento_favorito') !== null ? $Aplic->getEstado('instrumento_favorito') : 0;

if (isset($_REQUEST['filtro_prioridade_instrumento']))	$Aplic->setEstado('filtro_prioridade_instrumento', getParam($_REQUEST, 'filtro_prioridade_instrumento', null));
$filtro_prioridade_instrumento = $Aplic->getEstado('filtro_prioridade_instrumento') !== null ? $Aplic->getEstado('filtro_prioridade_instrumento') : null;


$painel_filtro = $Aplic->getEstado('painel_filtro') !== null ? $Aplic->getEstado('painel_filtro') : 0;

if (isset($_REQUEST['instrumentostextobusca'])) $Aplic->setEstado('instrumentostextobusca', getParam($_REQUEST, 'instrumentostextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('instrumentostextobusca') ? $Aplic->getEstado('instrumentostextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('ListaInstrumentosTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('ListaInstrumentosTab') !== null ? $Aplic->getEstado('ListaInstrumentosTab') : 0);
...
if (isset($_REQUEST['instrumento_ano']) || $houve_filtro) $Aplic->setEstado('instrumento_ano', getParam($_REQUEST, 'instrumento_ano', null));
$instrumento_ano = ($Aplic->getEstado('instrumento_ano') ? $Aplic->getEstado('instrumento_ano') : ($houve_filtro ? null : -1));

if (isset($_REQUEST['instrumento_tipo']) || $houve_filtro) $Aplic->setEstado('instrumento_tipo', getParam($_REQUEST, 'instrumento_tipo', null));
$instrumento_tipo = ($Aplic->getEstado('instrumento_tipo') ? $Aplic->getEstado('instrumento_tipo') : ($houve_filtro ? null : -1));

if (isset($_REQUEST['instrumento_situacao']) || $houve_filtro) $Aplic->setEstado('instrumento_situacao', getParam($_REQUEST, 'instrumento_situacao', null));
$instrumento_situacao = ($Aplic->getEstado('instrumento_situacao') ? $Aplic->getEstado('instrumento_situacao') : ($houve_filtro ? null : -1));
...
$sql->adOnde('favorito_instrumento=1');
...
echo '<input type="hidden" name="filtro_prioridade_instrumento" id="filtro_prioridade_instrumento" value="'.$filtro_prioridade_instrumento.'" />';
...
if (!$dialogo){
	...
	if($plano_acao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_acao'].' '.$config['acao'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['acao']).':'.dicaF();
		$nome=nome_acao($plano_acao_id);
		}
	elseif($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela agenda que estão relacionad'.$config['genero_instrumento'].'s.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo indicador que estão relacionad'.$config['genero_instrumento'].'s.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionad'.$config['genero_instrumento'].'s.').''.ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($pg_meta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_meta'].' '.$config['meta'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['meta']).':'.dicaF();
		$nome=nome_meta($pg_meta_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo monitoramento que estão relacionad'.$config['genero_instrumento'].'s.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela ata de reunião que estão relacionad'.$config['genero_instrumento'].'s.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}		
	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela matriz SWOT que estão relacionad'.$config['genero_instrumento'].'s.').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($mswot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo camo de matriz SWOT que estão relacionad'.$config['genero_instrumento'].'s.').'Campo SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo plano operativo que estão relacionad'.$config['genero_instrumento'].'s.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo recurso que estão relacionad'.$config['genero_instrumento'].'s.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela demanda que estão relacionad'.$config['genero_instrumento'].'s.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo evento que estão relacionad'.$config['genero_instrumento'].'s.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($link_id){
		$legenda_filtro=dica('Filtrar pelo Link', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo link que estão relacionad'.$config['genero_instrumento'].'s.').'Link:'.dicaF();
		$nome=nome_link($link_id);
		}
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela avaliação que estão relacionad'.$config['genero_instrumento'].'s.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionad'.$config['genero_instrumento'].'s.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo brainstorm que estão relacionad'.$config['genero_instrumento'].'s.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pela matriz GUT que estão relacionad'.$config['genero_instrumento'].'s.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo diagrama de causa-efeito que estão relacionad'.$config['genero_instrumento'].'s.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($arquivo_id){
		$legenda_filtro=dica('Filtrar pelo Arquivo', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo arquivo que estão relacionad'.$config['genero_instrumento'].'s.').'Arquivo:'.dicaF();
		$nome=nome_arquivo($arquivo_id);
		}	
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo fórum que estão relacionad'.$config['genero_instrumento'].'s.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($checklist_id){
		$legenda_filtro=dica('Filtrar pelo Checklist', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo checklist que estão relacionad'.$config['genero_instrumento'].'s.').'Checklist:'.dicaF();
		$nome=nome_checklist($checklist_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo compromisso que estão relacionad'.$config['genero_instrumento'].'s.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo agrupamento que estão relacionad'.$config['genero_instrumento'].'s.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo patrocinador que estão relacionad'.$config['genero_instrumento'].'s.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' pelo modelo que estão relacionad'.$config['genero_instrumento'].'s.').'Modelo:'.dicaF();
		$nome=nome_template($template_id);
		}		
	else{
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar '.$config['genero_instrumento'].'s '.$config['instrumentos'].'.').'Filtro:'.dicaF();
		}
	
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionad'.$config['genero_instrumento'],'A qual parte do sistema '.$config['genero_instrumento'].'s '.$config['instrumentos'].' estão relacionad'.$config['genero_instrumento'].'s.').'Relacionad'.$config['genero_instrumento'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional){	
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['instrumentos']), 'instrumento.png', $m, "$m.$a");
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$imprimir='<tr><td nowrap="nowrap" align="right">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['instrumentos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=instrumento&a=instrumento_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';


	$novo=($podeEditar ? '<tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Criar '.($config['genero_instrumento']=='o' ? 'um' : 'uma').' nov'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=instrumento&a=instrumento_editar\');" ><img src="'.acharImagem('instrumento_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' :'');

	$botao_excel='<tr><td align=right><a href="javascript: void(0)" onclick="exportar_excel();">'.imagem('icones/excel_p.gif', 'Exportar para Excel' , 'Clique neste ícone '.imagem('icones/excel_p.gif').' para exportar a lista de '.$config['instrumentos'].' para o formato excel.').'</a>'.dicaF().'</td></tr>';
	
	$sql = new BDConsulta();
	$sql->adTabela('instrumento');
	$sql->adCampo('DISTINCT instrumento_ano');
	...
	$anos = array(''=>'')+$sql->carregarColuna();
	$sql->limpar();


	$procurar_ano='<tr><td align="right" nowrap="nowrap">'.dica('Ano', 'Preencha neste campo o ano d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Ano:'.dicaF().'</td><td align="left">'.selecionaVetor($anos, 'instrumento_ano', 'class="texto" style="width:250px" multiple', $instrumento_ano).'</td></tr>';
	$procurar_tipo='<tr><td align="right" nowrap="nowrap">'.dica('Tipo', 'Selecione qual o tipo de '.$config['instrumento'].'.').'Tipo:'.dicaF().'</td><td align="left" >'.selecionaVetor(getSisValor('TipoInstrumento'), 'instrumento_tipo', 'class="texto" style="width:250px" multiple', $instrumento_tipo).'</td></tr>';
	$procurar_situacao='<tr><td align="right" nowrap="nowrap">'.dica('Situação', 'Selecione qual a situação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Situação:'.dicaF().'</td><td align="left" >'.selecionaVetor(getSisValor('SituacaoInstrumento'), 'instrumento_situacao', 'class="texto" style="width:250px" multiple', $instrumento_situacao).'</td></tr>';
	...
	$botao_favorito='<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&instrumento=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	if($filtro_prioridade_instrumento) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procuraBuffer.$procurar_usuario.$procurar_ano.$procurar_tipo.$procurar_situacao.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$botao_filtrar.$novo.$imprimir.$botao_prioridade.$botao_excel.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['instrumentos']), 'instrumento.png', $m, "$m.$a");
	$botoesTitulo->adicionaCelula('<table cellspacing=0 cellpadding=0>'.$procurar_om.'</tr></table>');
	if ($podeEditar)$botoesTitulo->adicionaCelula('<table width=75><tr><td nowrap="nowrap">'.dica('Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Criar '.($config['genero_instrumento']=='o' ? 'um' : 'uma').' nov'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=instrumento&a=instrumento_editar\');" ><span>nov'.$config['genero_instrumento'].'</span></a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->mostrar();
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de '.ucfirst($config['instrumentos']).'</center></font>';
	}	
...
if($Aplic->profissional){
	...
	if (is_array($instrumento_ano)) $instrumento_ano=implode(',', $instrumento_ano);
	if (is_array($instrumento_tipo)) $instrumento_tipo=implode(',', $instrumento_tipo);
	if (is_array($instrumento_situacao)) $instrumento_situacao=implode(',', $instrumento_situacao);
  ...
  echo '$jq(function(){';
	echo '  $jq("#instrumento_ano").multiSelect();';
	echo '  $jq("#instrumento_tipo").multiSelect();';
  echo '  $jq("#instrumento_situacao").multiSelect();';
  echo '});';
	echo '</script>';  
	}
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Ativ'.$config['genero_instrumento'].'s',null,null,'Ativ'.$config['genero_instrumento'].'s','Visualizar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' ativ'.$config['genero_instrumento'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Inativ'.$config['genero_instrumento'].'s',null,null,'Inativ'.$config['genero_instrumento'].'s','Visualizar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' inativ'.$config['genero_instrumento'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Tod'.$config['genero_instrumento'].'s',null,null,'Tod'.$config['genero_instrumento'].'s','Visualizar todos '.$config['genero_instrumento'].'s '.$config['instrumentos'].'.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx.php');
...
function exportar_excel(){
  url_passar(1, 'm=instrumento&a=instrumentos_exportar_excel_pro&sem_cabecalho=1');
	}
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&acao=1&filtro_prioridade='+env.filtro_prioridade_instrumento.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_instrumento){
	env.filtro_prioridade_instrumento.value=filtro_prioridade_instrumento;
	env.submit();
	}	
