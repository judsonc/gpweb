...
if (!$dialogo){
	...
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar os arquivos pela ata de reunião a qual estão relacionados.').'Ata:'.dicaF();
	...
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar os arquivos pelo campo SWOT relacionado.').'Campo SWOT:'.dicaF();
	...
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar os arquivos pelo instrumento jurídico relacionado.').'Instrumento Jurídico:'.dicaF();
	...
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar os arquivos pela avaliação relacionado.').'Avaliação:'.dicaF();
	...
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar os arquivos pelo fórum relacionado.').'Fórum:'.dicaF();
	...
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionado','A qual parte do sistema os arquivos estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito='<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&arquivo=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procura_categoria.$procurar_dono.$procuraBuffer.$icone_pasta.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_arquivo.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
