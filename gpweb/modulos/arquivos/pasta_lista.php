...
if (isset($_REQUEST['arquivo_pastatextobusca'])) $Aplic->setEstado('arquivo_pastatextobusca', getParam($_REQUEST, 'arquivo_pastatextobusca', true));
$pesquisar_texto = ($Aplic->getEstado('arquivo_pastatextobusca') ? $Aplic->getEstado('arquivo_pastatextobusca') : '');

if (isset($_REQUEST['arquivo_pasta_usuario'])) $Aplic->setEstado('arquivo_pasta_usuario', getParam($_REQUEST, 'arquivo_pasta_usuario', null));
$arquivo_pasta_usuario = $Aplic->getEstado('arquivo_pasta_usuario', null);
...
if (!$dialogo){
	...
	if($plano_acao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Filtrar as pastas pel'.$config['genero_acao'].' '.$config['acao'].' que estão relacionadas.').ucfirst($config['acao']).':'.dicaF();
		$nome=nome_acao($plano_acao_id);
		}
	elseif($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar as pastas pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionadas.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar as pastas pela agenda que estão relacionadas.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar as pastas pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionadas.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar as pastas pelo indicador que estão relacionadas.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar as pastas pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionadas.').''.ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar as pastas pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionadas.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar as pastas pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionadas.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar as pastas pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionadas.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar as pastas pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionadas.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar as pastas pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionadas.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($pg_meta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar as pastas pel'.$config['genero_meta'].' '.$config['meta'].' que estão relacionadas.').ucfirst($config['meta']).':'.dicaF();
		$nome=nome_meta($pg_meta_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar as pastas pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionadas.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar as pastas pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionadas.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar as pastas pelo monitoramento que estão relacionadas.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar as pastas pela ata de reunião a qual estão relacionadas.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}	
	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar as pastas pela matriz SWOT a qual estão relacionadas..').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($mswot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo Campo de Matriz SWOT', 'Filtrar as pastas pelo campo de matriz SWOT a qual estão relacionadas.').'Matriz SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar as pastas pelo plano operativo que estão relacionadas.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar as pastas pelo instrumento jurídico que estão relacionadas.').'Instrumento Jurídico:'.dicaF();
		$nome=nome_instrumento($instrumento_id);
		}	
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar as pastas pelo recurso que estão relacionadas.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar as pastas pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionadas.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar as pastas pela demanda que estão relacionadas.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar as pastas pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionadas.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar as pastas pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionadas.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar as pastas pelo evento que estão relacionadas.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($link_id){
		$legenda_filtro=dica('Filtrar pelo Link', 'Filtrar as pastas pelo link que estão relacionadas.').'Link:'.dicaF();
		$nome=nome_link($link_id);
		}
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar as pastas pela avaliação que estão relacionadas.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar as pastas pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionadas.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar as pastas pelo brainstorm que estão relacionadas.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar as pastas pela matriz GUT que estão relacionadas.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar as pastas pelo diagrama de causa-efeito que estão relacionadas.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar as pastas pelo fórum que estão relacionadas.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($checklist_id){
		$legenda_filtro=dica('Filtrar pelo Checklist', 'Filtrar as pastas pelo checklist que estão relacionadas.').'Checklist:'.dicaF();
		$nome=nome_checklist($checklist_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar as pastas pelo compromisso que estão relacionadas.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar as pastas pelo agrupamento que estão relacionadas.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar as pastas pelo patrocinador que estão relacionadas.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar as pastas pelo modelo que estão relacionadas.').'Modelo:'.dicaF();
	...
	elseif($arquivo_pasta_usuario){
		$legenda_filtro=dica('Filtrar pelos Particulares', 'Filtrar pelos seus arquivo_pastas particulares.').'Arquivos:'.dicaF();
		$nome='Particulares';
		}		
	else{
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar as pastas.').'Filtro:'.dicaF();
		}
	
	$procuraBuffer = '<tr><td align=right nowrap="nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="arquivo_pastatextobusca" onChange="document.frm_filtro.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&tab='.$tab.'&arquivo_pastatextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionado','A qual parte do sistema as pastas estão relacionadas.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	$icone_limpar='<td><a href="javascript:void(0);" onclick="limpar_tudo(); frm_filtro.submit();">'.imagem('icones/limpar_p.gif','Cancelar Filtro', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para cancelar o filtro aplicado.').'</a></td>';
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Pastas', 'pasta_grande.png', $m, "$m.$a");
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$podeAdicionar = $Aplic->checarModulo('arquivo_pastas', 'adicionar');
	$novo_arquivo_pasta=($podeEditar && $podeAdicionar ? '<tr><td><a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=arquivos&a=editar_pasta\');">'.imagem('icones/pasta_novo.png', 'Nova Pasta', 'Clique neste ícone '.imagem('icones/pasta_novo.png').' para inserir uma nova pasta.').'</a>'.dicaF().'</td></tr>' : '');

	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_dono.$procuraBuffer.$popFiltro.$filtros.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo_arquivo_pasta.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo&& !$Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Pastas', 'pasta_grande.png', $m, "$m.$a");
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0>'.$procurar_om.$procurar_dono.$procuraBuffer.$popFiltro.$filtros.'</table>');
	$podeAdicionar = $Aplic->checarModulo('arquivo_pastas', 'adicionar');
	if ($podeEditar && $podeAdicionar) $botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0 width=90><tr><td>'.dica('Nova Pasta', 'Inserir uma nova pasta onde será possível armazenar arquivo_pastas de forma organizada.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=arquivos&a=editar_pasta'.($projeto_id ? '&arquivo_pasta_projeto='.$projeto_id  : '').($pratica_id ? '&arquivo_pasta_pratica='.$pratica_id  : '').($plano_acao_id ? '&arquivo_pasta_acao='.$plano_acao_id  : '').($pratica_indicador_id ? '&arquivo_pasta_indicador='.$pratica_indicador_id  : '').($tema_id ? '&arquivo_pasta_tema='.$tema_id  : '').($objetivo_id ? '&arquivo_pasta_objetivo='.$objetivo_id  : '').($fator_id ? '&arquivo_pasta_fator='.$fator_id  : '').($pg_estrategia_id ? '&arquivo_pasta_estrategia='.$pg_estrategia_id  : '').($calendario_id ? '&arquivo_pasta_calendario='.$calendario_id  : '').($ata_id ? '&arquivo_pasta_ata='.$ata_id  : '').($arquivo_pasta_usuario ? '&arquivo_pasta_usuario='.$arquivo_pasta_usuario  : '').'\');"><span>nova&nbsp;pasta</span></a>'.dicaF().'</td></tr><tr><td colspan=20>&nbsp;</td></tr></table>');
	$botoesTitulo->mostrar();
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Pastas</center></font>';
	}	
...
$caixaTab = new CTabBox('m=arquivos&a=pasta_lista', '', $tab);
$caixaTab->adicionar(BASE_DIR.'/modulos/arquivos/pasta_tabela', 'Ativas',null,null,'Ativas','Visualizar as pastas ativas.');
$caixaTab->adicionar(BASE_DIR.'/modulos/arquivos/pasta_tabela', 'Inativas',null,null,'Inativas','Visualizar as pastas inativas.');
$caixaTab->adicionar(BASE_DIR.'/modulos/arquivos/pasta_tabela', 'Todas',null,null,'Todas','Visualizar as pastas, tanto ativas quanto inativas.');
...
function popUpload(campo) {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Responsável pelo upload', 500, 500, 'm=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setUpload&cia_id='+document.getElementById('cia_id').value+'&arquivo_pasta_upload='+document.getElementById('arquivo_pasta_upload').value, window.setUpload, window);
	else window.open('./index.php?m=publico&a=selecao_unico_usuario&dialogo=1&chamar_volta=setUpload&cia_id='+document.getElementById('cia_id').value+'&arquivo_pasta_upload='+document.getElementById('arquivo_pasta_upload').value, 'Responsável pelo upload','height=500,width=500,resizable,scrollbars=yes, left=0, top=0');
	}

function setUpload(arquivo_pasta_upload, posto, nome, funcao, campo, nome_cia){
	document.getElementById('arquivo_pasta_upload').value=(arquivo_pasta_upload ? arquivo_pasta_upload : 0);
...
function popRelacao(relacao){
	...
	frm_filtro.tipo_relacao.value='';
	}
	
function limpar_tudo(){
	document.frm_filtro.projeto_id .value = null;
	document.frm_filtro.pg_perspectiva_id .value = null;
	document.frm_filtro.tema_id .value = null;
	document.frm_filtro.objetivo_id .value = null;
	document.frm_filtro.fator_id .value = null;
	document.frm_filtro.pg_estrategia_id.value = null;
	document.frm_filtro.pg_meta_id .value = null;
	document.frm_filtro.pratica_id .value = null;
	document.frm_filtro.pratica_indicador_id .value = null;
	document.frm_filtro.plano_acao_id .value = null;
	document.frm_filtro.canvas_id .value = null;
	document.frm_filtro.risco_id.value = null;
	document.frm_filtro.risco_resposta_id.value = null;
	document.frm_filtro.calendario_id .value = null;
	document.frm_filtro.monitoramento_id .value = null;
	document.frm_filtro.instrumento_id.value = null;
	document.frm_filtro.recurso_id.value = null;
	document.frm_filtro.problema_id.value = null;
	document.frm_filtro.demanda_id.value = null;
	document.frm_filtro.programa_id.value = null;
	document.frm_filtro.licao_id.value = null;
	document.frm_filtro.evento_id.value = null;
	document.frm_filtro.link_id.value = null;
	document.frm_filtro.avaliacao_id.value = null;
	document.frm_filtro.tgn_id.value = null;
	document.frm_filtro.brainstorm_id.value = null;
	document.frm_filtro.gut_id.value = null;
	document.frm_filtro.causa_efeito_id.value = null;
	document.frm_filtro.arquivo_pasta_id.value = null;
	document.frm_filtro.forum_id.value = null;
	document.frm_filtro.checklist_id.value = null;
	document.frm_filtro.agenda_id.value = null;
	document.frm_filtro.template_id.value = null;
	document.frm_filtro.painel_id.value = null;
	document.frm_filtro.painel_odometro_id.value = null;
	document.frm_filtro.painel_composicao_id.value = null;
	<?php 
	if($swot_ativo) {
		echo 'document.frm_filtro.mswot_id.value = null;';
		echo 'document.frm_filtro.swot_id.value = null;';
		}
	if($ata_ativo) echo 'document.frm_filtro.ata_id.value = null;';
	if($operativo_ativo) echo 'document.frm_filtro.operativo_id.value = null;';
	if($agrupamento_ativo) echo 'document.frm_filtro.agrupamento_id.value = null;';
	if($patrocinador_ativo) echo 'document.frm_filtro.patrocinador_id.value = null;';
	if($tr_ativo) echo 'document.frm_filtro.tr_id.value = null;';
	if(isset($config['exibe_me']) && $config['exibe_me']) echo 'document.frm_filtro.me_id.value = null;';
	?>
	}
...
function setProjeto(chave, valor){
	limpar_tudo();
	document.frm_filtro.projeto_id.value = chave;
	frm_filtro.submit();
	}
...
function setPerspectiva(chave, valor){
	limpar_tudo();
	document.frm_filtro.pg_perspectiva_id.value = chave;
	frm_filtro.submit();
	}
...
function setTema(chave, valor){
	limpar_tudo();
	document.frm_filtro.tema_id.value = chave;
	frm_filtro.submit();
	}	
...
function setObjetivo(chave, valor){
	limpar_tudo();
	document.frm_filtro.objetivo_id.value = chave;
	frm_filtro.submit();
	}	
...
function setFator(chave, valor){
	limpar_tudo();
	document.frm_filtro.fator_id.value = chave;
	frm_filtro.submit();
	}
...
function setEstrategia(chave, valor){
	limpar_tudo();
	document.frm_filtro.pg_estrategia_id.value = chave;
	frm_filtro.submit();
	}	
...
function setMeta(chave, valor){
	limpar_tudo();
	document.frm_filtro.pg_meta_id.value = chave;
	frm_filtro.submit();
	}	
...
function setPratica(chave, valor){
	limpar_tudo();
	document.frm_filtro.pratica_id.value = chave;
	frm_filtro.submit();
	}
...
function setIndicador(chave, valor){
	limpar_tudo();
	document.frm_filtro.pratica_indicador_id.value = chave;
	frm_filtro.submit();
	}
...
function setAcao(chave, valor){
	limpar_tudo();
	document.frm_filtro.plano_acao_id.value = chave;
	frm_filtro.submit();
	}	
...
function setCanvas(chave, valor){
	limpar_tudo();
	document.frm_filtro.canvas_id.value = chave;
	frm_filtro.submit();
	}
<?php }?>	
...
function setRisco(chave, valor){
	limpar_tudo();
	document.frm_filtro.risco_id.value = chave;
	frm_filtro.submit();
	}
<?php }?>	
...
function setRiscoResposta(chave, valor){
	limpar_tudo();
	document.frm_filtro.risco_resposta_id.value = chave;
	frm_filtro.submit();
	}
<?php }?>	
...
function setCalendario(chave, valor){
	limpar_tudo();
	document.frm_filtro.calendario_id.value = chave;
	frm_filtro.submit();
	}
...
function setMonitoramento(chave, valor){
	limpar_tudo();
	document.frm_filtro.monitoramento_id.value = chave;
	frm_filtro.submit();
	}	
...
function setAta(chave, valor){
	limpar_tudo();
	document.frm_filtro.ata_id.value = chave;
	frm_filtro.submit();
	}	
...
function setMSWOT(chave, valor){
	limpar_tudo();
	document.frm_filtro.mswot_id.value = chave;
	frm_filtro.submit();
	}		
...
function setSWOT(chave, valor){
	limpar_tudo();
	document.frm_filtro.swot_id.value = chave;
	frm_filtro.submit();
	}	
...
function setOperativo(chave, valor){
	limpar_tudo();
	document.frm_filtro.operativo_id.value = chave;
	frm_filtro.submit();
	}		
...
function setInstrumento(chave, valor){
	limpar_tudo();
	document.frm_filtro.instrumento_id.value = chave;
	frm_filtro.submit();
	}	
...
function setRecurso(chave, valor){
	limpar_tudo();
	document.frm_filtro.recurso_id.value = chave;
	frm_filtro.submit();
	}
...
function setProblema(chave, valor){
	limpar_tudo();
	document.frm_filtro.problema_id.value = chave;
	frm_filtro.submit();
	}
<?php } ?>
...
function setDemanda(chave, valor){
	limpar_tudo();
	document.frm_filtro.demanda_id.value = chave;
	frm_filtro.submit();
	}
...
function setPrograma(chave, valor){
	limpar_tudo();
	document.frm_filtro.programa_id.value = chave;
	frm_filtro.submit();
	}	
<?php } ?>
...
function setLicao(chave, valor){
	limpar_tudo();
	document.frm_filtro.licao_id.value = chave;
	frm_filtro.submit();
	}
...
function setEvento(chave, valor){
	limpar_tudo();
	document.frm_filtro.evento_id.value = chave;
	frm_filtro.submit();
	}
...
function setLink(chave, valor){
	limpar_tudo();
	document.frm_filtro.link_id.value = chave;
	frm_filtro.submit();
	}
...
function setAvaliacao(chave, valor){
	limpar_tudo();
	document.frm_filtro.avaliacao_id.value = chave;
	frm_filtro.submit();
	}
...
function setTgn(chave, valor){
	limpar_tudo();
	document.frm_filtro.tgn_id.value = chave;
	frm_filtro.submit();
	}
<?php } ?>
...
function setBrainstorm(chave, valor){
	limpar_tudo();
	document.frm_filtro.brainstorm_id.value = chave;
	frm_filtro.submit();
	}
...
function setGut(chave, valor){
	limpar_tudo();
	document.frm_filtro.gut_id.value = chave;
	frm_filtro.submit();
	}
...
function setCausa_efeito(chave, valor){
	limpar_tudo();
	document.frm_filtro.causa_efeito_id.value = chave;
	frm_filtro.submit();
	}

function popArquivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Arquivo', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo_pastas&cia_id='+document.getElementById('cia_id').value, window.setArquivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setArquivo&tabela=arquivo_pastas&cia_id='+document.getElementById('cia_id').value, 'Arquivo','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}

function setArquivo(chave, valor){
	limpar_tudo();
	document.frm_filtro.arquivo_pasta_id.value = chave;
	frm_filtro.submit();
	}
...
function setForum(chave, valor){
	limpar_tudo();
	document.frm_filtro.forum_id.value = chave;
	frm_filtro.submit();
	}
...
function setChecklist(chave, valor){
	limpar_tudo();
	document.frm_filtro.checklist_id.value = chave;
	frm_filtro.submit();
	}
...
function setAgenda(chave, valor){
	limpar_tudo();
	document.frm_filtro.agenda_id.value = chave;
	frm_filtro.submit();
	}
...
<?php  if ($Aplic->profissional) { ?>
	...
	function setAgrupamento(chave, valor){
		limpar_tudo();
		document.frm_filtro.agrupamento_id.value = chave;
		frm_filtro.submit();
		}
	...
	function setPatrocinador(chave, valor){
		limpar_tudo();
		document.frm_filtro.patrocinador_id.value = chave;
		frm_filtro.submit();
		}
	...
	function setTemplate(chave, valor){
		limpar_tudo();
		document.frm_filtro.template_id.value = chave;
		frm_filtro.submit();
		}		
	...
	function setPainel(chave, valor){
		limpar_tudo();
		document.frm_filtro.painel_id.value = chave;
		frm_filtro.submit();
		}		
	...
	function setOdometro(chave, valor){
		limpar_tudo();
		document.frm_filtro.painel_odometro_id.value = chave;
		frm_filtro.submit();
		}			
	...
	function setComposicaoPaineis(chave, valor){
		limpar_tudo();
		document.frm_filtro.painel_composicao_id.value = chave;
		frm_filtro.submit();
		}	
	...
	function setTR(chave, valor){
		limpar_tudo();
		document.frm_filtro.tr_id.value = chave;
		frm_filtro.submit();
		}	
