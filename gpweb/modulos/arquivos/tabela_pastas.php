...
function getPastas($superior, $nivel = 0) {
	...
	if ($Aplic->profissional){
		...
		elseif ($objetivo_id) $sql->adOnde('arquivo_pasta_gestao_objetivo='.(int)$objetivo_id);
		elseif ($fator_id) $sql->adOnde('arquivo_pasta_gestao_fator='.(int)$fator_id);
		...
		elseif ($mswot_id) $sql->adOnde('arquivo_pasta_gestao_mswot='.(int)$mswot_id);
...
function mostrarArquivos($arquivo_pasta_id) {
	...
	$sql->esqUnir('arquivo_pasta', 'ff', 'ff.arquivo_pasta_id = arquivo_pasta');
	$sql->adCampo('arquivo.*,count(arquivo_versao) as arquivo_versoes,round(max(arquivo_versao), 2) as arquivo_ultimaversao,arquivo_pasta_id, arquivo_pasta_nome, contato_id, usuario_id');
	if ($arquivo_pasta_id) $sql->adOnde('arquivo_pasta = '.(int)$arquivo_pasta_id);
	else $sql->adOnde('arquivo_pasta = 0 OR arquivo_pasta IS NULL');
	$sql->adGrupo('arquivo_pasta, arquivo_versao_id, arquivo.arquivo_id, ff.arquivo_pasta_id, contato_id, usuario_id');
	...
	$qv->adCampo('arquivo.*, usuario_login as arquivo_dono, arquivo_pasta_nome, usuario_id');
	if ($arquivo_pasta_id) $qv->adOnde('arquivo_pasta = '.(int)$arquivo_pasta_id);
	else $qv->adOnde('arquivo_pasta = 0 OR arquivo_pasta IS NULL');
	if ($arquivo_pasta_id) $qv->adOnde('arquivo_pasta = '.(int)$arquivo_pasta_id);
	
	if ($Aplic->profissional){
		$qv->esqUnir('arquivo_gestao','arquivo_gestao','arquivo_gestao_arquivo = arquivo.arquivo_id');
		...
		elseif ($objetivo_id) $qv->adOnde('arquivo_gestao_objetivo='.(int)$objetivo_id);
		elseif ($fator_id) $qv->adOnde('arquivo_gestao_fator='.(int)$fator_id);
		...
		elseif ($mswot_id) $qv->adOnde('arquivo_gestao_mswot='.(int)$mswot_id);
		...
		else if ($objetivo_id) $qv->adOnde('arquivo_objetivo = '.(int)$objetivo_id);
		else if ($pg_estrategia_id) $qv->adOnde('arquivo_estrategia = '.(int)$pg_estrategia_id);
		else if ($fator_id) $qv->adOnde('arquivo_fator = '.(int)$fator_id);
