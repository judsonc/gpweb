...
if ($swot_ativo) {
	echo '<tr '.(isset($arquivo_mswot) && $arquivo_mswot ? '' : 'style="display:none"').' id="mswot" ><td align="right" nowrap="nowrap">'.dica('Matriz SWOT', 'Caso o arquivo seja específico de uma matriz SWOT neste campo deverá constar o nome da matriz SWOT').'Matriz SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="arquivo_mswot" value="'.(isset($arquivo_mswot) ? $arquivo_mswot : '').'" /><input type="text" id="mswot_nome" name="mswot_nome" value="'.nome_mswot((isset($arquivo_mswot) ? $arquivo_mswot : null)).'" style="width:288px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popMSWOT();">'.imagem('../../../modulos/swot/imagens/mswot_p.png','Selecionar Matriz SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/mswot_p.png').' para selecionar uma matriz SWOT.').'</a></td></tr></table></td></tr>';
	echo '<tr '.(isset($arquivo_swot) && $arquivo_swot ? '' : 'style="display:none"').' id="swot" ><td align="right" nowrap="nowrap">'.dica('Campo SWOT', 'Caso o arquivo seja específico de um campo de matriz SWOT neste campo deverá constar o nome do campo de matriz SWOT').'Campo SWOT:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="arquivo_swot" value="'.(isset($arquivo_swot) ? $arquivo_swot : '').'" /><input type="text" id="swot_nome" name="swot_nome" value="'.nome_swot((isset($arquivo_swot) ? $arquivo_swot : null)).'" style="width:288px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popSWOT();">'.imagem('../../../modulos/swot/imagens/swot_p.png','Selecionar Campo SWOT','Clique neste ícone '.imagem('../../../modulos/swot/imagens/swot_p.png').' para selecionar um campo de matriz SWOT.').'</a></td></tr></table></td></tr>';
	}
else {
	echo '<input type="hidden" name="arquivo_mswot" value="" id="mswot" /><input type="hidden" id="mswot_nome" name="mswot_nome" value="">';
	echo '<input type="hidden" name="arquivo_swot" value="" id="swot" /><input type="hidden" id="swot_nome" name="swot_nome" value="">';
	}
...
if ($Aplic->profissional){
  ...
  $sql->limpar();
	...
	foreach($lista as $gestao_data){
		...
		elseif ($gestao_data['arquivo_gestao_mswot']) echo '<td align=left>'.imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['arquivo_gestao_mswot']).'</td>';
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($indicadores)>1) echo '<tr><td align="right" nowrap="nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores relacionados mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'arquivo_principal_indicador', 'class="texto" style="width:284px;"', $obj->arquivo_principal_indicador).'</td></tr>';
	}
...
if ($Aplic->profissional && $exibir['moeda']){
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'Escolha a moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td>'.selecionaVetor($moedas, 'arquivo_moeda', 'class=texto size=1', ($obj->arquivo_moeda ? $obj->arquivo_moeda : 1)).'</td></tr>';
	}	
...
function popPasta() {
	...
	<?php if (!$Aplic->profissional) { ?>
		...
		if (tipo=='projeto' && f.arquivo_projeto.value<1) alert('Necessita escolher um<?php echo ($config["genero_projeto"]=="o" ? "" : "a")." ".$config["projeto"]?>!');
			...
			'&cia_id='+document.getElementById('arquivo_cia').value, 'Pastas','left=0,top=0,height=600,width=350,scrollbars=yes, resizable=yes');
			}
	<?php } else { ?>
...
function popObjetivo() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["objetivo"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('arquivo_cia').value, window.setObjetivo, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setObjetivo&tabela=objetivo&cia_id='+document.getElementById('arquivo_cia').value, '<?php echo ucfirst($config["objetivo"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popFator() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["fator"])?>', 500, 500, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('arquivo_cia').value, window.setFator, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setFator&tabela=fator&cia_id='+document.getElementById('arquivo_cia').value, '<?php echo ucfirst($config["fator"])?>','left=0,top=0,height=600,width=600,scrollbars=yes, resizable=yes');
	}
...
function popMSWOT() {
	parent.gpwebApp.popUp('Matriz SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setMSWOT&tabela=mswot&cia_id='+document.getElementById('arquivo_cia').value, window.setMSWOT, window);
	}
...
function popSWOT() {
	parent.gpwebApp.popUp('Campo SWOT', 500, 500, 'm=swot&a=selecionar&dialogo=1&chamar_volta=setSWOT&tabela=swot&cia_id='+document.getElementById('arquivo_cia').value, window.setSWOT, window);
	}
...
function limpar_tudo(){
	...
	<?php
	if($swot_ativo) {
		echo 'document.env.mswot_nome.value = \'\';	document.env.arquivo_mswot.value = null;';
		echo 'document.env.swot_nome.value = \'\';	document.env.arquivo_swot.value = null;';
		}
