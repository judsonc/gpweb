...
global $tab, $dialogo, $estilo_interface, $arquivo_pasta, $usuario_id, $arquivo_upload, $arquivo_participante, $pesquisar_texto, $arquivo_categoria, $lista_depts, $lista_cias, $Aplic, 
$negar1, $podeAcessar, $podeAdmin, $perms, $cia_id, $dept_id,$m, $a, $arquivo_pasta_id, $favorito_id, $podeEditar, $filtro_prioridade_arquivo,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('arquivo_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('arquivo_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('arquivo_gestao_mswot='.(int)$mswot_id);
...
$sql->adCampo('arquivo_id, arquivo_acesso, arquivo_nome, arquivo_tipo, arquivo_categoria, arquivo_descricao, arquivo_versao, arquivo_dono, arquivo_usuario_upload, arquivo_tamanho, arquivo_data, arquivo_pasta_nome, arquivo_pasta_id, arquivo_aprovado');
if ($arquivo_pasta_id) $sql->adOnde('arquivo_pasta='.(int)$arquivo_pasta_id);
...
if ($Aplic->profissional){
	...
	else if ($arquivo_usuario) $sql->adOnde('arquivo.arquivo_usuario = '.(int)$Aplic->usuario_id);
	//else $sql->adOnde('arquivo.arquivo_usuario=0 OR arquivo.arquivo_usuario IS NULL OR arquivo.arquivo_usuario = '.(int)$Aplic->usuario_id);
	}
if ($arquivo_pasta) $sql->adOnde('arquivo_pasta = '.(int)$arquivo_pasta);
if ($arquivo_categoria > -1) $sql->adOnde('arquivo_categoria = '.(int)$arquivo_categoria);
...
echo '<tr>';
	...
	if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=arquivo_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='arquivo_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
	if ($filtro_prioridade_arquivo) echo '<th><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($arquivos as $linha) {
	...
	if (permiteAcessarArquivo($linha['arquivo_acesso'], $linha['arquivo_id'])){	
		...
		echo '<tr>';
		echo '<td align="center">'.($editar && $podeEditar && !$bloquear ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=arquivos&a=editar&arquivo_id='.$linha['arquivo_id'].'\');">'.imagem('icones/editar.gif', 'Editar Arquivo', 'Ao clicar neste ícone '.imagem('icones/editar.gif').' será possivel editar o arquivo.').'</a>' :'&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['arquivo_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_arquivo) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
		...
		if ($Aplic->profissional){
			...
			if (count($lista)){	
				foreach($lista as $gestao_data){	
					...
					elseif ($gestao_data['arquivo_gestao_mswot']) echo ($qnt_gestao++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['arquivo_gestao_mswot']);
