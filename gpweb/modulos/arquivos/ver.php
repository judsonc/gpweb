...
else if ($Aplic->profissional && !$dialogo){
	$botoesTitulo = new CBlocoTitulo('Visualizar Arquivo', 'arquivo.png', $m, $m.'.'.$a);
	$botoesTitulo->mostrar();
	}
...
if ($Aplic->profissional && !$dialogo){
	...
	if ($editar){
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_arquivo=".$arquivo_id."\");");
	...
	$bloquear=($obj->arquivo_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin);
...
if ($Aplic->profissional){
	...
	if (count($lista)){	
		...
		foreach($lista as $gestao_data){	
			...
			elseif ($gestao_data['arquivo_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['arquivo_gestao_mswot']);
			...
			elseif ($gestao_data['arquivo_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['arquivo_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->arquivo_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o arquivo se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->arquivo_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
//echo '<tr><td align="right" nowrap="nowrap">'.dica('Responsável pela Atualização', 'Nome d'.$config['genero_usuario'].' '.$config['usuario'].' que enviou o arquivo atualizado.').'Atualizado por:'.dicaF().'</td><td align="left" class="realce"><a href="./codigo/arquivo_visualizar.php?arquivo_id='.$linha['arquivo_id'].'">'.$linha['arquivo_nome'].dicaF().'</a></td></tr>';
...
if (!$dialogo && $Aplic->profissional) {
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(evento_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(link_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(forum_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(pratica_indicador_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			...
			$sql->adCampo('count(plano_acao_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			}
		else $existe=true;	
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		...
		$sql->adCampo('count(msg_gestao_arquivo)');
		$existe=$sql->resultado();
		$sql->limpar();
		...
		if ($config['doc_interno']) {
			...
			$sql->adCampo('count(modelo_gestao_arquivo)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		...
		$sql->adCampo('count(ata_gestao_arquivo)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		...
		$sql->adCampo('count(problema_gestao_arquivo)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		...
		$sql->adCampo('count(risco_gestao_arquivo)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
		...
		$sql->adOnde('risco_resposta_gestao_arquivo = '.(int)$arquivo_id.' OR risco_gestao_arquivo = '.(int)$arquivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		...
		$sql->adCampo('count(instrumento_gestao_arquivo)');
		$existe=$sql->resultado();
		$sql->limpar();
		if ($existe) {
			$qnt_aba++;
