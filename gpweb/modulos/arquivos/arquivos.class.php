...
class CArquivo extends CAplicObjeto {
	...
	function armazenar($atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($this->arquivo_id) {
			$ret = $sql->atualizarObjeto('arquivo', $this, 'arquivo_id', false);
			$sql->limpar();
			}
		else {
...
function getPastaListaSelecao() {
	//ERRO corrigir depois
	global $Aplic, $projeto_id, $pratica_id, $demanda_id, $instrumento_id, $pratica_indicador_id, $arquivo_usuario, $objetivo_id, $tema_id, $pg_perspectiva_id, $canvas_id, $pg_estrategia_id, $plano_acao_id, $fator_id, $pg_meta_id, $calendario_id;
	$q = new BDConsulta();
	$q->adTabela('arquivo_pasta');
	...
	if ($objetivo_id) $q->adOnde('arquivo_pasta_objetivo='.$objetivo_id);
	if ($pg_estrategia_id) $q->adOnde('arquivo_pasta_estrategia='.$pg_estrategia_id);
	if ($fator_id) $q->adOnde('arquivo_pasta_fator='.$fator_id);
