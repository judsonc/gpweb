...
if (!$Aplic->checarModulo('links', 'acesso')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
$sql->adOnde('favorito_trava_link=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('link_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('link_favorito') !== null ? $Aplic->getEstado('link_favorito') : 0;
...
if (!$dialogo){
	...
	if($plano_acao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Filtrar os links pel'.$config['genero_acao'].' '.$config['acao'].' que estão relacionados.').ucfirst($config['acao']).':'.dicaF();
		$nome=nome_acao($plano_acao_id);
		}
	elseif($pratica_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Filtrar os links pel'.$config['genero_pratica'].' '.$config['pratica'].' que estão relacionados.').ucfirst($config['pratica']).':'.dicaF();
		$nome=nome_pratica($pratica_id);
		}
	elseif($calendario_id){
		$legenda_filtro=dica('Filtrar pela Agenda', 'Filtrar os links pela agenda que estão relacionados.').'Agenda:'.dicaF();
		$nome=nome_calendario($calendario_id);
		}
	elseif($projeto_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar os links pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionados.').ucfirst($config['projeto']).':'.dicaF();
		$nome=nome_projeto($projeto_id);
		}
	elseif($pratica_indicador_id){
		$legenda_filtro=dica('Filtrar pelo Indicador', 'Filtrar os links pelo indicador que estão relacionados.').'Indicador:'.dicaF();
		$nome=nome_indicador($pratica_indicador_id);
		}
	elseif($objetivo_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Filtrar os links pel'.$config['genero_objetivo'].' '.$config['objetivo'].' que estão relacionados.').''.ucfirst($config['objetivo']).':'.dicaF();
		$nome=nome_objetivo($objetivo_id);
		}
	elseif($tema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Filtrar os links pel'.$config['genero_tema'].' '.$config['tema'].' que estão relacionados.').ucfirst($config['tema']).':'.dicaF();
		$nome=nome_tema($tema_id);
		}
	elseif($pg_estrategia_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Filtrar os links pel'.$config['genero_iniciativa'].' '.$config['iniciativa'].' que estão relacionados.').ucfirst($config['iniciativa']).':'.dicaF();
		$nome=nome_estrategia($pg_estrategia_id);
		}
	elseif($pg_perspectiva_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']), 'Filtrar os links pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' que estão relacionados.').ucfirst($config['perspectiva']).':'.dicaF();
		$nome=nome_perspectiva($pg_perspectiva_id);
		}
	elseif($canvas_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_canvas'].' '.ucfirst($config['canvas']), 'Filtrar os links pel'.$config['genero_canvas'].' '.$config['canvas'].' que estão relacionados.').ucfirst($config['canvas']).':'.dicaF();
		$nome=nome_canvas($canvas_id);
		}
	elseif($fator_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Filtrar os links pel'.$config['genero_fator'].' '.$config['fator'].' que estão relacionados.').ucfirst($config['fator']).':'.dicaF();
		$nome=nome_fator($fator_id);
		}
	elseif($pg_meta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_meta'].' '.ucfirst($config['meta']), 'Filtrar os links pel'.$config['genero_meta'].' '.$config['meta'].' que estão relacionados.').ucfirst($config['meta']).':'.dicaF();
		$nome=nome_meta($pg_meta_id);
		}	
	elseif($risco_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco'].' '.ucfirst($config['risco']), 'Filtrar os links pel'.$config['genero_risco'].' '.$config['risco'].' que estão relacionados.').ucfirst($config['risco']).':'.dicaF();
		$nome=nome_risco($risco_id);
		}
	elseif($risco_resposta_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_risco_resposta'].' '.ucfirst($config['risco_resposta']), 'Filtrar os links pel'.$config['genero_risco_resposta'].' '.$config['risco_resposta'].' que estão relacionados.').ucfirst($config['risco_resposta']).':'.dicaF();
		$nome=nome_risco_resposta($risco_resposta_id);
		}	
	elseif($monitoramento_id){
		$legenda_filtro=dica('Filtrar pelo Monitoramento', 'Filtrar os links pelo monitoramento que estão relacionados.').'Monitoramento:'.dicaF();
		$nome=nome_monitoramento($monitoramento_id);
		}		
	elseif($ata_id){
		$legenda_filtro=dica('Filtrar pela Ata de Reunião', 'Filtrar os links pela ata de reunião a qual estão relacionados.').'Ata:'.dicaF();
		$nome=nome_ata($ata_id);
		}	
	elseif($mswot_id){
		$legenda_filtro=dica('Filtrar pela Matriz SWOT', 'Filtrar os links pela matriz SWOT que estão relacionados.').'Matriz SWOT:'.dicaF();
		$nome=nome_mswot($swot_id);
		}	
	elseif($swot_id){
		$legenda_filtro=dica('Filtrar pelo campo SWOT', 'Filtrar os links pelo campo SWOT que estão relacionados.').'Campo SWOT:'.dicaF();
		$nome=nome_swot($swot_id);
		}		
	elseif($operativo_id){
		$legenda_filtro=dica('Filtrar pelo Plano Operativo', 'Filtrar os links pelo plano operativo que estão relacionados.').'Plano Operativo:'.dicaF();
		$nome=nome_operativo($operativo_id);
		}			
	elseif($instrumento_id){
		$legenda_filtro=dica('Filtrar pelo Instrumento Jurídico', 'Filtrar os links pelo instrumento jurídico que estão relacionados.').'Instrumento Jurídico:'.dicaF();
		$nome=nome_instrumento($instrumento_id);
		}	
	elseif($recurso_id){
		$legenda_filtro=dica('Filtrar pelo Recurso', 'Filtrar os links pelo recurso que estão relacionados.').'Recurso:'.dicaF();
		$nome=nome_recurso($recurso_id);
		}	
	elseif($problema_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_problema'].' '.ucfirst($config['problema']), 'Filtrar os links pel'.$config['genero_problema'].' '.$config['problema'].' que estão relacionados.').ucfirst($config['problema']).':'.dicaF();
		$nome=nome_problema($problema_id);
		}	
	elseif($demanda_id){
		$legenda_filtro=dica('Filtrar pela Demanda', 'Filtrar os links pela demanda que estão relacionados.').'Demanda:'.dicaF();
		$nome=nome_demanda($demanda_id);
		}
	elseif($programa_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_programa'].' '.ucfirst($config['programa']), 'Filtrar os links pel'.$config['genero_programa'].' '.$config['programa'].' que estão relacionados.').ucfirst($config['programa']).':'.dicaF();
		$nome=nome_programa($programa_id);
		}	
	elseif($licao_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Filtrar os links pel'.$config['genero_licao'].' '.$config['licao'].' que estão relacionados.').ucfirst($config['licao']).':'.dicaF();
		$nome=nome_licao($licao_id);
		}	
	elseif($evento_id){
		$legenda_filtro=dica('Filtrar pelo Evento', 'Filtrar os links pelo evento que estão relacionados.').'Evento:'.dicaF();
		$nome=nome_evento($evento_id);
		}		
	elseif($avaliacao_id){
		$legenda_filtro=dica('Filtrar pela Avaliação', 'Filtrar os links pela avaliação que estão relacionados.').'Avaliação:'.dicaF();
		$nome=nome_avaliacao($avaliacao_id);
		}
	elseif($tgn_id){
		$legenda_filtro=dica('Filtrar pel'.$config['genero_tgn'].' '.ucfirst($config['tgn']), 'Filtrar os links pel'.$config['genero_tgn'].' '.$config['tgn'].' que estão relacionados.').ucfirst($config['tgn']).':'.dicaF();
		$nome=nome_tgn($tgn_id);
		}	
	elseif($brainstorm_id){
		$legenda_filtro=dica('Filtrar pelo Brainstorm', 'Filtrar os links pelo brainstorm que estão relacionados.').'Brainstorm:'.dicaF();
		$nome=nome_brainstorm($brainstorm_id);
		}	
	elseif($gut_id){
		$legenda_filtro=dica('Filtrar pela Matriz GUT', 'Filtrar os links pela matriz GUT que estão relacionados.').'Matriz GUT:'.dicaF();
		$nome=nome_gut($gut_id);
		}		
	elseif($causa_efeito_id){
		$legenda_filtro=dica('Filtrar pelo Diagrama de Causa-Efeito', 'Filtrar os links pelo diagrama de causa-efeito que estão relacionados.').'Diagrama de Causa-Efeito:'.dicaF();
		$nome=nome_causa_efeito($causa_efeito_id);
		}		
	elseif($arquivo_id){
		$legenda_filtro=dica('Filtrar pelo Arquivo', 'Filtrar os links pelo arquivo que estão relacionados.').'Arquivo:'.dicaF();
		$nome=nome_arquivo($arquivo_id);
		}	
	elseif($forum_id){
		$legenda_filtro=dica('Filtrar pelo Fórum', 'Filtrar os links pelo fórum que estão relacionados.').'Fórum:'.dicaF();
		$nome=nome_forum($forum_id);
		}	
	elseif($checklist_id){
		$legenda_filtro=dica('Filtrar pelo Checklist', 'Filtrar os links pelo checklist que estão relacionados.').'Checklist:'.dicaF();
		$nome=nome_checklist($checklist_id);
		}	
	elseif($agenda_id){
		$legenda_filtro=dica('Filtrar pelo Compromisso', 'Filtrar os links pelo compromisso que estão relacionados.').'Compromisso:'.dicaF();
		$nome=nome_compromisso($agenda_id);
		}	
	elseif($agrupamento_id){
		$legenda_filtro=dica('Filtrar pelo Agrupamento', 'Filtrar os links pelo agrupamento que estão relacionados.').'Agrupamento:'.dicaF();
		$nome=nome_agrupamento($agrupamento_id);
		}
	elseif($patrocinador_id){
		$legenda_filtro=dica('Filtrar pelo Patrocinador', 'Filtrar os links pelo patrocinador que estão relacionados.').'Patrocinador:'.dicaF();
		$nome=nome_patrocinador($patrocinador_id);
		}
	elseif($template_id){
		$legenda_filtro=dica('Filtrar pelo Modelo', 'Filtrar os links pelo modelo que estão relacionados.').'Modelo:'.dicaF();
	...
	elseif($me_id){
		...
		$nome='';
		$legenda_filtro=dica('Filtrar', 'Selecione um campo para filtrar os links.').'Filtro:'.dicaF();
		}
	}
	
if (!$dialogo && $Aplic->profissional){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Lista de Links', 'links.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" nowrap="nowrap">'.dica('Relacionado','A qual parte do sistema os link estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$novo=($podeEditar ? '<tr><td nowrap="nowrap">'.dica('Novo Link', 'Criar um novo link.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=links&a=editar\');" >'.imagem('icones/links_novo.png').'</a>'.dicaF().'</td></tr>' : '');

	$botao_favorito='<tr><td nowrap="nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&link=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$popFiltro.$filtros.$favoritos.'</table></td><td><table cellspacing=0 cellpadding=0>'.$novo.$botao_prioridade.$botao_favorito.$botao_trava.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Links', 'links.png', $m, $m.'.'.$a);
	...
	if ($podeEditar)$botoesTitulo->adicionaCelula('<table width=75><tr><td nowrap="nowrap">'.dica('Novo Link', 'Criar um novo link.').'<a class="botao" href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=links&a=editar\');" ><span>novo</span></a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->mostrar();
	}
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Links</center></font>';
	}	
...
if (!$dialogo){
	$caixaTab->adicionar(BASE_DIR.'/modulos/links/index_tabela', 'Ativos',null,null,'Ativos','Visualizar os links ativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/links/index_tabela', 'Inativos',null,null,'Inativos','Visualizar os links inativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/links/index_tabela', 'Todos',null,null,'Todos','Visualizar todos os links.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/links/index_tabela.php');
...
function priorizacao() {
	parent.gpwebApp.popUp("<?php echo 'Priorização de '.ucfirst($config['projetos'])?>", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&link=1&filtro_prioridade='+env.filtro_prioridade_link.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_link){
	env.filtro_prioridade_link.value=filtro_prioridade_link;
	env.submit();
	}
