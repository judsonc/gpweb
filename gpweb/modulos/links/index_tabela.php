...
global $Aplic, $negar1, $podeAcessar, $podeEditar,  $dialogo, $estilo_interface, $usuario_id, $cia_id, $dept_id, $lista_depts, $lista_cias, $mostrarProjeto, $m, $tab,$filtro_prioridade_link, $favorito_id,
	$tarefa_id, 
	$projeto_id, 
	$pg_perspectiva_id, 
	$tema_id, 
...
if ($Aplic->profissional){
	...
	elseif ($objetivo_id) $sql->adOnde('link_gestao_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('link_gestao_fator='.(int)$fator_id);
	...
	elseif ($mswot_id) $sql->adOnde('link_gestao_mswot='.(int)$mswot_id);
	...
	elseif ($objetivo_id) $sql->adOnde('link_objetivo='.(int)$objetivo_id);
	elseif ($fator_id) $sql->adOnde('link_fator='.(int)$fator_id);
...
$sql->adCampo('link_id, link_nome, link_acesso, link_dono, link_descricao, link_categoria, link_url, link_aprovado');
...
if ($Aplic->profissional) echo '<th nowrap="nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.($tab ? '&tab='.$tab : '').'&ordenar=link_aprovado&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_aprovado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprovado', 'Neste campo consta se foi aprovado.').'Ap.'.dicaF().'</a></th>';
if ($filtro_prioridade_link) echo '<th nowrap="nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.($tab ? '&tab='.$tab : '').'&a='.$a.'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
...
foreach ($linhas as $linha) {
	if (permiteAcessarLink($linha['link_acesso'],$linha['link_id'])) {
		...
		echo '<tr>';
		echo '<td nowrap="nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar Link', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o link.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=links&a=editar&link_id='.$linha['link_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional) echo '<td width=30 align="center">'.($linha['link_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		if ($filtro_prioridade_link) echo '<td align="right" nowrap="nowrap" width=50>'.($linha['priorizacao']).'</td>';	
		echo '<td align="center" valign="middle" width=16>'.dica('Link', 'Clique neste ícone '.imagem('icones/link.png').' para  acessar:<ul><li>'.$linha['link_url'].'</ul>').'<a href="'.$linha['link_url'].'" target="_blank">'.imagem('icones/link.png').'</a>'.dicaF().'</td>';
