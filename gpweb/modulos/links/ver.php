...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_link='.(int)$link_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
if (!$dialogo && !$Aplic->profissional) {
	...
	elseif ($obj->link_objetivo)$botoesTitulo->adicionaBotao('m=praticas&a=obj_estrategico_ver&objetivo_id='.$obj->link_objetivo, $config['objetivo'],'',ucfirst($config['objetivo']),'Visualizar '.$config['genero_objetivo'].' '.$config['objetivo'].' do qual este link faz parte.');
	...
	elseif ($obj->link_fator)$botoesTitulo->adicionaBotao('m=praticas&a=fator_ver&fator_id='.$obj->link_fator, $config['fator'],'',ucfirst($config['fator']),'Visualizar '.$config['genero_fator'].' '.$config['fator'].' d'.$config['genero_fator'].' qual este link faz parte.');
...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($podeEditar && $editar) {
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("inserir","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_link=".$link_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->link_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Link</center></font>';
	}
		  ...
		  if (count($lista)) {
		  	...
		  	echo '<tr><td align="right" nowrap="nowrap">'.dica('Relacionado', 'Neste campo fica a quais áreas do sistema o link está relacionado.').'Relacionado:'.dicaF().'</td><td align="left" class="realce">';
				...
				foreach($lista as $gestao_data){
					...
					elseif ($gestao_data['link_gestao_mswot']) echo ($qnt++ ? '<br>' : '').imagem('../../../modulos/swot/imagens/mswot_p.png').link_mswot($gestao_data['link_gestao_mswot']);
					...
					elseif ($gestao_data['link_gestao_template']) echo ($qnt++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['link_gestao_template']);
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	echo '<tr><td align="right" nowrap="nowrap">'.dica('Moeda', 'A moeda padrão utilizada na meta.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->link_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" nowrap="nowrap">'.dica('Aprovado', 'Se o link se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->link_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional) {
	...
	if ($Aplic->profissional){
		...
		if ($Aplic->profissional) {
			...
			$sql->adOnde('log_link = '.(int)$link_id);
		...
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('calendario', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('evento_gestao_link = '.(int)$link_id);
				$sql->adCampo('count(evento_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('arquivo_gestao_link = '.(int)$link_id);
				$sql->adCampo('count(arquivo_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('forum_gestao_link = '.(int)$link_id);
				$sql->adCampo('count(forum_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'indicador')) {
			if ($Aplic->profissional) {
				...
				$sql->adCampo('count(pratica_indicador_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('plano_acao_gestao_link = '.(int)$link_id);
				$sql->adCampo('count(plano_acao_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				}
			else $existe=true;	
		...
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
			if ($Aplic->profissional) {
				...
				$sql->adOnde('projeto_gestao_link = '.(int)$link_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
			...
			$sql->adOnde('msg_gestao_link = '.(int)$link_id);
			$sql->adCampo('count(msg_gestao_link)');
			$existe=$sql->resultado();
			$sql->limpar();
			...
			if ($config['doc_interno']) {
				...
				$sql->adOnde('modelo_gestao_link = '.(int)$link_id);
				$sql->adCampo('count(modelo_gestao_link)');
				$existe=$sql->resultado();
				$sql->limpar();
				if ($existe) {
					$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
			...
			$sql->adOnde('ata_gestao_link = '.(int)$link_id);
			$sql->adCampo('count(ata_gestao_link)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
			...
			$sql->adOnde('problema_gestao_link = '.(int)$link_id);
			$sql->adCampo('count(problema_gestao_link)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
			...
			$sql->adOnde('risco_gestao_link = '.(int)$link_id);
			$sql->adCampo('count(risco_gestao_link)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
			...
			$sql->adOnde('risco_resposta_gestao_link = '.(int)$link_id.' OR risco_gestao_link = '.(int)$link_id);
		...
		if ($Aplic->profissional && $Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
			...
			$sql->adOnde('instrumento_gestao_link = '.(int)$link_id);
			$sql->adCampo('count(instrumento_gestao_link)');
			$existe=$sql->resultado();
			$sql->limpar();
			if ($existe) {
				$qnt_aba++;
