 ...
 *
 * The ADOdb arquivos are formatted so that doxygen can be used to generate documentation.
...
/**
	...
	ADO, SAP DB, SQLite and ODBC. We have had successful relatorios of connecting to Progress and
...
/*
		...
		var $not_null = false; 
		// actually, this has already been built-in in the postgres, fbsql AND mysql modulo? ^-^
		// so we can as well make not_null standard (leaving it at "false" does not harm anyways)

		var $has_default = false; // this one I have done only in mysql and postgres for now ... 
			// others to come (dannym)
		var $valor_padrao; // default, if any, and supported. Check has_default first.
*/
	}
	...
	var $cacheSecs = 3600; /// cache for 1 hora

	// memcache
	var $memCache = false; /// should we use memCache instead of caching in arquivos
	...
	*/
	...
	/**
	 * Should prepare the sql statement and return the stmt recurso.
	 * For databases that do not support this, we return the $sql. To ensure
	 * compatibility with databases that do not support prepare:
	 *
	 *   $stmt = $bd->Prepare("insert into table (id, name) values (?,?)");
	 *   $bd->Execute($stmt,array(1,'Jill')) or die('insert failed');
	 *   $bd->Execute($stmt,array(2,'Joe')) or die('insert failed');
	 *
	 ...
	 *
	 * Should prepare the stored procedure  and return the stmt recurso.
	 * For databases that do not support this, we return the $sql. To ensure
	 * compatibility with databases that do not support prepare:
	 *
	 * @param sql	SQL to send to database
	...
	*/
	...
	/* 
	Usage in oracle
		$stmt = $bd->Prepare('select * from table where id =:myid and group=:group');
		$bd->Parameter($stmt,$id,'myid');
		$bd->Parameter($stmt,$group,'group',64);
		$bd->Execute();
				...
				foreach($inputarr as $arr) {
					...
					while(list(, $v) = each($arr)) {
						...
						if ($typ == 'string')
							...
							$sql .= str_replace(',','.',$v); // localidades fix so 1.1 does not get converted to 1,1
						else if ($typ == 'boolean')
	...
	*/
	function &_rs2rs(&$rs,$nrows=-1,$offset=-1,$close=true)
	{
		...
		$tipoBd = $rs->databaseType;
		if (!$tipoBd) {
			$rs = &$rs;  // required to prevento crashing in 4.2.1, but does not happen in 4.3.1 -- why ?
			return $rs;
		}
		if (($tipoBd == 'array' || $tipoBd == 'csv') && $nrows == -1 && $offset == -1) {
			$rs->MoveFirst();
			$rs = &$rs; // required to prevento crashing in 4.2.1, but does not happen in 4.3.1-- why ?
			return $rs;
		}
	...
	*/
	...
	/*
		...
		If diaFraction=1.5 means 1.5 dias from now, 1.0/24 for 1 hora.
	*/
	function OffsetData($diaFraction,$data=false)
	{		
		if (!$data) $data = $this->sysDate;
		return  '('.$data.'+'.$diaFraction.')';
	}
	...
	* ADOdb's Replace() uses update-insert semantics, not insert-delete-duplicars of MySQL.
	...
	*
	* @param [secs2cache]	segundos to cache data, set to 0 to force query. This is optional
	* @param sql
	* @param [offset]	is the row to start calculations from (1-based)
	* @param [nrows]	is the number of rows to get
	* @param [inputarr]	array of bind variables
	...
	* If $sql == false, then we purge all arquivos in the cache.
 	*/
   ...
   * If $sql == false, then we purge all arquivos in the cache.
   ...
   /**
   * Private function to erase all of the arquivos and subdirectories in a directory.
   *
   ...
   */
	...
	* Assuming that we can have 50,000 arquivos per directory with good performance, 
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	static $notSafeMode;
		...
		$m = md5($sql.$this->databaseType.$this->database.$this->user.$modo);
	 ...
	 *
	 * @param [secs2cache]	segundos to cache data, set to 0 to force query. 
	...
	/* 
		...
		$modo can be 'INSERT' or 'UPDATE' or DB_AUTOQUERY_INSERT or DB_AUTOQUERY_UPDATE
		If $modo == 'UPDATE', then $where is compulsory as a safety measure.
		
		$forceUpdate means that even if the data has not changed, perform update.
	 */
	function& AutoExecute($table, $fields_values, $modo = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false) 
	{
		$false = false;
		$sql = 'SELECT * FROM '.$table;  
		if ($where!==FALSE) $sql .= ' WHERE '.$where;
		else if ($modo == 'UPDATE' || $modo == 2 /* DB_AUTOQUERY_UPDATE */) {
		...
		case '1':
			...
			break;
		default:
			ADOConnection::outp("AutoExecute: Unknown mode=$modo");
			return $false;
		}
		$ret = false;
	...
	*/
	function UpdateBlobFile($table,$column,$caminho,$where,$blobtype='BLOB')
	{
		$fd = fopen($caminho,'rb');
		if ($fd === false) return false;
		$val = fread($fd,filesize($caminho));
		fclose($fd);
	...
	*/
	...
	function MetaType($t,$tamanho=-1,$fieldobj=false)
	{
		...
		return $this->_metars->MetaType($t,$tamanho,$fieldobj);
	}
	...
	*/
	...
	function &GetActiveRecordsClass($classe, $table,$whereOrderBy=false,$bindarr=false, $primkeyArr=false)
	{
	global $_ADODB_ACTIVE_DBS;
		...
		if (!class_exists($classe)) {
			ADOConnection::outp("Unknown class $classe in GetActiveRcordsClass()");
			return $false;
		}
		$arr = array();
		foreach($rows as $row) {
		
			$obj = new $classe($table,$primkeyArr,$this);
			if ($obj->ErrorMsg()){
...
*/
	function MetaTransaction($modo,$bd)
	{
		$modo = strtoupper($modo);
		$modo = str_replace('ISOLATION LEVEL ','',$modo);
	 ...
	 * 		If false, both vers and tables are returned.
	 *		"VIEW" returns only vers
	 *		"TABLE" returns only tables
	 * @param mostrarSchema returns the schema/user with the table name, eg. USER.TABLE
    ...
    /**
      ...
      * @param table  table name to query
      * @param primary true to only mostrar primary keys. Not actually used for most databases
	  *
	...
	function UnixDate($v)
	{
		if (is_object($v)) {
		// odbtp support
		//( [ano] => 2004 [mes] => 9 [dia] => 4 [hora] => 12 [minuto] => 44 [segundo] => 8 [fraction] => 0 )
			return adodb_mktime($v->hora,$v->minuto,$v->segundo,$v->mes,$v->dia, $v->ano);
		}
	...
	*/
	 ...
	 * An example is  $bd->qstr("Don't bother",magic_quotes_runtime());
	 * 
   ...
   /**
	 ...
	 * @param [size]		#rows to mostrar for listbox. not used by popup
   ...
   */
	...
	* 	or false if EOF
	*
	* Fixed bug relatorioed by tim@orotech.net
	*/
	...
	*/
	...
	static $typeMap = array(
		...
		case 'C':
			...
			if ($this->blobSize >= 0) {
				if ($tamanho > $this->blobSize) return 'X';
			} else if ($tamanho > 250) {
				return 'X';
			}
			return 'C';
	...
	class ADORecordSet_array extends ADORecordSet
	{
		...
		function Fields($colname)
		{
			$modo = isset($this->adodbFetchMode) ? $this->adodbFetchMode : $this->fetchMode;
			
			if ($modo & ADODB_FETCH_ASSOC) {
	...
	global $ADODB_LASTDB;
		...
		$bd = strtolower($dbType);
		switch ($bd) {
			case 'ado': 
				if (PHP_VERSION >= 5) $bd = 'ado5';
				$classe = 'ado'; 
				break;
			case 'ifx':
			case 'maxsql': $classe = $bd = 'mysqlt'; break;
			case 'postgres':
			case 'postgres8':
			case 'pgsql': $classe = $bd = 'postgres7'; break;
			default:
				$classe = $bd; break;
		}
		
		$file = ADODB_DIR."/drivers/adodb-".$bd.".inc.php";
		@include_once($file);
		$ADODB_LASTDB = $classe;
		if (class_exists("ADODB_" . $classe)) return $classe;
	 ...
	 */
	function &NewADOConnection($bd='')
	{
		$tmp =& ADONewConnection($bd);
		return $tmp;
	}
	 ...
	 */
	function &ADONewConnection($bd='')
	{
	GLOBAL $ADODB_NEWCONNECTION, $ADODB_LASTDB;
		
		if (!defined('ADODB_ASSOC_CASE')) define('ADODB_ASSOC_CASE',2);
		$errofn = (defined('ADODB_ERROR_HANDLER')) ? ADODB_ERROR_HANDLER : false;
		$false = false;
		if ($at = strpos($bd,'://')) {
			$origdsn = $bd;
			if (PHP_VERSION < 5) $dsna = @parse_url($bd);
			else {
				$fakedsn = 'fake'.substr($bd,$at);
				$dsna = @parse_url($fakedsn);
				$dsna['scheme'] = substr($bd,0,$at);
			
				if (strncmp($bd,'pdo',3) == 0) {
			...
			if (!$dsna) {
				...
				$bd = str_replace('@/','@adodb-fakehost/',$bd);
				$dsna = parse_url($bd);
	 ...
	 */
		if (!empty($ADODB_NEWCONNECTION)) {
			$obj = $ADODB_NEWCONNECTION($bd);

		} else {
		
			if (!isset($ADODB_LASTDB)) $ADODB_LASTDB = '';
			if (empty($bd)) $bd = $ADODB_LASTDB;
			
			if ($bd != $ADODB_LASTDB) $bd = ADOLoadCode($bd);
			
			if (!$bd) {
				if (isset($origdsn)) $bd = $origdsn;
				if ($errofn) {
					// raise an error
					$ignore = false;
					$errofn('ADONewConnection', 'ADONewConnection', -998,
							 "could not load the database driver for '$bd'",
							 $bd,false,$ignore);
				} else
					 ADOConnection::outp( "<p>ADONewConnection: Unable to load database driver '$bd'</p>",false);
		...
		if ($obj) {
			if ($errofn)  $obj->raiseErrorFn = $errofn;
			if (isset($dsna)) {
				...
				foreach($opt as $k => $v) {
					...
					#ibase
					case 'perfil':		$obj->perfil = $v; break;
	...
	function _adodb_getdriver($provider,$drivername,$perf=false)
	{
		...
		case 'ado'  :   if (strncmp('afazer_',$drivername,4)==0) return substr($drivername,4);
		case 'native':  break;
		default:
			return $provider;
		}
	...
	function &NewPerfMonitor(&$conn)
	{
		...
		$classe = "Perf_$drivername";
		if (!class_exists($classe)) return $false;
		$perf = new $classe($conn);
	...
	function &NewDataDictionary(&$conn,$drivername=false)
	{
		...
		$caminho = ADODB_DIR."/datadict/datadict-$drivername.inc.php";

		if (!file_exists($caminho)) {
			ADOConnection::outp("Dictionary driver '$caminho' not available");
			return $false;
		}
		include_once($caminho);
		$classe = "ADODB2_$drivername";
		$dict = new $classe();
	...
	*/
	function adodb_backtrace($printOrArr=true,$nivels=9999)
	{
		global $ADODB_INCLUDED_LIB;
		if (empty($ADODB_INCLUDED_LIB)) include(ADODB_DIR.'/adodb-lib.inc.php');
		return _adodb_backtrace($printOrArr,$nivels);
	}
