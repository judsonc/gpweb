...
*/
	...
	function &MetaTables()
	{
		...
		if ($adors){
			$f = $adors->Fields(2);//table/ver name
	...
	function &MetaColumns($table)
	{
		...
		if ($adors){
			$t = $adors->Fields(2);//table/ver name
			while (!$adors->EOF){
...
class ADORecordSet_ado extends ADORecordSet {	
	...
	function ADORecordSet_ado($id,$modo=false)
	{
		if ($modo === false) { 
		...
		$this->fetchMode = $modo;
		return $this->__construct($id,$modo);
	}
			...
			case 135: // timestamp
				if (!strlen((string)$f->value)) $this->fields[] = false;
					...
					if (!is_numeric($f->value)) # $val = variant_date_to_timestamp($f->value);
						// VT_DATE stores dates as (float) fractional dias since 1899/12/30 00:00:00
