...
extend(Chart.prototype, {
    ...
    getSVG: function (additionalOptions) {
        ...
        if(options.series && options.overwrite){
            each(options.series, function (serie) {
                serie.animation = false;
                serie.showCheckbox = false;

                if (!serie.isInternal) { // used for the navigator series that has its own option set
                    // remove image markers
                    if (serie.options && serie.options.marker && /^url\(/.test(serie.options.marker.symbol)) {
                        serie.options.marker.symbol = 'circle';
                    }
                }
            });
        }
