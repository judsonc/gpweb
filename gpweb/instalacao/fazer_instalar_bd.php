...
if (($fazer_bd || $fazer_bd_cfg)) {
    if($dbc){
        ...
        if (!$atualizacao){
            ...
            if(count($lstErros)){
                $erros[] = array('arquivo' => $nomeArquivoSQL, 'erros' => $lstErros);
            }
        ...
        if (count($erros)){
            $msgBD = 'Configuração da Base de Dados incompleta - os seguintes erros ocorreram:<br>';

            foreach($erros as $errosArquivo){
                $msgBD .= '<div style="padding: 10px;">Arquivo: '.$errosArquivo['arquivo'];

                $msgBD .= '<ul>';
                foreach($errosArquivo['erros'] as $erro){
                    $msgBD .= '<li>Comando No.: '.$erro['linha'].' - '.$erro['cmd'].'<br/>'.$erro['msg'].'</li>';
                }
            ...
            $msgBD = 'Base de Dados configurada com sucesso<br>';
        }
    }
    else {
        $msgBD = 'Não criado';
