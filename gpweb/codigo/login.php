...
echo '<input type="hidden" name="protocolo" value="" />';
...
if (!$expirado){
	...
	if ($config['militar']!=11) echo '<tr><td style="padding:2px" align="center" colspan="2" nowrap="nowrap">'.dica('Esqueceu a Senha','Clique neste link caso não se lembre de sua senha cadastrada para acesso ao '.$config['gpweb'].'.').'<a href="javascript: void(0);" onclick="document.frmlogin.perdeu_senha.value=1;document.frmlogin.submit();">Esqueci a senha</a>'.dicaF().'</td></tr>';
...
if(window.parent){
    frmlogin.protocolo.value = window.parent.location.protocol;
}
