...
if (is_file(BASE_DIR.'/config.php')) require_once BASE_DIR.'/config.php';
	...
	echo '<html><head><meta http-equiv="refresh" content="5; URL='.get_base_url().'/instalacao/index.php" charset="iso-8859-1"><LINK REL="SHORTCUT ICON" href="estilo/rondon/imagens/organizacao/10/favicon.ico"><link rel="stylesheet" type="text/css" href="estilo/rondon/estilo_'.(isset($config['estilo_css']) ? $config['estilo_css'] : 'metro').'.css"></head><body>';
	echo '<br><br><br>';
...
if(isset($_REQUEST['protocolo'])){
    $_SESSION['gpweb_protocolo'] = $_REQUEST['protocolo'];
    }
...
if (getParam($_REQUEST, 'perdeu_senha', 0)){
	...
	require BASE_DIR.'/codigo/perdeu_senha.php';
	session_unset();
	session_destroy();
	exit;
